...
from DomainSurfacePanelConnector import DomainSurfacePanelBlockSignals, DomainSurfacePanelConnector
from Gui2Neg import theGuiMsgHandler
from PartDomainSurfaceVis import PartDomainSurfaceVisParams
...
class DomainSurfacePanel(QtWidgets.QWidget,Ui_GenericVisualizerPanelBase):
    ...
    def __getParams(self):
        ...
        data = PartDomainSurfaceVisParams()
    ...
    def __panelAccordingTaskType( self ):
        ...
        self.vrpLabelTitle.setText(self.__tr("Edit DomainSurface"))
...
#            self.vrpComboBoxVariable.addItem(v)
    ...
    def __setParams( self, params ):
        ...
        DomainSurfacePanelBlockSignals(self, True)
        ...
        if hasattr(params, 'variable' ):
            indexTab = self.TabWidgetGeneralAdvanced.indexOf(self.__generalTab) 
            if params.variable == None or params.variable == 'unset':
                if not indexTab == -1:
                    self.TabWidgetGeneralAdvanced.removeTab(indexTab)
                self.vrpLineEditVariable.setText(self.__tr('None'))
            else: 
                self.vrpLineEditVariable.setText(params.variable)
                if indexTab == -1:
                    self.TabWidgetGeneralAdvanced.addTab(self.__generalTab, self.__generalTabText)
        ...
        if hasattr(params, 'colorTableKey'):
            self.colorCreator=params.colorTableKey

        DomainSurfacePanelBlockSignals(self, False)
