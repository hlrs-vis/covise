...
int main(int argc, char* argv[])
{
    ...
    if ((vive::vvCommandLine::argc() >= 5) && (!strcmp(vive::vvCommandLine::argv(1), "-c")))
    {
        myID = atoi(vive::vvCommandLine::argv(2));
        if (vive::vvCommandLine::argc() >= 6)
        {
            mastername = vive::vvCommandLine::argv(5);
        }
    }
    ...
    vive::config::Access config(my_hostname, mastername, myID);
...
#error "unknown MPI flavor"
    ...
    if (forceMpi)
    {
        Renderer = new vive::vvVIVE(&comm, nullptr);
    }
    else
#endif
    {
        Renderer = new vive::vvVIVE();
    }
    Renderer->run();
    config.save();
    delete Renderer; 
