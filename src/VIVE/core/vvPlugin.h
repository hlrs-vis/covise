...
public:
    ...
    vvPlugin(const std::string &name);

    //! called before plugin is unloaded
    virtual ~vvPlugin() = default;
    ...
    virtual void addNode(vsg::Node *, const vvRenderObject * = NULL)
    {
    }

    virtual void addNodeFromPlugin(vsg::Node *node, const vvRenderObject *ro, vvPlugin *addingPlugin)
    {
        return addNode(node, ro);
    }

    //! this function is called if a node in the scene graph is removed
    virtual void removeNode(vsg::Node *, bool isGroup, vsg::Node *realNode)
    {
        (void)isGroup;
    }
    ...
    virtual void newInteractor(const vvRenderObject *container, vvInteractor *it)
    {
        (void)container;
        (void)it;
    }
    ...
    virtual bool requestInteraction(vvInteractor *inter, vsg::Node *triggerNode, bool isMouse)
    {
        (void)inter;
        (void)triggerNode;
        (void)isMouse;
    ...
    virtual void getMatrix(int station, vsg::dmat4 &mat)
    {
        (void)station;
        (void)mat;
    }
