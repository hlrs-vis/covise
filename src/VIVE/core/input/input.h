 ...
 */
class VVCORE_EXPORT Input
{
public:
    static Input *instance(); //< access input subsystem
    ~Input();
    bool init(); //< initialize input subsytem, legacy driver requires cover/vvPluginSupport to exist
    ...
    const vsg::dmat4 &getMouseMat() const; //< mouse matrix of active persion
    const vsg::dmat4 &getHeadMat() const; //< head matrix of active persion
    const vsg::dmat4 &getRelativeMat() const; //< relative matrix of active persion
    const vsg::dmat4 &getHandMat(int num = 0) const; //< hand matrix of active person
