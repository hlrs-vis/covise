...
Input::~Input()
{
		...
		++it)
	{
		...
		if (handle)
		{
			vive::vvDynLib::dlclose(handle); // this has to be called after the destructor, otherwise the code gets unloaded while it is still executing
		}
	}
	plugins.clear();
...
const vsg::dmat4 &Input::getHeadMat() const
{

    return activePerson->getHeadMat();
}

const vsg::dmat4 &Input::getRelativeMat() const
{
    return activePerson->getRelativeMat();
}

const vsg::dmat4 &Input::getHandMat(int num) const
{
...
DriverFactoryBase *Input::getDriverPlugin(const std::string &type)
{
    ...
    if (!fact)
    {
        ...
        NewDriverFactory newDriverFactory = (NewDriverFactory)vvDynLib::dlsym(handle, "newDriverFactory");
        if (!newDriverFactory)
        {
...
InputDevice *Input::getDevice(const std::string &name)
{
    ...
    if (!dev)
    {
        ...
        DriverFactoryBase *plug = getDriverPlugin(vvMSController::instance()->isMaster() ? type : "const");
        if (!plug)
        {
        ...
        {
            if (vvPlugin *coverPlugin = vvPluginList::instance()->addPlugin(name.c_str(), vvPluginList::Input))
            {
        ...
        {
            if (vvPlugin *coverPlugin = vvPluginList::instance()->addPlugin(type.c_str(), vvPluginList::Input))
            {
                dev = findInMap(drivers, name);
                if (dev)
                    return dev;
...
bool Input::initPersons()
{
    ...
    bool vgl = vvMSController::instance()->syncBool(vvConfig::instance()->useVirtualGL());
    if (vgl)
    {
...
const vsg::dmat4 &Input::getMouseMat() const
{
...
bool Input::update()
{
    ...
    if (vvMSController::instance()->isMaster())
    {
        ...
        {
            const vsg::dmat4 old = ob->second->getMat();
            ob->second->update();
    ...
    else
    {
        ...
        {
            std::cerr << "Input (id=" << vvMSController::instance()->getID() << "): active persion is " << getActivePerson() << ", should be " << activePerson << std::endl;
            exit(1);
        }

        if (nButtons != buttondevices.size())
        {
            std::cerr << "Input (id=" << vvMSController::instance()->getID() << "): buttondevices.size() is " << buttondevices.size() << ", should be " << nButtons << std::endl;
            exit(1);
        }

        if (nValuators != valuators.size())
        {
            std::cerr << "Input (id=" << vvMSController::instance()->getID() << "): valuators.size() is " << valuators.size() << ", should be " << nValuators << std::endl;
            exit(1);
        }

        if (nBodies != trackingbodies.size())
        {
            std::cerr << "Input (id=" << vvMSController::instance()->getID() << "): trackingbodies.size() is " << trackingbodies.size() << ", should be " << nBodies << std::endl;
            exit(1);
        }
