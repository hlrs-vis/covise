...
class VVCORE_EXPORT vvInteractor
{
public:
    vvInteractor();
    ...
    virtual bool isSameModule(vvInteractor *i) const = 0;

    /// returns true, if Interactor is exactly the same as interactor i;
    virtual bool isSame(vvInteractor *i) const = 0;
    ...
    virtual vvRenderObject *getObject() = 0;
...
class VVCORE_EXPORT InteractorReference
{
public:
    InteractorReference(vive::vvInteractor *inter)
    : m_interactor(inter)
    {
        if (m_interactor)
    ...
    vive::vvInteractor *interactor() const
    {
        return m_interactor;
    }

private:
    vive::vvInteractor *m_interactor = nullptr;
};
