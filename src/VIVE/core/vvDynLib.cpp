...
//
//
int vvDynLib::dlclose(CO_SHLIB_HANDLE handle)
{
...
const char *vvDynLib::dlerror(void)
{
...
CO_SHLIB_HANDLE vvDynLib::dlopen(const std::string &filename, bool showErrors)
{
...
CO_SHLIB_HANDLE try_dlopen(const char *filename, bool showErrors)
{
    ...
    if (handle == NULL)
    {
        if (vvPluginSupport::instance()->debugLevel(2) && showErrors)
            cerr << "vvDynLib::try_dlopen(" << filename << ") failed: " << vvDynLib::dlerror() << endl;
    }
    else
    {
...
CO_SHLIB_HANDLE vvDynLib::dlopen(const char *filename, bool showErrors)
{
    ...
    {
        cerr << "vvDynLib::dlopen() error: " << dlerror() << endl;
...
void *vvDynLib::dlsym(CO_SHLIB_HANDLE handle, const char *symbolname)
{
...
std::string vvDynLib::libName(const std::string &name)
{
