...
#undef KP
    ...
    if (numEventsToSync)
    {
        vvMSController::instance()->syncData(&eventQueue[0], numEventsToSync * sizeof(eventQueue[0]));
    }
    ...
    while (index < eventQueue.size())
    {
        ...
        vvVIVE::instance()->handleEvents(event.event, event.mod, event.key);
        index++;
...
bool MSEventHandler::handle(const osgGA::GUIEventAdapter &ea, osgGA::GUIActionAdapter &)
{
    ...
    {
        //modBuffer[numEventsToSync] = (int) (((ea.getXnormalized()+1.0)/2.0) * vv->windows[0].sx);
        //keyBuffer[numEventsToSync] = (int) (((ea.getYnormalized()+1.0)/2.0) * vv->windows[0].sy);
