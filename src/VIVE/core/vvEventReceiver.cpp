...
#ifdef _WIN32
void vvEventReceiver::_receiveLoop(void *receiver)
#else
void *vvEventReceiver::_receiveLoop(void *receiver)
#endif
{

    vvEventReceiver *er = (vvEventReceiver *)receiver;
...
vvEventReceiver::vvEventReceiver(int port, int window)
    : _port(port)
    , _windowID(window)
{
    _socket = -1;
...
vvEventReceiver::~vvEventReceiver()
{
}

void vvEventReceiver::_createSocket()
{
#ifdef _WIN32
    WSADATA wsaData;
...
void vvEventReceiver::_disconnectSocket()
{
    //closeSocket(_socket);
}

void vvEventReceiver::_mainLoop()
{
#ifdef _WIN32
    // Not used: "uintptr_t thread = "
    _beginthread(vvEventReceiver::_receiveLoop, 0, this);
#else
    pthread_t thread;
    pthread_create(&thread, NULL, vvEventReceiver::_receiveLoop, this);
#endif
}

void vvEventReceiver::_readData()
{
#ifdef _WIN32
    if (_socket == INVALID_SOCKET)
#else
...
// check wether we already received package
    ...
    if (bytes <= 0)
    {
        ...
        {
            //fprintf(stderr, "vvEventReceiver: no data received\n");
        }
    }
    ...
    if (numbytes < 0)
    {
        fprintf(stderr, "vvEventReceiver: error reading\n");
    } // fi
    ...
    // fprintf(stderr, "vvEventReceiver::read %s %d [%d, %d, %d]\n", msg, numRead, param1, param2, param3);

    // get event queue
    if (!vvConfig::instance()->windows[_windowID].window)
        return;
    /*osgGA::EventQueue* queue = vvConfig::instance()->windows[_windowID].window->getEventQueue();*/
    ...
    else if (numRead == 3)
    {
        ...
        {
            vvWindows::instance()->setOrigHSize(_windowID, param1);
            vvWindows::instance()->setOrigVSize(_windowID, param2);
           // vvConfig::instance()->windows[_windowID].window->resized(0, 0, param1, param2);
        }
