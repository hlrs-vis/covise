...
coVRDePee::coVRDePee(vsg::Group* parent, vsg::Node* subgraph, unsigned width, unsigned height)
{
...
bool coVRDePee::createHUD()
{
    ...
    vsg::vec3 position(5.0f,7.0f,0.0f);
    vsg::vec3 delta(0.0f,-120.0f,0.0f);
...

    {
      ...
      _hudText->setColor(vsg::vec4(0.5, 0.5, 0.5, 1.0));
...

    {
      ...
      vertices->push_back(vsg::vec3(bb.xMin(),bb.yMax(),depth));
      vertices->push_back(vsg::vec3(bb.xMin(),bb.yMin(),depth));
      vertices->push_back(vsg::vec3(bb.xMax(),bb.yMin(),depth));
      vertices->push_back(vsg::vec3(bb.xMax(),bb.yMax(),depth));
      ...
      colors->push_back(vsg::vec4(0.0f,0.0,0.0f,0.3f));
    ...
    camera->setProjectionMatrix(vsg::dmat4::ortho2D(0,1280,0,1024));
...
bool coVRDePee::createFinal()
{
  ...
  screenAlignedProjectionMatrix->matrix = (vsg::dmat4::ortho2D(0,_width,0,_height));
  screenAlignedProjectionMatrix->setCullingActive(false);

  vsg::MatrixTransform* screenAlignedModelViewMatrix = vsg::MatrixTransform::create();
  screenAlignedModelViewMatrix->matrix = (vsg::dmat4::identity());

  // Make sure the model view matrix is not affected by any transforms
  // above it in the scene graph:
  screenAlignedModelViewMatrix->setReferenceFrame(vsg::MatrixTransform::ABSOLUTE_RF);
  ...
  vsg::ref_ptr<osg::Program> program = new osg::Program;
...
//create the edge map of the first normal and depth map
{
  ...
  _coVRDePeePasses.back()->Cameras[EDGE_MAP]->setClearColor(vsg::vec4(0.3,0.3f,0.3f,1.0f));
  ...
  //set view
  _coVRDePeePasses.back()->Cameras[EDGE_MAP]->setReferenceFrame(vsg::MatrixTransform::ABSOLUTE_RF);

  _coVRDePeePasses.back()->Cameras[EDGE_MAP]->setViewMatrixAsLookAt(vsg::vec3(0.0f,0.0f,2.0f)*bs.radius(), vsg::vec3(0.0,0.0,0.0),vsg::vec3(0.0f,1.0f,0.0f));
...
bool coVRDePee::createNormalDepthColorMap(MapMode mapMode, bool first)
{
  ...
  // set up the background color and clear mask
  pass->Cameras[mapMode]->setClearColor(vsg::vec4(0.f,0.f,1.f,1.f));
  ...
  // setup view
  pass->Cameras[mapMode]->setReferenceFrame(vsg::MatrixTransform::ABSOLUTE_RF);

  pass->Cameras[mapMode]->setViewMatrixAsLookAt(bs.center()-vsg::vec3(0.0f,2.0f,0.0f)*bs.radius(),
							       bs.center(),
  ...
  stateset->setMode(GL_LIGHTING,osg::StateAttribute::OVERRIDE |
	...
	pass->Cameras[mapMode]->setClearColor(vsg::vec4(1.f,1.f,1.f,1.f));
	stateset->setMode(GL_LIGHTING,osg::StateAttribute::OVERRIDE | osg::StateAttribute::OFF);
...
{
  const char *file = vvFileManager::instance()->getName(fName);
