...
std::string vvSidecarConfigBridge::Key::configSection() const
{
    std::locale C("C");
...
std::string vvSidecarConfigBridge::Key::configName() const
{
    return name;
}
...
vvSidecarConfigBridge::~vvSidecarConfigBridge()
{
    if (!m_save)
        return;
                                       ...
                                       bool seen)
{
    ...
    if (const auto *value = dynamic_cast<const vive::config::Value<V> *>(entry))
    {
        if (!seen && node)
        {
            if (auto v = node->as<V>())
    ...
    if (const auto *array = dynamic_cast<const vive::config::Array<V> *>(entry))
    {
        if (!seen && node)
        {
...
bool vvSidecarConfigBridge::wasChanged(const vive::config::ConfigBase *entry)
{
