...
#define CLUSTER_MARK() \
    ;                  \
    vvMSController::msController->checkMark(__FILE__, __LINE__);
#else
#define CLUSTER_MARK() ;
#endif
...
class Rel_Mcast;
class coClusterStat;
class VVCORE_EXPORT vvMSController
{
public:
    enum
    {
    ...
    static vvMSController *instance();
    static void destroy();
    vvMSController(int AmyID = -1, const char *addr = NULL, int port = 0);
#ifdef HAS_MPI
    vvMSController(const MPI_Comm *comm, pthread_barrier_t *shmBarrier = nullptr);
#endif
    ~vvMSController();
    void startSlaves();
...
private:
    ...
    vvSlave *slaves[MAX_NUMBER_OF_SLAVES];
    ...
    static vvMSController *s_singleton;
};

extern template VVCORE_EXPORT std::vector<signed short> vvMSController::syncVector(const std::vector<signed short> &vec);
extern template VVCORE_EXPORT std::vector<unsigned short>
vvMSController::syncVector(const std::vector<unsigned short> &vec);
extern template VVCORE_EXPORT std::vector<signed> vvMSController::syncVector(const std::vector<signed> &vec);
extern template VVCORE_EXPORT std::vector<unsigned> vvMSController::syncVector(const std::vector<unsigned> &vec);
extern template VVCORE_EXPORT std::vector<signed long> vvMSController::syncVector(const std::vector<signed long> &vec);
extern template VVCORE_EXPORT std::vector<unsigned long>
vvMSController::syncVector(const std::vector<unsigned long> &vec);
extern template VVCORE_EXPORT std::vector<signed long long>
vvMSController::syncVector(const std::vector<signed long long> &vec);
extern template VVCORE_EXPORT std::vector<unsigned long long>
vvMSController::syncVector(const std::vector<unsigned long long> &vec);
extern template VVCORE_EXPORT std::vector<float> vvMSController::syncVector(const std::vector<float> &vec);
extern template VVCORE_EXPORT std::vector<double> vvMSController::syncVector(const std::vector<double> &vec);
