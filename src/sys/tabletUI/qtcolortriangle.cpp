...
/*!
   ...
   called \a name.
*/
ColorTriangle::ColorTriangle(QWidget *parent)
    : QFrame(parent)
    , bg(sizeHint(), QImage::Format_RGB32)
    , selMode(Idle)
{
    setLineWidth(1);
    setMidLineWidth(1);
    ...
    for (int y = 0; y < bg.height(); ++y)
    {
        ...
        for (int x = 0; x < bg.width(); ++x)
        {
            ...
            if (radius < outerRadius && radius > innerRadius)
            {
                ...
                double angle = angle2 - PI / 2.0;
                if (angle < 0)
                    angle += TWOPI;
                color.setHsv(360 - (int)((angle * 360.0) / TWOPI), 255, 255);

                // Shade the color depending on	where on the donut
                // this	point is.
                if (radius < innerRadius + border || radius > outerRadius - border)
                {
                    QColor bc = palette().mid().color();

                    int r = (color.red() + bc.red()) / 2;
                    int g = (color.green() + bc.green()) / 2;
                    int b = (color.blue() + bc.blue()) / 2;

                    double n = angle2 + PI / 4;
                    if (n < 0)
                        n += TWOPI;

                    double fact;
                    if (radius > outerRadius - border)
                        fact = (1.5 - cos(n) / 2);
                    else
                        fact = (1.5 + cos(n) / 2);
...
*/
void ColorTriangle::paintEvent(QPaintEvent *e)
{
    QFrame::paintEvent(e);
...
*/
DoublePoint ColorTriangle::movePointToTriangle(double x, double y, const Vertex &a,
...
void ColorDisplay::paintEvent(QPaintEvent *e)
{
    QFrame::paintEvent(e);
    QPainter p(this);
...
/*!	\internal
   ...
   Sets the current color value of	the	color display, and creates a
   tooltip	that shows the hex triplet.
*/
...
/*!
   Constructs a QtColorTriangle. The \a parent	and	\a name	arguments
   are	passed to QWidget's	constructor.
*/
    ...
    : QWidget(parent)
{
    ...
    connect(triangle, SIGNAL(colorChanged(const QColor &)), display, SLOT(setColor(const QColor &)));
    connect(triangle, SIGNAL(colorChanged(const QColor &)), SIGNAL(colorChanged(const QColor &)));
    connect(triangle, SIGNAL(released(const QColor &)), display, SLOT(setColor(const QColor &)));
    connect(triangle, SIGNAL(released(const QColor &)), SIGNAL(released(const QColor &)));
}
