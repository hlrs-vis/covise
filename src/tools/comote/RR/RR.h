...
{
    unsigned int size;
    /* If this frame is compressed, the size (in bytes)
                                  of the compressed image that represents it */
    unsigned int winid;
    /* The ID of the window on the client into which
    ...
    unsigned char subsamp;
    /* Chrominance subsampling of destination JPEG
    ...
    /* Display number on the client machine that
...
typedef struct _rrframeheader_v1
{
    ...
    unsigned char qual; /* Quality of destination JPEG (1-100) */
    unsigned char subsamp;
    /* YUV subsampling of destination JPEG
                                  (RR_411, RR_422, or RR_444) */
    unsigned char flags; /* See enum below */
    unsigned char dpynum;
    /* Display number on the client machine that
                                  contains the window into which this frame will be
                                  drawn */
} rrframeheader_v1;
...
{
    RR_EOF = 1,
    /* this tile is an End-of-Frame marker and contains no real
                  image data */
