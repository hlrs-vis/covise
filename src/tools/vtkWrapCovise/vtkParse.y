...
void SetParamType(Param *p, int t)
{
...
var_array: { $<integer>$ = 0; }
     | ARRAY_NUM { char temp[100]; snprintf(temp, sizeof(temp),"[%i]",$<integer>1); 
                   postSig(temp); } 
...
type_primitive: 
  ...
  ID     {       
      char ctmpid[2048];
      snprintf(ctmpid, sizeof(ctmpid),"%s ",$<str>1);
      postSig(ctmpid);
      $<integer>$ = 0x8;} |
  VTK_ID  
    { 
...
macro:
  SetMacro '(' any_id ',' 
   ...
   {
   postSig(");");
   snprintf(temps, sizeof(temps),"Set%s",$<str>3); 
   ...
   {
      p = dynamic_cast<ScalarParam *>(paramMap[name]);
      assert(p);
   }
   int t = $<integer>6;
   SetParamType(p, t);
   p->setter = true;
   p->ispublic &= in_public;
   }
| GetMacro '('{postSig("Get");} any_id ',' {postSig(" ();"); invertSig = 1;} 
    type_red2 ')'
   { 
   snprintf(temps, sizeof(temps),"Get%s",$<str>4); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   currentFunction->ReturnType = $<integer>7;
   output_function();

   std::string name($<str>4);
   ParamMap::iterator it = paramMap.find(name);
   ScalarParam *p = NULL;
   if(it == paramMap.end())
   {
...
| SetStringMacro '(' {preSig("void Set");} any_id ')'
   {
   postSig(" (char *);"); 
   snprintf(temps, sizeof(temps),"Set%s",$<str>4); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 1;
   ...
   {
      p = dynamic_cast<StringParam *>(paramMap[name]);
      assert(p);
   }
   p->setter = true;
...
| SetClampMacro  '(' any_id ',' 
   ...
   snprintf(temps, sizeof(temps),"Get%sMinValue",$<str>3);
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   ...
   snprintf(temps, sizeof(temps),"Get%sMaxValue",$<str>3);
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   currentFunction->ReturnType = $<integer>6;
   output_function();
...
| BooleanMacro '(' any_id 
    {preSig("void "); postSig("On ();"); openSig = 0; } 
        ',' type_red2 ')'
   { 
   snprintf(temps, sizeof(temps),"%sOn",$<str>3); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   currentFunction->ReturnType = 0x2;
   output_function();
   ...
   snprintf(temps, sizeof(temps),"%sOff",$<str>3); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   output_function();
...
| SetVector2Macro '(' any_id ',' 
   ...
   {
      p = dynamic_cast<VecParam *>(paramMap[name]);
      assert(p);
   }
   int t = $<integer>6;
   SetParamType(p, t);
...
| GetVector2Macro  '(' any_id ',' 
   ...
   snprintf(temps, sizeof(temps),"Get%s",$<str>3); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
...
| ViewportCoordinateMacro '(' any_id ')'
   { 
     ...
     snprintf(temps, sizeof(temps),"Get%sCoordinate",$<str>3); 
     currentFunction->Name = vtkstrdup(temps);
     currentFunction->NumberOfArguments = 0;
     currentFunction->ReturnType = 0x309;
...
| TypeMacro '(' any_id ',' any_id ')'
   ...
   snprintf(temps, sizeof(temps),"GetClassName"); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   ...
   snprintf(temps, sizeof(temps),"IsA"); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 1;
   ...
   sprintf(currentFunction->Signature, "%s *NewInstance ();",
           $<str>3);
   snprintf(temps, sizeof(temps),"NewInstance"); 
   currentFunction->Name = vtkstrdup(temps);
   currentFunction->NumberOfArguments = 0;
   currentFunction->ReturnType = 0x309;
   ...
   if ( data.IsConcrete )
     ...
     sprintf(currentFunction->Signature, "%s *SafeDownCast (vtkObject* o);",
             $<str>3);
     snprintf(temps, sizeof(temps),"SafeDownCast"); 
     currentFunction->Name = vtkstrdup(temps);
     currentFunction->NumberOfArguments = 1;
     currentFunction->ArgTypes[0] = 0x309;
     currentFunction->ArgCounts[0] = 1;
...
/* move a function structure */
void MoveFunction(FunctionInfo *d, FunctionInfo *s)
{
  d->Name = s->Name; s->Name = NULL;
  d->ClassName = s->ClassName; s->Name = NULL;
  d->NumberOfArguments = s->NumberOfArguments;
  for(int i=0; i<MAX_ARGS; ++i)
  {
     d->ArgCounts[i] = s->ArgCounts[i];
     d->ArgTypes[i] = s->ArgTypes[i];
     d->ArgClasses[i] = s->ArgClasses[i]; s->ArgClasses[i] = NULL;
  }
  d->ArrayFailure = s->ArrayFailure;
  d->IsPureVirtual = s->IsPureVirtual;
  d->IsPublic = s->IsPublic;
  d->IsOperator = s->IsOperator;
  d->HaveHint = s->HaveHint;
  d->HintSize = s->HintSize;
  d->ReturnType = s->ReturnType;
  d->ReturnClass = s->ReturnClass; s->ReturnClass=NULL;
  d->Comment = s->Comment; s->Comment=NULL;
  d->Signature = s->Signature; s->Signature = NULL;
  d->IsLegacy = s->IsLegacy;
  InitFunction(s);
}
...
std::string canonify(const std::string &sig)
{
   std::string ret = sig;
   size_t found = ret.rfind(" = 0;");
   if(found == ret.length()-5)
       ret= ret.substr(0, ret.length()-5)+";";
   found = ret.find("virtual ");
   if(found == 0)
      ret=ret.substr(8, ret.length());
...
output_function()
{
  ...
  if (HaveComment)
   ...
   currentFunction->ClassName = vtkstrdup(data.ClassName);

#ifdef DEBUG
   if(!currentFunction->Name)
   {
   fprintf(stderr, "no name for %s\n", currentFunction->Signature);
   }
#endif

  int override = 0;
  for(i=0; i<data.NumberOfFunctions-1; ++i)
  {
      override = 0;
      if(currentFunction->Name && data.Functions[i].Name
      && !strcmp(currentFunction->Name, data.Functions[i].Name))
      {
         if(currentFunction->NumberOfArguments==data.Functions[i].NumberOfArguments)
         {
            override = 1;
            for(int j=0; j<currentFunction->NumberOfArguments; ++j)
            {
               if(currentFunction->ArgTypes[j] != data.Functions[i].ArgTypes[j])
               {
                  override = 0;
                  break;
               }
      ...
      {
#ifdef DEBUG
         fprintf(stderr, "overriding:\n%s\n%s\n\n",
            currentFunction->Signature, data.Functions[i].Signature);
#endif
         MoveFunction(&data.Functions[i], currentFunction);
         break;
      }
  }
