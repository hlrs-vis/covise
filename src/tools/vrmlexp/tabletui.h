...
public:
    ...
    const TCHAR* GetObjectName(bool localized) const override { return localized ? GetString(IDS_TABLETUI) : _T("TabletUI"); }
#else
...
#if MAX_PRODUCT_VERSION_MAJOR > 23
    void GetClassName(MSTR& s, bool localized) const override { s = localized ? GetString(IDS_TABLETUI_CLASS) : _T("TabletUI"); }
#else

    void GetClassName(TSTR& s)
    {
        s = GetString(IDS_TABLETUI_CLASS);
    }
#endif
    int IsKeyable()
    {
