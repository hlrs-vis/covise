...
void
ToolParameterSettings::addListUI(unsigned int paramIndex, ToolParameter *p)
{
    addParamUI(paramIndex, p);
    setLabels(paramIndex, QString("%1 elements selected").arg(tool_->getObjectCount(p->getToolId(), p->getParamToolId())), buttonGroup_->button(paramIndex)->text());
}
    ...
    if (p->getType() == ToolParameter::ParameterTypes::OBJECT_LIST)
    {
        ...
        if (objectCount == tool_->getListSize())
        {
            ...
            do
            {
                ...
                {
                    addParamUI(paramIndex, p);  // paramIt.value statt p ???
                    break;
                }
...
ToolParameterSettings::generateUI(QFrame *box)
{
    ...
    QList<ToolParameter *>invalidParams;
    QMap<unsigned int, ToolParameter *>::const_iterator paramIt = params_->constBegin();
    while (paramIt != params_->constEnd())
    {
        ToolParameter *param = paramIt.value();
        if (param->isValid())
        {
            addParamUI(paramIt.key(), param);
        }
        else
        {
    ...
    while (it != paramList_->constEnd())
    {
        ToolParameter *lastParam = it.value().last();
        addListUI(it.key(), lastParam);
        foreach(ToolParameter * param, invalidParams)
        {
            if ((param->getToolId() == lastParam->getToolId()) && (param->getParamToolId() == lastParam->getParamToolId()))
            {
                addParamUI(params_->key(param), param);
                invalidParams.removeOne(param);
                continue;
            }
        }
        it++;
