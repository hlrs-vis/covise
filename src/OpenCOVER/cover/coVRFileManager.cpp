...
int coVRFileManager::isUTF8(const char* data, size_t size)
{
...
osg::Node *coVRFileManager::loadFile(const char *fileName, coTUIFileBrowserButton *fb, osg::Group *parent, const char *covise_key)
{
    ...
    if (isUTF8(fileName, strlen(fileName)))
    {

        validFileName = boost::locale::conv::from_utf<char>(validFileName, "ISO-8859-1");
    }
