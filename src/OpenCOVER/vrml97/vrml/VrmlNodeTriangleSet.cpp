...
{
    //cerr << "VrmlNodeTriangleSet::insertGeometry" << endl;
    ...
    if (d_coord.get())
    {
        ...
        int coordIndexSize = numPoints * 4;
        int *coordIndexData = new int[coordIndexSize];

        for (int i = 0; i < numPoints; i++)
        {
            for (int j = 0; j < 3; j++)
                coordIndexData[i * 4 + j] = i * 3 + j;
        ...
        {
            std::cerr << "Warning: According to chapter 11.4.13 TriangleSet of "
