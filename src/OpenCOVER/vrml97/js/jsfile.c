...
#define JSFILE_CHECK_NATIVE(op)                                                                 \
    if (file->isNative)                                                                         \
    {                                                                                           \
        JS_ReportWarning(cx, "Cannot call or access \"%s\" on native file %s", op, file->path); \
        goto out;                                                                               \
    }
...
#define JSFILE_CHECK_WRITE                                                                           \
    if (!file->isOpen)                                                                               \
    {                                                                                                \
        JS_ReportWarning(cx, "File %s is closed, will open it for writing, proceeding", file->path); \
        js_FileOpen(cx, obj, file, "write,append,create");                                           \
    }                                                                                                \
    if (!js_canWrite(cx, file))                                                                      \
    {                                                                                                \
        JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL, JSFILEMSG_CANNOT_WRITE, file->path);  \
        goto out;                                                                                    \
    }
...
#define JSFILE_CHECK_READ                                                                            \
    if (!file->isOpen)                                                                               \
    {                                                                                                \
        JS_ReportWarning(cx, "File %s is closed, will open it for reading, proceeding", file->path); \
        js_FileOpen(cx, obj, file, "read");                                                          \
    }                                                                                                \
    if (!js_canRead(cx, file))                                                                       \
    {                                                                                                \
        JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL, JSFILEMSG_CANNOT_READ, file->path);   \
        goto out;                                                                                    \
    }
...
#define JSFILE_CHECK_OPEN(op)                                                                      \
    if (!file->isOpen)                                                                             \
    {                                                                                              \
        JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL, JSFILEMSG_FILE_MUST_BE_CLOSED, op); \
        goto out;                                                                                  \
    }
...
#define JSFILE_CHECK_CLOSED(op)                                                                  \
    if (file->isOpen)                                                                            \
    {                                                                                            \
        JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL, JSFILEMSG_FILE_MUST_BE_OPEN, op); \
        goto out;                                                                                \
    }
...
js_fileBaseName(JSContext *cx, const char *pathname)
{
    ...
    while (index > 0 && (pathname[index] == FILESEPARATOR || pathname[index] == FILESEPARATOR2))
    {
        --index;
    }
                                   ...
                                   uint16 onechar)
{
    ...
    {
    case 3:
        *--tobufp = (onechar | BYTE_MARK) & BYTE_MASK;
        onechar >>= 6;
        *--tobufp = (onechar | BYTE_MARK) & BYTE_MASK;
        onechar >>= 6;
...
utf8_to_ucs2_char(const unsigned char *utf8p, int16 buflen, uint16 *ucs2p)
{
    ...
    { /* not a valid utf8/ucs2 character */
        return -1;
    }
}
...
file_writeln(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (!file_write(cx, obj, argc, argv, rval))
        return JS_FALSE;
...
file_list(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (argc == 1)
    {
        ...
        else if (JSVAL_IS_FUNCTION(cx, argv[0]))
        {
...
file_toURL(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    *rval = STRING_TO_JSVAL(js_NewString(cx, urlChars, strlen(url), 0));
    if (!js_str_escape(cx, obj, 0, rval, rval))
        return JS_FALSE;
