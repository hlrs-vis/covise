        ...
        do
        {
            if (ts->linebuf.ptr == ts->linebuf.limit)
            {
                ...
                if (nl < ts->userbuf.limit)
                {
                    ...
                    {
                        if (nl > ts->userbuf.base && nl[-1] == '\r' && ts->linebuf.base[len - 2] == '\r')
                        {
                            len--;
                         ...
                         JSBool charArgs, va_list ap)
{
    ...
    if (onError)
    {
        ...
        if (onError)
        {
             ...
             */
            if (hook && !hook(cx, message, report, cx->runtime->debugErrorHookData))
            {
                onError = NULL;
            }
        }
...
GetXMLEntity(JSContext *cx, JSTokenStream *ts)
{
    ...
    if (length > 2 && bp[1] == '#')
    {
        ...
        else
        {
            ...
            if (c != 0x9 && c != 0xA && c != 0xD && !(0x20 <= c && c <= 0xD7FF) && !(0xE000 <= c && c <= 0xFFFD))
            {
                goto badncr;
            }
        }
...
js_PeekTokenSameLine(JSContext *cx, JSTokenStream *ts)
{
    ...
    JS_ASSERT(ts->lookahead == 0 || ON_CURRENT_LINE(ts, CURRENT_TOKEN(ts).pos) || ts->tokens[(ts->cursor + ts->lookahead) & NTOKENS_MASK].type == TOK_EOL);
...
GetUnicodeEscape(JSTokenStream *ts)
{
    ...
    if (PeekChars(ts, 5, cp) && cp[0] == 'u' && JS7_ISHEX(cp[1]) && JS7_ISHEX(cp[2]) && JS7_ISHEX(cp[3]) && JS7_ISHEX(cp[4]))
    {
        c = (((((JS7_UNHEX(cp[1]) << 4)
                + JS7_UNHEX(cp[2]))
               << 4)
...
NewToken(JSTokenStream *ts, ptrdiff_t adjust)
{
    ...
    tp->pos.begin.index = ts->linepos + PTRDIFF(tp->ptr, ts->linebuf.base, jschar) - ts->ungetpos;
        ...
        if (JS_ISXMLNSSTART(c))
        {
            ...
            while ((c = GetChar(ts)) != EOF && JS_ISXMLNAME(c))
            {
                if (c == ':')
                {
                    ...
                    if (sawColon || (nextc = PeekChar(ts),
                                     ((ts->flags & TSF_XMLONLYMODE) || nextc != '{') && !JS_ISXMLNAME(nextc)))
                    {
                        js_ReportCompileErrorNumber(cx, ts,
                                                    JSREPORT_TS | JSREPORT_ERROR,
                                                    JSMSG_BAD_XML_QNAME);
...
retry:
    ...
    if (JS_ISIDSTART(c) || (c == '\\' && (c = GetUnicodeEscape(ts),
                                          hadUnicodeEscape = JS_ISIDSTART(c))))
    {
        ...
        if (!hadUnicodeEscape && ATOM_KEYWORD(atom))
        {
            ...
            else if (JS_VERSION_IS_ECMA(cx) || kw->version <= (cx->version & JSVERSION_MASK))
            {
                tt = kw->tokentype;
                tp->t_op = (JSOp)kw->op;
                goto out;
        ...
        while ((c = GetChar(ts)) != qc)
        {
            ...
            if (c == '\\')
            {
                ...
                default:
                    ...
                    {
                        jschar cp[4];
                        if (PeekChars(ts, 4, cp) && JS7_ISHEX(cp[0]) && JS7_ISHEX(cp[1]) && JS7_ISHEX(cp[2]) && JS7_ISHEX(cp[3]))
                        {
                            c = (((((JS7_UNHEX(cp[0]) << 4)
                                    + JS7_UNHEX(cp[1]))
                                   << 4)
                    ...
                    {
                        jschar cp[2];
                        if (PeekChars(ts, 2, cp) && JS7_ISHEX(cp[0]) && JS7_ISHEX(cp[1]))
                        {
    ...
    case '<':
         ...
         */
        if ((ts->flags & TSF_OPERAND) && (JS_HAS_XML_OPTION(cx) || PeekChar(ts) != '!'))
        {
            ...
            if (MatchChar(ts, '!'))
            {
                ...
                {
                    jschar cp[6];
                    if (PeekChars(ts, 6, cp) && cp[0] == 'C' && cp[1] == 'D' && cp[2] == 'A' && cp[3] == 'T' && cp[4] == 'A' && cp[5] == '[')
                    {
                        SkipChars(ts, 6);
                        while ((c = GetChar(ts)) != ']' || !PeekChars(ts, 2, cp) || cp[0] != ']' || cp[1] != '>')
                        {
                            if (c == EOF)
                                goto bad_xml_markup;
                            ADD_TO_TOKENBUF(c);
    ...
    case '/':
        if (MatchChar(ts, '/'))
        {
            ...
            if (JS_HAS_ATLINE_OPTION(cx))
            {
                ...
                if (PeekChars(ts, 5, cp) && cp[0] == '@' && cp[1] == 'l' && cp[2] == 'i' && cp[3] == 'n' && cp[4] == 'e')
                {
                    SkipChars(ts, 5);
                    while ((c = GetChar(ts)) != '\n' && JS_ISSPACE(c))
                        continue;
        ...
        if (MatchChar(ts, '*'))
        {
            while ((c = GetChar(ts)) != EOF && !(c == '*' && MatchChar(ts, '/')))
            {
    ...
    case '#':
    {
        ...
        if (JS_HAS_STRICT_OPTION(cx) && (c == '=' || c == '#'))
        {
            char buf[20];
...
eol_out:
    ...
    tp->pos.end.index = ts->linepos + PTRDIFF(ts->linebuf.ptr, ts->linebuf.base, jschar) - ts->ungetpos;
    tp->type = tt;
    return tt;
