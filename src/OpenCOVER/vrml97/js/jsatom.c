...
#define FROB(lval, str)                                                                                 \
    JS_BEGIN_MACRO if (!(state->lval = js_Atomize(cx, str, strlen(str), ATOM_PINNED))) return JS_FALSE; \
    JS_END_MACRO
...
js_atom_marker(JSHashEntry *he, intN i, void *arg)
{
    ...
    atom = (JSAtom *)he;
    args = (MarkArgs *)arg;
    if ((atom->flags & (ATOM_PINNED | ATOM_INTERNED)) || (args->gcflags & GC_KEEP_ATOMS))
    {
