...
#define JS_ARENA_ALLOCATE_CAST(p, type, pool, nb)  \
    JS_BEGIN_MACRO JSArena *_a = (pool)->current;  \
...
#define JS_ARENA_RELEASE(pool, mark)                                                           \
    JS_BEGIN_MACRO char *_m = (char *)(mark);                                                  \
    JSArena *_a = (pool)->current;                                                             \
    if (_a != &(pool)->first && JS_UPTRDIFF(_m, _a->base) <= JS_UPTRDIFF(_a->avail, _a->base)) \
    {                                                                                          \
...
#define JS_ARENA_DESTROY(pool, a, pnext)     \
    JS_BEGIN_MACRO JS_COUNT_ARENA(pool, --); \
    if ((pool)->current == (a))              \
