...
JS_InitArenaPool(JSArenaPool *pool, const char *name, size_t size, size_t align)
{
    ...
    pool->first.base = pool->first.avail = pool->first.limit = JS_ARENA_ALIGN(pool, &pool->first + 1);
...
#define PTR_TO_HEADER(pool, p) (JS_ASSERT(((jsuword)(p)&HEADER_BASE_MASK(pool)) \
                                          == 0),                                \
                                (JSArena ***)(p)-1)
    ...
    for (a = pool->current; a->avail > a->limit - nb; pool->current = a)
    {
        ...
        claim:
            ...
            *ap = a = b;
            ...
            if (extra)
            {
                a->base = a->avail = ((jsuword)a + hdrsz) & ~HEADER_BASE_MASK(pool);
