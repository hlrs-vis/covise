...
#define PN_MOVE_NODE(pn, pn2)                     \
    JS_BEGIN_MACRO(pn)->pn_type = (pn2)->pn_type; \
    (pn)->pn_op = (pn2)->pn_op;                   \
...
#define PN_CLEAR_NODE(pn)                  \
    JS_BEGIN_MACRO(pn)->pn_type = TOK_EOF; \
    (pn)->pn_op = JSOP_NOP;                \
...
#define PN_IS_CONSTANT(pn) \
    ((pn)->pn_type == TOK_NUMBER || (pn)->pn_type == TOK_STRING || ((pn)->pn_type == TOK_PRIMARY && (pn)->pn_op != JSOP_THIS))
...
#define PN_APPEND(list, pn)                 \
    JS_BEGIN_MACRO *(list)->pn_tail = (pn); \
    (list)->pn_tail = &(pn)->pn_next;       \
    (list)->pn_count++;                     \
    JS_END_MACRO
