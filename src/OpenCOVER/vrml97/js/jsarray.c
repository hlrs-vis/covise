...
js_IdIsIndex(jsval id, jsuint *indexp)
{
    ...
    if (JS7_ISDEC(*cp) && JSSTRING_LENGTH(str) < sizeof(MAXSTR))
    {
        ...
        if (*cp == 0 && (oldIndex < (MAXINDEX / 10) || (oldIndex == (MAXINDEX / 10) && c < (MAXINDEX % 10))))
        {
            *indexp = index;
            return JS_TRUE;
        }
...
#define v (*rval)
    ...
    for (index = 0; index < length; index++)
    {
        ...
        if ((op != TO_SOURCE || JS_VERSION_IS_1_2(cx)) && (JSVAL_IS_VOID(v) || JSVAL_IS_NULL(v)))
        {
            str = cx->runtime->emptyString;
            if (op == TO_SOURCE)
            {
        ...
        else
        {
            ...
            {
                if (!js_ValueToObject(cx, v, &obj2) || !js_TryMethod(cx, obj2, cx->runtime->atomState.toLocaleStringAtom, 0, NULL, &v))
                {
                    str = NULL;
                }
                else
              ...
              jsval *rval)
{
    ...
    for (i = 0; i < half; i++)
    {
        ...
        if (!PropertyExists(cx, obj, id, &idexists) || !PropertyExists(cx, obj, id2, &id2exists))
        {
            return JS_FALSE;
        }
         ...
         */
        if (!OBJ_GET_PROPERTY(cx, obj, id, tmproot) || !OBJ_GET_PROPERTY(cx, obj, id2, tmproot2))
        {
            return JS_FALSE;
        }
