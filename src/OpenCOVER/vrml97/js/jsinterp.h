...
#define PCE_LOAD(cache, pce, entry)                                     \
    JS_BEGIN_MACRO uint32 prefills_;                                    \
...
#define PCE_STORE(cache, pce, entry)                                                           \
    ...
    while (PCE_OBJECT(*pce) != PCE_OBJECT(entry) || PCE_PROPERTY(*pce) != PCE_PROPERTY(entry)) \
        ;                                                                                      \
    JS_END_MACRO
...
#define PROPERTY_CACHE_FILL(cache, obj, id, sprop)                          \
    JS_BEGIN_MACRO JSPropertyCache *cache_ = (cache);                       \
    if (!cache_->disabled)                                                  \
    {                                                                       \
        ...
        PCMETER(if (pce_sprop_ && pce_sprop_ != sprop) cache_->recycles++); \
...
#define PROPERTY_CACHE_TEST(cache, obj, id, sprop)                         \
    JS_BEGIN_MACRO uintN hashIndex_ = (uintN)PROPERTY_CACHE_HASH(obj, id); \
    ...
    if (pce_sprop_ && PCE_OBJECT(entry_) == obj && pce_sprop_->id == id)   \
    {                                                                      \
        sprop = pce_sprop_;                                                \
    }                                                                      \
    else                                                                   \
