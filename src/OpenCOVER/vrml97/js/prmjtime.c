...
#define IS_LEAP(year) \
    (year != 0 && ((((year & 0x3) == 0) && ((year - ((year / 100) * 100)) != 0)) || (year - ((year / 400) * 400)) == 0))
...
PRMJ_DSTOffset(JSInt64 local_time)
{
    ...
    diff = ((tm.tm_hour - prtm.tm_hour) * PRMJ_HOUR_SECONDS) + ((tm.tm_min - prtm.tm_min) * 60);
...
PRMJ_basetime(JSInt64 tsecs, PRMJTime *prtm)
{
    ...
    while (((month == 1 && isleap) ? (mday >= mtab[month] + 1) : (mday >= mtab[month])))
    {
        yday += mtab[month];
        days += mtab[month];
