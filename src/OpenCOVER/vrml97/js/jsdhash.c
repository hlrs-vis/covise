                       ...
                       const void *key)
{
    ...
    return stub->key == key || (stub->key && key && strcmp(stub->key, key) == 0);
}
                            ...
                            float minAlpha)
{
    ...
    {
        maxAlpha = (float)(JS_DHASH_MIN_SIZE - JS_MAX(JS_DHASH_MIN_SIZE / 256, 1))
                   / JS_DHASH_MIN_SIZE;
    }
...
#define ENSURE_LIVE_KEYHASH(hash0) \
    if (hash0 < 2)                 \
        hash0 -= 2;                \
    else                           \
    (void)0
...
JS_DHashTableFinish(JSDHashTable *table)
{
    ...
    if (!dumpfp)
        dumpfp = fopen("/tmp/jsdhash.bigdump", "w");
    if (dumpfp)
    {
...
JS_PUBLIC_API(JSDHashEntryHdr *) JS_DHASH_FASTCALL JS_DHashTableOperate(JSDHashTable *table, const void *key, JSDHashOperator op)
{
    ...
    case JS_DHASH_ADD:
        ...
        if (!ENTRY_IS_LIVE(entry))
        {
            ...
            if (table->ops->initEntry && !table->ops->initEntry(table, entry, key))
            {
    ...
    case JS_DHASH_REMOVE:
        ...
        if (ENTRY_IS_LIVE(entry))
        {
            ...
            size = JS_DHASH_TABLE_SIZE(table);
            if (size > JS_DHASH_MIN_SIZE && table->entryCount <= MIN_LOAD(table, size))
            {
...
JS_DHashTableEnumerate(JSDHashTable *table, JSDHashEnumerator etor, void *arg)
{
     ...
     */
    if (didRemove && (table->removedCount >= capacity >> 2 || (capacity > JS_DHASH_MIN_SIZE && table->entryCount <= MIN_LOAD(table, capacity))))
    {
...
JS_DHashTableDumpMeter(JSDHashTable *table, JSDHashEnumerator dump, FILE *fp)
{
    ...
    fprintf(fp, "      mean steps per search: %g\n", table->stats.searches ? (double)table->stats.steps / table->stats.searches
                                                                           : 0.);
