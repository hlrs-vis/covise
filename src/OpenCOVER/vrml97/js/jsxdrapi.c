...
#define MEM_LEFT(xdr, bytes)                                                                   \
    JS_BEGIN_MACRO if ((xdr)->mode == JSXDR_DECODE && MEM_COUNT(xdr) + bytes > MEM_LIMIT(xdr)) \
    {                                                                                          \
        JS_ReportErrorNumber((xdr)->cx, js_GetErrorMessage, NULL, JSMSG_END_OF_DATA);          \
        return 0;                                                                              \
    }                                                                                          \
    JS_END_MACRO

#define MEM_NEED(xdr, bytes)                                               \
    JS_BEGIN_MACRO if ((xdr)->mode == JSXDR_ENCODE)                        \
    {                                                                      \
        if (MEM_LIMIT(xdr) && MEM_COUNT(xdr) + bytes > MEM_LIMIT(xdr))     \
        {                                                                  \
...
mem_seek(JSXDRState *xdr, int32 offset, JSXDRWhence whence)
{
    ...
    case JSXDR_SEEK_SET:
        ...
        return JS_TRUE;
    case JSXDR_SEEK_END:
        if (offset >= 0 || xdr->mode == JSXDR_ENCODE || (int32)MEM_LIMIT(xdr) + offset < 0)
        {
            JS_ReportErrorNumber(xdr->cx, js_GetErrorMessage, NULL,
                                 JSMSG_END_SEEK);
            return JS_FALSE;
...
JS_XDRString(JSXDRState *xdr, JSString **strp)
{
    ...
    if (xdr->mode == JSXDR_ENCODE)
        len = (uint32)JSSTRING_LENGTH(*strp);
