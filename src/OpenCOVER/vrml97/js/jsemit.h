...
#define SET_STATEMENT_TOP(stmt, top) \
    ((stmt)->update = (top), (stmt)->breaks = (stmt)->continues = (stmt)->catchJump = (stmt)->gosub = (-1))
...
#define CHECK_AND_SET_JUMP_OFFSET(cx, cg, pc, off)                          \
    JS_BEGIN_MACRO if (!js_SetJumpOffset(cx, cg, pc, off)) return JS_FALSE; \
    JS_END_MACRO
...
#define SN_MAKE_NOTE(sn, t, d) (*(sn) = (jssrcnote)(((t) << SN_DELTA_BITS) \
                                                    | ((d)&SN_DELTA_MASK)))
#define SN_MAKE_XDELTA(sn, d) (*(sn) = (jssrcnote)((SRC_XDELTA << SN_DELTA_BITS) \
                                                   | ((d)&SN_XDELTA_MASK)))
...
#define CG_COUNT_FINAL_SRCNOTES(cg, cnt)                                                                                    \
    JS_BEGIN_MACRO ptrdiff_t diff_ = CG_PROLOG_OFFSET(cg) - (cg)->prolog.lastNoteOffset;                                    \
    cnt = (cg)->prolog.noteCount + (cg)->main.noteCount + 1;                                                                \
    if ((cg)->prolog.noteCount && (cg)->prolog.currentLine != (cg)->firstLine)                                              \
    {                                                                                                                       \
        if (diff_ > SN_DELTA_MASK)                                                                                          \
            cnt += (uint32)(JS_HOWMANY(diff_ - SN_DELTA_MASK, SN_XDELTA_MASK));                                             \
    ...
    {                                                                                                                       \
        ...
        {                                                                                                                   \
            ...
            diff_ -= SN_IS_XDELTA(sn_) ? SN_XDELTA_MASK - (*sn_ & SN_XDELTA_MASK) : SN_DELTA_MASK - (*sn_ & SN_DELTA_MASK); \
        }                                                                                                                   \
        if (diff_ > 0)                                                                                                      \
            cnt += (uint32)(JS_HOWMANY(diff_, SN_XDELTA_MASK));                                                             \
    }                                                                                                                       \
    JS_END_MACRO
...
#define CG_COUNT_FINAL_TRYNOTES(cg, cnt)                                                                                   \
    JS_BEGIN_MACRO cnt = (uint32)(((cg)->tryNext > (cg)->tryBase) ? PTRDIFF(cg->tryNext, cg->tryBase, JSTryNote) + 1 : 0); \
    JS_END_MACRO
