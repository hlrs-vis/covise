...
js_NewGCThing(JSContext *cx, uintN flags, size_t nbytes)
{
    ...
    for (;;)
    {
         ...
         * arenaList.
         */
        if (rt->gcBytes < rt->gcMaxBytes && (tried_gc || rt->gcMallocBytes < rt->gcMaxBytes))
        {
            ...
            METER(arenaList->stats.maxthings = JS_MAX(arenaList->stats.nthings, arenaList->stats.maxthings));
            break;
        }
...
#define GC_TYPE_IS_XML(t) ((unsigned)((t)-GCX_NAMESPACE) <= (unsigned)(GCX_XML - GCX_NAMESPACE))
#define GC_TYPE_IS_DEEP(t) ((t) == GCX_OBJECT || GC_TYPE_IS_XML(t))

#define IS_DEEP_STRING(t, o) (GC_TYPE_IS_STRING(t) && JSSTRING_IS_DEPENDENT((JSString *)(o)))
...
js_LockGCThingRT(JSRuntime *rt, void *thing)
{
    ...
    if (lock || deep)
    {
        ...
        {
#ifdef DEBUG
            JSDHashEntryHdr *hdr = JS_DHashTableOperate(rt->gcLocksHash, thing,
                                                        JS_DHASH_LOOKUP);
...
js_UnlockGCThingRT(JSRuntime *rt, void *thing)
{
    ...
    if (flags & GCF_LOCK)
    {
        if (!rt->gcLocksHash || (lhe = (JSGCLockHashEntry *)
...
#define GC_MARK_JSVALS(cx, len, vec, name)                 \
    JS_BEGIN_MACRO jsval _v, *_vp, *_end;                  \
                                                           \
