...
XDRAtomMap(JSXDRState *xdr, JSAtomMap *map)
{
    ...
    if (xdr->mode == JSXDR_DECODE)
    {
        ...
        for (i = 0; i < length; i++)
        {
            ...
            if (!ale || !XDRAtomListElement(xdr, ale))
            {
                if (!ale)
...
js_XDRScript(JSXDRState *xdr, JSScript **scriptp, JSBool *hasMagic)
{
    ...
    if (xdr->mode == JSXDR_ENCODE)
    {
        ...
        prologLength = (uint32)(PTRDIFF(script->main, script->code, jsbytecode));
        ...
        for (sn = notes; !SN_IS_TERMINATOR(sn); sn = SN_NEXT(sn))
            continue;
        nsrcnotes = (uint32)(PTRDIFF(sn, notes, jssrcnote));
    ...
    if (xdr->mode == JSXDR_DECODE)
    {
        ...
        script = js_NewScript(cx, (uint32)alloclength, nsrcnotes, ntrynotes);
        if (!script)
            return JS_FALSE;
        if (magic >= JSXDR_MAGIC_SCRIPT_2)
        {
     ...
     */
    if (!JS_XDRBytes(xdr, (char *)script->code, length * sizeof(jsbytecode)) || !XDRAtomMap(xdr, &script->atomMap))
    {
        goto error;
    }
    ...
    if (!JS_XDRBytes(xdr, (char *)notes, nsrcnotes * sizeof(jssrcnote)) || !JS_XDRCStringOrNull(xdr, (char **)&script->filename) || !JS_XDRUint32(xdr, &lineno) || !JS_XDRUint32(xdr, &depth) || (magic < JSXDR_MAGIC_SCRIPT_4 && !JS_XDRUint32(xdr, &ntrynotes)))
    {
        goto error;
    }
    ...
    if (magic >= JSXDR_MAGIC_SCRIPT_3)
    {
        ...
        if (xdr->mode == JSXDR_ENCODE)
        {
            ...
            if (!JS_XDRUint32(xdr, &encodeable))
                goto error;
            if (encodeable && !cx->runtime->principalsTranscoder(xdr, &principals))
            {
                goto error;
            }
        }
    ...
    if (xdr->mode == JSXDR_DECODE)
    {
        ...
        if (magic < JSXDR_MAGIC_SCRIPT_4)
        {
            ...
            newscript = (JSScript *)JS_realloc(cx, script,
                                               sizeof(JSScript) + length * sizeof(jsbytecode) + nsrcnotes * sizeof(jssrcnote) + ntrynotes * sizeof(JSTryNote));
            if (!newscript)
                goto error;
            ...
            *scriptp = script = newscript;
            ...
            if (ntrynotes)
            {
                script->trynotes = (JSTryNote *)((jsword)(SCRIPT_NOTES(script) + nsrcnotes) & ~(jsword)JSTRYNOTE_ALIGNMASK);
    ...
    while (ntrynotes)
    {
        ...
        if (!JS_XDRUint32(xdr, &start) || !JS_XDRUint32(xdr, &catchLength) || !JS_XDRUint32(xdr, &catchStart))
        {
            goto error;
        }
...
js_InitRuntimeScriptState(JSRuntime *rt)
{
    ...
    rt->scriptFilenameTable = JS_NewHashTable(16, JS_HashString, js_compare_strings, NULL,
...
js_NewScriptFromCG(JSContext *cx, JSCodeGenerator *cg, JSFunction *fun)
{
    ...
    mainLength = (uint32)(CG_OFFSET(cg));
    prologLength = (uint32)(CG_PROLOG_OFFSET(cg));
