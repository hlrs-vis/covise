...
args_setProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    default:
        if (fp->fun->interpreted && (uintN)slot < fp->argc && !ArgWasDeleted(cx, fp, slot))
        {
            fp->argv[slot] = *vp;
        }
        break;
             ...
             JSObject **objp)
{
    ...
    if (prop)
    {
        ...
        if ((sprop->flags & SPROP_IS_HIDDEN) && (obj2 == funobj || (JSFunction *)JS_GetPrivate(cx, obj2) == fp->fun))
        {
            if (getter == js_GetArgument)
            {
                vp = fp->argv;
...
fun_finalize(JSContext *cx, JSObject *obj)
{
    ...
    if (fun->interpreted && fun->u.i.script && js_IsAboutToBeFinalized(cx, fun))
    {
...
fun_xdrObject(JSXDRState *xdr, JSObject **objp)
{
    ...
    if (!JS_XDRStringOrNull(xdr, &atomstr) || !JS_XDRUint16(xdr, &fun->nargs) || !JS_XDRUint16(xdr, &extraUnused) || !JS_XDRUint16(xdr, &fun->u.i.nvars) || !JS_XDRUint32(xdr, &flagsword))
    {
        return JS_FALSE;
    }
    ...
    if (fun->object)
    {
        ...
        if (xdr->mode == JSXDR_ENCODE)
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                if (!propname || !JS_XDRUint32(xdr, &type) || !JS_XDRUint32(xdr, &userid) || !JS_XDRCString(xdr, &propname))
                {
                    if (mark)
        ...
        else
        {
            ...
            for (i = n; i != 0; i--)
            {
                ...
                if (!JS_XDRUint32(xdr, &type) || !JS_XDRUint32(xdr, &userid) || !JS_XDRCString(xdr, &propname))
                {
                    return JS_FALSE;
                }
                JS_ASSERT(type == JSXDR_FUNARG || type == JSXDR_FUNVAR || type == JSXDR_FUNCONST);
...
fun_apply(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (argc >= 2)
    {
        ...
        else
        {
            ...
            if (JSVAL_IS_PRIMITIVE(argv[1]) || (aobj = JSVAL_TO_OBJECT(argv[1]),
                                                OBJ_GET_CLASS(cx, aobj) != &js_ArgumentsClass && OBJ_GET_CLASS(cx, aobj) != &js_ArrayClass))
            {
                JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
...
Function(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    {
        ok = js_CompileFunctionBody(cx, ts, fun) && js_CloseTokenStream(cx, ts);
    }
    JS_ARENA_RELEASE(&cx->tempPool, mark);
    return ok;
...
js_ValueToFunctionObject(JSContext *cx, jsval *vp, uintN flags)
{
    ...
    *vp = OBJECT_TO_JSVAL(funobj);
    ...
    if (caller && !js_CheckPrincipalsAccess(cx, funobj, caller->script->principals, JS_GetFunctionName(fun)))
    {
        return NULL;
    }
    return funobj;
...
js_ValueToCallableObject(JSContext *cx, jsval *vp, uintN flags)
{
    ...
    if (callable && ((callable->map->ops == &js_ObjectOps)
