...
#if (defined XP_WIN || defined XP_OS2) && !defined WINCE && !defined __MWERKS__ && !defined MINGW && (defined _M_IX86 || (defined __GNUC__ && !defined __MINGW32__))
...
js_InitRuntimeNumberState(JSContext *cx)
{
    ...
    locale = localeconv();
    rt->thousandsSeparator = JS_strdup(cx, locale->thousands_sep ? locale->thousands_sep : "'");
    rt->decimalSeparator = JS_strdup(cx, locale->decimal_point ? locale->decimal_point : ".");
    rt->numGrouping = JS_strdup(cx, locale->grouping ? locale->grouping : "\3\0");
...
js_ValueToNumber(JSContext *cx, jsval v, jsdouble *dp)
{
    ...
    else if (JSVAL_IS_STRING(v))
    {
        ...
        if (!bp)
            return JS_FALSE;
        if ((!js_strtod(cx, bp, &ep, dp) || js_SkipWhiteSpace(ep) != bp + str->length) && (!js_strtointeger(cx, bp, &ep, 0, dp) || js_SkipWhiteSpace(ep) != bp + str->length))
        {
            goto badstr;
        }
    }
