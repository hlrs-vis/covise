...
#define GET_ATOM_QUOTE_AND_FMT(qfmt, ufmt, rval)        \
    JS_BEGIN_MACRO atom = GET_ATOM(cx, jp->script, pc); \
...
#define BEGIN_LITOPX_CASE(OP)           \
    case OP:                            \
        ...
        do_##OP : atom = js_GetAtom(cx, &jp->script->atomMap, atomIndex);
...
#define END_LITOPX_CASE \
    break;
            ...
            do_logical_connective:
                ...
                DECOMPILE_CODE(pc, len);
                rval = POP_STR();
                if (jp->pretty && jp->indent + 4 + strlen(lval) + 4 + strlen(rval) > 75)
                {
            ...
            case JSOP_SETCALL:
                ...
                for (i = 1; i <= argc; i++)
                {
                    if (!argv[i] || Sprint(&ss->sprinter, "%s%s", argv[i], (i < argc) ? ", " : "") < 0)
                    {
                        ok = JS_FALSE;
                        break;
                    }
            ...
            do_JSOP_NAMEDFUNOBJ:
                ...
                else
                {
                    if (!js_fun_toString(cx, ATOM_TO_OBJECT(atom),
                                         (pc + len < endpc && pc[len] == JSOP_GROUP)
                                             ? JS_IN_GROUP_CONTEXT | JS_DONT_PRETTY_PRINT
                    ...
                    for (i = j = 0; i < n; i++)
                    {
                        ...
                        if (off2)
                        {
                            ...
                            if (sn)
                            {
                                ...
                                table[j].label = js_GetAtom(cx, &jp->script->atomMap,
                                                            (jsatomid)
            ...
            case JSOP_LOOKUPSWITCHX:
            {
                ...
                for (k = 0; k < npairs; k++)
                {
                    ...
                    {
                        JS_ASSERT(SN_TYPE(sn) == SRC_LABEL);
                        table[k].label = js_GetAtom(cx, &jp->script->atomMap, (jsatomid)
                                                                                  js_GetSrcNoteOffset(sn, 0));
                    }
                    else
                    {
            ...
            case JSOP_CONDSWITCH:
            {
                ...
                {
                    pc2 += off2;
                    JS_ASSERT(*pc2 == JSOP_CASE || *pc2 == JSOP_DEFAULT || *pc2 == JSOP_CASEX || *pc2 == JSOP_DEFAULTX);
            ...
            do_initprop:
                ...
                todo = Sprint(&ss->sprinter, "%s%s%s%s%s:%s",
                              ...
                              (lastop == JSOP_GETTER || lastop == JSOP_SETTER)
                                  ? " "
                                  : "",
                              (lastop == JSOP_GETTER) ? js_getter_str : (lastop == JSOP_SETTER) ? js_setter_str : "",
                              rval);
#else
            ...
            case JSOP_DESCENDANTS:
                ...
                ok = QuoteString(&ss->sprinter, ATOM_TO_STRING(atom), 0) && SprintPut(&ss->sprinter, " ", 1) >= 0 && SprintPut(&ss->sprinter, rval, strlen(rval));
                JS_free(cx, (char *)rval);
                if (!ok)
                    return JS_FALSE;
