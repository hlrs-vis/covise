...
#define SET_SLOT_DONE(rt)                              \
    JS_BEGIN_MACRO JS_ACQUIRE_LOCK((rt)->setSlotLock); \
...
#define SET_SLOT_DONE(rt) /* nothing */
    ...
    if (slot == JSSLOT_PROTO && OBJ_IS_NATIVE(obj))
    {
        ...
        if (oldproto && OBJ_SCOPE(oldproto) == scope)
        {
            ...
            if (!pobj || !OBJ_IS_NATIVE(pobj) || OBJ_GET_CLASS(cx, pobj) != LOCKED_OBJ_GET_CLASS(oldproto))
            {
                /*
...
MarkSharpObjects(JSContext *cx, JSObject *obj, JSIdArray **idap)
{
    ...
    if (!he)
    {
        ...
        for (i = 0, length = ida->length; i < length; i++)
        {
            ...
            if (ok)
            {
                if (OBJ_IS_NATIVE(obj2) && (attrs & (JSPROP_GETTER | JSPROP_SETTER)))
                {
                    val = JSVAL_NULL;
            ...
            if (!ok)
                break;
            if (!JSVAL_IS_PRIMITIVE(val) && !MarkSharpObjects(cx, JSVAL_TO_OBJECT(val), NULL))
            {
                ok = JS_FALSE;
                break;
            }
                    ...
                    jschar **sp)
{
    ...
    if (JS_HAS_NATIVE_BRANCH_CALLBACK_OPTION(cx) && cx->branchCallback && !cx->branchCallback(cx, NULL))
    {
        return NULL;
    }
    ...
    for (i = 0, length = ida->length; i < length; i++)
    {
        ...
        if (prop)
        {
            ...
            if (OBJ_IS_NATIVE(obj2) && (attrs & (JSPROP_GETTER | JSPROP_SETTER)))
            {
                if (attrs & JSPROP_GETTER)
                {
                    ...
                    gsop[valcnt] = ATOM_TO_STRING(cx->runtime->atomState.getterAtom);
#else
...
#ifdef OLD_GETTER_SETTER
                    gsop[valcnt] = ATOM_TO_STRING(cx->runtime->atomState.setterAtom);
#else
        ...
        *rval = STRING_TO_JSVAL(idstr); /* local root */
        ...
        for (j = 0; j < valcnt; j++)
        {
            ...
            chars = (jschar *)
                realloc((ochars = chars),
                        (nchars + (comma ? 2 : 0) + idstrlength + 1 + (gsop[j] ? 1 + JSSTRING_LENGTH(gsop[j]) : 0) + vsharplength + vlength + (outermost ? 2 : 1) + 1) * sizeof(jschar));
            if (!chars)
            {
                         ...
                         JSPrincipals *principals, const char *caller)
{
    ...
    if (rt->findObjectPrincipals)
    {
        ...
        if (!principals || !scopePrincipals || !principals->subsume(principals, scopePrincipals))
        {
            JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
...
js_CheckScopeChainValidity(JSContext *cx, JSObject *scopeobj, const char *caller)
{
    ...
    while (scopeobj)
    {
        ...
        {
            xclasp = (JSExtendedClass *)clasp;
            if (xclasp->innerObject && xclasp->innerObject(cx, scopeobj) != scopeobj)
            {
                goto bad;
            }
        }
...
obj_eval(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (JS_VERSION_IS_ECMA(cx) && indirectCall && !JS_ReportErrorFlagsAndNumber(cx, JSREPORT_WARNING | JSREPORT_STRICT, js_GetErrorMessage, NULL, JSMSG_BAD_INDIRECT_CALL, js_eval_str))
    {
        return JS_FALSE;
    }
                        ...
                        uintN argc, jsval *argv, jsval *rval)
{
    ...
    else
    {
        ...
        xclasp = (clasp->flags & JSCLASS_IS_EXTENDED)
                     ...
                     : NULL;
        if (xclasp && xclasp->outerObject && xclasp->outerObject(cx, obj2) == obj)
        {
            *rval = JSVAL_TRUE;
        }
                         ...
                         jsval *rval)
{
     ...
     */
    if (obj2 != obj && !(OBJ_IS_NATIVE(obj2) && SPROP_IS_SHARED_PERMANENT((JSScopeProperty *)prop)))
    {
        OBJ_DROP_PROPERTY(cx, obj2, prop);
        *rval = JSVAL_FALSE;
        return JS_TRUE;
...
js_NewObject(JSContext *cx, JSClass *clasp, JSObject *proto, JSObject *parent)
{
     ...
     */
    if (proto && (map = proto->map)->ops == ops && ((protoclasp = OBJ_GET_CLASS(cx, proto)) == clasp || (!((protoclasp->flags ^ clasp->flags) & (JSCLASS_HAS_PRIVATE | (JSCLASS_RESERVED_SLOTS_MASK << JSCLASS_RESERVED_SLOTS_SHIFT))) && protoclasp->reserveSlots == clasp->reserveSlots)))
    {
        /*
...
#define CHECK_FOR_EMPTY_INDEX(id)                                   \
    JS_BEGIN_MACRO if (JSSTRING_LENGTH(_str) == 0) id = JSVAL_ZERO; \
    JS_END_MACRO
#else
...
#define CHECK_FOR_STRING_INDEX(id)                                                           \
    JS_BEGIN_MACRO if (JSID_IS_ATOM(id))                                                     \
    {                                                                                        \
        ...
        if (negative_)                                                                       \
            cp_++;                                                                           \
        if (JS7_ISDEC(*cp_) && str_->length - negative_ <= sizeof(JSVAL_INT_MAX_STRING) - 1) \
        {                                                                                    \
...
CheckForStringIndex(jsid id, const jschar *cp, JSBool negative)
{
    ...
    if (*cp == 0 && (oldIndex < (JSVAL_INT_MAX / 10) || (oldIndex == (JSVAL_INT_MAX / 10) && c <= (JSVAL_INT_MAX % 10))))
    {
        if (negative)
            index = 0 - index;
...
js_LookupHiddenProperty(JSContext *cx, JSObject *obj, jsid id, JSObject **objp,
                        JSProperty **propp)
{
    return HidePropertyName(cx, &id) && js_LookupProperty(cx, obj, id, objp, propp);
}
...
#define ADD_PROPERTY_HELPER(cx, clasp, obj, scope, sprop, vp, cleanup) \
    JS_BEGIN_MACRO if ((clasp)->addProperty != JS_PropertyStub)        \
    {                                                                  \
        jsval nominal_ = *(vp);                                        \
                        ...
                        uintN flags, intN shortid, JSProperty **propp)
{
    ...
    if (attrs & (JSPROP_GETTER | JSPROP_SETTER))
    {
        ...
        if (!js_LookupProperty(cx, obj, id, &pobj, &prop))
            return JS_FALSE;
        sprop = (JSScopeProperty *)prop;
        if (sprop && pobj == obj && (sprop->attrs & (JSPROP_GETTER | JSPROP_SETTER)))
        {
...
Detecting(JSContext *cx, jsbytecode *pc)
{
    ...
    for (endpc = script->code + script->length; pc < endpc; pc++)
    {
        ...
        if (op == JSOP_NAME)
        {
            ...
            if (atom == cx->runtime->atomState.typeAtoms[JSTYPE_VOID] && (pc += js_CodeSpec[op].length) < endpc)
            {
                op = (JSOp)*pc;
                return op == JSOP_EQ || op == JSOP_NE || op == JSOP_NEW_EQ || op == JSOP_NEW_NE;
            }
            break;
        }
            ...
            if (resolve != JS_ResolveStub)
            {
                ...
                *propp = NULL;
                ...
                if (clasp->flags & JSCLASS_NEW_RESOLVE)
                {
                    ...
                    if (!(flags & JSRESOLVE_CLASSNAME) && cx->fp && (pc = cx->fp->pc))
                    {
                        cs = &js_CodeSpec[*pc];
                        format = cs->format;
                        if ((format & JOF_MODEMASK) != JOF_NAME)
                            flags |= JSRESOLVE_QUALIFIED;
                        if ((format & JOF_ASSIGNING) || (cx->fp->flags & JSFRAME_ASSIGNING))
                        {
...
js_GetProperty(JSContext *cx, JSObject *obj, jsid id, jsval *vp)
{
    ...
    if (!prop)
    {
        ...
        *vp = default_val;
         ...
         */
        if (JS_HAS_STRICT_OPTION(cx) && *vp == default_val && cx->fp && cx->fp->pc && (*cx->fp->pc == JSOP_GETPROP || *cx->fp->pc == JSOP_GETELEM))
        {
            jsbytecode *pc;
            JSString *str;
            ...
            str = js_DecompileValueGenerator(cx, JSDVG_IGNORE_STACK,
                                             ID_TO_VALUE(id), NULL);
            if (!str || !JS_ReportErrorFlagsAndNumber(cx, JSREPORT_WARNING | JSREPORT_STRICT, js_GetErrorMessage, NULL, JSMSG_UNDEFINED_PROP, JS_GetStringBytes(str)))
            {
                return JS_FALSE;
            }
        }
...
js_SetProperty(JSContext *cx, JSObject *obj, jsid id, jsval *vp)
{
    ...
    if (sprop)
    {
        ...
        attrs = sprop->attrs;
        if ((attrs & JSPROP_READONLY) || (SCOPE_IS_SEALED(scope) && pobj == obj))
        {
            JS_UNLOCK_SCOPE(cx, scope);
                 ...
                 uintN *attrsp)
{
    ...
    sprop = js_ChangeNativePropertyAttrs(cx, obj, sprop,
                                         *attrsp & ~(JSPROP_GETTER | JSPROP_SETTER), 0,
...
js_DefaultValue(JSContext *cx, JSObject *obj, JSType hint, jsval *vp)
{
    ...
    default:
        ...
        if (!JSVAL_IS_PRIMITIVE(v))
        {
            ...
            if (type == hint || (type == JSTYPE_FUNCTION && hint == JSTYPE_OBJECT))
            {
                goto out;
            }
...
#endif
                        (sprop->attrs & JSPROP_ENUMERATE)) && !(sprop->flags & SPROP_IS_ALIAS) && (!SCOPE_HAD_MIDDLE_DELETE(scope) || SCOPE_HAS_PROPERTY(scope, sprop)))
                {
                    length++;
                }
            }
               ...
               jsval *vp, uintN *attrsp)
{
    ...
    if (!prop)
    {
        ...
        *attrsp = 0;
        clasp = OBJ_GET_CLASS(cx, obj);
        return !clasp->checkAccess || clasp->checkAccess(cx, obj, ID_TO_VALUE(id), mode, vp);
    }
    if (!OBJ_IS_NATIVE(pobj))
    {
        OBJ_DROP_PROPERTY(cx, pobj, prop);
...
js_HasInstance(JSContext *cx, JSObject *obj, jsval v, JSBool *bp)
{
...
#ifdef NARCISSUS
    {
        ...
        if (JSVAL_IS_FUNCTION(cx, fval))
        {
            return js_InternalCall(cx, obj, fval, 1, &v, &rval) && js_ValueToBoolean(cx, rval, bp);
        }
    }
#endif
...
js_Mark(JSContext *cx, JSObject *obj, void *arg)
{
    ...
    JS_ASSERT(!SCOPE_LAST_PROP(scope) || SCOPE_HAS_PROPERTY(scope, SCOPE_LAST_PROP(scope)));
