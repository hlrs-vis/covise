...
logit(JSScope *scope, char op, const char *file, int line)
{
    ...
    if (!logtbl.entryStore && !JS_DHashTableInit(&logtbl, JS_DHashGetStubOps(), NULL, sizeof(logentry), 100))
    {
        return;
    }
...
#define MAKE_STRING_IMMUTABLE(rt, v, vp)                                   \
    JS_BEGIN_MACRO JSString *str_ = JSVAL_TO_STRING(v);                    \
    uint8 *flagp_ = js_GetGCThingFlags(str_);                              \
    if (*flagp_ & GCF_MUTABLE)                                             \
    {                                                                      \
        if (JSSTRING_IS_DEPENDENT(str_) && !js_UndependString(NULL, str_)) \
        {                                                                  \
...
ClaimScope(JSScope *scope, JSContext *cx)
{
    ...
    while ((ownercx = scope->ownercx) != NULL)
    {
         ...
         */
        if (!scope->u.link && (!js_ValidContextPointer(rt, ownercx) || !ownercx->requestDepth || ownercx->thread == cx->thread))
        {
         ...
         */
        if (rt->gcThread == cx->thread || (ownercx->scopeToShare && WillDeadlock(ownercx->scopeToShare, cx)))
        {
            ShareScope(rt, scope);
            break;
        }
...
js_GetSlotThreadSafe(JSContext *cx, JSObject *obj, uint32 slot)
{
     ...
     */
    if (CX_THREAD_IS_RUNNING_GC(cx) || (SCOPE_IS_SEALED(scope) && scope->object == obj) || (scope->ownercx && ClaimScope(scope, cx)))
    {
...
js_TransferScopeLock(JSContext *cx, JSScope *oldscope, JSScope *newscope)
{
    ...
    {
        JS_ASSERT(newscope->ownercx == cx || (!newscope->ownercx && newscope->u.count == 1));
        cx->lockedSealedScope = NULL;
        return;
    }
...
js_LockObj(JSContext *cx, JSObject *obj)
{
    ...
    {
        scope = OBJ_SCOPE(obj);
        if (SCOPE_IS_SEALED(scope) && scope->object == obj && !cx->lockedSealedScope)
        {
