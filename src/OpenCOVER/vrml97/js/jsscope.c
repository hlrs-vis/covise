...
js_HashScopeProperty(JSDHashTable *table, const void *key)
{
    ...
    gsop = sprop->getter;
    if (gsop)
        hash = (JSDHashNumber)((hash >> (JS_DHASH_BITS - 4)) ^ (hash << 4) ^ (jsword)gsop);
    gsop = sprop->setter;
    if (gsop)
        hash = (JSDHashNumber)((hash >> (JS_DHASH_BITS - 4)) ^ (hash << 4) ^ (jsword)gsop);
    ...
    hash = (hash >> (JS_DHASH_BITS - 4)) ^ (hash << 4) ^ (JSDHashNumber)(sprop->id);
    return hash;
}
...
#define SPROP_MATCH_PARAMS(sprop, aid, agetter, asetter, aslot, aattrs, aflags, ashortid)        \
    ((sprop)->id == (aid) && SPROP_MATCH_PARAMS_AFTER_ID(sprop, agetter, asetter, aslot, aattrs, \
                                                         aflags, ashortid))

#define SPROP_MATCH_PARAMS_AFTER_ID(sprop, agetter, asetter, aslot, aattrs, aflags, ashortid) \
    ((sprop)->getter == (agetter) && (sprop)->setter == (asetter) && (sprop)->slot == (aslot) && (sprop)->attrs == (aattrs) && (((sprop)->flags ^ (aflags)) & ~SPROP_FLAGS_NOT_MATCHED) == 0 && (sprop)->shortid == (ashortid))
...
#define FREENODE_INSERT(list, sprop)                    \
    JS_BEGIN_MACRO FREENODE(sprop)->next = (list);      \
...
#define FREENODE_REMOVE(sprop)                                           \
    JS_BEGIN_MACRO *FREENODE(sprop)->prevp = FREENODE(sprop)->next;      \
...
#define KIDS_TO_CHUNK(kids) ((PropTreeKidsChunk *)((jsuword)(kids) & ~CHUNKY_KIDS_TAG))
#define CHUNK_TO_KIDS(chunk) ((JSScopeProperty *)((jsuword)(chunk) | CHUNKY_KIDS_TAG))
...
#define CHECK_ANCESTOR_LINE(scope, sparse)                               \
    JS_BEGIN_MACRO if ((scope)->table) CheckAncestorLine(scope, sparse); \
    JS_END_MACRO
                    ...
                    uintN attrs, uintN flags, intN shortid)
{
    ...
    if (!sprop)
    {
        ...
        if (scope->entryCount + scope->removedCount >= size - (size >> 2))
        {
            ...
            if (!ChangeScope(cx, scope, change) && scope->entryCount + scope->removedCount == size - 1)
            {
                METER(addFailures);
                return NULL;
            }
    ...
    else
    {
         ...
         */
        if (!(attrs & JSPROP_SHARED) && slot == SPROP_INVALID_SLOT && SPROP_HAS_VALID_SLOT(sprop, scope))
        {
            slot = sprop->slot;
        }
    ...
    if (!sprop)
    {
         ...
         */
        if (!JS_CLIST_IS_EMPTY(&cx->runtime->watchPointList) && js_FindWatchPoint(cx->runtime, scope, id))
        {
                            ...
                            JSPropertyOp getter, JSPropertyOp setter)
{
    ...
    JS_ASSERT(!((attrs ^ sprop->attrs) & JSPROP_SHARED) || !(attrs & JSPROP_SHARED));
    if (getter == JS_PropertyStub)
        getter = NULL;
    if (setter == JS_PropertyStub)
        setter = NULL;
