         ...
         wp = (JSWatchPoint *)wp->links.next)
    {
        ...
        if (wp->object == obj && SPROP_USERID(sprop) == id)
        {
            ...
            if (ok)
            {
                ...
                frame.down = cx->fp;
                cx->fp = &frame;
                ok = !wp->setter || ((sprop->attrs & JSPROP_SETTER)
                 ...
                 JSWatchPointHandler handler, void *closure)
{
    ...
    else if (pobj != obj)
    {
        ...
        {
            if (!OBJ_GET_PROPERTY(cx, pobj, id, &value) || !OBJ_GET_ATTRIBUTES(cx, pobj, id, prop, &attrs))
            {
...
JS_EvalFramePrincipals(JSContext *cx, JSStackFrame *fp, JSStackFrame *caller)
{
    ...
    return (callerPrincipals && principals && callerPrincipals->subsume(callerPrincipals, principals))
               ? principals
               : callerPrincipals;
}
                   ...
                   JSPropertyDesc *pd)
{
    ...
    if (wasThrowing)
    {
        ...
        if (JSVAL_IS_GCTHING(lastException) && !js_AddRoot(cx, &lastException, "lastException"))
        {
            return JS_FALSE;
        }
    ...
    if (OBJ_GET_CLASS(cx, obj) == &js_CallClass && getter == js_CallClass.getProperty)
    {
        /*
