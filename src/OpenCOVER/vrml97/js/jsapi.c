...
#define SYNC_OPTIONS_TO_VERSION(cx)                                                     \
    JS_BEGIN_MACRO if ((cx)->options & JSOPTION_XML)(cx)->version |= JSVERSION_HAS_XML; \
    else(cx)->version &= ~JSVERSION_HAS_XML;                                            \
    JS_END_MACRO
...
JS_InitStandardClasses(JSContext *cx, JSObject *obj)
{
    ...
    return js_InitArrayClass(cx, obj) && js_InitBooleanClass(cx, obj) && js_InitMathClass(cx, obj) && js_InitNumberClass(cx, obj) && js_InitStringClass(cx, obj) &&
...
JS_EnumerateStandardClasses(JSContext *cx, JSObject *obj)
{
    ...
    atom = rt->atomState.typeAtoms[JSTYPE_VOID];
    if (!AlreadyHasOwnProperty(obj, atom) && !OBJ_DEFINE_PROPERTY(cx, obj, ATOM_TO_JSID(atom), JSVAL_VOID, NULL, NULL, JSPROP_PERMANENT, NULL))
    {
        return JS_FALSE;
    }
#endif
    ...
    {
        atom = OFFSET_TO_ATOM(rt, standard_class_atoms[i].atomOffset);
        if (!AlreadyHasOwnProperty(obj, atom) && !standard_class_atoms[i].init(cx, obj))
        {
            return JS_FALSE;
        }
    }
...
#if JS_MAP_GCROOT_NEXT == JS_DHASH_NEXT && JS_MAP_GCROOT_STOP == JS_DHASH_STOP && JS_MAP_GCROOT_REMOVE == JS_DHASH_REMOVE
...
JS_MaybeGC(JSContext *cx)
{
    ...
    if ((bytes > 8192 && bytes > lastBytes + lastBytes / 2) || rt->gcMallocBytes > rt->gcMaxBytes)
    {
        /*
             ...
             JSPropertySpec *static_ps, JSFunctionSpec *static_fs)
{
    ...
    if ((ps && !JS_DefineProperties(cx, proto, ps)) || (fs && !JS_DefineFunctions(cx, proto, fs)) || (static_ps && !JS_DefineProperties(cx, ctor, static_ps)) || (static_fs && !JS_DefineFunctions(cx, ctor, static_fs)))
    {
        goto bad;
    }
    js_LeaveLocalRootScope(cx);
...
JS_NextProperty(JSContext *cx, JSObject *iterobj, jsid *idp)
{
    ...
    if (i < 0)
    {
         ...
         */
        while (sprop && (!(sprop->attrs & JSPROP_ENUMERATE) || (sprop->flags & SPROP_IS_ALIAS) || (SCOPE_HAD_MIDDLE_DELETE(scope) && !SCOPE_HAS_PROPERTY(scope, sprop))))
        {
            sprop = sprop->parent;
        }
...
#define LAST_FRAME_EXCEPTION_CHECK(cx, result)                                                                        \
    JS_BEGIN_MACRO if (!(result) && !((cx)->options & JSOPTION_DONT_REPORT_UNCAUGHT)) js_ReportUncaughtException(cx); \
    JS_END_MACRO
#else
                          ...
                          const char *bytes, size_t length)
{
    ...
    if (ts)
    {
        ...
        if (!js_ParseTokenStream(cx, obj, ts) && (ts->flags & TSF_UNEXPECTED_EOF))
        {
            /*
