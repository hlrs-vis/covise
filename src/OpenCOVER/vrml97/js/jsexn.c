...
exn_newPrivate(JSContext *cx, JSErrorReport *report)
{
    ...
    if (report->uclinebuf)
    {
        ...
        newReport->uclinebuf = (const jschar *)JS_malloc(cx, capacity * sizeof(jschar));
    ...
    if (report->ucmessage)
    {
        ...
        if (report->messageArgs)
        {
            for (i = 0; report->messageArgs[i]; i++)
                continue;
            JS_ASSERT(i);
            newReport->messageArgs = (const jschar **)JS_malloc(cx, (i + 1) * sizeof(jschar *));
            if (!newReport->messageArgs)
                goto error;
            for (i = 0; report->messageArgs[i]; i++)
            {
                capacity = js_strlen(report->messageArgs[i]) + 1;
                newReport->messageArgs[i] = (const jschar *)JS_malloc(cx, capacity * sizeof(jschar));
...
#define APPEND_CHAR_TO_STACK(c)                                           \
    JS_BEGIN_MACRO if (stacklen == stackmax)                              \
    {                                                                     \
        void *ptr_;                                                       \
...
#define APPEND_STRING_TO_STACK(str)                                       \
    JS_BEGIN_MACRO JSString *str_ = str;                                  \
...
exn_toSource(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    *rval = STRING_TO_JSVAL(name);
    ...
    if (!JS_GetProperty(cx, obj, js_message_str, &vp[0]) || !(message = js_ValueToSource(cx, vp[0])))
    {
        return JS_FALSE;
    }
    vp[0] = STRING_TO_JSVAL(message);

    if (!JS_GetProperty(cx, obj, js_filename_str, &vp[1]) || !(filename = js_ValueToSource(cx, vp[1])))
    {
        return JS_FALSE;
    }
    vp[1] = STRING_TO_JSVAL(filename);

    if (!JS_GetProperty(cx, obj, js_lineno_str, &vp[2]) || !js_ValueToECMAUint32(cx, vp[2], &lineno))
    {
        return JS_FALSE;
    }
...
js_ReportUncaughtException(JSContext *cx)
{
    ...
    if (!reportp && exnObject && OBJ_GET_CLASS(cx, exnObject) == &ExceptionClass)
    {
        const char *filename;
        uint32 lineno;
