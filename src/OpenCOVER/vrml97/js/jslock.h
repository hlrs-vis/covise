...
#define JS_LOCK_OBJ_VOID(cx, obj, e)     \
    JS_BEGIN_MACRO JS_LOCK_OBJ(cx, obj); \
    e;                                   \
    JS_UNLOCK_OBJ(cx, obj);              \
    JS_END_MACRO

#define JS_LOCK_VOID(cx, e)                        \
    JS_BEGIN_MACRO JSRuntime *_rt = (cx)->runtime; \
    JS_LOCK_RUNTIME_VOID(_rt, e);                  \
    JS_END_MACRO

#if defined(JS_USE_ONLY_NSPR_LOCKS) || !((defined(_WIN32) && defined(_M_IX86)) || (defined(__GNUC__) && defined(__i386__)) || ((defined(__USLC__) || defined(_SCO_DS)) && defined(i386)) || (defined(SOLARIS) && defined(sparc) && defined(ULTRA_SPARC)) || defined(AIX))
...
#define JS_LOCK_RUNTIME_VOID(rt, e)     \
    JS_BEGIN_MACRO JS_LOCK_RUNTIME(rt); \
    e;                                  \
