...
#define ASSERT_CACHE_IS_EMPTY(cache)                          \
    JS_BEGIN_MACRO JSPropertyCacheEntry *end_, *pce_, entry_; \
...
#define FETCH_INT(cx, n, i)                   \
    JS_BEGIN_MACRO jsval v_ = FETCH_OPND(n);  \
    if (JSVAL_IS_INT(v_))                     \
    {                                         \
        i = JSVAL_TO_INT(v_);                 \
    }                                         \
...
#define VALUE_TO_NUMBER(cx, v, d)         \
    JS_BEGIN_MACRO if (JSVAL_IS_INT(v))   \
    {                                     \
...
#define VALUE_TO_OBJECT(cx, v, obj)            \
    JS_BEGIN_MACRO if (!JSVAL_IS_PRIMITIVE(v)) \
    {                                          \
...
js_ComputeThis(JSContext *cx, JSObject *thisp, JSStackFrame *fp)
{
    ...
    else
    {
        ...
        if (JSVAL_IS_PRIMITIVE(fp->argv[-2]) || !(parent = OBJ_GET_PARENT(cx, JSVAL_TO_OBJECT(fp->argv[-2]))))
        {
...
js_compare_call_keys(const void *k1, const void *k2)
{
    ...
    return ck1->callee == ck2->callee && ((ck1->filename && ck2->filename)
                                              ? strcmp(ck1->filename, ck2->filename) == 0
                                              : ck1->filename == ck2->filename) && ck1->lineno == ck2->lineno;
}
...
js_Invoke(JSContext *cx, uintN argc, uintN flags)
{
    ...
    if (clasp != &js_FunctionClass)
    {
         ...
         */
        if (JS_VERSION_IS_1_2(cx) || ((ops == &js_ObjectOps) ? clasp->call : ops->call))
        {
                    ...
                    JSAccessMode mode, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (cx->runtime->checkObjectAccess && JSVAL_IS_FUNCTION(cx, fval) && ((JSFunction *)JS_GetPrivate(cx, JSVAL_TO_OBJECT(fval)))->interpreted && !cx->runtime->checkObjectAccess(cx, obj, ID_TO_VALUE(id), mode, &fval))
    {
        return JS_FALSE;
    }
...
#define CHECK_ELEMENT_ID(obj, id)                                      \
    JS_BEGIN_MACRO if (JSID_IS_OBJECT(id) && !OBJECT_IS_XML(cx, obj))  \
    {                                                                  \
        SAVE_SP_AND_PC(fp);                                            \
 ...
 */
#if __GNUC__ >= 3 || (__IBMC__ >= 700 && defined __IBM_COMPUTED_GOTO) || __SUNPRO_C >= 0x570
...
#define CHECK_BRANCH(len)                    \
    JS_BEGIN_MACRO if (len <= 0 && onbranch) \
    {                                        \
        SAVE_SP_AND_PC(fp);                  \
...
#define LOAD_INTERRUPT_HANDLER(rt)                            \
    JS_BEGIN_MACRO interruptHandler = (rt)->interruptHandler; \
    LOAD_JUMP_TABLE();                                        \
    JS_END_MACRO
    ...
    if (++cx->interpLevel == MAX_INTERP_LEVEL || !JS_CHECK_STACK_SIZE(cx, stackDummy))
    {
...
#define FETCH_ELEMENT_ID(n, id)                      \
    JS_BEGIN_MACRO jsval idval_ = FETCH_OPND(n);     \
        ...
        *vp = OBJECT_TO_JSVAL(propobj);
        ...
        ok =
            ...
            (foreach && OBJECT_IS_XML(cx, obj))
                ? ((JSXMLObjectOps *)obj->map->ops)->enumerateValues(cx, obj, JSENUMERATE_INIT, &iter_state, NULL, NULL)
                :
#endif
...
enum_next_property:
{
...
#if JS_HAS_XML_SUPPORT
        (foreach && OBJECT_IS_XML(cx, obj))
            ? ((JSXMLObjectOps *)obj->map->ops)->enumerateValues(cx, obj, JSENUMERATE_NEXT, &iter_state, &fid, &rval)
            :
#endif
    ...
    if (obj != obj2)
    {
        ...
        if (clasp->flags & JSCLASS_IS_EXTENDED)
        {
            ...
            cond = xclasp->outerObject && xclasp->outerObject(cx, obj2) == obj;
        }
        if (!cond)
            goto enum_next_property;
    }
...
#define PROPERTY_OP(n, call)                                                                                      \
    JS_BEGIN_MACRO /* Fetch the left part and resolve it to a non-null object. */ FETCH_OBJECT(cx, n, lval, obj); \
                                                                                                                  \
...
#define ELEMENT_OP(n, call)                                                                              \
    JS_BEGIN_MACRO /* Fetch the right part and resolve it to an internal id. */ FETCH_ELEMENT_ID(n, id); \
                                                                                                         \
...
#define CACHED_GET_VP(call, vp)                                                                 \
    JS_BEGIN_MACRO if (!OBJ_IS_NATIVE(obj))                                                     \
    {                                                                                           \
        ok = call;                                                                              \
    }                                                                                           \
    else                                                                                        \
    {                                                                                           \
        ...
        {                                                                                       \
            ...
            *(vp) = (slot != SPROP_INVALID_SLOT) ? LOCKED_OBJ_GET_SLOT(obj, slot) : JSVAL_VOID; \
...
#define CACHED_SET(call)                                                                                       \
    ...
    {                                                                                                          \
        ...
        JS_LOCK_OBJ(cx, obj);                                                                                  \
        PROPERTY_CACHE_TEST(&rt->propertyCache, obj, id, sprop);                                               \
        if (sprop && !(sprop->attrs & JSPROP_READONLY) && (scope_ = OBJ_SCOPE(obj), !SCOPE_IS_SEALED(scope_))) \
        {                                                                                                      \
            JS_UNLOCK_SCOPE(cx, scope_);                                                                       \
...
#define BEGIN_LITOPX_CASE(OP, PCOFF)                       \
    BEGIN_CASE(OP) atomIndex = GET_ATOM_INDEX(pc + PCOFF); \
    do_##OP : atom = js_GetAtom(cx, &script->atomMap, atomIndex);
...
#define INTEGER_OP(OP, EXTRA_CODE)       \
    JS_BEGIN_MACRO FETCH_INT(cx, -1, j); \
    FETCH_INT(cx, -2, i);                \
    EXTRA_CODE i = i OP j;               \
    sp--;                                \
    STORE_INT(cx, -1, i);                \
...
#define RELATIONAL_OP(OP)                                              \
    JS_BEGIN_MACRO rval = FETCH_OPND(-1);                              \
    lval = FETCH_OPND(-2);                                             \
...
#define XML_EQUALITY_OP(OP)                                                                                                                                                         \
    if ((ltmp == JSVAL_OBJECT && (obj2 = JSVAL_TO_OBJECT(lval)) && OBJECT_IS_XML(cx, obj2)) || (rtmp == JSVAL_OBJECT && (obj2 = JSVAL_TO_OBJECT(rval)) && OBJECT_IS_XML(cx, obj2))) \
    {                                                                                                                                                                               \
        JSXMLObjectOps *ops;                                                                                                                                                        \
                                                                                                                                                                                    \
...
#define EXTENDED_EQUALITY_OP(OP)                                                                                                    \
    if (ltmp == JSVAL_OBJECT && (obj2 = JSVAL_TO_OBJECT(lval)) && ((clasp = OBJ_GET_CLASS(cx, obj2))->flags & JSCLASS_IS_EXTENDED)) \
    {                                                                                                                               \
        JSExtendedClass *xclasp;                                                                                                    \
                                                                                                                                    \
        xclasp = (JSExtendedClass *)clasp;                                                                                          \
...
#define EQUALITY_OP(OP, IFNAN)                                                                                     \
    ...
    ltmp = JSVAL_TAG(lval);                                                                                        \
    rtmp = JSVAL_TAG(rval);                                                                                        \
    XML_EQUALITY_OP(OP) if (ltmp == rtmp)                                                                          \
    {                                                                                                              \
        ...
        {                                                                                                          \
            EXTENDED_EQUALITY_OP(OP) /* Handle all undefined (=>NaN) and int combinations. */ cond = lval OP rval; \
        }                                                                                                          \
    }                                                                                                              \
    else                                                                                                           \
    {                                                                                                              \
...
#undef SIGNED_SHIFT_OP
    ...
    lval = FETCH_OPND(-2);
#if JS_HAS_XML_SUPPORT
    if (!JSVAL_IS_PRIMITIVE(lval) && (obj2 = JSVAL_TO_OBJECT(lval), OBJECT_IS_XML(cx, obj2)) && VALUE_IS_XML(cx, rval))
    {
        JSXMLObjectOps *ops;
...
#define BINARY_OP(OP)                        \
    JS_BEGIN_MACRO FETCH_NUMBER(cx, -1, d2); \
    FETCH_NUMBER(cx, -2, d);                 \
    d = d OP d2;                             \
    sp--;                                    \
    STORE_NUMBER(cx, -1, d);                 \
...
do_new:
    ...
    obj2 = NULL;
    lval = *vp;
    if (!JSVAL_IS_OBJECT(lval) || (obj2 = JSVAL_TO_OBJECT(lval)) == NULL ||
        /* XXX clean up to avoid special cases above ObjectOps layer */
        OBJ_GET_CLASS(cx, obj2) == &js_FunctionClass || !obj2->map->ops->construct)
    {
...
do_incop:
{
    ...
    if (JSVAL_IS_INT(rval) && rval != INT_TO_JSVAL(JSVAL_INT_MIN) && rval != INT_TO_JSVAL(JSVAL_INT_MAX))
    {
...
#define NONINT_INCREMENT_OP_MIDDLE()              \
    JS_BEGIN_MACRO VALUE_TO_NUMBER(cx, rval, d);  \
    if (cs->format & JOF_POST)                    \
    {                                             \
        rtmp = rval;                              \
    ...
    JS_END_MACRO
        ...
        if (cs->format & JOF_POST)
        {
            ...
            vp = NULL; /* suppress bogus gcc warnings */
#endif
...
#define LAZY_ARGS_THISP ((JSObject *)1)
    ...
    if (JSVAL_IS_FUNCTION(cx, lval) && (obj = JSVAL_TO_OBJECT(lval),
                                        fun = (JSFunction *)JS_GetPrivate(cx, obj),
                                        fun->interpreted && !(fun->flags & (JSFUN_HEAVYWEIGHT | JSFUN_BOUND_METHOD)) && argc >= (uintN)fun->nargs))
    /* inline_call: */
    {
    ...
    case JSOP_ANONFUNOBJ:
        goto do_JSOP_ANONFUNOBJ;
    case JSOP_BINDNAME:
        goto do_JSOP_BINDNAME;
    case JSOP_CLOSURE:
        goto do_JSOP_CLOSURE;
    case JSOP_DEFCONST:
        goto do_JSOP_DEFCONST;
    case JSOP_DEFFUN:
        goto do_JSOP_DEFFUN;
    case JSOP_DEFLOCALFUN:
        goto do_JSOP_DEFLOCALFUN;
    case JSOP_DEFVAR:
        goto do_JSOP_DEFVAR;
#if JS_HAS_EXPORT_IMPORT
    case JSOP_EXPORTNAME:
        goto do_JSOP_EXPORTNAME;
#endif
#if JS_HAS_XML_SUPPORT
    case JSOP_GETMETHOD:
        goto do_JSOP_GETMETHOD;
    case JSOP_SETMETHOD:
        goto do_JSOP_SETMETHOD;
#endif
    case JSOP_INITCATCHVAR:
        goto do_JSOP_INITCATCHVAR;
    case JSOP_NAMEDFUNOBJ:
        goto do_JSOP_NAMEDFUNOBJ;
    case JSOP_NUMBER:
        goto do_JSOP_NUMBER;
    case JSOP_OBJECT:
        goto do_JSOP_OBJECT;
    ...
    case JSOP_QNAMECONST:
        goto do_JSOP_QNAMECONST;
    case JSOP_QNAMEPART:
        goto do_JSOP_QNAMEPART;
#endif
    case JSOP_REGEXP:
        goto do_JSOP_REGEXP;
    case JSOP_SETCONST:
        goto do_JSOP_SETCONST;
    case JSOP_STRING:
        goto do_JSOP_STRING;
#if JS_HAS_XML_SUPPORT
    case JSOP_XMLCDATA:
        goto do_JSOP_XMLCDATA;
    case JSOP_XMLCOMMENT:
        goto do_JSOP_XMLCOMMENT;
    case JSOP_XMLOBJECT:
        goto do_JSOP_XMLOBJECT;
    case JSOP_XMLPI:
        goto do_JSOP_XMLPI;
#endif
    default:
...
do_JSOP_OBJECT:
    ...
    /*
             ...
             */
    if ((cx->version & JSVERSION_MASK) == JSVERSION_DEFAULT || (cx->version & JSVERSION_MASK) >= JSVERSION_1_4)
    {
        rval = POP_OPND();
    ...
    if (!JSVAL_IS_NUMBER(lval) && !JSVAL_IS_STRING(lval) && !JSVAL_IS_BOOLEAN(lval))
    {
        DO_NEXT_OP(len);
    }
...
#define SEARCH_PAIRS(MATCH_CODE)                 \
    ...
    if (JSVAL_IS_STRING(lval))
    {
        str = JSVAL_TO_STRING(lval);
        SEARCH_PAIRS(
            match = (JSVAL_IS_STRING(rval) && ((str2 = JSVAL_TO_STRING(rval)) == str || !js_CompareStrings(str2, str)));)
    }
    else if (JSVAL_IS_DOUBLE(lval))
    {
        d = *JSVAL_TO_DOUBLE(lval);
        SEARCH_PAIRS(
            match = (JSVAL_IS_DOUBLE(rval) && *JSVAL_TO_DOUBLE(rval) == d);)
    }
    else
    {
        SEARCH_PAIRS(
...
do_JSOP_DEFVAR:
    ...
    /*
             ...
             */
    if (atomIndex < script->numGlobalVars && (attrs & JSPROP_PERMANENT) && obj2 == obj && OBJ_IS_NATIVE(obj))
    {
        sprop = (JSScopeProperty *)prop;
        if (SPROP_HAS_VALID_SLOT(sprop, OBJ_SCOPE(obj)) && SPROP_HAS_STUB_GETTER(sprop) && SPROP_HAS_STUB_SETTER(sprop))
        {
            /*
...
do_init:
    ...
    obj = fp->scopeChain;
    while (OBJ_GET_CLASS(cx, obj) == &js_WithClass && JSVAL_TO_INT(OBJ_GET_SLOT(cx, obj, JSSLOT_PRIVATE)) > i)
    {
    ...
    SAVE_SP_AND_PC(fp);
    rval = FETCH_OPND(-1);
    if (JSVAL_IS_PRIMITIVE(rval) || !(obj = JSVAL_TO_OBJECT(rval))->map->ops->hasInstance)
    {
        str = js_DecompileValueGenerator(cx, -1, rval, NULL);
        if (str)
        {
