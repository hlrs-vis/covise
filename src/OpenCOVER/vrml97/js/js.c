...
ValueToScript(JSContext *cx, jsval v)
{
    ...
    if (!JSVAL_IS_PRIMITIVE(v) && JS_GET_CLASS(cx, JSVAL_TO_OBJECT(v)) == &js_ScriptClass)
    {
            ...
            int32 *ip)
{
    ...
    *ip = 0;
    if (argc != 0)
    {
        v = argv[0];
        intarg = 0;
        if (!JSVAL_IS_PRIMITIVE(v) && (JS_GET_CLASS(cx, JSVAL_TO_OBJECT(v)) == &js_FunctionClass || JS_GET_CLASS(cx, JSVAL_TO_OBJECT(v)) == &js_ScriptClass))
        {
...
Disassemble(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (argc > 0 && JSVAL_IS_STRING(argv[0]) && !strcmp(JS_GetStringBytes(JSVAL_TO_STRING(argv[0])), "-l"))
    {
        lines = JS_TRUE;
        argv++, argc--;
    }
...
#define SHOW_FLAG(flag)       \
    if (flags & JSFUN_##flag) \
        fputs(" " #flag, stdout);
...
#define DUMP_ATTR(name)               \
    if (sprop->attrs & JSPROP_##name) \
    fputs(" " #name, fp)
        DUMP_ATTR(ENUMERATE);
        DUMP_ATTR(READONLY);
        DUMP_ATTR(PERMANENT);
...
GetPDA(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    *rval = OBJECT_TO_JSVAL(aobj);
    ...
    for (i = 0; i < pda.length; i++)
    {
        ...
        ok = JS_SetProperty(cx, pdobj, "id", &pd->id) && JS_SetProperty(cx, pdobj, "value", &pd->value) && (v = INT_TO_JSVAL(pd->flags),
                                                                                                            JS_SetProperty(cx, pdobj, "flags", &v)) && (v = INT_TO_JSVAL(pd->slot),
                                                                                                                                                        JS_SetProperty(cx, pdobj, "slot", &v)) && JS_SetProperty(cx, pdobj, "alias", &pd->alias);
        if (!ok)
            break;
                       ...
                       void **listenerTSData, JSDContext *jsdc)
{
    ...
    if (!jsdsrc)
    {
        ...
        else
        {
            ...
            if (jsdsrc && JSD_SOURCE_PARTIAL != JSD_GetSourceStatus(jsdc, jsdsrc))
            {
                jsdsrc = NULL;
            }
        }
...
its_setProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    if (JSVAL_IS_STRING(id) && !strcmp(JS_GetStringBytes(JSVAL_TO_STRING(id)), "noisy"))
    {
