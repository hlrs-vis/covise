...
js_ConstructXMLQNameObject(JSContext *cx, jsval nsval, jsval lnval)
{
     ...
     * production, step 2.
     */
    if (!JSVAL_IS_PRIMITIVE(nsval) && OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(nsval)) == &js_AnyNameClass)
    {
        nsval = JSVAL_NULL;
    }
...
js_IsXMLName(JSContext *cx, jsval v)
{
     ...
     */
    if (!JSVAL_IS_PRIMITIVE(v) && (clasp = OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(v)),
                                   clasp == &js_QNameClass.base || clasp == &js_AttributeNameClass || clasp == &js_AnyNameClass))
    {
...
Namespace(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (argc == 1)
    {
        ...
        else if (isQName && (qn = (JSXMLQName *)JS_GetPrivate(cx, uriobj))->uri)
        {
            ns->uri = qn->uri;
            ns->prefix = qn->prefix;
        }
    ...
    {
        if (isQName && (qn = (JSXMLQName *)JS_GetPrivate(cx, uriobj))->uri)
        {
            ns->uri = qn->uri;
        }
        else
...
QName(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    nameval = argv[argc > 1];
    isQName = !JSVAL_IS_PRIMITIVE(nameval) && OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(nameval)) == &js_QNameClass.base;
    ...
    else
    {
        ...
        else if (isQName && (qn = (JSXMLQName *)JS_GetPrivate(cx, nsobj))->uri)
        {
            uri = qn->uri;
            prefix = qn->prefix;
        }
...
XMLArraySetCapacity(JSContext *cx, JSXMLArray *array, uint32 capacity)
{
    ...
    {
        if ((size_t)capacity > ~(size_t)0 / sizeof(void *) || !(vector = (void **)realloc(array->vector, capacity * sizeof(void *))))
        {
            if (cx)
...
#define XMLARRAY_HAS_MEMBER(a, e, f) (XMLArrayFindMember(a, (void *)(e), f) != XML_NOT_FOUND)
...
#define IS_XML_CHARS(chars) \
    (JS_TOLOWER((chars)[0]) == 'x' && JS_TOLOWER((chars)[1]) == 'm' && JS_TOLOWER((chars)[2]) == 'l')

#define HAS_NS_AFTER_XML(chars) \
    (JS_TOLOWER((chars)[3]) == 'n' && JS_TOLOWER((chars)[4]) == 's')
...
#define PN2X_SKIP_CHILD ((JSXML *)1)
    ...
    case TOK_XMLELEM:
        ...
        while ((pn2 = pn2->pn_next) != NULL)
        {
            ...
            if ((flags & XSF_IGNORE_WHITESPACE) && n > 1 && pn2->pn_type == TOK_XMLSPACE)
            {
                --n;
                continue;
            }
            ...
            if ((flags & XSF_IGNORE_WHITESPACE) && n > 1 && kid->xml_class == JSXML_CLASS_TEXT)
            {
    ...
    case TOK_XMLPTAGC:
        ...
        while ((pn2 = *pnp) != NULL)
        {
            ...
            if (length >= 5 && IS_XMLNS_CHARS(chars) && (length == 5 || chars[5] == ':'))
            {
                JSString *uri, *prefix;
                ...
                {
                    if (!XMLARRAY_APPEND(cx, inScopeNSes, ns) || !XMLARRAY_APPEND(cx, &xml->xml_namespaces, ns))
                    {
                        goto fail;
                    }
                }
        ...
        for (i = 0; (pn2 = pn2->pn_next) != NULL; i++)
        {
            ...
            for (j = 0; j < i; j++)
            {
                ...
                attrjqn = attrj->name;
                if (!js_CompareStrings(attrjqn->uri, qn->uri) && !js_CompareStrings(attrjqn->localName, qn->localName))
                {
                    js_ReportCompileErrorNumber(cx, pn2,
                                                JSREPORT_PN | JSREPORT_ERROR,
                                                JSMSG_DUPLICATE_XML_ATTR,
...
#define constrlen(constr) (sizeof(constr) - 1)
    ...
    length = constrlen(prefix) + urilen + constrlen(middle) + srclen + constrlen(suffix);
...
ToXML(JSContext *cx, jsval v)
{
    ...
    else
    {
        ...
        if (clasp != &js_StringClass && clasp != &js_NumberClass && clasp != &js_BooleanClass)
        {
            goto bad;
        }
    }
                     ...
                     const jschar *suffix, size_t suffixlength)
{
    ...
    newlength = STRING_BUFFER_OFFSET(sb) + prefixlength + length + ((length2 != 0) ? 1 + length2 : 0) + suffixlength;
    bp = base = (jschar *)
...
GetNamespace(JSContext *cx, JSXMLQName *qn, const JSXMLArray *inScopeNSes)
{
    ...
    if (inScopeNSes)
    {
        for (i = 0, n = inScopeNSes->length; i < n; i++)
        {
             ...
             * declared namespace.
             */
            if (!js_CompareStrings(ns->uri, qn->uri) && (ns->prefix == qn->prefix || ((ns->prefix && qn->prefix)
...
GeneratePrefix(JSContext *cx, JSString *uri, JSXMLArray *decls)
{
    ...
    do
    {
        ...
        for (i = 0, n = decls->length; i < n; i++)
        {
            ...
            if (ns->prefix && JSSTRING_LENGTH(ns->prefix) == length && !memcmp(JSSTRING_CHARS(ns->prefix), cp, length * sizeof(jschar)))
            {
                if (!bp)
                {
               ...
               uintN indentLevel)
{
    ...
    if (!ns->prefix)
    {
         ...
         */
        if (!XMLARRAY_APPEND(cx, &ancdecls, ns) || !XMLARRAY_APPEND(cx, &decls, ns))
        {
            goto out;
        }
    }
    ...
    for (i = 0, n = xml->xml_attrs.length; i < n; i++)
    {
        ...
        if (!ns2->prefix)
        {
             ...
             * matters here: code at label out: releases strong refs in decls.
             */
            if (!XMLARRAY_APPEND(cx, &ancdecls, ns2) || !XMLARRAY_APPEND(cx, &decls, ns2))
            {
                goto out;
            }
        }
    ...
    else
    {
        ...
        js_AppendChar(&sb, '>');
        indentKids = n > 1 || (n == 1 && XMLARRAY_MEMBER(&xml->xml_kids, 0, JSXML)->xml_class != JSXML_CLASS_TEXT);
...
out:
    ...
    if (qn->uri && atom && (qn->uri == ATOM_TO_STRING(atom) || !js_CompareStrings(qn->uri, ATOM_TO_STRING(atom))))
    {
                 ...
                 uintN flags)
{
    ...
    for (i = j = 0; i < n; i++)
    {
        ...
        if ((flags & XSF_IGNORE_COMMENTS) && kid->xml_class == JSXML_CLASS_COMMENT)
        {
            continue;
        }
        if ((flags & XSF_IGNORE_PROCESSING_INSTRUCTIONS) && kid->xml_class == JSXML_CLASS_PROCESSING_INSTRUCTION)
        {
            continue;
        }
        if ((flags & XSF_IGNORE_WHITESPACE) && (kid->xml_flags & XMLF_WHITESPACE_TEXT))
        {
            continue;
        }
        ...
        if ((flags & XSF_IGNORE_WHITESPACE) && n > 1 && kid2->xml_class == JSXML_CLASS_TEXT)
        {
...
MatchAttrName(JSXMLQName *nameqn, JSXML *attr)
{
    ...
    return (IS_STAR(nameqn->localName) || !js_CompareStrings(attrqn->localName, nameqn->localName)) && (!nameqn->uri || !js_CompareStrings(attrqn->uri, nameqn->uri));
}

static JSBool
MatchElemName(JSXMLQName *nameqn, JSXML *elem)
{
    return (IS_STAR(nameqn->localName) || (elem->xml_class == JSXML_CLASS_ELEMENT && !js_CompareStrings(elem->name->localName, nameqn->localName))) && (!nameqn->uri || (elem->xml_class == JSXML_CLASS_ELEMENT && !js_CompareStrings(elem->name->uri, nameqn->uri)));
}
...
DescendantsHelper(JSContext *cx, JSXML *xml, JSXMLQName *nameqn, JSXML *list)
{
    ...
    if (xml->xml_class == JSXML_CLASS_ELEMENT && OBJ_GET_CLASS(cx, nameqn->object) == &js_AttributeNameClass)
    {
        for (i = 0, n = xml->xml_attrs.length; i < n; i++)
        {
            attr = XMLARRAY_MEMBER(&xml->xml_attrs, i, JSXML);
    ...
    {
        kid = XMLARRAY_MEMBER(&xml->xml_kids, i, JSXML);
        if (OBJ_GET_CLASS(cx, nameqn->object) != &js_AttributeNameClass && MatchElemName(nameqn, kid))
        {
            if (!Append(cx, list, kid))
                return JS_FALSE;
        }
...
retry:
    ...
    if (qn)
    {
        *bp = vqn && !js_CompareStrings(qn->localName, vqn->localName) && !js_CompareStrings(qn->uri, vqn->uri);
    }
    else
    {
        *bp = vqn == NULL;
    ...
    do
    {
        ...
        if (JSVAL_IS_FUNCTION(cx, fval))
        {
            if (xml && OBJECT_IS_XML(cx, obj))
            {
                ...
                if (!fun->interpreted && fun->u.n.spare && (fun->u.n.spare & CLASS_TO_MASK(xml->xml_class)) == 0)
                {
        ...
        else
        {
            ...
            if (JSXML_HAS_KIDS(xml))
            {
                ...
                for (i = 0, n = array->length; i < n; i++)
                {
                    ...
                    if (matcher(nameqn, kid))
                    {
                        if (array == &xml->xml_kids && kid->xml_class == JSXML_CLASS_ELEMENT)
                        {
...
PutProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    if (xml->xml_class == JSXML_CLASS_LIST)
    {
        ...
        if (js_IdIsIndex(id, &index))
        {
                ...
                if (kid->xml_class != JSXML_CLASS_ATTRIBUTE)
                {
                    ...
                    if (rxml)
                    {
                        ...
                        if (n != 0 && i != 0)
                        {
                            for (n = j, j = 0; j < n; j++)
                            {
                                if (rxml->xml_kids.vector[j] == xml->xml_kids.vector[i - 1])
                                {
                                    break;
                                }
                            }
            ...
            /* 2(d). */
            if (!vxml || vxml->xml_class == JSXML_CLASS_TEXT || vxml->xml_class == JSXML_CLASS_ATTRIBUTE)
            {
    ...
    else
    {
        ...
        if (OBJ_GET_CLASS(cx, nameobj) == &js_AttributeNameClass)
        {
            ...
            for (i = 0, n = xml->xml_attrs.length; i < n; i++)
            {
                attr = XMLARRAY_MEMBER(&xml->xml_attrs, i, JSXML);
                attrqn = attr->name;
                if (!js_CompareStrings(attrqn->localName, nameqn->localName) && (!nameqn->uri || !js_CompareStrings(attrqn->uri, nameqn->uri)))
                {
                    if (!match)
                    {
                        match = attr;
        ...
        /* 8-9. */
        if (!js_IsXMLName(cx, OBJECT_TO_JSVAL(nameobj)) && !IS_STAR(nameqn->localName))
        {
            goto out;
        }
...
ResolveValue(JSContext *cx, JSXML *list, JSXML **result)
{
    ...
    if (!target || !targetprop || OBJ_GET_CLASS(cx, targetprop->object) == &js_AttributeNameClass || IS_STAR(targetprop->localName))
    {
        *result = NULL;
        return JS_TRUE;
    }
...
xml_defineProperty(JSContext *cx, JSObject *obj, jsid id, jsval value,
                   ...
                   JSProperty **propp)
{
    if (JSVAL_IS_FUNCTION(cx, value) || getter || setter || (attrs & JSPROP_ENUMERATE) == 0 || (attrs & (JSPROP_READONLY | JSPROP_PERMANENT | JSPROP_SHARED)))
    {
...
xml_equality(JSContext *cx, JSObject *obj, jsval v, JSBool *bp)
{
    ...
    else if (vxml)
    {
        ...
        {
            if (((xml->xml_class == JSXML_CLASS_TEXT || xml->xml_class == JSXML_CLASS_ATTRIBUTE) && HasSimpleContent(vxml)) || ((vxml->xml_class == JSXML_CLASS_TEXT || vxml->xml_class == JSXML_CLASS_ATTRIBUTE) && HasSimpleContent(xml)))
            {
                ok = JS_EnterLocalRootScope(cx);
                if (ok)
                {
...
#define XML_METHOD_PROLOG                                                             \
    JS_BEGIN_MACRO xml = (JSXML *)JS_GetInstancePrivate(cx, obj, &js_XMLClass, argv); \
    if (!xml)                                                                         \
        return JS_FALSE;                                                              \
    JS_END_MACRO
...
xml_child(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (xml->xml_class == JSXML_CLASS_LIST)
    {
        ...
        for (i = 0, n = xml->xml_kids.length; i < n; i++)
        {
            ...
            if ((!JSXML_HAS_KIDS(vxml) || vxml->xml_kids.length != 0) && !Append(cx, list, vxml))
            {
                return JS_FALSE;
            }
        }
             ...
             jsval *rval)
{
    ...
    else
    {
        ...
        {
            kid = XMLARRAY_MEMBER(&xml->xml_kids, i, JSXML);
            if (kid->xml_class == JSXML_CLASS_ELEMENT && MatchElemName(nameqn, kid))
            {
                ok = Append(cx, list, kid);
                if (!ok)
                    break;
              ...
              jsval *rval)
{
    ...
    XML_METHOD_PROLOG;
    if (argc == 0 && (xml->xml_class == JSXML_CLASS_TEXT || xml->xml_class == JSXML_CLASS_COMMENT || xml->xml_class == JSXML_CLASS_PROCESSING_INSTRUCTION))
    {
        *rval = JSVAL_NULL;
        return JS_TRUE;
    }
              ...
              jsval *rval)
{
    ...
    *rval = OBJECT_TO_JSVAL(obj);
    ...
    for (i = 0, n = xml->xml_kids.length; i < n; i++)
    {
        ...
        {
            while (i + 1 < n && (kid2 = XMLARRAY_MEMBER(&xml->xml_kids, i + 1, JSXML))->xml_class == JSXML_CLASS_TEXT)
            {
                           ...
                           jsval *argv, jsval *rval)
{
    ...
    else
    {
        ...
        {
            kid = XMLARRAY_MEMBER(&xml->xml_kids, i, JSXML);
            if (kid->xml_class == JSXML_CLASS_PROCESSING_INSTRUCTION && (IS_STAR(nameqn->localName) || !js_CompareStrings(nameqn->localName, kid->name->localName)))
            {
                ok = Append(cx, list, kid);
                if (!ok)
                    break;
...
namespace_full_match(const void *a, const void *b)
{
    ...
    if (nsa->prefix && nsb->prefix && js_CompareStrings(nsa->prefix, nsb->prefix))
    {
        return JS_FALSE;
    }
    return !js_CompareStrings(nsa->uri, nsb->uri);
                 ...
                 jsval *rval)
{
    ...
    name = argv[0];
    if (!JSVAL_IS_PRIMITIVE(name) && OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(name)) == &js_QNameClass.base)
    {
...
xml_setName(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    name = argv[0];
    if (!JSVAL_IS_PRIMITIVE(name) && OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(name)) == &js_QNameClass.base && !(nameqn = (JSXMLQName *)JS_GetPrivate(cx, JSVAL_TO_OBJECT(name)))->uri)
    {
                 ...
                 jsval *rval)
{
    ...
    XML_METHOD_PROLOG;
    if (xml->xml_class != JSXML_CLASS_ELEMENT && xml->xml_class != JSXML_CLASS_ATTRIBUTE)
    {
        return JS_TRUE;
    }
...
xml_toString_helper(JSContext *cx, JSXML *xml)
{
    ...
    if (xml->xml_class == JSXML_CLASS_ATTRIBUTE || xml->xml_class == JSXML_CLASS_TEXT)
    {
    ...
    {
        kid = XMLARRAY_MEMBER(&xml->xml_kids, i, JSXML);
        if (kid->xml_class != JSXML_CLASS_COMMENT && kid->xml_class != JSXML_CLASS_PROCESSING_INSTRUCTION)
        {
...
XML(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    *rval = OBJECT_TO_JSVAL(xobj);
    ...
    if ((cx->fp->flags & JSFRAME_CONSTRUCTING) && !JSVAL_IS_PRIMITIVE(v))
    {
        ...
        if (clasp == &js_XMLClass || (clasp->flags & JSCLASS_DOCUMENT_OBSERVER))
        {
    ...
    *flagp |= GCF_MARK;
    ...
    for (;;)
    {
        ...
        for (n = JSXML_LENGTH(xml); i <= n; i++)
        {
             ...
             */
            if (JSXML_HAS_KIDS(kid) && (JSXML_HAS_VALUE(xml) || !xml->xml_kids.cursors))
            {
                if (i < n)
...
js_GetXMLObject(JSContext *cx, JSXML *xml)
{
     ...
     */
    JS_ASSERT(!xml->parent || !xml->parent->object || JS_IS_OBJ_LOCKED(cx, xml->parent->object));
...
js_GetDefaultXMLNamespace(JSContext *cx, jsval *vp)
{
    ...
    if (!nsobj)
        return JS_FALSE;
    v = OBJECT_TO_JSVAL(nsobj);
    if (obj && !OBJ_DEFINE_PROPERTY(cx, obj, JS_DEFAULT_XML_NAMESPACE_ID, v, JS_PropertyStub, JS_PropertyStub, JSPROP_PERMANENT, NULL))
    {
        return JS_FALSE;
    }
...
js_AddAttributePart(JSContext *cx, JSBool isName, JSString *str, JSString *str2)
{
    ...
    if (JSSTRING_IS_DEPENDENT(str) || !(*js_GetGCThingFlags(str) & GCF_MUTABLE))
    {
...
js_CloneXMLObject(JSContext *cx, JSObject *obj)
{
    ...
    if (!GetXMLSettingFlags(cx, &flags))
        return NULL;
    xml = (JSXML *)JS_GetPrivate(cx, obj);
    if (flags & (XSF_IGNORE_COMMENTS | XSF_IGNORE_PROCESSING_INSTRUCTIONS | XSF_IGNORE_WHITESPACE))
    {
                       ...
                       JSString *value)
{
    ...
    if ((xml_class == JSXML_CLASS_COMMENT && (flags & XSF_IGNORE_COMMENTS)) || (xml_class == JSXML_CLASS_PROCESSING_INSTRUCTION && (flags & XSF_IGNORE_PROCESSING_INSTRUCTIONS)))
    {
