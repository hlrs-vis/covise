...
#define REOP_IS_SIMPLE(op) ((unsigned)((op)-REOP_SIMPLE_START) < (unsigned)REOP_SIMPLE_END)
...
#define RE_IS_LETTER(c) (((c >= 'A') && (c <= 'Z')) || ((c >= 'a') && (c <= 'z')))
          ...
          intN operandSP)
{
    ...
    case REOP_ALT:
         ...
         */
        if (((RENode *)result->kid)->op == REOP_FLAT && ((RENode *)result->u.kid2)->op == REOP_FLAT && (state->flags & JSREG_FOLD) == 0)
        {
        ...
        else if (((RENode *)result->kid)->op == REOP_CLASS && ((RENode *)result->kid)->u.ucclass.index < 256 && ((RENode *)result->u.kid2)->op == REOP_FLAT && (state->flags & JSREG_FOLD) == 0)
        {
        ...
        else if (((RENode *)result->kid)->op == REOP_FLAT && ((RENode *)result->u.kid2)->op == REOP_CLASS && ((RENode *)result->u.kid2)->u.ucclass.index < 256 && (state->flags & JSREG_FOLD) == 0)
        {
            result->op = REOP_ALTPREREQ2;
            result->u.altprereq.ch1 = ((RENode *)result->kid)->u.flat.chr;
            result->u.altprereq.ch2 = ((RENode *)result->u.kid2)->u.ucclass.index;
            /* ALTPREREQ2, <end>, uch1, uch2, <next>, ...,
                                          JUMP, <end> ... ENDALT */
            state->progLength += 13;
        }
...
ParseRegExp(CompilerState *state)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            case '(':
                ++state->cp;
                if (state->cp + 1 < state->cpend && *state->cp == '?' && (state->cp[1] == '=' || state->cp[1] == '!' || state->cp[1] == ':'))
                {
                    switch (state->cp[1])
                    {
                    case '=':
            ...
            case ')':
                ...
                for (i = operatorSP - 1;; i--)
                {
                    ...
                    if (operatorStack[i].op == REOP_ASSERT || operatorStack[i].op == REOP_ASSERT_NOT || operatorStack[i].op == REOP_LPARENNON || operatorStack[i].op == REOP_LPAREN)
                    {
                        break;
                    }
                }
    ...
    restartOperator:
        ...
        case '|':
            ...
            ++state->cp;
            while (operatorSP && operatorStack[operatorSP - 1].op == REOP_CONCAT)
            {
                --operatorSP;
                if (!ProcessOp(state, &operatorStack[operatorSP],
                               operandStack, operandSP))
        ...
        case ')':
            ...
            for (;;)
            {
                ...
                case REOP_LPAREN:
                    ...
                    if (!operand)
                        goto out;
                    operand->u.parenIndex = operatorStack[operatorSP].parenIndex;
                    JS_ASSERT(operandSP);
    ...
    case '\\':
        ...
        case '0':
            ...
            if (!js_ReportCompileErrorNumber(state->context,
                                             state->tokenStream,
                                             JSREPORT_TS | JSREPORT_WARNING | JSREPORT_STRICT,
               ...
               jsbytecode *pc, RENode *t)
{
    ...
    {
        emitStateStack = (EmitStateStackEntry *)JS_malloc(state->context,
                                                          sizeof(EmitStateStackEntry) * treeDepth);
        if (!emitStateStack)
            return NULL;
    }
    ...
    for (;;)
    {
        *pc++ = op;
        ...
        case REOP_ENDALT:
             ...
             */
            if ((size_t)(pc - re->program) > OFFSET_MAX && emitStateSP > emitStateStack)
            {
                ...
                {
                    if (esp->continueOp == REOP_ENDALT && !esp->jumpToJumpFlag && esp->nextTermFixup + OFFSET_LEN == alt && (size_t)(pc - ((esp->continueNode->op != REOP_ALT) ? esp->endTermFixup : esp->nextTermFixup)) > OFFSET_MAX)
                    {
                        alt = esp->altHead;
        ...
        case REOP_FLAT:
             ...
             */
            if (t->kid && GetCompactIndexWidth((jschar *)t->kid - state->cpbegin) <= 4)
            {
                while (t->next && t->next->op == REOP_FLAT && (jschar *)t->kid + t->u.flat.length == (jschar *)t->next->kid)
                {
...
{
    size_t i;
    REBackTrackData *result = (REBackTrackData *)((char *)gData->backTrackSP + gData->cursz);

    size_t sz = sizeof(REBackTrackData) + gData->stateStackTop * sizeof(REProgState) + parenCount * sizeof(RECapture);

    ptrdiff_t btsize = gData->backTrackStackSize;
    ptrdiff_t btincr = ((char *)result + sz) - ((char *)gData->backTrackStack + btsize);
    ...
    if (parenCount != 0)
    {
        ...
        memcpy((char *)(result + 1) + sizeof(REProgState) * result->saveStateStackTop,
               &x->parens[parenIndex],
...
#define PUSH_STATE_STACK(data)                                                          \
    JS_BEGIN_MACRO++(data)->stateStackTop;                                              \
    if ((data)->stateStackTop == (data)->stateStackLimit && !ReallocStateStack((data))) \
    {                                                                                   \
        return NULL;                                                                    \
    }                                                                                   \
    JS_END_MACRO
            ...
            jsbytecode **startpc, JSBool updatecp)
{
    ...
    case REOP_BOL:
        ...
        {
            if (!gData->cx->regExpStatics.multiline && !(gData->regexp->flags & JSREG_MULTILINE))
            {
                break;
            }
    ...
    case REOP_EOL:
        ...
        break;
    case REOP_WBDRY:
        if ((x->cp == gData->cpbegin || !JS_ISWORD(x->cp[-1])) ^ !(x->cp != gData->cpend && JS_ISWORD(*x->cp)))
        {
            result = x;
        }
        break;
    ...
    case REOP_CLASS:
        ...
        if (x->cp != gData->cpend)
        {
            ...
            ch = *x->cp;
            index = ch >> 3;
            if (charSet->length != 0 && ch <= charSet->length && (charSet->u.bits[index] & (1 << (ch & 0x7))))
            {
                result = x;
                result->cp++;
            }
...
ExecuteREBytecode(REGlobalData *gData, REMatchState *x)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            case REOP_ALTPREREQ2:
                ...
                if (x->cp != gData->cpend)
                {
                    ...
                    matchCh1 = *x->cp;
                    k = matchCh1 >> 3;
                    if ((charSet->length == 0 || matchCh1 > charSet->length || !(charSet->u.bits[k] & (1 << (matchCh1 & 0x7)))) ^ charSet->sense)
                    {
                        goto doAlt;
                    }
                }
            ...
            case REOP_ALTPREREQ:
                ...
                matchCh2 = GET_ARG(pc);
                pc += ARG_LEN;
                if (x->cp == gData->cpend || (*x->cp != matchCh1 && *x->cp != matchCh2))
                {
                    result = NULL;
                    break;
                }
            ...
            case REOP_ASSERT:
                ...
                op = (REOp)*pc++;
                testpc = pc;
                if (REOP_IS_SIMPLE(op) && !SimpleMatch(gData, x, op, &testpc, JS_FALSE))
                {
                    result = NULL;
                    break;
                }
                curState->u.assertion.top = (char *)gData->backTrackSP - (char *)gData->backTrackStack;
            ...
            case REOP_ASSERT_NOT:
                ...
                op = (REOp)*pc++;
                testpc = pc;
                if (REOP_IS_SIMPLE(op) /* Note - fail to fail! */ && SimpleMatch(gData, x, op, &testpc, JS_FALSE) && *testpc == REOP_ASSERTNOTTEST)
                {
                    result = NULL;
                    break;
                }
            ...
            case REOP_ASSERTTEST:
                ...
                gData->backTrackSP = (REBackTrackData *)((char *)gData->backTrackStack + curState->u.assertion.top);
            ...
            quantcommon:
                ...
                curState->parenSoFar = parenSoFar;
                PUSH_STATE_STACK(gData);
                if (curState->u.quantifier.min == 0 && !PushBackTrackState(gData, REOP_REPEAT, pc, x, startcp, 0, 0))
                {
                    return NULL;
                }
                pc = nextpc;
            ...
            case REOP_REPEAT:
                ...
                do
                {
                    ...
                    if (curState->u.quantifier.min == 0 && x->cp == gData->cpbegin + curState->index)
                    {
                    ...
                    curState->index = startcp - gData->cpbegin;
                    PUSH_STATE_STACK(gData);
                    if (curState->u.quantifier.min == 0 && !PushBackTrackState(gData, REOP_REPEAT, pc, x, startcp, curState->parenSoFar, parenSoFar - curState->parenSoFar))
                    {
                        return NULL;
                    }
            ...
            case REOP_MINIMALREPEAT:
                ...
                if (!result)
                {
                     ...
                     */
                    if (curState->u.quantifier.max == (uintN)-1 || curState->u.quantifier.max > 0)
                    {
                        curState->index = x->cp - gData->cpbegin;
                        curState->continue_op = REOP_MINIMALREPEAT;
                        curState->continue_pc = pc;
        ...
        if (!result)
        {
            ...
            gData->backTrackSP = (REBackTrackData *)((char *)backTrackData - backTrackData->sz);
            ...
            if (backTrackData->parenCount)
            {
                memcpy(&x->parens[backTrackData->parenIndex],
                       (char *)(backTrackData + 1) + sizeof(REProgState) * backTrackData->saveStateStackTop,
...
#define DEFVAL(val, id)                                                                                     \
    {                                                                                                       \
        ok = js_DefineProperty(cx, obj, id, val, JS_PropertyStub, JS_PropertyStub, JSPROP_ENUMERATE, NULL); \
        if (!ok)                                                                                            \
        {                                                                                                   \
...
regexp_setProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    if (slot == REGEXP_LAST_INDEX)
    {
        ...
        ok = js_NewNumberValue(cx, lastIndex, vp) && JS_SetReservedSlot(cx, obj, 0, *vp);
    }
    return ok;
}
...
regexp_xdrObject(JSXDRState *xdr, JSObject **objp)
{
    ...
    if (xdr->mode == JSXDR_DECODE)
    {
        ...
        if (!re)
            return JS_FALSE;
        if (!JS_SetPrivate(xdr->cx, obj, re) || !js_SetLastIndex(xdr->cx, obj, 0))
        {
...
RegExp(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    if (!(cx->fp->flags & JSFRAME_CONSTRUCTING))
    {
         ...
         */
        if ((argc < 2 || JSVAL_IS_VOID(argv[1])) && !JSVAL_IS_PRIMITIVE(argv[0]) && OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(argv[0])) == &js_RegExpClass)
        {
            *rval = argv[0];
            return JS_TRUE;
        }
...
js_InitRegExpClass(JSContext *cx, JSObject *obj)
{
    ...
    if (!proto || !(ctor = JS_GetConstructor(cx, proto)))
        return NULL;
    if (!JS_AliasProperty(cx, ctor, "input", "$_") || !JS_AliasProperty(cx, ctor, "multiline", "$*") || !JS_AliasProperty(cx, ctor, "lastMatch", "$&") || !JS_AliasProperty(cx, ctor, "lastParen", "$+") || !JS_AliasProperty(cx, ctor, "leftContext", "$`") || !JS_AliasProperty(cx, ctor, "rightContext", "$'"))
    {
        goto bad;
    }
...
js_GetLastIndex(JSContext *cx, JSObject *obj, jsdouble *lastIndex)
{
    ...
    return JS_GetReservedSlot(cx, obj, 0, &v) && js_ValueToNumber(cx, v, lastIndex);
}
...
js_SetLastIndex(JSContext *cx, JSObject *obj, jsdouble lastIndex)
{
    ...
    return js_NewNumberValue(cx, lastIndex, &v) && JS_SetReservedSlot(cx, obj, 0, v);
}
