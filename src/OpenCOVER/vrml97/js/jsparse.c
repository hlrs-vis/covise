...
#define MUST_MATCH_TOKEN(tt, errno)                                               \
    JS_BEGIN_MACRO if (js_GetToken(cx, ts) != tt)                                 \
    {                                                                             \
        js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_ERROR, errno); \
        return NULL;                                                              \
    }                                                                             \
    JS_END_MACRO

#define CHECK_RECURSION()                                                                       \
    JS_BEGIN_MACRO int stackDummy;                                                              \
    if (!JS_CHECK_STACK_SIZE(cx, stackDummy))                                                   \
    {                                                                                           \
        js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_ERROR, JSMSG_OVER_RECURSED); \
        return NULL;                                                                            \
    }                                                                                           \
    JS_END_MACRO
          ...
          JSTreeContext *tc)
{
     ...
     */
    if (left->pn_type == tt && left->pn_op == op && (js_CodeSpec[op].format & JOF_LEFTASSOC))
    {
     ...
     * to "3pt", not "12pt").
     */
    if (tt == TOK_PLUS && left->pn_type == TOK_NUMBER && right->pn_type == TOK_NUMBER)
    {
...
CheckGetterOrSetter(JSContext *cx, JSTokenStream *ts, JSTokenType tt)
{
    ...
    if (!name || !js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_WARNING | JSREPORT_STRICT, JSMSG_DEPRECATED_USAGE, name))
    {
        return TOK_ERROR;
    }
    return tt;
                      ...
                      JSCodeGenerator *cg)
{
    ...
    flags = cx->fp->flags;
    cx->fp->flags = flags | (JS_HAS_COMPILE_N_GO_OPTION(cx)
             ...
             JSTreeContext *tc)
{
    ...
    fp = cx->fp;
    funobj = fun->object;
    if (!fp || fp->fun != fun || fp->varobj != funobj || fp->scopeChain != funobj)
    {
        memset(&frame, 0, sizeof frame);
        frame.fun = fun;
...
/* Make a TOK_FUNCTION node. */
    ...
    if (!lambda && funAtom)
    {
        ...
        if (ale)
        {
            ...
            if (JS_HAS_STRICT_OPTION(cx) || prevop == JSOP_DEFCONST)
            {
                ...
                if (!name || !js_ReportCompileErrorNumber(cx, ts, (prevop != JSOP_DEFCONST) ? JSREPORT_TS | JSREPORT_WARNING | JSREPORT_STRICT : JSREPORT_TS | JSREPORT_ERROR, JSMSG_REDECLARED_VAR, (prevop == JSOP_DEFFUN || prevop == JSOP_CLOSURE) ? js_function_str : (prevop == JSOP_DEFCONST) ? js_const_str : js_var_str, name))
                {
                    return NULL;
                }
            }
        ...
        if (!tc->topStmt && (tc->flags & TCF_IN_FUNCTION))
        {
            ...
            if (!prop || pobj != varobj || ((JSScopeProperty *)prop)->getter != js_GetLocalVariable)
            {
    ...
    if (!js_MatchToken(cx, ts, TOK_RP))
    {
        do
        {
            ...
            {
                ok = JS_TRUE;
                if (pobj == fun->object && ((JSScopeProperty *)prop)->getter == js_GetArgument)
                {
                     ...
                     */
                    ok = name && js_ReportCompileErrorNumber(cx, ts,
                                                             JSREPORT_TS | JSREPORT_WARNING | JSREPORT_STRICT,
    ...
    else
    {
         ...
         */
        if ((!lambda && funAtom && tc->topStmt) || (funtc.flags & TCF_FUN_USES_NONLOCALS))
        {
            tc->flags |= TCF_FUN_HEAVYWEIGHT;
        }
    }
...
Statements(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    while ((tt = js_PeekToken(cx, ts)) > TOK_EOF && tt != TOK_RC)
    {
        ...
        {
            if (cx->fp->fun && JS_HAS_STRICT_OPTION(cx) && (tc->flags & TCF_RETURN_EXPR))
            {
                /*
                ...
                if ((tt == TOK_EOF || tt == TOK_RC) && !CheckFinalReturn(cx, ts, pn2))
                {
                    tt = TOK_ERROR;
                    break;
                }
            ...
            if (!js_FoldConstants(cx, pn2, tc) || !js_AllocTryNotes(cx, (JSCodeGenerator *)tc) || !js_EmitTree(cx, (JSCodeGenerator *)tc, pn2))
            {
                tt = TOK_ERROR;
                break;
            }
...
Condition(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
     ...
     */
    if (pn->pn_type == TOK_ASSIGN && pn->pn_op == JSOP_NOP && pn->pn_right->pn_type > TOK_EQOP)
    {
...
Statement(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    case TOK_SWITCH:
    {
        ...
        while ((tt = js_GetToken(cx, ts)) != TOK_RC)
        {
            ...
            PN_INIT_LIST(pn4);
            ts->flags |= TSF_OPERAND;
            while ((tt = js_PeekToken(cx, ts)) != TOK_RC && tt != TOK_CASE && tt != TOK_DEFAULT)
            {
                ts->flags &= ~TSF_OPERAND;
                if (tt == TOK_ERROR)
                    return NULL;
...
/* Set pn1 to a var list or an initializing expression. */
        ...
        if (pn1 && js_MatchToken(cx, ts, TOK_IN))
        {
            ...
            if ((pn1->pn_type == TOK_VAR)
                    ...
                    : (pn1->pn_type != TOK_NAME && pn1->pn_type != TOK_DOT &&
#if JS_HAS_LVALUE_RETURN
                       pn1->pn_type != TOK_LP &&
#endif
#if JS_HAS_XML_SUPPORT
                       (pn1->pn_type != TOK_UNARYOP || pn1->pn_op != JSOP_XMLNAME) &&
#endif
                       pn1->pn_type != TOK_LB))
            {
                js_ReportCompileErrorNumber(cx, ts,
            ...
            if (pn2->pn_type == TOK_NAME && pn2->pn_atom == cx->runtime->atomState.argumentsAtom)
            {
                tc->flags |= TCF_FUN_HEAVYWEIGHT;
            }
    ...
    case TOK_RETURN:
        ...
        if (JS_HAS_STRICT_OPTION(cx) && (~tc->flags & (TCF_RETURN_EXPR | TCF_RETURN_VOID)) == 0)
        {
            /*
    ...
    case TOK_DEFAULT:
        ...
        if (!pn)
            return NULL;
        if (!js_MatchToken(cx, ts, TOK_NAME) || CURRENT_TOKEN(ts).t_atom != cx->runtime->atomState.xmlAtom || !js_MatchToken(cx, ts, TOK_NAME) || CURRENT_TOKEN(ts).t_atom != cx->runtime->atomState.namespaceAtom || !js_MatchToken(cx, ts, TOK_ASSIGN) || CURRENT_TOKEN(ts).t_op != JSOP_NOP)
        {
            js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_ERROR,
    ...
    do
    {
        ...
        if (ale)
        {
                    ...
                    : pn->pn_op == JSOP_DEFCONST || prevop == JSOP_DEFCONST)
            {
                ...
                if (!name || !js_ReportCompileErrorNumber(cx, ts, (pn->pn_op != JSOP_DEFCONST && prevop != JSOP_DEFCONST) ? JSREPORT_TS | JSREPORT_WARNING | JSREPORT_STRICT : JSREPORT_TS | JSREPORT_ERROR, JSMSG_REDECLARED_VAR, (prevop == JSOP_DEFFUN || prevop == JSOP_CLOSURE) ? js_function_str : (prevop == JSOP_DEFCONST) ? js_const_str : js_var_str, name))
                {
                    return NULL;
                }
            }
            ...
            else
            {
                if (fun)
                {
                    ...
                    if (clasp == &js_FunctionClass)
                    {
                        ...
                        JS_ASSERT((sprop->flags & SPROP_HAS_SHORTID) && (uint16)sprop->shortid < fun->u.i.nvars);
                    }
        ...
        else
        {
            ...
            if (currentGetter == js_GetLocalVariable && atom != cx->runtime->atomState.argumentsAtom && fp->scopeChain == obj && !js_InWithStatement(tc))
            {
...
RelExpr(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    while (pn && (js_MatchToken(cx, ts, TOK_RELOP)
#if JS_HAS_IN_OPERATOR
                      /*
...
AddExpr(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    pn = MulExpr(cx, ts, tc);
    while (pn && (js_MatchToken(cx, ts, TOK_PLUS) || js_MatchToken(cx, ts, TOK_MINUS)))
    {
        tt = CURRENT_TOKEN(ts).type;
...
MulExpr(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    while (pn && (js_MatchToken(cx, ts, TOK_STAR) || js_MatchToken(cx, ts, TOK_DIVOP)))
    {
        tt = CURRENT_TOKEN(ts).type;
        op = CURRENT_TOKEN(ts).t_op;
           ...
           const char *name)
{
    while (kid->pn_type == TOK_RP)
        kid = kid->pn_kid;
    if (kid->pn_type != TOK_NAME && kid->pn_type != TOK_DOT &&
#if JS_HAS_LVALUE_RETURN
           ...
           JSBool allowCallSyntax)
{
    ...
    else
    {
        ...
        if (pn->pn_type == TOK_ANYNAME || pn->pn_type == TOK_AT || pn->pn_type == TOK_DBLCOLON)
        {
    ...
    while ((tt = js_GetToken(cx, ts)) > TOK_EOF)
    {
        if (tt == TOK_DOT)
        {
            ...
            if (!pn3)
                return NULL;
            tt = pn3->pn_type;
            if (tt == TOK_NAME || (tt == TOK_DBLCOLON && pn3->pn_arity == PN_NAME && pn3->pn_expr->pn_type == TOK_FUNCTION))
            {
        ...
        else if (allowCallSyntax && tt == TOK_LP)
        {
            ...
            pn2->pn_op = JSOP_CALL;
            if (pn->pn_op == JSOP_NAME && pn->pn_atom == cx->runtime->atomState.evalAtom)
            {
                pn2->pn_op = JSOP_EVAL;
                tc->flags |= TCF_FUN_HEAVYWEIGHT;
            }
...
#define XML_CHECK_FOR_ERROR_AND_EOF(tt, result)                                                         \
    JS_BEGIN_MACRO if ((tt) <= TOK_EOF)                                                                 \
    {                                                                                                   \
        if ((tt) == TOK_EOF)                                                                            \
        {                                                                                               \
            js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_ERROR, JSMSG_END_OF_XML_SOURCE); \
        }                                                                                               \
        return result;                                                                                  \
    }                                                                                                   \
    JS_END_MACRO
                  ...
                  JSTreeContext *tc)
{
    ...
    for (;;)
    {
        ...
        {
            JS_ASSERT(tt == TOK_XMLCDATA || tt == TOK_XMLCOMMENT || tt == TOK_XMLPI);
            pn2 = XMLAtomNode(cx, ts, tc);
        }
        if (!pn2)
            return JS_FALSE;
        ...
        if (!js_MatchToken(cx, ts, TOK_RC))
        {
            do
            {
                ...
                case TOK_NAME:
                    ...
                    atom = CURRENT_TOKEN(ts).t_atom;
                    rt = cx->runtime;
                    if (atom == rt->atomState.getAtom || atom == rt->atomState.setAtom)
                    {
...
/* FALL THROUGH */
    ...
    case TOK_OBJECT:
        ...
        if (tt == TOK_NAME)
        {
            ...
            if (pn->pn_atom == cx->runtime->atomState.parentAtom || pn->pn_atom == cx->runtime->atomState.protoAtom)
            {
                tc->flags |= TCF_FUN_HEAVYWEIGHT;
            }
            else
            {
                ...
                if (ale && !(fp = cx->fp)->fun && fp->scopeChain == fp->varobj && !js_InWithStatement(tc) && !js_InCatchBlock(tc, pn->pn_atom))
                {
                    tc->globalUses++;
                    for (stmt = tc->topStmt; stmt; stmt = stmt->down)
                    {
...
ContainsVarStmt(JSParseNode *pn)
{
    ...
    case PN_LIST:
        ...
        break;
    case PN_TERNARY:
        return ContainsVarStmt(pn->pn_kid1) || ContainsVarStmt(pn->pn_kid2) || ContainsVarStmt(pn->pn_kid3);
    case PN_BINARY:
        /*
...
FoldXMLConstants(JSContext *cx, JSParseNode *pn, JSTreeContext *tc)
{
    ...
    for (pn2 = pn1, i = j = 0; pn2; pn2 = pn2->pn_next, i++)
    {
        ...
        default:
            JS_ASSERT(*pnp == pn1);
            if ((tt == TOK_XMLSTAGO || tt == TOK_XMLPTAGC) && (i & 1) ^ (j & 1))
            {
...
js_FoldConstants(JSContext *cx, JSParseNode *pn, JSTreeContext *tc)
{
    ...
    do_binary_op:
        ...
        {
            JS_ASSERT(pn->pn_arity == PN_BINARY);
            if (!FoldType(cx, pn1, TOK_NUMBER) || !FoldType(cx, pn2, TOK_NUMBER))
            {
                return JS_FALSE;
            }
