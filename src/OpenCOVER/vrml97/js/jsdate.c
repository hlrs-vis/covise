...
date_getYear(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (version == JSVERSION_1_0 || version == JSVERSION_1_1 || version == JSVERSION_1_2)
    {
    ...
    else
    {
        ...
        if (PRMJ_FormatTime(tzbuf, sizeof tzbuf, "(%Z)", &split) != 0)
        {
            ...
            else
            {
                ...
                {
                    jschar c = tzbuf[i];
                    if (c > 127 || !(isalpha(c) || isdigit(c) || c == ' ' || c == '(' || c == ')'))
                    {
                        usetz = JS_FALSE;
                    }
                }
                    ...
                    jsval *argv, jsval *rval, char *format)
{
    ...
    else
    {
        ...
        if (strcmp(format, "%x") == 0 && result_len >= 6 &&
            ...
            !isdigit(buf[result_len - 3]) && isdigit(buf[result_len - 2]) && isdigit(buf[result_len - 1]) &&
            /* ...but not if starts with 4-digit year, like 2022/3/11. */
            !(isdigit(buf[0]) && isdigit(buf[1]) && isdigit(buf[2]) && isdigit(buf[3])))
        {
