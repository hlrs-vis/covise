...
#define SCOPE_REMOVE_LAST_PROP(scope) ((scope)->lastProp = (scope)->lastProp->parent)
...
#define SPROP_FLAG_COLLISION(spp, sprop) (*(spp) = (JSScopeProperty *)((jsuword)(sprop) | SPROP_COLLISION))
...
#define SPROP_CALL_GETTER(cx, sprop, getter, obj, obj2, vp) \
    (!(getter) || (getter)(cx, OBJ_THIS_OBJECT(cx, obj), SPROP_USERID(sprop), vp))
#define SPROP_CALL_SETTER(cx, sprop, setter, obj, obj2, vp) \
    (!(setter) || (setter)(cx, OBJ_THIS_OBJECT(cx, obj), SPROP_USERID(sprop), vp))
...
#define ID_TO_VALUE(id) (JSID_IS_ATOM(id) ? ATOM_JSID_TO_JSVAL(id) : JSID_IS_OBJECT(id) ? OBJECT_JSID_TO_JSVAL(id) : (jsval)(id))
#define HASH_ID(id) (JSID_IS_ATOM(id) ? JSID_TO_ATOM(id)->number : JSID_IS_OBJECT(id) ? (jsatomid)JSID_CLRTAG(id) : (jsatomid)JSID_TO_INT(id))
