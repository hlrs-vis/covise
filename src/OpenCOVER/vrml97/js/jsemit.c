           ...
           ptrdiff_t off)
{
    ...
    if ((index & (index - 1)) == 0 && (!(sdbase = cg->spanDeps) || index >= SPANDEPS_MIN))
    {
        if (!sdbase)
        {
...
OptimizeSpanDeps(JSContext *cx, JSCodeGenerator *cg)
{
    ...
    do
    {
        ...
        for (sd = sdbase; sd < sdlimit; sd++)
        {
            ...
            if (sd->top != top)
            {
                ...
                JS_ASSERT(type == JOF_JUMP || type == JOF_TABLESWITCH || type == JOF_LOOKUPSWITCH);
            }
...
#ifdef DEBUG_brendan
    {
        ...
        for (sd = sdbase; sd < sdlimit; sd++)
        {
            ...
            if (sd->top != top)
            {
                JS_ASSERT(top == -1 || !JOF_TYPE_IS_EXTENDED_JUMP(type) || bigspans != 0);
                bigspans = 0;
                top = sd->top;
                JS_ASSERT(top == sd->before);
                op = (JSOp)base[offset];
                type = (js_CodeSpec[op].format & JOF_TYPEMASK);
                JS_ASSERT(type == JOF_JUMP || type == JOF_JUMPX || type == JOF_TABLESWITCH || type == JOF_TABLESWITCHX || type == JOF_LOOKUPSWITCH || type == JOF_LOOKUPSWITCHX);
                pivot = offset;
            }
            ...
            if (JOF_TYPE_IS_EXTENDED_JUMP(type))
            {
                ...
                {
                    JS_ASSERT(type == JOF_TABLESWITCHX || type == JOF_LOOKUPSWITCHX);
                }
            }
            else
            {
...
js_PopStatementCG(JSContext *cx, JSCodeGenerator *cg)
{
    ...
    if (!BackPatch(cx, cg, stmt->breaks, CG_NEXT(cg), JSOP_GOTO) || !BackPatch(cx, cg, stmt->continues, CG_CODE(cg, stmt->update), JSOP_GOTO))
    {
        return JS_FALSE;
    }
                             ...
                             jsval *vp)
{
    ...
    *vp = JSVAL_VOID;
    ...
    do
    {
        ...
        obj = fp->varobj;
        if (obj == fp->scopeChain && !js_InWithStatement(&cg->treeContext) && !js_InCatchBlock(&cg->treeContext, atom))
        {
            ...
            if (ok)
            {
                if (pobj == obj && (fp->flags & (JSFRAME_EVAL | JSFRAME_COMPILE_N_GO)))
                {
                    /*
...
#define EMIT_UINT16_IMM_OP(op, i)                                                                     \
    JS_BEGIN_MACRO if (js_Emit3(cx, cg, op, ATOM_INDEX_HI(i), ATOM_INDEX_LO(i)) < 0) return JS_FALSE; \
    JS_END_MACRO
...
LookupArgOrVar(JSContext *cx, JSTreeContext *tc, JSParseNode *pn)
{
    ...
    if (clasp != &js_FunctionClass && clasp != &js_CallClass)
    {
         ...
         */
        optimizeGlobals = (tc->globalUses >= 100 || (tc->loopyGlobalUses && tc->loopyGlobalUses >= tc->globalUses / 2));
        if (!optimizeGlobals)
            return JS_TRUE;
    }
    else
     ...
     */
    atom = pn->pn_atom;
    if (fp->scopeChain != obj || js_InWithStatement(tc) || js_InCatchBlock(tc, atom))
    {
        return JS_TRUE;
    }
    ...
    if (optimizeGlobals || getter)
    {
        if (optimizeGlobals)
        {
            ...
            case JSOP_SETNAME:
                op = JSOP_SETGVAR;
                break;
            case JSOP_SETCONST: /* NB: no change */
                break;
            case JSOP_INCNAME:
                op = JSOP_INCGVAR;
                break;
            ...
            case JSOP_NAMEDEC:
                op = JSOP_GVARDEC;
                break;
            case JSOP_FORNAME: /* NB: no change */
                break;
            case JSOP_DELNAME: /* NB: no change */
                break;
            default:
                JS_ASSERT(0);
            }
        }
        else if (getter == js_GetLocalVariable || getter == js_GetCallVariable)
        {
            switch (op)
            {
            case JSOP_NAME:
        ...
        else if (getter == js_GetArgument || (getter == js_CallClass.getProperty && fp->fun && (uintN)slot < fp->fun->nargs))
        {
            switch (op)
            {
            case JSOP_NAME:
    ...
    if (pn->pn_slot < 0)
    {
         ...
         * object.
         */
        if (pn->pn_op == JSOP_NAME && atom == cx->runtime->atomState.argumentsAtom)
        {
                 ...
                 JSBool *answer)
{
    ...
    case PN_LIST:
        if (pn->pn_type == TOK_NEW || pn->pn_type == TOK_LP || pn->pn_type == TOK_LB)
        {
            /*
    ...
    case PN_TERNARY:
        ok = CheckSideEffects(cx, tc, pn->pn_kid1, answer) && CheckSideEffects(cx, tc, pn->pn_kid2, answer) && CheckSideEffects(cx, tc, pn->pn_kid3, answer);
        break;
    ...
    case PN_BINARY:
        ...
        else
        {
            ...
            ok = CheckSideEffects(cx, tc, pn->pn_left, answer) && CheckSideEffects(cx, tc, pn->pn_right, answer);
        }
        break;

    case PN_UNARY:
        if (pn->pn_type == TOK_INC || pn->pn_type == TOK_DEC || pn->pn_type == TOK_DELETE || pn->pn_type == TOK_THROW || pn->pn_type == TOK_DEFSHARP)
        {
    ...
    case PN_NAME:
        ...
        if (pn->pn_type == TOK_DOT)
        {
            if (pn2->pn_type == TOK_NAME && !LookupArgOrVar(cx, tc, pn2))
                return JS_FALSE;
            if (!(pn2->pn_op == JSOP_ARGUMENTS && pn->pn_atom == cx->runtime->atomState.lengthAtom))
            {
                /*
...
{
    return ((op == JSOP_GETMETHOD && !(pn->pn_attrs & JSPROP_IMPLICIT_FUNCTION_NAMESPACE)) || op == JSOP_SETMETHOD)
               ? SRC_PCDELTA
               : SRC_PCBASE;
}
...
EmitPropOp(JSContext *cx, JSParseNode *pn, JSOp op, JSCodeGenerator *cg)
{
    ...
    pn2 = pn->pn_expr;
    if (op == JSOP_GETPROP && pn->pn_type == TOK_DOT && pn2->pn_type == TOK_NAME)
    {
...
EmitElemOp(JSContext *cx, JSParseNode *pn, JSOp op, JSCodeGenerator *cg)
{
    ...
    if (pn->pn_arity == PN_LIST)
    {
        ...
        if (left->pn_type == TOK_NAME && next->pn_type == TOK_NUMBER)
        {
            if (!LookupArgOrVar(cx, &cg->treeContext, left))
                return JS_FALSE;
            if (left->pn_op == JSOP_ARGUMENTS && JSDOUBLE_IS_INT(next->pn_dval, slot) && (jsuint)slot < JS_BIT(16))
            {
    ...
    else
    {
        ...
        if (op == JSOP_GETELEM && left->pn_type == TOK_NAME && right->pn_type == TOK_NUMBER)
        {
            if (!LookupArgOrVar(cx, &cg->treeContext, left))
                return JS_FALSE;
            if (left->pn_op == JSOP_ARGUMENTS && JSDOUBLE_IS_INT(right->pn_dval, slot) && (jsuint)slot < JS_BIT(16))
            {
           ...
           JSStmtInfo *stmtInfo)
{
    ...
    if (caseCount == 0 || (caseCount == 1 && (hasDefault = (pn2->pn_head->pn_type == TOK_DEFAULT))))
    {
        caseCount = 0;
        low = 0;
        high = -1;
...
#define INTMAP_LENGTH 256
        ...
        for (pn3 = pn2->pn_head; pn3; pn3 = pn3->pn_next)
        {
            ...
            JS_ASSERT(JSVAL_IS_NUMBER(pn3->pn_val) || JSVAL_IS_STRING(pn3->pn_val) || JSVAL_IS_BOOLEAN(pn3->pn_val));
            ...
            if (i >= intmap_bitlen)
            {
                if (!intmap && i < (INTMAP_LENGTH << JS_BITS_PER_WORD_LOG2))
                {
                    intmap = intmap_space;
    ...
    else
    {
         ...
         */
        switchSize = (size_t)(JUMP_OFFSET_LEN + ATOM_INDEX_LEN + (ATOM_INDEX_LEN + JUMP_OFFSET_LEN) * caseCount);
    }
    ...
    else
    {
        ...
        if (constPropagated)
        {
            ...
            if (switchOp == JSOP_TABLESWITCH)
            {
                ...
                {
                    pn3 = table[i];
                    if (pn3 && (pn4 = pn3->pn_left) != NULL && pn4->pn_type == TOK_NAME)
                    {
                    ...
                    JSFunction *fun)
{
    ...
    fp = cx->fp;
    funobj = fun->object;
    JS_ASSERT(!fp || (fp->fun != fun && fp->varobj != funobj && fp->scopeChain != funobj));
...
#define UPDATE_LINE_NUMBER_NOTES(cx, cg, pn)                               \
    JS_BEGIN_MACRO uintN line_ = (pn)->pn_pos.begin.lineno;                \
...
js_EmitTree(JSContext *cx, JSCodeGenerator *cg, JSParseNode *pn)
{
    ...
    case TOK_FUNCTION:
    {
         ...
         */
        if (cg2->treeContext.flags & (TCF_FUN_USES_NONLOCALS | TCF_FUN_HEAVYWEIGHT))
        {
    ...
    case TOK_DO:
        ...
        if (js_NewSrcNote(cx, cg, SRC_WHILE) < 0 || js_Emit1(cx, cg, JSOP_NOP) < 0)
        {
            return JS_FALSE;
        }
...
/* FALL THROUGH */
    ...
    case TOK_TRY:
    {
        ...
        if (iter)
        {
            ...
            for (;;)
            {
                ...
                if (catchJump != -1)
                {
                    ...
                    if (js_NewSrcNote(cx, cg, SRC_HIDDEN) < 0 || js_Emit1(cx, cg, JSOP_LEAVEWITH) < 0)
                    {
                        return JS_FALSE;
                    }
                }
                ...
                if (js_Emit1(cx, cg, JSOP_PUSHOBJ) < 0 || js_Emit1(cx, cg, JSOP_NEWINIT) < 0 || js_Emit1(cx, cg, JSOP_EXCEPTION) < 0)
                {
                    return JS_FALSE;
                }
                ...
                if (js_NewSrcNote(cx, cg, SRC_HIDDEN) < 0 || js_Emit1(cx, cg, JSOP_ENTERWITH) < 0)
                {
                    return JS_FALSE;
                }
                ...
                off = cg->stackDepth - 1;
                if (js_NewSrcNote2(cx, cg, SRC_CATCH, off) < 0 || js_Emit1(cx, cg, JSOP_LEAVEWITH) < 0)
                {
                    return JS_FALSE;
                }
         ...
         */
        if (pn->pn_kid3 || (catchJump != -1 && iter->pn_kid1->pn_expr))
        {
            /*
            ...
            if (js_NewSrcNote(cx, cg, SRC_HIDDEN) < 0 || js_Emit1(cx, cg, JSOP_EXCEPTION) < 0 || js_NewSrcNote(cx, cg, SRC_HIDDEN) < 0 || js_Emit1(cx, cg, JSOP_THROW) < 0)
            {
                return JS_FALSE;
            }
        ...
        if (pn->pn_kid3)
        {
            ...
            if (!UpdateLineNumberNotes(cx, cg, pn->pn_kid3))
                return JS_FALSE;
            if (js_Emit1(cx, cg, JSOP_FINALLY) < 0 || !js_EmitTree(cx, cg, pn->pn_kid3) || js_Emit1(cx, cg, JSOP_RETSUB) < 0)
            {
                return JS_FALSE;
            }
        }
    ...
    case TOK_VAR:
        ...
        for (pn2 = pn->pn_head;; pn2 = pn2->pn_next)
        {
            ...
            else
            {
                ...
                if ((js_CodeSpec[op].format & JOF_TYPEMASK) == JOF_CONST && (!(cg->treeContext.flags & TCF_IN_FUNCTION) || (cg->treeContext.flags & TCF_FUN_HEAVYWEIGHT)))
                {
                ...
                if (pn2->pn_expr)
                {
                    ...
                    pn3 = pn2->pn_expr;
                    if (pn->pn_op == JSOP_DEFCONST && !js_DefineCompileTimeConstant(cx, cg, pn2->pn_atom, pn3))
                    {
                        return JS_FALSE;
                    }
                    if (!js_EmitTree(cx, cg, pn3))
            ...
            if (pn2 == pn->pn_head && js_NewSrcNote(cx, cg, (pn->pn_op == JSOP_DEFCONST) ? SRC_CONST : SRC_VAR) < 0)
            {
                return JS_FALSE;
            }
            if (op == JSOP_ARGUMENTS)
    ...
    case TOK_RETURN:
         ...
         * mislead.
         */
        if (js_InStatement(&cg->treeContext, STMT_SUBROUTINE) && (js_NewSrcNote(cx, cg, SRC_HIDDEN) < 0 || js_Emit1(cx, cg, JSOP_EXCEPTION) < 0 || js_NewSrcNote(cx, cg, SRC_HIDDEN) < 0 || js_Emit1(cx, cg, JSOP_POP) < 0))
        {
            return JS_FALSE;
        }
    ...
    case TOK_SEMI:
        ...
        if (pn2)
        {
             ...
             */
            useful = wantval = !cx->fp->fun || !cx->fp->fun->interpreted || (cx->fp->flags & JSFRAME_SPECIAL);
            if (!useful)
            {
            ...
            if (!useful)
            {
                ...
                if (!js_ReportCompileErrorNumber(cx, cg,
                                                 JSREPORT_CG | JSREPORT_WARNING | JSREPORT_STRICT,
                                                 JSMSG_USELESS_EXPR))
                {
                    return JS_FALSE;
                }
    ...
    case TOK_ASSIGN:
        ...
        if (op != JSOP_NOP)
        {
            if (js_NewSrcNote(cx, cg, SRC_ASSIGNOP) < 0 || js_Emit1(cx, cg, op) < 0)
            {
                return JS_FALSE;
            }
        }
        ...
        if (pn2->pn_type != TOK_NAME && js_NewSrcNote2(cx, cg, SrcNoteForPropOp(pn2, pn2->pn_op), CG_OFFSET(cg) - top) < 0)
        {
            return JS_FALSE;
        }
...
/* FALL THROUGH */
    ...
    case TOK_UNARYOP:
        ...
        op = pn->pn_op;
#if JS_HAS_XML_SUPPORT
        if (op == JSOP_XMLNAME && js_NewSrcNote2(cx, cg, SRC_PCBASE, CG_OFFSET(cg) - pn2->pn_offset) < 0)
        {
            return JS_FALSE;
        }
#endif
    ...
    case TOK_DEC:
         ...
         */
        if (pn2->pn_type != TOK_NAME && (js_CodeSpec[op].format & JOF_POST) && (uintN)cg->stackDepth == cg->maxStackDepth)
        {
            ++cg->maxStackDepth;
        }
        break;
