...
js_ReportOutOfMemory(JSContext *cx)
{
    ...
    const JSErrorFormatString *efs = js_GetLocalizedErrorMessage(cx, NULL, NULL, JSMSG_OUT_OF_MEMORY);
...
js_ReportErrorAgain(JSContext *cx, const char *message, JSErrorReport *reportp)
{
    ...
    {
        JSDebugErrorHook hook = cx->runtime->debugErrorHook;
        if (hook && !hook(cx, cx->lastMessage, reportp, cx->runtime->debugErrorHookData))
        {
            onError = NULL;
        }
    }
