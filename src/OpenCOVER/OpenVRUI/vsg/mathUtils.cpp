...
void snapTo45Degrees(vsg::dmat4 *mat)
{
    coCoord coord = *mat;
...
void snapToDegrees(float degree, vsg::dmat4 *mat)
{
    coCoord coord = *mat;
    //fprintf(stderr,"H: %f P: %f R: %f\n",coord.hpr[0],coord.hpr[1],coord.hpr[2]);
...
coCoord &coCoord::operator=(const vsg::dmat4 &right)
{
    initFromMatrix(right);
    return *this;
}

void coCoord::makeMat(vsg::dmat4 &right) const
{
    right = makeEulerMat(hpr);
    setTrans(right,xyz);
}

coCoord::coCoord(const vsg::dmat4 &right)
{
    initFromMatrix(right);
}

void
coCoord::initFromMatrix(const vsg::dmat4 &right)
{
    //GET_HPR_VEC(right,hpr);
    float cp;
    vsg::dmat4 m;
    vsg::dvec3 v1(right(0, 0), right(0, 1), right(0, 2));
    vsg::dvec3 v2(right(1, 0), right(1, 1), right(1, 2));
    vsg::dvec3 v3(right(2, 0), right(2, 1), right(2, 2));
    v1 = vsg::normalize(v1);
    v2 = vsg::normalize(v2);
    v3 = vsg::normalize(v3);
    m(0, 0) = v1[0];
    ...
    else
    {
        ...
        if (d > 1.0)
        {
            hpr[0] = (vsg::dvec3::value_type) - M_PI_2;
        }
        else if (d < -1.0)
        {
            hpr[0] = (vsg::dvec3::value_type)M_PI_2;
        ...
        if (d > 1.0)
            ...
            hpr[2] = (vsg::dvec3::value_type)M_PI;
        else
            hpr[2] = acos(d);
