...
float OSGVruiRectButtonGeometry::A = 0.5f;
float OSGVruiRectButtonGeometry::B = 0.4f;

ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::coord1 = 0;
ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::coord2 = 0;
ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::coordt1 = 0;
ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::coordt2 = 0;
ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::normal1 = 0;
ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::normal2 = 0;
ref_ptr<Vec3Array> OSGVruiRectButtonGeometry::normalt = 0;
ref_ptr<Vec2Array> OSGVruiRectButtonGeometry::texCoord = 0;
ref_ptr<Vec4Array> OSGVruiRectButtonGeometry::color = 0;

ref_ptr<StateSet> OSGVruiRectButtonGeometry::normalStateSet = 0;
ref_ptr<Material> OSGVruiRectButtonGeometry::textureMat = 0;

ref_ptr<DrawElementsUShort> OSGVruiRectButtonGeometry::coordIndex = 0;
...
*/
OSGVruiRectButtonGeometry::OSGVruiRectButtonGeometry(coRectButtonGeometry *button)
    ...
    , myDCS(0)
{
    ...
    C = button->getInnerWidth();
    D = button->getInnerHeight();
}

/// Destructor.
OSGVruiRectButtonGeometry::~OSGVruiRectButtonGeometry()
{
    delete myDCS;
    myDCS = 0;
}

void OSGVruiRectButtonGeometry::createSharedLists()
{
    if (coord1 == 0)
    {
        ...
        (*coord1)[0].set(0.0, 2.0 * (A + B) + D, 0.0);
        (*coord1)[1].set(2.0 * (A + B) + C, 2.0 * (A + B) + D, 0.0);
        (*coord1)[2].set(2.0 * (A + B) + C, 0.0, 0.0);
        (*coord1)[3].set(0.0, 0.0, 0.0);
        (*coord1)[4].set(A, A + B + D + B, A);
        (*coord1)[5].set(A + B + C + B, A + B + D + B, A);
        (*coord1)[6].set(A + B + C + B, A, A);
        (*coord1)[7].set(A, A, A);
        (*coord1)[8].set(A + B, A + B + D, A + B);
        (*coord1)[9].set(A + B + C, A + B + D, A + B);
        (*coord1)[10].set(A + B + C, A + B, A + B);
        (*coord1)[11].set(A + B, A + B, A + B);

        (*coord2)[0].set(0.0, (2.0 * (A + B) + D), 0.0);
        (*coord2)[1].set(2.0 * (A + B) + C, (2.0 * (A + B) + D), 0.0);
        (*coord2)[2].set(2.0 * (A + B) + C, 0.0, 0.0);
        (*coord2)[3].set(0.0, 0.0, 0.0);
        (*coord2)[4].set(A, A + B + D + B, A);
        (*coord2)[5].set(A + B + C + B, A + B + D + B, A);
        (*coord2)[6].set(A + B + C + B, A, A);
        (*coord2)[7].set(A, A, A);
        (*coord2)[8].set(A + B, A + B + D, A - B);
        (*coord2)[9].set(A + B + C, A + B + D, A - B);
        ...
        (*coordt1)[3].set(A + B, A + B + D, A + B);
        (*coordt1)[2].set(A + B + C, A + B + D, A + B);
        (*coordt1)[1].set(A + B + C, A + B, A + B);
        (*coordt1)[0].set(A + B, A + B, A + B);

        (*coordt2)[3].set(A + B, A + B + D, A - B);
        (*coordt2)[2].set(A + B + C, A + B + D, A - B);
...
ref_ptr<Node> OSGVruiRectButtonGeometry::createGeode(const string &textureName, int style)
{
    ...
    {
        VRUILOG("OSGVruiRectButtonGeometry::createBox err: texture image " << textureName << " not found")
    }
...
void OSGVruiRectButtonGeometry::createGeometry()
{
...
void OSGVruiRectButtonGeometry::resizeGeometry()
{
}

vruiTransformNode *OSGVruiRectButtonGeometry::getDCS()
{
    createGeometry();
    return myDCS;
}

void OSGVruiRectButtonGeometry::switchGeometry(coButtonGeometry::ActiveGeometry active)
{
    createGeometry();
