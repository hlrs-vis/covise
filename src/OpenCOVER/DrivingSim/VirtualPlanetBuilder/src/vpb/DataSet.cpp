...
void DataSet::addSource(Source *source, unsigned int revisionNumber)
{
    ...
    if (!_sourceGraph.valid())
        _sourceGraph = new CompositeSource;
...
{
    if (!_destinationExtents.intersects(extents))
        return false;
...
bool DataSet::computeOptimumLevel(Source *source, int maxLevel, int &level)
{
    ...
    if (maxLevel < static_cast<int>(source->getMinLevel()))
        return false;
...
int DataSet::computeMaximumLevel(int maxNumLevels)
{
    ...
    for (CompositeSource::source_iterator itr(_sourceGraph.get()); itr.valid(); ++itr)
    {
        ...
        int k = 0;
        if (!computeOptimumLevel(source, maxNumLevels - 1, k))
            continue;
...
CompositeDestination *DataSet::createDestinationTile(int currentLevel, int currentX, int currentY)
{
    ...
    if (currentLevel == 0)
        _destinationGraph = destinationGraph;
                                                      ...
                                                      unsigned int maxNumLevels)
{
    ...
    {
        if ((currentX != getSubtileX()) || (currentY != getSubtileY()))
            return 0;
    }
...
bool DataSet::prepareForDestinationGraphCreation()
{
    ...
    for (CompositeSource::source_iterator sitr(_sourceGraph.get()); sitr.valid(); ++sitr)
    {
        ...
        if (source)
        {
            if (maxTextureUnit < source->getLayer())
                maxTextureUnit = source->getLayer();
        }
    }
...
{
    if (!prepareForDestinationGraphCreation())
        return;
...
{
    if (!_destinationGraph || !_sourceGraph)
        return;
...
void DataSet::_writeNodeFile(osg::Node &node, const std::string &filename)
{
    ...
    if (_archive.valid())
        _archive->writeNode(node, filename);
    else
    {
        ...
        if (vpb::hasWritePermission(filename))
        {
            ...
            osgDB::ReaderWriter::WriteResult result = osgDB::Registry::instance()->writeNode(node, filename, osgDB::Registry::instance()->getOptions());
            ...
            if (result.success())
            {
                if (_databaseRevision.valid())
                {
                    ...
                    {
                        if (_databaseRevision->getFilesModified())
                            _databaseRevision->getFilesModified()->addFile(filename);
                    }
                    else
                    {
                        if (_databaseRevision->getFilesAdded())
                            _databaseRevision->getFilesAdded()->addFile(filename);
                    }
                }
            }
            else
            {
                if (!result.message().empty())
                    log(notifylevel, result.message());
                else
                {
...
void DataSet::_writeImageFile(osg::Image &image, const std::string &filename)
{
    ...
    if (_archive.valid())
        _archive->writeImage(image, simpliedFileName);
    else
    {
        osg::NotifySeverity notifylevel = getAbortTaskOnError() ? osg::FATAL : osg::WARN;
        ...
        if (FilePathManager::instance()->checkWritePermissionAndEnsurePathAvailability(simpliedFileName))
        {
            ...
            osgDB::ReaderWriter::WriteResult result = osgDB::Registry::instance()->writeImage(image, simpliedFileName, osgDB::Registry::instance()->getOptions());
            ...
            {
                if (!result.message().empty())
                    log(notifylevel, result.message().c_str());
                else
                {
                    switch (result.status())
                    {
...
public:
        ...
        , _writeHint(osg::Image::STORE_INLINE)
    {
        ...
        if (dataSet->getDestinationTileExtension() == ".osg" || dataSet->getDestinationTileExtension() == ".osgt" || dataSet->getDestinationTileExtension() == ".osgx")
            _writeHint = osg::Image::EXTERNAL_FILE;
    }
...
public:
    ...
    virtual void build()
    {
        ...
        {
            if (_buildLog.valid())
                _buildLog->log(osg::NOTICE, "   writeSubTile filename= %s", _filename.c_str());
...
osg::Node *DataSet::decorateWithTerrain(osg::Node *subgraph)
{
    ...
    if (getConvertFromGeographicToGeocentric())
        terrain->setEllipsoidModel(getEllipsoidModel());
...
osg::Node *DataSet::decorateWithCoordinateSystemNode(osg::Node *subgraph)
{
    ...
    // set the ellipsoid model if geocentric coords are used.
    if (getConvertFromGeographicToGeocentric())
        csn->setEllipsoidModel(getEllipsoidModel());
    ...
    if (_destinationGraph.valid())
    {
        ...
        else // _databaseType==PagedLOD_DATABASE
        {
                 ...
                 ++qitr)
            {
                ...
                if (getGenerateSubtile() && qitr->first <= getSubtileLevel())
                    continue;

                if (getRecordSubtileFileNamesOnLeafTile() && qitr->first >= getMaximumNumOfLevels())
                    continue;
        ...
        if (_writeThreadPool.valid())
            _writeThreadPool->waitForCompletion();
    }
    else
    {
...
bool DataSet::addLayer(Source::Type type, osgTerrain::Layer *layer, unsigned layerNum, unsigned int revisionNumber)
{
    ...
    if (hfl)
    {
        ...
        {
            vpb::Source::ParameterPolicy geoTransformPolicy = layer->getLocator()->getTransformScaledByResolution() ? vpb::Source::PREFER_CONFIG_SETTINGS : vpb::Source::PREFER_CONFIG_SETTINGS_BUT_SCALE_BY_FILE_RESOLUTION;
...
public:
    MyGraphicsContext(BuildLog *buildLog)
    {
        ...
        {
            if (buildLog)
                buildLog->log(osg::NOTICE, "Failed to create pbuffer, failing back to normal graphics window.");
...

        {
            ...
            if (buildLog)
                buildLog->log(osg::NOTICE, "Realized window");
        }
    }
        ...
        if (computeCoverage(sp._extents, level, i_min, j_min, i_max, j_max))
        {
            for (int j = j_min; j < j_max; ++j)
            {
                for (int i = i_min; i < i_max; ++i)
                {
                    ...
                    {
                        if (k > static_cast<int>(itr->second))
                            itr->second = k;
                    }
                    else
                    {
...
void DataSet::selectAppropriateSplitLevels()
{
    ...
    else
    {
        if (maxLevel >= 10 && getDistributedBuildSecondarySplitLevel() == 0)
        {
            ...
            setDistributedBuildSecondarySplitLevel(
                getDistributedBuildSplitLevel() + (maxLevel - getDistributedBuildSplitLevel()) / 2);
        }
        else
        {
    ...
    if (getDistributedBuildSecondarySplitLevel() != 0 && getDistributedBuildSecondarySplitLevel() <= getDistributedBuildSplitLevel())
    {
...
bool DataSet::generateTasksImplementation(TaskManager *taskManager)
{
    ...
    int bottomDistributedBuildLevel = getDistributedBuildSecondarySplitLevel() == 0 ? getDistributedBuildSplitLevel() : getDistributedBuildSecondarySplitLevel();
    ...
    if (System::instance()->getFileCache())
        fileCacheName = System::instance()->getFileCache()->getFileName();
    std::string xodrName;
...
int DataSet::_run()
{
    ...
    if (getRecordSubtileFileNamesOnLeafTile())
        ++numLevels;
    ...
    if (requiresGenerationOfTiles)
    {
        ...
        if (getOutputTaskDirectories())
        {
            ...
            if (getGenerateSubtile())
                log(osg::NOTICE, "We are a subtile");
            if (getRecordSubtileFileNamesOnLeafTile())
                log(osg::NOTICE, "We have to record ../task/name");
        }
        else
        {
