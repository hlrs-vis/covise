        ...
        while (!fr.eof())
        {
            ...
            if (fr.matchSequence("FileDetails {"))
            {
                ...
                while (!fr.eof() && fr[0].getNoNestedBrackets() > local_entry)
                {
                    ...
                    if (fr.read("cs", str))
                    {
                        if (!fd->getSpatialProperties()._cs)
                            fd->getSpatialProperties()._cs = new osg::CoordinateSystemNode;
...
std::string FileCache::getOptimimumFile(const std::string &filename, const SpatialProperties &sp)
{
         ...
         ++vitr)
    {
        ...
        if (fd_sp.compatible(sp))
        {
            ...
            {
                if (resolutionRatio > res_closest_below || (resolutionRatio == res_closest_below && fd->getHostName() == hostname))
                {
                    fd_closest_below = fd;
            ...
            {
                if (resolutionRatio < res_closest_above || (resolutionRatio == res_closest_above && fd->getHostName() == hostname))
                {
                    fd_closest_above = fd;
