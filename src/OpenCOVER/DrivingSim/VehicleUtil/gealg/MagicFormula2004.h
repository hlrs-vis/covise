...
template <class ExprRadiusType, class ExprOrientType, class ExprVelType>
struct ContactWrench
{
    typedef typename ExprRadiusType::result_type RadiusType;
    typedef typename ExprOrientType::result_type OrientType;
    typedef typename ExprVelType::result_type VelType;

    typedef gealg::Multivector<6, 0x060504030201> MultivectorType;

    ContactWrench(const ExprRadiusType &setr, const ExprOrientType &setR, const ExprVelType &setV, const TyrePropertyPack &setPars)
        : rExpr(setr)
        , RExpr(setR)
        , VExpr(setV)
        , pars(setPars)
    ...
    {
        return wrench.template element<EB>(vars);
    }

    template <class T>
    void evaluate(const T &vars)
    {
        RadiusType r = rExpr(vars); //tyre frame
        OrientType R = RExpr(vars); //surface frame
        VelType V = VExpr(vars); //tyre frame

        gealg::mv<3, 0x040201>::type Vc = part<3, 0x040201>(V);
        gealg::mv<2, 0x0201>::type Vs = part<2, 0x0201>(V) + gealg::mv<1, 0x04>::type(-pars.R_0) * part<1, 0x05>(V); //assumption R_e = R_0
        ...
        double rho_z = std::max(pars.R_0 - part<1, 0x04>(r)()[0], 0.0);

        //Velocity contact point
        //double V_c = magnitude(part<2, 0x0201>(Vc)).element<0x00>();
        double V_c = magnitude(part<2, 0x0201>(Vc))()[0];
        ...
        double F_z = pars.C_Fz * rho_z + pars.D_Fz * V.element<0x04>();
        ...
        double gamma_star = grade<1>(R * gealg::mv<1, 0x04>::type(1.0) * (~R))()[1];
    ...
    ExprRadiusType rExpr;
    ExprOrientType RExpr;
    ExprVelType VExpr;

    MultivectorType wrench;

    TyrePropertyPack pars;
};

template <class L, class M, class H>
inline gealg::Expression<ContactWrench<gealg::Expression<L>, gealg::Expression<M>, gealg::Expression<H> > >
wrench(const gealg::Expression<L> &l, const gealg::Expression<M> &m, const gealg::Expression<H> &h, const TyrePropertyPack &pp = TyrePropertyPack())
{
    typedef ContactWrench<gealg::Expression<L>, gealg::Expression<M>, gealg::Expression<H> > ContactWrenchType;
    return gealg::Expression<ContactWrenchType>(ContactWrenchType(l, m, h, pp));
}
}
#endif
