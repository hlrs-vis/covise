...
fasi2 *fasi2::myFasi = NULL;

fasi2::fasi2(const char *filename)
{
    system = NULL;
    myFasi = this;
    ...
    vehicleDynamics = new FourWheelDynamicsRealtime2();
...
bool fasi2::loadRoadSystem(const char *filename_chars)
{
...
xercesc::DOMElement *fasi2::getOpenDriveRootElement(std::string filename)
{
    try
    {
...
void fasi2::parseOpenDrive(xercesc::DOMElement *rootElement)
{
...
int fasi2::getAutoGearDiff(float downRPM, float upRPM)
{
    int diff = 0;
	
	double shiftParameter = vehicleDynamics->getShiftParameter();
	double engineSpeed = vehicleDynamics->getEngineSpeed();
	double speed = vehicleDynamics->getVelocity();
	double downShiftPoint = downRPM * shiftParameter;
	double upShiftPoint = upRPM * shiftParameter;
...
bool fasi2::readClientVal(void *buf, unsigned int numBytes)
{
...
void fasi2::run()
{
    ...
    while (running)
    {
        ...
        if (toClientConn)
        {
			...
			remoteData.slipFL = vehicleDynamics->getTyreSlipFL();
			remoteData.slipFR = vehicleDynamics->getTyreSlipFR();
			remoteData.slipRL = vehicleDynamics->getTyreSlipRL();
			remoteData.slipRR = vehicleDynamics->getTyreSlipRR();
...
p_klsm->p_CANProv->GW_SVB_D.values.canmsg.cansignals.SVB_GRA_D = 0;
        ...
        else if (p_ignitionLock->getLockState() == IgnitionLock::ENGINESTOP)
        {
			...
			vehicleDynamics->setEngineSpeed(0.0);
			vehicleDynamics->setTyreSlipFL(0.0);
        ...
        CarSound::instance()->start(CarSound::Ignition);
             ...
             anlasserSource->play();
				...
				vehicleDynamics->setEngineSpeed(1000 / (60.0));
            }
        }
      ...
      cardynCGA->centerSteeringWheel();
        ...
        if (p_kombi->getButtonState() & KI::Button_PSM && !(oldButtonState & KI::Button_PSM))
        {
			...
			//std::cout << "psm" << sharedState.PSMState << std::endl;
        }
        if (p_kombi->getButtonState() & KI::Button_Spoiler && !(oldButtonState & KI::Button_Spoiler))
        {
			...
			//std::cout << "spoiler" << sharedState.SpoilerState << std::endl;
        }
        ...
        if (automatic)
        {
            ...
            if (sharedState.frameTime - oldShiftTime > 1)
            {
                int gearDiff;
                ...
                gearDiff = getAutoGearDiff(65, 110);
        ...
        if (sharedState.shiftTimer < 400)
		{
			sharedState.shiftTimer += 1;
		}
    ...
    std::cout << "fasi not running" << std::endl;
}
