    ...
    while (it != roads_.constEnd())
    {
        ...
        while(laneSectionsIt != road->getLaneSections().constEnd() - 1)
        {
            ...
            while (laneIt != laneSection->getLanes().constEnd())
            {
                ...
                if (lane->getId() != 0)
                {
                    ...
                    lane->setSuccessor(laneId);
                    nextLaneSection->getLane(laneId)->setPredecessor(lane->getId());
                }
        ...
        while (laneIt != startLaneSection->getLanes().constEnd())
        {
            ...
            if (lane->getId() != 0)
            {
                ...
                {
                    RSystemElementRoad * predRoad = getRoad(roadPredecessor->getElementId());
                    if (predRoad)
                    {
                        ...
                        {
                            QVector2D normal = predRoad->getGlobalNormal(predRoad->getLength());
                            ...
                            if (road->getJunction() == "-1")
                            {
                                nextLaneSection->getLane(laneId)->setSuccessor(lane->getId());
                            }
                        }
                    }
                }
        ...
        while (laneIt != endLaneSection->getLanes().constEnd())
        {
            ...
            if (lane->getId() != 0)
            {
                ...
                {
                    RSystemElementRoad * succRoad = getRoad(roadSuccessor->getElementId());
                    if (succRoad)
                    {
...
/*
    ...
    while (it != roads_.constEnd())
    {
        ...
        QMap<int, Lane *>::const_iterator laneIt = startLaneSection->getLanes().constBegin();
        while (laneIt != startLaneSection->getLanes().constEnd())
        {
            Lane *lane = laneIt.value();
        ...
        while (laneIt != endLaneSection->getLanes().constEnd())
        {
            ...
            {
                RoadLink * roadSuccessor = road->getSuccessor();
...
double
    RoadSystem::getTValue(LaneSection * laneSection, Lane * lane, double s, double laneWidth)
{
    double t = 0.0;
