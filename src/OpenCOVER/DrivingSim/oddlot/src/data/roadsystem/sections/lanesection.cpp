   ...
   */
   ...
   LaneSection::addLane(Lane *lane)
   {
      ...
      {
      qDebug("ERROR 1010151532! A lane with the same ID already exists! Road::%s", getParentRoad()->getName().toUtf8().constData());
      return;
      }
...
int
LaneSection::getRightmostLaneId() const
{
...
int
LaneSection::getLeftmostLaneId() const
{
...
*/
void
LaneSection::addLaneSectionChanges(int changes)
{
    if (changes)
    {
...
*/
void
LaneSection::accept(Visitor *visitor)
{
    visitor->visit(this);
}
...
*/
void
LaneSection::acceptForLanes(Visitor *visitor)
{
    foreach (Lane *child, lanes_)
    {
