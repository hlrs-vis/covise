...
namespace op
{
    ...
    template <conf_t LC, conf_t RC, typename list, int op = (BitCount<LC ^ RC>::value == (BitCount<LC>::value + BitCount<RC>::value)) ? (((LC ^ RC) == list::conf) ? 0 : (((LC ^ RC) < list::conf) ? 1 : -1))
...
/// \ingroup ga_ops
template <class L, class R>
inline gaalet::outer_product<L, R>
