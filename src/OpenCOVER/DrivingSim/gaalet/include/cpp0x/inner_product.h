...
namespace ip
{
    ...
    template <conf_t LC, conf_t RC, typename list, int op = ((BitCount<LC ^ RC>::value == ((((int)BitCount<LC>::value - (int)BitCount<RC>::value) < 0)
                                                                ...
                                                                : -10 > struct insert_element_to_melist
    {
...
struct inner_product : public expression<inner_product<L, R> >
{
    ...
    typedef typename ip::build_multiplication_element_list<typename L::clist, typename R::clist, metric>::melist melist;
...
/// \ingroup ga_ops
template <class L, class R>
inline gaalet::inner_product<L, R>
