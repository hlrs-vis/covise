...
#define __MEDIAN3(a, b, c) (((a) >= (b)) ? (((c) >= (a)) ? (a) : (((c) <= (b)) ? (b) : (c))) : (((c) >= (b)) ? (b) : (((c) <= (a)) ? (a) : (c))))
...
/* ### state.hx=state.x-state.hx, tmp1=||state.hx|| */
        ...
        *ffpls = tmp1;
        ...
        if (fpls <= f + slp * alpha * lambda)
        { /* solution found */
            *iretcd = 0;
            if (lambda == LM_CNST(1.) && sln > stepmx * LM_CNST(.99))
                *mxtake = 1;
            return;
        }
...
/* ### e=x-hx, p_eL2=||e|| */
     ...
     */
        for (i = j = numactive = 0, p_L2 = jacTe_inf = 0.0; i < m; ++i)
        {
            ...
            else if (jacTe_inf < (tmp = FABS(jacTe[i])))
                jacTe_inf = tmp;
...
/* ### hx=x-hx, pDp_eL2=||hx|| */
       ...
       */
            ...
            if (jacTeDp <= -rho * pow(Dp_L2, _POW_ / LM_CNST(2.0)))
            {
                ...
                if (iretcd != 0)
                    goto gradproj; /* rather inelegant but effective way to handle LNSRCH() failures... */
#else
                ...
                for (t = tini; t > tmin; t *= beta)
                {
                    ...
                    (*func)(pDp, hx, m, n, adata);
                    ++nfev; /* evaluate function at p + t*Dp */
                    for (i = 0, pDp_eL2 = 0.0; i < n; ++i)
                    { /* compute ||e(pDp)||_2 */
                        hx[i] = tmp = x[i] - hx[i];
                        pDp_eL2 += tmp * tmp;
                    ...
                    if (!LM_FINITE(pDp_eL2))
                        goto gradproj; /* treat as line search failure */

                    //if(LM_CNST(0.5)*pDp_eL2<=LM_CNST(0.5)*p_eL2 + t*alpha*jacTeDp) break;
                    if (pDp_eL2 <= p_eL2 + LM_CNST(2.0) * t * alpha * jacTeDp)
                        break;
                }
#endif
                ++nLSsteps;
                      ...
                      */
{
    ...
    if (info)
    { /* correct the number of function calls */
        if (data.ffdif)
