...
static int LMLEC_ELIM(LM_REAL *A, LM_REAL *b, LM_REAL *c, LM_REAL *Y, LM_REAL *Z, int m, int n)
{
   ...
   */
    ...
    for (i = rank = 0; i < mintmn; ++i)
        if (a[i * (tm + 1)] > tmp || a[i * (tm + 1)] < -tmp)
            ++rank; /* loop across R's diagonal elements */
        else
                      ...
                      */
{
   ...
   */
    ...
    if (!info)
        info = locinfo; /* make sure that LEVMAR_DER() is called with non-null info */
                      ...
                      */
{
   ...
   */
    ...
    if (!info)
        info = locinfo; /* make sure that LEVMAR_DIF() is called with non-null info */
