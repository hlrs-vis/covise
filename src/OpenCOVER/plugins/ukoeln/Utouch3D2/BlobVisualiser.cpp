...
BlobVisualiser::BlobVisualiser(const Vec3f &center, float radius, int numOfSegments, const Vec4f &color)
{
...
Geometry *BlobVisualiser::getGeometry() const
{
...
void BlobVisualiser::createBlob()
{
    ...
    Vec3Array *vertices = this->createCircle();
    blobGeometry->setVertexArray(vertices);

    Vec4Array *colors = new Vec4Array();
    colors->push_back(color);
    blobGeometry->setColorArray(colors);
    blobGeometry->setColorBinding(Geometry::BIND_OVERALL);

    Vec3Array *normals = new Vec3Array;
    normals->push_back(Vec3f(0.0f, 0.0f, 1.0f));
    blobGeometry->setNormalArray(normals);
...
Vec3Array *BlobVisualiser::createCircle()
{
