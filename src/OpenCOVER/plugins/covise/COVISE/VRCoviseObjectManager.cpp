...
{
    for (ButtonsMap::iterator it = buttonsMap.begin();
         it != buttonsMap.end();
         ++it)
    {
        const std::vector<buttonSpecCell *> &buttons = it->second;
        for (std::vector<buttonSpecCell *>::const_iterator bit = buttons.begin();
             bit != buttons.end();
             ++bit)
        {
            cover->removeButton((*bit)->name, "COVISE");
        }
    }
...
void
ObjectManager::removeButtonsForContainer(const char *container)
{
    ButtonsMap::iterator it = buttonsMap.find(container);
...
{
    char buttonName[200];
    const int i = VRPinboard::instance()->getIndex(buttonId);
    sprintf(buttonName, "%s %d",
            VRPinboard::instance()->functions[i].customButtonName, moduleInstance);
    if (!name)
        name = buttonName;
    VRPinboard::instance()->makeButton(buttonId,
                                       name,
                                       VRPinboard::instance()->functions[i].customMenuName,
                                       VRPinboard::BTYPE_NAVGROUP, feedbackCallback, this, const_cast<char *>(feedback_info));
}

/*______________________________________________________________________*/
void
ObjectManager::addFeedbackButton(const char *container, const char *feedback_info, const char *name)
{
    char tmp[200];
    ...
    CoviseRender::set_feedback_info(feedback_info);
    ...
    {
        if (cover->debugLevel(2))
            cerr << "ObjectManager::addFeedbackButton: sscanf1 failed" << endl;
    }

    // we put the covise buttons into the same menu as execute
    switch (CoviseRender::get_feedback_type())
    {
    case 'Z': // CuttingSurface
    case 'C': // CuttingSurface
        if (strncmp(moduleName, "CuttingSurface", 14) == 0)
        {
            addPinboardButton("CUTTINGSURFACE", moduleInstance, feedback_info, name);
        }
        break;

    case 'G': // CutGeometry
        if (strcmp(moduleName, "CutGeometry") == 0 || strcmp(moduleName, "CutSollIst") == 0)
        {
            addPinboardButton("CUTGEOMETRY", moduleInstance, feedback_info, name);
        }
        break;

    case 'I': // IsoSurface
        if (strncmp(moduleName, "IsoSurface", 10) == 0)
        {
            char s1[200];
            char s2[200];
            char s3[200];
            float min = -1, max = 1, value = 0.1;
            char *newInfo = new char[strlen(feedback_info) + 1];
            if (sscanf(feedback_info, "I%s\n%s\n%s\n%f\n%f\n%f\n", s1, s2, s3, &min, &max, &value) != 6)
            {
                if (cover->debugLevel(2))
                    cerr << "ObjectManager::addFeedbackButton: sscanf2 failed" << endl;
            }
            sprintf(newInfo, "I%s\n%s\n%s\n", s1, s2, s3);
            addPinboardButton("ISOSURFACE", moduleInstance, newInfo, name);
            const int i = VRPinboard::instance()->getIndex("ISOSURFACE");
            char buttonName[200];
            sprintf(buttonName, "%s_%d",
                    VRPinboard::instance()->functions[i].customButtonName, moduleInstance);
            cover->addSliderButton(buttonName, "COVISE", min, max, value, feedbackCallback, this, newInfo);

            //fprintf(stderr,"min %f max %f value %f %s \n",min,max,value,feedback_info);
            VRMenu *menu = VRPinboard::instance()->namedMenu("COVISE");
            if (!menu)
            {
                if (cover->debugLevel(2))
                    fprintf(stderr, "did not find COVISE Menu \n");
                return;
            }
        ...
        break;
    case 'T': // Tracer startline
        if (strcmp(moduleName, "TracerUsg") == 0)
        {
            addPinboardButton("TRACERUSG", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "Tracer") == 0 || strcmp(moduleName, "TracerComp") == 0)
        {
            addPinboardButton("TRACER", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "TetraTrace") == 0)
        {
            addPinboardButton("TETRATRACE", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "CellTracer") == 0)
        {
            addPinboardButton("CELLTRACER", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "LTracer") == 0)
        {
            addPinboardButton("LTRACER", moduleInstance, feedback_info, name);
        }
        // BlockSTracer is TracerStr
        else if (strcmp(moduleName, "TracerStr") == 0)
        {
            addPinboardButton("TRACERSTR", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "MagTracer") == 0)
        {
            addPinboardButton("MAGTRACER", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "STracer") == 0)
        {
            addPinboardButton("STRACER", moduleInstance, feedback_info, name);
        }
        break;

    case 'P': // tracer startplane
        if (strcmp(moduleName, "TetraTrace") == 0)
        {
            addPinboardButton("TETRATRACE", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "LTracer") == 0)
        ...
        // MagBlockTracer is now TracerUsg
        else if (strcmp(moduleName, "TracerUsg") == 0)
        {
            addPinboardButton("TRACERUSG", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "Tracer") == 0)
        {
            addPinboardButton("TRACER", moduleInstance, feedback_info, name);
        }
        else if (strcmp(moduleName, "MagTracer") == 0)
    ...
    default:
        //  all other which are not really supported and therefore not configurable
        cover->addGroupButton(tmp, NULL, 0, feedbackCallback, 0, this, const_cast<char *>(feedback_info));
        fprintf(stderr,
                "WARNING: VRCoviseObjectManager::addFeedbackButton - feedback type %c not handled\n",
                CoviseRender::get_feedback_type());
        break;
    }
    addCoviseMenu(container, spec);
}

void ObjectManager::addCoviseMenu(const char *container, buttonSpecCell *spec)
{
    if (!container)
        container = "";
    ...
    if (!found)
    {
        buttonsMap[container].push_back(spec);
    }
    else
    {
        delete spec;
...
static std::map<char, int> feedbackIconMap;
static std::map<char, char> feedbackTypeMap;

/*______________________________________________________________________*/
void
ObjectManager::feedback(buttonSpecCell *spec)
{
    if (feedbackIconMap.empty())
    {
        feedbackIconMap['A'] = HAND_PROBE;
        feedbackTypeMap['A'] = 'c';
        feedbackIconMap['S'] = HAND_ANCHOR;
        feedbackTypeMap['S'] = 'c';
        feedbackIconMap['C'] = HAND_PLANE;
        feedbackTypeMap['C'] = 'c';
        feedbackIconMap['G'] = HAND_PLANE;
        feedbackTypeMap['G'] = 'c';
        feedbackIconMap['Z'] = HAND_PLANE;
        feedbackTypeMap['Z'] = 'c';

        feedbackIconMap['T'] = HAND_SPHERE;
        feedbackTypeMap['T'] = 't';
        feedbackIconMap['P'] = HAND_SPHERE;
        feedbackTypeMap['P'] = 't';

        feedbackIconMap['I'] = HAND_SPHERE;
        feedbackTypeMap['I'] = 'i';
    }
    ...
    if (CoviseRender::get_feedback_type() == 'I'
        && spec->actionType == BUTTON_SLIDER)
    {
        if (spec->dragState == BUTTON_RELEASED && spec->state != spec->oldState)
        {
            char buf[1000];
            fprintf(stdout, "\a");
            fflush(stdout);
            sprintf(buf, "isovalue\nFloatSlider\n%f %f %f",
                    spec->sliderMin, spec->sliderMax, spec->state);
            CoviseRender::send_feedback_message("PARAM", buf);
            buf[0] = '\0';
            CoviseRender::send_feedback_message("EXEC", buf);
        }
    ...
    {
        const char feedbackType = CoviseRender::get_feedback_type();
        std::map<char, int>::iterator it = feedbackIconMap.find(feedbackType);
        if (it == feedbackIconMap.end())
        {
        ...
        else
        {
            ...
            VRSceneGraph::instance()->setPointerType(
                spec->state ? feedbackIconMap[feedbackType] : HAND_LINE);

            switch (feedbackTypeMap[feedbackType])
            {
            case 'c':
                this->c_feedback = spec->state;
                break;
            case 't':
                this->t_feedback = spec->state;
                break;
            case 'i':
                this->i_feedback = spec->state;
                break;
            default:
                assert("Programmer error" == NULL);
                break;
            }
        }
    }
