...
class KardanikConstructor : public std::enable_shared_from_this<KardanikConstructor>
{
private:
public:
    KardanikConstructor();
    ...
    void InstanciateConstruction(std::shared_ptr<KardanikXML::Construction> construction);

    void SetMotionStateFactory(KardanikXML::MotionStateFactory *motionStateFactory);
    btRigidBody *GetRigidBody(std::shared_ptr<KardanikXML::Body> body) const;

    void MakeHigherBodiesStatic(std::shared_ptr<KardanikXML::Body> body, bool freeHigher);
...
private:
    void AddBody(std::shared_ptr<KardanikXML::Body> body);
    void AddJoint(std::shared_ptr<KardanikXML::Joint> body);
    void AddLineToCompound(btCompoundShape *shape, std::shared_ptr<KardanikXML::Point> start,
                           std::shared_ptr<KardanikXML::Point> end, float radius, bool objectIsEmpty = false);
    ...
    btTransform GetCenterOfMass(std::shared_ptr<KardanikXML::Body> body) const;
    btVector3 PointToVector(std::shared_ptr<KardanikXML::Point> point) const;
    ...
    void LockConnectedJoints(std::shared_ptr<KardanikXML::Body> body);
    void UnlockConnectedJoints(std::shared_ptr<KardanikXML::Body> body, bool unlockLower = false);
    ...
    typedef std::map<std::shared_ptr<KardanikXML::Body>, RigidBodyInfo> BodyToRigidBody;
    ...
    typedef std::map<std::shared_ptr<KardanikXML::Joint>, btHingeConstraint *> JointToHinge;
    JointToHinge m_JointToHinge;

    typedef std::vector<std::shared_ptr<KardanikXML::Joint> > Joints;
    Joints m_Joints;
