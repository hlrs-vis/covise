...
#undef RR_DEFAULTPORT
#define RR_DEFAULTPORT 31042
...
int _glXInitThreadsSUN(void)
{
    ...
    if ((__glXInitThreadsSUN = (_glXInitThreadsSUNType)dlsym(RTLD_NEXT, "glXInitThreadsSUN")) != NULL)
...
void killproc(bool useronly)
{
    ...
    try
    {
        ...
        if (nps)
            for (i = 0; i < nps; i++)
            {
                if (pid[i] == GetCurrentProcessId())
                    continue;
                if (!(ph = OpenProcess(PROCESS_ALL_ACCESS, FALSE, pid[i])))
                    continue;
    ...
    if (!useronly && getuid() != 0)
        _throw("Only root can do that");
    ...
    try
    {
        ...
        if (len < sizeof(kinfo_proc))
            _throw("Process table is empty");
...
void getenvironment(void)
{
    ...
    if ((env = getenv("VGLCLIENT_LOG")) != NULL && strlen(env) > 0)
    {
        ...
        if (!f)
        {
            rrout.println("Could not open log file %s", logfile);
            _throwunix();
        }
        else
            fclose(f);
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (argc > 1)
            for (i = 1; i < argc; i++)
            {
...
#ifdef USESSL
                if (!stricmp(argv[i], "-sslonly"))
                {
                    ssl = true;
                    nonssl = false;
                }
                if (!stricmp(argv[i], "-nossl"))
                {
                    ssl = false;
                    nonssl = true;
                }
                ...
                if (!stricmp(argv[i], "-force"))
                    force = true;
                if (!stricmp(argv[i], "-detach"))
                    detach = true;
                if (!stricmp(argv[i], "-kill"))
                {
                    killproc(true);
                    return 0;
                }
                if (!stricmp(argv[i], "-killall"))
                {
                    killproc(false);
                    return 0;
                }
        ...
        if (!child)
        {
            ...
            rrout.println("\n%s Client v%s (Build %s %d-bit)", "rrclient", "0", "0", sizeof(long) * 8);
            if (printversion)
                return 0;
            if (detach)
                daemonize();
...
void start(char *displayname)
{
    ...
    unsigned short actualsslport = 0;
#endif
    bool newlistener = false;

    if (!XInitThreads())
    {
        rrout.println("XInitThreads() failed");
        return;
    }
#ifdef SUNOGL
    if (!_glXInitThreadsSUN())
        _throw("glXInitThreadsSUN() failed");
#endif
    ...
    try
    {
        ...
        if (ssl)
        {
            ...
            if (actualsslport == 0)
            {
                ...
                {
                    bool success = false;
                    unsigned short i = RR_DEFAULTSSLPORT;
                    do
                    {
                        try
                        {
...
#endif
        {
            ...
            if (actualport == 0)
            {
                ...
                {
                    bool success = false;
                    unsigned short i = RR_DEFAULTPORT;
                    do
                    {
                        try
                        {
