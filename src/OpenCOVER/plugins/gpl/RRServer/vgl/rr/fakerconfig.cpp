...
#if defined(FCONFIG_USESHM) && FCONFIG_USESHM == 1
#include <sys/shm.h>
#include <sys/ipc.h>
#include <sys/types.h>
#endif
    ...
    if (fc == NULL)
    {
        ...
        if (fc == NULL)
        {
            ...
            if ((fc_shmid = shmget(IPC_PRIVATE, sizeof(FakerConfig), IPC_CREAT | 0600)) == -1)
                _throwunix();
            if ((addr = shmat(fc_shmid, 0, 0)) == (void *)-1)
                _throwunix();
            if (!addr)
            ...
            fc = new FakerConfig;
            if (!fc)
                _throw("Could not allocate config structure");
...
#define fetchenv_str(envvar, s)                                                                                     \
    {                                                                                                               \
        if ((env = getenv(envvar)) != NULL && strlen(env) > 0 && (!fcenv_set || strncmp(env, fcenv.s, MAXSTR - 1))) \
        {                                                                                                           \
...
#define fetchenv_int(envvar, i, min, max)                                                          \
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            if (t && t != env && itemp >= min && itemp <= max && (!fcenv_set || itemp != fcenv.i)) \
...
#define fetchenv_dbl(envvar, d, min, max)                                                           \
    {                                                                                               \
        ...
        {                                                                                           \
            char *t = NULL;                                                                         \
            double dtemp = strtod(temp, &t);                                                        \
            if (t && t != temp && dtemp >= min && dtemp <= max && (!fcenv_set || dtemp != fcenv.d)) \
...
void fconfig_setdefaultsfromdpy(Display *dpy)
{
    ...
    if (fconfig.compress < 0)
    {
        ...
        {
            if (usesunray)
                fconfig_setcompress(fconfig, RRCOMP_XV);
            else
    ...
    found:
        ...
        ai = NULL;
        if (port != -1)
            fconfig.transvalid[RRTRANS_XV] = 1;
    }
#endif
}
#endif
...
{
    if (i < 0 || (i >= RR_COMPRESSOPT && strlen(fc.transport) == 0))
        return;
    rrcs::safelock l(fcmutex);

    bool is = (fc.compress >= 0);
    fc.compress = i;
    if (strlen(fc.transport) > 0)
        return;
    if (!is)
        fc.transvalid[_Trans[fc.compress]] = fc.transvalid[RRTRANS_X11] = 1;
    if (fc.subsamp < 0)
