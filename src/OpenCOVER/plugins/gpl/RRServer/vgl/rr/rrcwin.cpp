    ...
    try
    {
        ...
        while (!_deadyet)
        {
            ...
            if (_deadyet)
                break;
            if (!f)
                throw(rrerror("rrcwin::run()", "Invalid image received from queue"));
            rrcs::safelock l(_mutex);
#ifdef USEXV
            if (f->_isxv)
            {
...
#endif
            {
                if (f->_h.flags == RR_EOF)
                {
                    pb.startframe();
                    if (_b->_isgl)
                        ((rrglframe *)_b)->init(f->_h, _stereo);
                    else
                        ((rrfb *)_b)->init(f->_h);
                    if (_b->_isgl)
                        ((rrglframe *)_b)->redraw();
                ...
                else
                {
                    pd.startframe();
                    if (_b->_isgl)
                        *((rrglframe *)_b) = *((rrcompframe *)f);
                    else
                        *((rrfb *)_b) = *((rrcompframe *)f);
                    pd.endframe(f->_h.width * f->_h.height, 0, (double)(f->_h.width * f->_h.height) / (double)(f->_h.framew * f->_h.frameh));
                    bytes += f->_h.size;
                }
            }
            f->complete();
