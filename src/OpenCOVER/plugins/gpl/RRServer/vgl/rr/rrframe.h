...
#include <turbojpeg.h>
#include <tjplanar.h>
#include "../include/rrmutex.h"
#include "../include/rrlog.h"
#include "../include/rrutil.h"
#include <string.h>
#ifdef USEMEDIALIB
#include <mlib.h>
#endif
...
#define RRBMP_CUDAALLOC 8 // use cudaHostAlloc instead of new
...
public:
    ...
    {
        if (pixelsize < 1)
            throw(rrerror("rrframe::init", "Invalid argument"));
        _flags = flags;
        if (h.size == 0)
            h.size = h.framew * h.frameh * pixelsize;
        checkheader(h);
        size_t size = h.framew * h.frameh * pixelsize + 1;
        if (h.framew != _h.framew || h.frameh != _h.frameh || pixelsize != _pixelsize
            || !_bits)
        {
#ifdef HAVE_CUDA
            if (flags & RRBMP_CUDAALLOC)
            {
                if (_bits)
                    cudaFreeHost(_bits);
                cudaHostAlloc((void **)&_bits, size, cudaHostAllocPortable);
            }
            else
#endif
            {
                if (_bits)
                    delete[] _bits;
                newcheck(_bits = new unsigned char[size]);
            }
        }
        if (stereo)
        {
                ...
                || !_rbits)
            {
                ...
                if (flags & RRBMP_CUDAALLOC)
                {
                    if (_rbits)
                        cudaFreeHost(_rbits);
                    cudaHostAlloc((void **)&_rbits, size, cudaHostAllocPortable);
                }
                else
#endif
                {
                    if (_rbits)
                        delete[] _rbits;
                    newcheck(_rbits = new unsigned char[size]);
                }
            }
        }
        else
    ...
    void yuvbaseaddrs(const rrframe &b, unsigned char **planes) const
    {
        return yuvbaseaddrs(b, planes, b._h.x, b._h.y, b._h.height);
    }

    void yuvbaseaddrs(const rrframe &b, unsigned char **planes, int xx, int yy, int hh) const
    {
        int ps = b._pixelsize;
        unsigned char *base = b._bits;
        bool bu = b._flags & RRBMP_BOTTOMUP;
        int y = bu ? b._h.frameh - b._h.y - b._h.height : b._h.y;
        base -= y * b._pitch;
        int x = b._h.x;
        base -= x * ps;
        int h = b._h.frameh;
        int hs = tjMCUWidth[jpegsub(b._h.subsamp)] / 8;
        int vs = tjMCUHeight[jpegsub(b._h.subsamp)] / 8;
        int pitch = b._pitch / ps;
        int pitchsub = (pitch + hs - 1) / hs;
        int hsub = (h + vs - 1) / vs;
        if (bu)
            yy = b._h.frameh - yy - hh;
        planes[0] = base + yy * pitch + xx;
        planes[1] = base + h * pitch + yy / vs * pitchsub + xx / hs;
        planes[2] = base + h * pitch + hsub * pitchsub + yy / vs * pitchsub + xx / hs;
    }

    rrframe *gettile(int x, int y, int w, int h)
    {
        rrframe *f;
        if (!_bits || !_pitch || !_pixelsize)
    ...
    bool tileequals(rrframe *last, int x, int y, int w, int h)
    {
            ...
            && _h.dpynum == last->_h.dpynum)
        {
            if (_bits && last->_bits)
            {
                if (_h.compress == RRCOMP_YUV2JPEG)
                {
                    unsigned char *oldplanes[4], *newplanes[4];
                    yuvbaseaddrs(*last, oldplanes, x, y, h);
                    yuvbaseaddrs(*this, newplanes, x, y, h);

                    int oldpitch = last->_pitch / last->_pixelsize;
                    int newpitch = _pitch / _pixelsize;
                    for (int i = 0; i < h; i++)
                    {
                        if (memcmp(&newplanes[0][newpitch * i], &oldplanes[0][oldpitch * i], w))
                            return false;
                    }
                    int hs = tjMCUWidth[jpegsub(_h.subsamp)] / 8;
                    oldpitch = (oldpitch + hs - 1) / hs;
                    newpitch = (newpitch + hs - 1) / hs;
                    for (int p = 1; p < 3; ++p)
                    {
                        for (int i = 0; i < h; i++)
                        {
                            if (memcmp(&newplanes[p][newpitch * i], &oldplanes[p][oldpitch * i], w))
                                return false;
                        }
                    }
                }
    ...
    void decompressrgb(rrframe &f, int w, int h, bool righteye)
    {
        ...
        unsigned char *srcptr = righteye ? f._rbits : f._bits,
                      *dstptr = righteye ? &_rbits[_pitch * startline + f._h.x * _pixelsize] : &_bits[_pitch * startline + f._h.x * _pixelsize];
        unsigned char *srcptr2, *dstptr2;
        if (!dstbu)
        {
            srcptr = &srcptr[(h - 1) * f._pitch];
            srcstride = -srcstride;
        }
    ...
    void addlogo(void)
    {
        ...
        if (!_bits || _h.width < 1 || _h.height < 1)
            return;
...
public:
    ...
    rrcompframe &operator=(rrframe &b)
    {
        ...
        case RRCOMP_JPEG:
            compressjpeg(b);
            break;
        case RRCOMP_YUV2JPEG:
            compressjpegfromyuv(b);
            break;
        case RRCOMP_YUV:
            compressyuv(b);
            break;
        default:
            _throw("Invalid compression type");
        }
        return *this;
    }
    ...
    void compressyuv(rrframe &b)
    {
        ...
        int tjflags = TJ_YUV;
        if (b._h.subsamp != 4)
            throw(rrerror("YUV encoder", "Invalid argument"));
        init(b._h, 0);
        if (b._flags & RRBMP_BOTTOMUP)
            tjflags |= TJ_BOTTOMUP;
        if (b._flags & RRBMP_BGR)
            tjflags |= TJ_BGR;
        unsigned long size;
    ...
    void compressjpegfromyuv(rrframe &b)
    {
        int tjflags = 0;
        if (b._h.qual > 100 || b._h.subsamp > 16 || !isPow2(b._h.subsamp))
            throw(rrerror("JPEG compressor", "Invalid argument"));
        ...
        unsigned long size;
        unsigned char *empty = (unsigned char *)malloc(b._h.framew * b._h.framew);
        memset(empty, '\0', b._h.framew * b._h.framew);

        unsigned char *srcplanes[4];
        yuvbaseaddrs(b, srcplanes);
        srcplanes[3] = NULL;
        int pitch = b._pitch / b._pixelsize;
        tj(tjCompressPlanar(_tjhnd, srcplanes, b._h.width, pitch, b._h.height, b._pixelsize,
                            _bits, &size, jpegsub(b._h.subsamp), b._h.qual, tjflags));
        _h.size = (unsigned int)size;
        if (b._stereo && b._rbits)
        {
            init(b._h, RR_RIGHT);
            if (_rbits)
                tj(tjCompressPlanar(_tjhnd, srcplanes, b._h.width, b._pitch / b._pixelsize, b._h.height,
                                    b._pixelsize, _rbits, &size, jpegsub(b._h.subsamp), b._h.qual,
                                    tjflags));
            _rh.size = (unsigned int)size;
        }
...
{
public:
    rrfb(Display *dpy, Window win)
        : rrframe()
    {
        if (!dpy || !win)
            throw(rrerror("rrfb::rrfb", "Invalid argument"));
#ifdef XDK
    ...
    rrfb(char *dpystring, Window win)
        : rrframe()
    {
#ifdef XDK
        rrcs::safelock l(_Mutex);
    ...
    void init(char *dpystring, Window win)
    {
        ...
        if (!dpystring || !win)
            throw(rrerror("rrfb::init", "Invalid argument"));
    ...
    void init(rrframeheader &h)
    {
        ...
        _h = h;
        if (_h.framew > _fb.width)
            _h.framew = _fb.width;
        if (_h.frameh > _fb.height)
            _h.frameh = _fb.height;
        _pixelsize = fbx_ps[_fb.format];
        _pitch = _fb.pitch;
        _bits = (unsigned char *)_fb.bits;
        _flags = 0;
        if (fbx_bgr[_fb.format])
            _flags |= RRBMP_BGR;
        if (fbx_alphafirst[_fb.format])
            _flags |= RRBMP_ALPHAFIRST;
    }
...
{
public:
    rrxvframe(Display *dpy, Window win)
        : rrframe()
    {
        if (!dpy || !win)
            throw(rrerror("rrxvframe::rrxvframe", "Invalid argument"));
        XFlush(dpy);
        init(DisplayString(dpy), win);
    }

    rrxvframe(char *dpystring, Window win)
        : rrframe()
    {
        init(dpystring, win);
    }
    ...
    void init(char *dpystring, Window win)
    {
        ...
        if (!dpystring || !win)
            throw(rrerror("rrxvframe::init", "Invalid argument"));
    ...
    void init(rrframeheader &h)
    {
        ...
        _h = h;
        if (_h.framew > _fb.xvi->width)
            _h.framew = _fb.xvi->width;
        if (_h.frameh > _fb.xvi->height)
            _h.frameh = _fb.xvi->height;
