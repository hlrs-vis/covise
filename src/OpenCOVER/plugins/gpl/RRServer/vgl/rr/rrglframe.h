...
{
public:
    rrglframe(char *dpystring, Window win)
        : rrframe()
        ,
#ifndef XDK
        _dpy(NULL)
    ...
    rrglframe(Display *dpy, Window win)
        : rrframe()
        ,
#ifndef XDK
        _dpy(NULL)
    ...
    void init(Display *dpy, Window win)
    {
        ...
        try
        {
            ...
            int n = 0;
            if (!xwa.visual)
                _throw("Could not get window attributes");
...
#ifdef GL_BGR_EXT
        if (littleendian() && h.compress != RRCOMP_RGB)
            flags |= RRBMP_BGR;
#endif
