...
void genericQ::spoil(void *myval, qspoilfct f)
{
    if (deadyet)
        return;
    if (myval == NULL)
        _throw("NULL argument in genericQ::spoil()");
...
void genericQ::add(void *myval)
{
    if (deadyet)
        return;
    if (myval == NULL)
        _throw("NULL argument in genericQ::add()");
    rrcs::safelock l(qmutex);
    if (deadyet)
        return;
...
void genericQ::get(void **myval, bool nonblocking)
{
    if (deadyet)
        return;
    if (myval == NULL)
        _throw("NULL argument in genericQ::get()");
    if (nonblocking)
    {
        if (!qhasitem.trywait())
        {
