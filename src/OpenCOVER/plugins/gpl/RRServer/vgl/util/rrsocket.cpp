...
static X509 *newcert(EVP_PKEY *priv)
{
    ...
    try
    {
        ...
        if ((pub = X509_PUBKEY_new()) == NULL)
            _throwssl();
        X509_PUBKEY_set(&pub, priv);
        if ((pk = X509_PUBKEY_get(pub)) == NULL)
            _throwssl();
        ...
        pub = NULL;
        if (X509_sign(cert, priv, EVP_md5()) <= 0)
            _throwssl();
...
void rrsocket::connect(char *servername, unsigned short port)
{
    ...
    if (_sd != INVALID_SOCKET)
        _throw("Already connected");
#ifdef USESSL
    if (_ssl && _sslctx && _dossl)
        _throw("SSL already connected");
#endif
    ...
    {
        if ((hent = gethostbyname(servername)) == 0)
            _throwsock();
    ...
    if (_dossl)
    {
        if ((_sslctx = SSL_CTX_new(SSLv23_client_method())) == NULL)
            _throwssl();
        if ((_ssl = SSL_new(_sslctx)) == NULL)
            _throwssl();
        if (!SSL_set_fd(_ssl, _sd))
            _throwssl();
        int ret = SSL_connect(_ssl);
        if (ret != 1)
            throw(sslerror("rrsocket::connect", _ssl, ret));
...
unsigned short rrsocket::listen(unsigned short port, bool findport)
{
    ...
    if (_dossl)
    {
        try
        {
            if ((_sslctx = SSL_CTX_new(SSLv23_server_method())) == NULL)
                _throwssl();
...
rrsocket *rrsocket::accept(void)
{
    ...
    if (_sd == INVALID_SOCKET)
        _throw("Not connected");
#ifdef USESSL
    if (!_sslctx && _dossl)
        _throw("SSL not initialized");
#endif
    ...
    if (_dossl)
    {
        if (!(tempssl = SSL_new(_sslctx)))
            _throwssl();
        if (!(SSL_set_fd(tempssl, sd_client)))
            _throwssl();
        int ret = SSL_accept(tempssl);
        if (ret != 1)
            throw(sslerror("rrsocket::accept", tempssl, ret));
...
void rrsocket::send(char *buf, int len)
{
    ...
    while (bytessent < len)
    {
        ...
        if (_dossl)
        {
            ...
            if (retval <= 0)
                throw(sslerror("rrsocket::send", _ssl, retval));
        }
        else
#endif
        {
    ...
    if (bytessent != len)
        _throw("Incomplete send");
}

int rrsocket::recv(char *buf, int len)
{
    if (_sd == INVALID_SOCKET)
        _throw("Not connected");
#ifdef USESSL
    ...
    while (bytesrecd < len)
    {
        ...
        if (_dossl)
        {
            ...
            if (retval <= 0)
                throw(sslerror("rrsocket::recv", _ssl, retval));
        }
        else
#endif
        {
    ...
    if (bytesrecd != len)
        _throw("Incomplete receive");
    return bytesrecd;
}

int rrsocket::set_blocking(int state)
{
