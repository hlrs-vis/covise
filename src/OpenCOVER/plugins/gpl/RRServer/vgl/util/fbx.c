...
const int fbx_rmask[FBX_FORMATS] = { 0x0000FF, 0x0000FF, 0xFF0000, 0xFF0000, 0x0000FF, 0xFF0000, 0 };
const int fbx_gmask[FBX_FORMATS] = { 0x00FF00, 0x00FF00, 0x00FF00, 0x00FF00, 0x00FF00, 0x00FF00, 0 };
const int fbx_bmask[FBX_FORMATS] = { 0xFF0000, 0xFF0000, 0x0000FF, 0x0000FF, 0xFF0000, 0x0000FF, 0 };
const char *_fbx_formatname[FBX_FORMATS] = { "RGB", "RGBA", "BGR", "BGRA", "ABGR", "ARGB", "INDEX" };
...
#define w32(f)                                                                                                                                           \
    {                                                                                                                                                    \
        if (!(f))                                                                                                                                        \
        {                                                                                                                                                \
            FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)__lasterror, 1024, NULL); \
            __line = __LINE__;                                                                                                                           \
            goto finally;                                                                                                                                \
        }                                                                                                                                                \
    }
...
int _fbx_xhandler(Display *dpy, XErrorEvent *e)
{
    ...
    if (prevhandler && prevhandler != _fbx_xhandler)
...
{
    if (format >= 0 && format <= FBX_FORMATS - 1)
        return _fbx_formatname[format];
    else
...
int fbx_init(fbx_struct *s, fbx_wh wh, int width, int height, int useshm)
{
    ...
    if (s->wh == wh)
    {
        if (w == s->width && h == s->height && s->hmdc && s->hdib && s->bits)
            return 0;
        else if (fbx_term(s) == -1)
            return -1;
    }
    ...
    w32(GetDIBits(s->hmdc, hmembmp, 0, 1, NULL, &bminfo.bmi, DIB_RGB_COLORS));
    w32(DeleteObject(hmembmp));
    hmembmp = 0; // We only needed it to get the screen properties
    ps = bminfo.bmi.bmiHeader.biBitCount / 8;
    if (width > 0)
        bminfo.bmi.bmiHeader.biWidth = width;
    if (height > 0)
        bminfo.bmi.bmiHeader.biHeight = height;
    ...
    for (i = 0; i < FBX_FORMATS; i++)
        if (rmask == fbx_rmask[i] && gmask == fbx_gmask[i] && bmask == fbx_bmask[i]
            ...
            s->format = i;
    if (s->format == -1)
        _throw("Display has unsupported pixel format");
    ...
    {
        if (w == s->width && h == s->height && s->xi && s->xgc && s->bits)
            return 0;
        else if (fbx_term(s) == -1)
            return -1;
    }
...
noshm:
    ...
    rmask = s->xi->red_mask;
    gmask = s->xi->green_mask;
    bmask = s->xi->blue_mask;
    alphafirst = 0;
    if (s->xi->byte_order == MSBFirst)
    {
        if (ps < 4)
        {
            rmask = s->xi->blue_mask;
            gmask = s->xi->green_mask;
            bmask = s->xi->red_mask;
        }
        else
...
int fbx_read(fbx_struct *s, int winx, int winy)
{
...
#else
    int x = 0, y = 0;
    XWindowAttributes xwinattrib, rwinattrib;
    Window dummy;
#endif
    if (!s)
        _throw("Invalid argument");
    ...
    return 0;
#else
    if (!s->wh.dpy || !s->wh.win || !s->xi || !s->bits)
        _throw("Not initialized");
#ifdef USESHM
    if (!s->xattach && s->shm)
    {
        x11(XShmAttach(s->wh.dpy, &s->shminfo));
        s->xattach = 1;
    }
#endif
    ...
    if (s->wh.win != xwinattrib.root && s->format != FBX_INDEX)
    {
        ...
        if (y < 0)
            y = 0;
        if (x > rwinattrib.width - s->width)
            x = rwinattrib.width - s->width;
        if (y > rwinattrib.height - s->height)
            y = rwinattrib.height - s->height;
#ifdef USESHM
        if (s->shm)
        {
...
int fbx_write(fbx_struct *s, int bmpx, int bmpy, int winx, int winy, int w, int h)
{
    ...
    return 0;
#else
    if (fbx_awrite(s, bmpx, bmpy, winx, winy, w, h) == -1)
        return -1;
    if (s->pm)
    {
...
int fbx_term(fbx_struct *s)
{
    ...
    if (s->shm)
    {
        if (s->xattach)
        {
            XShmDetach(s->wh.dpy, &s->shminfo);
            XSync(s->wh.dpy, False);
        }
#ifdef XWIN32
        if (s->shminfo.shmaddr != NULL)
            UnmapViewOfFile(s->shminfo.shmaddr);
        if (s->filemap)
            CloseHandle(s->filemap);
#else
        if (s->shminfo.shmaddr != NULL)
            shmdt(s->shminfo.shmaddr);
        if (s->shminfo.shmid != -1)
            shmctl(s->shminfo.shmid, IPC_RMID, 0);
#endif
    }
#endif
    if (s->xgc)
        XFreeGC(s->wh.dpy, s->xgc);
#endif
    memset(s, 0, sizeof(fbx_struct));
    return 0;
