...
public:
    ...
    operator bool()
    {
        return (_method != NULL && _message[0] != 0);
    }
...
#define errifnot(f)                              \
    {                                            \
        if (!(f))                                \
            _throw("Unexpected NULL condition"); \
    }
#define newcheck(f)                            \
    try                                        \
    {                                          \
...
public:
    ...
    w32error(const char *method, DWORD lasterr)
        : rrerror(method, (char *)NULL)
    {
...
{
public:
    unixerror(const char *method)
        : rrerror(method, strerror(errno))
    {
    }
