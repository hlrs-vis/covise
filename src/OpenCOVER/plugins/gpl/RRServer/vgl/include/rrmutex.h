...
public:
    ...
    {
#ifdef _WIN32
        if (!ReleaseMutex(cs) && errorcheck)
            throw(w32error("rrcs::unlock()"));
#else
        int ret;
...
public:
    rrsem(long initialCount = 0)
    {
        ...
        if (sem == SEM_FAILED)
            throw(unixerror("rrsem: sem_open()"));
        else
            sem_unlink(sem_name);
#else
    ...
    void post(void)
    {
        ...
        if (!ReleaseSemaphore(sem, 1, NULL))
            throw(w32error("rrsem::post()"));
#elif defined(__APPLE__)
        if (sem_post(sem) == -1)
            throw(unixerror("rrsem::post()"));
#else
        if (sem_post(&sem) == -1)
            throw(unixerror("rrsem::post()"));
#endif
