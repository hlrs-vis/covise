...
public:
    ...
    void endframe(long pixels, long bytes, double incframes)
    {
        ...
        if (start != 0.0)
        {
            totaltime += now - start;
            if (pixels)
                mpixels += (double)pixels / 1000000.;
            if (bytes)
                mbytes += (double)bytes / 1000000.;
            if (incframes != 0.0)
                frames += incframes;
        }
        if (lastframe == 0.0)
        ...
        if (totaltime > interval || (now - lastframe) > interval)
        {
            ...
            if (mbytes)
            {
                snprintf(&temps[i], 255 - i, "- %7.2f Mbits/sec (%.1f:1)", mbytes * 8.0 / totaltime, mpixels * 3. / mbytes);
                i = strlen(temps);
            }
            rrout.PRINT("%s\n", temps);
            totaltime = 0.;
