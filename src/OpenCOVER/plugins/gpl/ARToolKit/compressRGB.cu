...
#if BLOCKSPERBLOCK == 1
  #define COMPRESS compressRGB1
  #define DECOMPRESS decompressRGB1
#elif BLOCKSPERBLOCK == 2
  #define COMPRESS compressRGB2
  #define DECOMPRESS decompressRGB2
#elif BLOCKSPERBLOCK == 4
  #define COMPRESS compressRGB4
  #define DECOMPRESS decompressRGB4
#endif 
...
//! COMPRESS //
__global__ void COMPRESS(const unsigned char * image, uint2 * result, const uint picWidth) {
    ...
    // COLORS //
    colors[offset + id].x = image[i*3 + j*picWidth*3 + 0];
    colors[offset + id].y = image[i*3 + j*picWidth*3 + 1];
    colors[offset + id].z = image[i*3 + j*picWidth*3 + 2];
...
//! DECOMPRESS //
__global__ void DECOMPRESS(const uint2 * input, unsigned char * output, const uint picWidth) {
    const int BPB = BLOCKSPERBLOCK;
    ...
    // interpolated colors //
    colors[offset2 + 1] = (colors[offset2 + 3] + 2*(colors[offset2 + 0]))/uint(3);
    colors[offset2 + 2] = (colors[offset2 + 3]*2 + (colors[offset2 + 0]))/uint(3);
    ...
    // output //
    if(inp==0){
      output[i*3 + j*picWidth*3 + 0] = colors[offset2 + 0].x;
      output[i*3 + j*picWidth*3 + 1] = colors[offset2 + 0].y;
      output[i*3 + j*picWidth*3 + 2] = colors[offset2 + 0].z;
    } else if(inp==1){
      output[i*3 + j*picWidth*3 + 0] = colors[offset2 + 1].x;
      output[i*3 + j*picWidth*3 + 1] = colors[offset2 + 1].y;
      output[i*3 + j*picWidth*3 + 2] = colors[offset2 + 1].z;
    } else if(inp==2){
      output[i*3 + j*picWidth*3 + 0] = colors[offset2 + 2].x;
      output[i*3 + j*picWidth*3 + 1] = colors[offset2 + 2].y;
      output[i*3 + j*picWidth*3 + 2] = colors[offset2 + 2].z;
    } else if(inp==3){
      output[i*3 + j*picWidth*3 + 0] = colors[offset2 + 3].x;
      output[i*3 + j*picWidth*3 + 1] = colors[offset2 + 3].y;
      output[i*3 + j*picWidth*3 + 2] = colors[offset2 + 3].z;
    }
