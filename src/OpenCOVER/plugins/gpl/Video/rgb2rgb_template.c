...
/*static inline*/ void RENAME(rgb24to32)(const uint8_t *src, uint8_t *dst, long src_size)
{
    ...
    asm volatile(PREFETCH "    %0" ::"m"(*s)
                 : "memory");
    mm_end = end - 23;
    asm volatile("movq        %0, %%mm7" ::"m"(mask32)
                 : "memory");
    while (s < mm_end)
    {
        asm volatile(PREFETCH
                     "    32%1           \n\t"
                     "movd          %1, %%mm0    \n\t"
                     "punpckldq    3%1, %%mm0    \n\t"
                     "movd         6%1, %%mm1    \n\t"
...
#ifdef HAVE_MMX
    mm_end = end - 15;
#if 1 // is faster only if multiplies are reasonably fast (FIXME figure out on which CPUs this is faster, on
    // Athlon it is slightly faster)
    asm volatile(
        "movq           %3, %%mm5   \n\t"
        "movq           %4, %%mm6   \n\t"
        "movq           %5, %%mm7   \n\t"
...
/*static inline*/ void RENAME(rgb24tobgr24)(const uint8_t *src, uint8_t *dst, long src_size)
{
    ...
    if (mmx_size == 23)
        return; // finished, was multiple of 8
...
/*static inline*/ void RENAME(yuvPlanartoyuy2)(const uint8_t *ysrc, const uint8_t *usrc, const uint8_t *vsrc,
                                               uint8_t *dst, long width, long height, long lumStride,
                                               long chromStride, long dstStride, long vertLumPerChroma)
{
    long y;
    ...
    {
#ifdef HAVE_MMX
        // FIXME handle 2 lines at once (fewer prefetches, reuse some chroma, but very likely memory-limited
        // anyway)
        asm volatile("xor                 %%" REG_a ", %%" REG_a
                     ...
                     MOVNTQ "                  %%mm3,   (%0, %%" REG_a ", 4)    \n\t" MOVNTQ
                                           ...
                                           "   \n\t"
                                           " jb                        1b              \n\t" ::"r"(dst),
                     "r"(ysrc), "r"(usrc), "r"(vsrc), "g"(chromWidth)
                     : "%" REG_a);
#else
...
#define pl2yuy2(n)                           \
    ...
    qdst2[n] = yuv2;
        ...
        {
            uint64_t k, l;
            k = yc[0] + (uc[0] << 8) + (yc[1] << 16) + (vc[0] << 24);
            l = yc[2] + (uc[1] << 8) + (yc[3] << 16) + (vc[1] << 24);
            *ldst++ = k + (l << 32);
            yc += 4;
            uc += 2;
            vc += 2;
        ...
        {
#ifdef WORDS_BIGENDIAN
            *idst++ = (yc[0] << 24) + (uc[0] << 16) + (yc[1] << 8) + (vc[0] << 0);
#else
 ...
 */
/*static inline*/ void RENAME(yv12toyuy2)(const uint8_t *ysrc, const uint8_t *usrc, const uint8_t *vsrc,
                                          uint8_t *dst, long width, long height, long lumStride,
                                          long chromStride, long dstStride)
{
    // FIXME interpolate chroma
    RENAME(yuvPlanartoyuy2)(ysrc, usrc, vsrc, dst, width, height, lumStride, chromStride, dstStride, 2);
}

/*static inline*/ void RENAME(yuvPlanartouyvy)(const uint8_t *ysrc, const uint8_t *usrc, const uint8_t *vsrc,
                                               uint8_t *dst, long width, long height, long lumStride,
                                               long chromStride, long dstStride, long vertLumPerChroma)
{
    long y;
...
// FIXME adapt the Alpha ASM code from yv12->yuy2
        ...
        {
            uint64_t k, l;
            k = uc[0] + (yc[0] << 8) + (vc[0] << 16) + (yc[1] << 24);
            l = uc[1] + (yc[2] << 8) + (vc[1] << 16) + (yc[3] << 24);
            *ldst++ = k + (l << 32);
            yc += 4;
            uc += 2;
            vc += 2;
        ...
        {
#ifdef WORDS_BIGENDIAN
            *idst++ = (uc[0] << 24) + (yc[0] << 16) + (vc[0] << 8) + (yc[1] << 0);
#else
 ...
 * (If this is a problem for anyone then tell me, and I will fix it.)
 */
/*static inline*/ void RENAME(yv12touyvy)(const uint8_t *ysrc, const uint8_t *usrc, const uint8_t *vsrc,
                                          uint8_t *dst, long width, long height, long lumStride,
                                          long chromStride, long dstStride)
{
    // FIXME interpolate chroma
 ...
 */
/*static inline*/ void RENAME(yuv422ptoyuy2)(const uint8_t *ysrc, const uint8_t *usrc, const uint8_t *vsrc,
                                             uint8_t *dst, long width, long height, long lumStride,
                                             long chromStride, long dstStride)
{
    ...
    for (y = 0; y < height; y += 2)
    {
        ...
        asm volatile("xor                 %%" REG_a ", %%" REG_a
                     ...
                     MOVNTQ "                  %%mm0, (%3, %%" REG_a ")     \n\t" MOVNTQ
                                 ...
                                 "   \n\t"
                                 " jb                        1b              \n\t" ::"r"(src),
                     "r"(ydst), "r"(udst), "r"(vdst), "g"(chromWidth)
                     : "memory", "%" REG_a);
...
/*static inline*/ void RENAME(yvu9toyv12)(const uint8_t *ysrc, const uint8_t *usrc, const uint8_t *vsrc,
                                          uint8_t *ydst, uint8_t *udst, uint8_t *vdst, long width,
                                          long height, long lumStride, long chromStride)
{
    /* Y Plane */
...
/*static inline*/ void RENAME(planar2x)(const uint8_t *src, uint8_t *dst, long srcWidth, long srcHeight,
                                        long srcStride, long dstStride)
{
    ...
    for (y = 1; y < srcHeight; y++)
    {
        ...
        asm volatile(
            ...
            "            \n\t"
            " js                       1b                       \n\t" ::"r"(src + mmxSize),
            "r"(src + srcStride + mmxSize), "r"(dst + mmxSize * 2), "r"(dst + dstStride + mmxSize * 2),
            "g"(-mmxSize)
            : "%" REG_a);
#else
...
void RENAME(interleaveBytes)(uint8_t *src1, uint8_t *src2, uint8_t *dest, long width, long height,
                             long src1Stride, long src2Stride, long dstStride)
{
    long h;
    ...
    for (h = 0; h < height; h++)
    {
        ...
        asm("xor              %%" REG_a ", %%" REG_a
            ...
            "punpcklbw           %%xmm2, %%xmm0     \n\t"
            "punpckhbw           %%xmm2, %%xmm1     \n\t"
            "movntdq             %%xmm0,   (%0, %%" REG_a
            ", 2)   \n\t"
            "movntdq             %%xmm1, 16(%0, %%" REG_a
            ", 2)   \n\t"
            "add                    $16, %%" REG_a
            "  \n\t"
            "cmp                     %3, %%" REG_a
...
/*static inline*/ void RENAME(vu9_to_vu12)(const uint8_t *src1, const uint8_t *src2, uint8_t *dst1,
                                           uint8_t *dst2, long width, long height, long srcStride1,
                                           long srcStride2, long dstStride1, long dstStride2)
{
    long y, x, w, h;
    w = width / 2;
    h = height / 2;
...
/*static inline*/ void RENAME(yvu9_to_yuy2)(const uint8_t *src1, const uint8_t *src2, const uint8_t *src3,
                                            uint8_t *dst, long width, long height, long srcStride1,
                                            long srcStride2, long srcStride3, long dstStride)
{
    long y, x, w, h;
    w = width / 2;
    h = height;
