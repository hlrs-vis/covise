...
osg::Matrix ListenerCover::getCurrentTransform() const
{
    if (Vrml97Plugin::plugin && Vrml97Plugin::plugin->viewer)
...
vec ListenerCover::getPositionWC() const
{
...
vec ListenerCover::getPositionVC() const
{
    // vec w = getPositionWC();
...
vec ListenerCover::getPositionOC() const
{
    // fprintf(stderr, "getPosOC\n");
    return WCtoOC(getPositionWC());
}

vec ListenerCover::getVelocity() const
{
...
void ListenerCover::getOrientation(vec *at, vec *up) const
{
...
vec ListenerCover::OCtoWC(vec p) const
{
...
vec ListenerCover::OCtoVC(vec p) const
{
    osg::Vec4 pos(p.x, p.y, p.z, 1.0);
    osg::Matrix tr = getCurrentTransform();
    osg::Vec4 v = pos * tr;
...
vec ListenerCover::WCtoVC(vec p) const
{
...
vec ListenerCover::WCtoOC(vec p) const
{
    osg::Vec4 pos(p.x, p.y, p.z, 1.0);
    osg::Matrix tr = getCurrentTransform();
    tr.postMult(getVrmlBaseMat());
...
vec ListenerCover::VCtoOC(vec p) const
{
    osg::Vec4 pos(p.x, p.y, p.z, 1.0);
    osg::Matrix tr = getCurrentTransform();
    osg::Matrix inv = osg::Matrix::inverse(tr);
...
vec ListenerCover::VCtoWC(vec p) const
{
    osg::Vec4 pos(p.x, p.y, p.z, 1.0);
    osg::Matrix tr = getVrmlBaseMat();
    osg::Vec4 v = pos * tr;
