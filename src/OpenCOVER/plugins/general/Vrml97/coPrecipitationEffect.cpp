...
coPrecipitationEffect::coPrecipitationEffect():
...
coPrecipitationEffect::coPrecipitationEffect(const coPrecipitationEffect& copy, const osg::CopyOp& copyop):
...
void coPrecipitationEffect::rain(float intensity)
{
    _wind.set(0.0f,0.0f,0.0f);
    _particleSpeed = -2.0f + -5.0f*intensity/1000.0;
    _particleSize = (0.01 + 0.02*intensity)*1000.0;
...
void coPrecipitationEffect::snow(float intensity)
{
    _wind.set(0.0f,0.0f,0.0f);
    _particleSpeed = -0.75f - 0.25f*intensity/1000.0;
    _particleSize = (0.02f + 0.03f*intensity)*1000.0;
...
void coPrecipitationEffect::compileGLObjects(osg::RenderInfo& renderInfo) const
{
...
void coPrecipitationEffect::traverse(osg::NodeVisitor& nv)
{
...
void coPrecipitationEffect::update()
{
    _dirty = false;

    OSG_INFO<<"coPrecipitationEffect::update()"<<std::endl;
...
void coPrecipitationEffect::createGeometry(unsigned int numParticles,
...
void coPrecipitationEffect::setUpGeometries(unsigned int numParticles)
{
    ...
    OSG_INFO<<"coPrecipitationEffect::setUpGeometries("<<numParticles<<")"<<std::endl;
    ...
    if (!_quadStateSet)
    {
        ...
        char vertexShaderSource[] =
            ...
            "    \n"
            "    if(dv[1] <0.01 && dv[1] >-0.01)\n"
            "        dv[1]=0.01;\n"
            "    vec2 dv_normalized = normalize(dv.xy);\n"
            "    dv.xy += dv_normalized * particleSize;\n"
            "    vec2 dp = vec2( -dv_normalized.y, dv_normalized.x ) * particleSize/5.0;\n"
            "    \n"
...
void coPrecipitationEffect::cull(PrecipitationDrawableSet& pds, osgUtil::CullVisitor* cv) const
{
#ifdef DO_TIMING
...
bool coPrecipitationEffect::build(const osg::Vec3 eyeLocal, int i, int j, int k, float startTime, PrecipitationDrawableSet& pds, osg::Polytope& frustum, osgUtil::CullVisitor* cv) const
{
...
coPrecipitationEffect::PrecipitationDrawable::PrecipitationDrawable():
...
coPrecipitationEffect::PrecipitationDrawable::PrecipitationDrawable(const PrecipitationDrawable& copy, const osg::CopyOp& copyop):
...
void coPrecipitationEffect::PrecipitationDrawable::drawImplementation(osg::RenderInfo& renderInfo) const
{
