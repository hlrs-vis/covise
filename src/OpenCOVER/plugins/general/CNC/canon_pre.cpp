...
void print_nc_line_number()
{
    ...
    for (k SET_TO 0;
         ((k < 256)AND((text[k] IS '\t')OR(text[k] IS ' ') OR(text[k] IS '/')));
         k++)
        ;
...
#define PRINT10(control, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10)            \
    ...
    {                                                                                            \
        fprintf(_outfile, "%5d ", _line_number++);                                               \
        print_nc_line_number();                                                                  \
        fprintf(_outfile, control, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10); \
    }                                                                                            \
    else
    ...
    )
{
    ...
    CNCPlugin::instance()->straightFeed(x, y, z, a, b, c, _traverse_rate);
...
void SELECT_PLANE(CANON_PLANE in_plane)
{
    PRINT1("SELECT_PLANE(CANON_PLANE_%s)\n",
           ((in_plane IS CANON_PLANE_XY) ? "XY" : (in_plane IS CANON_PLANE_YZ) ? "YZ" : (in_plane IS CANON_PLANE_XZ) ? "XZ" : "UNKNOWN"));
...
void START_CUTTER_RADIUS_COMPENSATION(int side)
{
    PRINT1("START_CUTTER_RADIUS_COMPENSATION(%s)\n",
           (side IS CANON_SIDE_LEFT) ? "LEFT" : (side IS CANON_SIDE_RIGHT) ? "RIGHT" : "UNKNOWN");
}
...
{   
    CNCPlugin::instance()->arcFeed(first_end, second_end, axis_end_point, first_axis, second_axis, rotation, _traverse_rate);
    fprintf(_outfile, "%5d ", _line_number++);
    print_nc_line_number();
...
{
    CNCPlugin::instance()->straightFeed(x, y, z, a, b, c, _feed_rate);
    /* fprintf(_outfile, "%5d ", _line_number++);
    print_nc_line_number();
...
void ORIENT_SPINDLE(double orientation, CANON_DIRECTION direction)
{
    PRINT2("ORIENT_SPINDLE(%.4f, %s)\n", orientation,
           (direction IS CANON_CLOCKWISE) ? "CANON_CLOCKWISE" : "CANON_COUNTERCLOCKWISE");
}
...
{
    PRINT1("CLAMP_AXIS(%s)\n",
           (axis IS CANON_AXIS_X) ? "CANON_AXIS_X" : (axis IS CANON_AXIS_Y) ? "CANON_AXIS_Y" : (axis IS CANON_AXIS_Z) ? "CANON_AXIS_Z" : (axis IS CANON_AXIS_A) ? "CANON_AXIS_A" : (axis IS CANON_AXIS_C) ? "CANON_AXIS_C" : "UNKNOWN");
}
...
void UNCLAMP_AXIS(CANON_AXIS axis)
{
    PRINT1("UNCLAMP_AXIS(%s)\n",
           (axis IS CANON_AXIS_X) ? "CANON_AXIS_X" : (axis IS CANON_AXIS_Y) ? "CANON_AXIS_Y" : (axis IS CANON_AXIS_Z) ? "CANON_AXIS_Z" : (axis IS CANON_AXIS_A) ? "CANON_AXIS_A" : (axis IS CANON_AXIS_B) ? "CANON_AXIS_B" : (axis IS CANON_AXIS_C) ? "CANON_AXIS_C" : "UNKNOWN");
}
