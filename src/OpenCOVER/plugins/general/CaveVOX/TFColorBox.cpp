...
const float TFColorBox::DEFAULT_TEXTURE_WIDTH = 2;
const float TFColorBox::DEFAULT_TEXTURE_HEIGHT = 2;
const float TFColorBox::DEFAULT_LABEL_HEIGHT = 0.5;
const float TFColorBox::DEFAULT_FONT_SIZE = 0.4;

TFColorBox::TFColorBox(cui::Interaction *interaction, float width, float height,
                       vvTransFunc *tf)
    : Widget()
    , Events()
{
...
*/
void TFColorBox::createBackground()
{
    _geom = new Geometry();
...
void TFColorBox::createTexturesGeometry()
{
...
void TFColorBox::initVertices()
{
    _vertices = new Vec3Array(4);
    setVertices();
}

void TFColorBox::setVertices()
{
    (*_vertices)[0].set(-_width / 2.0, -_height / 2.0 - DEFAULT_LABEL_HEIGHT / 2.0, 0.0); // bottom left
    (*_vertices)[1].set(_width / 2.0, -_height / 2.0 - DEFAULT_LABEL_HEIGHT / 2.0, 0.0); // bottom right
    (*_vertices)[2].set(_width / 2.0, _height / 2.0 + DEFAULT_LABEL_HEIGHT / 2.0, 0.0); // top right
    (*_vertices)[3].set(-_width / 2.0, _height / 2.0 + DEFAULT_LABEL_HEIGHT / 2.0, 0.0); // top left
}

void TFColorBox::initTextures()
{
...
void TFColorBox::createDefaultTexImage()
{
...
void TFColorBox::addGeomToLeft(Drawable *geom)
{
    _leftGeode->addDrawable(geom);
}

void TFColorBox::addGeomToRight(Drawable *geom)
{
    _rightGeode->addDrawable(geom);
}

void TFColorBox::setImage(int num, Image *img)
{
    switch (num)
    {
    case 0:
...
void TFColorBox::showTexture(int num)
{
    switch (num)
    {
    case 0:
...
void TFColorBox::setLabelText(int num, const std::string &text)
{
    switch (num)
    {
    case 0:
...
float TFColorBox::getWidth()
{
    return _width;
}

float TFColorBox::getHeight()
{
    return (_height + DEFAULT_LABEL_HEIGHT);
}

void TFColorBox::cursorEnter(cui::InputDevice *)
{
}

void TFColorBox::cursorLeave(cui::InputDevice *)
{
}

bool TFColorBox::isInside()
{
    return false;
}

void TFColorBox::cursorUpdate(cui::InputDevice *dev)
{
    if (_moveWidget == true)
    {
        ...
        {
            Matrix l2w = CUI::computeLocal2Root(getNode());
...
void TFColorBox::addBarListener(BarListener *b)
{
    _listeners.push_back(b);
}

void TFColorBox::selectWidget(vvTFWidget *w)
{
    std::list<BarListener *>::iterator iter;
...
void TFColorBox::buttonEvent(InputDevice *dev, int button)
{
    if (button == 0)
    {
...
*/
bool TFColorBox::loadImage(const std::string &)
{
    return false;
}

void TFColorBox::joystickEvent(InputDevice *)
{
}

void TFColorBox::wheelEvent(InputDevice *, int)
{
}
