...
void Vive::preFrame()
{
    ...
    if (!trackingOnly)
    {
        ...
        {
            fprintf(stderr, "configure two channels\n");
        }
    }
}
...
void Vive::postFrame()
{
    ...
    for (vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++)
    {
        ...
        {
            if (Input::instance()->debug(Input::Driver) && Input::instance()->debug(Input::Buttons) && state.ulButtonPressed != 0)
            {
            ...
            for (unsigned int i = 0; i < vr::k_unControllerStateAxisCount; ++i)
            {
                ...
                m_valuatorRanges[(m_ControllerID[unDevice] * vr::k_unControllerStateAxisCount + i) * 2] = m_valuatorRanges[(m_ControllerID[unDevice] * vr::k_unControllerStateAxisCount + i) * 2 + 1] = std::make_pair(0., 0.);
                if (type == vr::k_eControllerAxis_TrackPad || type == vr::k_eControllerAxis_Joystick)
                {
                    m_valuatorRanges[(m_ControllerID[unDevice] * vr::k_unControllerStateAxisCount + i) * 2] = m_valuatorRanges[(m_ControllerID[unDevice] * vr::k_unControllerStateAxisCount + i) * 2 + 1] = std::make_pair(-1., +1.);
                }
                else if (type == vr::k_eControllerAxis_Trigger)
                {
                    m_valuatorRanges[(m_ControllerID[unDevice] * vr::k_unControllerStateAxisCount + i) * 2] = std::make_pair(0., +1.);
                }
                m_valuatorValues[(m_ControllerID[unDevice] * vr::k_unControllerStateAxisCount + i) * 2] = state.rAxis[i].x;
