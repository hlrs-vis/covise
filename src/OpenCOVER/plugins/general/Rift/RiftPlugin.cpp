...
bool RiftPlugin::init()
{
    ...
    hmd->applyShaderParameters(leftEyeStateSet, program.get(), coVRConfig::instance()->channels[0].renderTargetTexture.get(), OculusDevice::LEFT);
    hmd->applyShaderParameters(rightEyeStateSet, program.get(), coVRConfig::instance()->channels[1].renderTargetTexture.get(), OculusDevice::RIGHT);
