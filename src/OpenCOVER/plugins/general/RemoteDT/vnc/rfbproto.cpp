...
bool VNCRFBproto::connectToRFBServer()
{
    ...
    {
        // was error
        fprintf(stderr, "VNCRFBproto::connectToRFBServer() err: unable to connect to VNC server\n");
        return false;
    }
...
void VNCRFBproto::printPixelFormat(rfbPixelFormat *format)
{
    if (format->bitsPerPixel == 1)
    {
        ...
        fprintf(stderr, "VNCRFBproto::printPixelFormat(): %s significant bit in each byte is leftmost on the screen\n",
    ...
    else
    {
        ...
        {
            fprintf(stderr, "VNCRFBproto::printPixelFormat(): %s significant byte first in each pixel\n",
                    (format->bigEndian ? "Most" : "Least"));
        }
        if (format->trueColour)
        {
            fprintf(stderr, "VNCRFBproto::printPixelFormat(): True color: max red %d green %d blue %d, shift red %d green %d blue %d\n",
                    format->redMax, format->greenMax, format->blueMax,
                    format->redShift, format->greenShift, format->blueShift);
        }
        else
            fprintf(stderr, "VNCRFBproto::printPixelFormat(): Colour map (not true colour)\n");
    }
}
