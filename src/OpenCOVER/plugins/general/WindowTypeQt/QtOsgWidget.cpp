...
void QtGraphicsWindow::setCursor(MouseCursor cursor)
{
    ...
    case InfoCursor:
        ...
        break;
    case DestroyCursor:
        m_currentCursor = Qt::ForbiddenCursor;
        break;
    case HelpCursor:
        m_currentCursor = Qt::WhatsThisCursor;
        break;
    case CycleCursor:
        m_currentCursor = Qt::ForbiddenCursor;
        break;
    case SprayCursor:
        ...
        break;
    case WaitCursor:
        m_currentCursor = Qt::WaitCursor;
        break;
    case TextCursor:
        m_currentCursor = Qt::IBeamCursor;
        break;
    case CrosshairCursor:
        m_currentCursor = Qt::CrossCursor;
        break;
    case HandCursor:
        m_currentCursor = Qt::OpenHandCursor;
        break;
    case UpDownCursor:
        m_currentCursor = Qt::SizeVerCursor;
        break;
    case LeftRightCursor:
        m_currentCursor = Qt::SizeHorCursor;
        break;
    case TopSideCursor:
    case BottomSideCursor:
        m_currentCursor = Qt::UpArrowCursor;
        break;
    case LeftSideCursor:
    case RightSideCursor:
        ...
        break;
    case TopLeftCorner:
        m_currentCursor = Qt::SizeBDiagCursor;
        break;
    case TopRightCorner:
...
void QtOsgWidget::setKeyboardModifiers(QInputEvent *event)
{
    ...
    if (modkey & Qt::MetaModifier)
        mask |= osgGA::GUIEventAdapter::MODKEY_META;
...
void QtOsgWidget::wheelEvent(QWheelEvent *event)
{
    ...
    if (std::fabs(event->angleDelta().y()) > std::fabs(event->angleDelta().x()))
    {
        auto motion = event->angleDelta().y() > 0 ? osgGA::GUIEventAdapter::SCROLL_UP : osgGA::GUIEventAdapter::SCROLL_DOWN;
        getEventQueue()->mouseScroll(motion);
    }
    if (std::fabs(event->angleDelta().x()) > std::fabs(event->angleDelta().y()))
    {
        auto motion = event->angleDelta().x() > 0 ? osgGA::GUIEventAdapter::SCROLL_LEFT : osgGA::GUIEventAdapter::SCROLL_RIGHT;
        getEventQueue()->mouseScroll(motion);
    }
#else
