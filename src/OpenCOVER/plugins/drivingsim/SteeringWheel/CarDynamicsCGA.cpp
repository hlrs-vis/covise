...
CarDynamicsCGA::CarDynamicsCGA()
    ...
    ,
    /*tyreFL(0.325, 140000.0, 20.0, 1.0),
      tyreFR(0.325, 140000.0, 20.0, 1.0),
      tyreRL(0.325, 210000.0, 20.0, 1.0),
      tyreRR(0.325, 210000.0, 20.0, 1.0),*/
...
void CarDynamicsCGA::resetState()
{
...
void CarDynamicsCGA::run()
{
    ...
    {
        //Steering
        a_steer = InputDevice::instance()->getSteeringWheelAngle() * 0.5;
        if (a_steer < (-M_PI * 0.4))
            a_steer = -M_PI * 0.4;
        if (a_steer > (M_PI * 0.4))
            a_steer = M_PI * 0.4;
...
void CarDynamicsCGA::move(VrmlNodeVehicle *vehicle)
{
    if (firstMoveCall)
    {
        start();
...
void CarDynamicsCGA::setVehicleTransformation(const osg::Matrix &m)
{
    resetState();

    osg::Vec3d t_c = chassisTrans.getTrans();
    //chassisTrans.setTrans(osg::Vec3(-p_b[1], p_b[2], -p_b[0]));
    //cardyncga::cm::mv<1,2,4>::type p_vrml = {t_c[0], t_c[1], t_c[2]};
    //auto p_b = eval(grade<1>(R_vrml*p_vrml*~R_vrml));
    cardyncga::cm::mv<1, 2, 4>::type p_b = { -m(3, 2), -m(3, 0), m(3, 1) };
    auto T_b = cardyncga::one + cardyncga::einf * p_b * 0.5;

    //osg::Quat q_c = chassisTrans.getRotate();
    //cardyncga::cm::mv<0,3,5,6>::type R_b_vrml = {q_c[3], -q_c[2], q_c[1], q_c[0]};
...
cardyncga::Plane CarDynamicsCGA::getRoadTangentPlane(Road *&road, Vector2D v_c)
{
    if (road)
    {
...
void CarDynamicsCGA::getRoadSystemContactPoint(const cardyncga::Point &p_w, Road *&road, double &u, cardyncga::Plane &s_c)
{
...
void CarDynamicsCGA::getFirstRoadSystemContactPoint(const cardyncga::Point &p_w, Road *&road, double &u, cardyncga::Plane &s_c)
{
    cardyncga::Vector r_w = part_type<cardyncga::Vector>(p_w);
    Vector3D v_w(-r_w[1], r_w[0], r_w[2]);
...
std::pair<Road *, Vector2D> CarDynamicsCGA::getStartPositionOnRoad()
{
    double targetS = 5.0;
