...
osgSimBuffer *ParticleRenderer::GetCudaBufferPosition() { return mCudaBufferPosition; };
osgSimBuffer *ParticleRenderer::GetCudaBufferColors() { return mCudaBufferColor; };

ParticleRenderer::ParticleRenderer(fluidDrawable *fd)
    : m_pos(0)
    , m_numParticles(0)
    , m_pointSize(1.0f)
    , m_spriteSize(2.0f)
    ...
    , m_bFp64Positions(false)
{
    ...
    mCudaBufferPosition = new osgSimBuffer(fd);
    mCudaBufferColor = new osgSimBuffer(fd);
}
...
{
    m_pbo = mCudaBufferPosition->getVBO();
    m_vboColor = mCudaBufferColor->getVBO();
    if (true) //!m_pbo)
    {
...
{
    int numActiveParticles = (mCudaBufferPosition->GetSize() / sizeof(float)) / 4;
    //mCudaBufferPosition->MapBuffer();
    switch (mode)
    {
    ...
    //mCudaBufferPosition->UnmapBuffer();
