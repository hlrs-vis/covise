...
Traffic::Traffic() : coVRPlugin(COVER_PLUGIN_NAME), opencover::ui::Owner("Traffic", opencover::cover->ui) {
    thisPlugin = this;
    initUI();
...
void Traffic::processNewResults() {
    for (auto& vehicleState : currentSimulationState.vehicles) {
        ...
        if (currentEntity == vehicles.end()) { // not found, create new one
...
void Traffic::loadVehicleClasses() {
    ...
    for (auto &it : entries) {
        ...
        const auto& entries =
            covise::coCoviseConfig::getScopeEntries("COVER.Plugin.Traffic.VehicleClasses." + vehicleClassName + ".Models");
