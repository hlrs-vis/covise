	...
	case MSG_ElevatorPart:
        {
            ...
            {
                if (elev->elevatorName == elevatorName)
                {
                    elevator = elev;
                    break;
                }
            ...
            if (elevator == nullptr)
            {
                elevator = new Elevator(ID, name, elevatorName);
                elevators.push_back(elevator);
            }
            elevator->addPart(familyName,subType, newTrans, tb);
            RevitInfo *info = new RevitInfo();
            info->ObjectID = ID;
            info->DocumentID = docID;
            OSGVruiUserDataCollection::setUserData(newTrans, "RevitInfo", info);
		...
		if (it != ElementIDMap[docID].end())
		{
			...
			for (std::list<osg::Node *>::iterator nodesIt = ei->nodes.begin(); nodesIt != ei->nodes.end(); nodesIt++)
			{
				...
				{// this could be a door
					...
					{
						if ((*it)->cabin && (*it)->cabin->transformNode == mt)
						{
							Elevator *di = *it;
							elevators.erase(it);
							delete di;
