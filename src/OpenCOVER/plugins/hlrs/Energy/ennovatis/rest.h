 ...
 *
 * It contains the necessary information to make a REST request, such as the
...
struct rest_request {
  ...
  /**
   * @brief This function generates a string representation of the RESTRequest
   * object.
   *
...
/*
 * @brief The `rest_request_handler` struct provides utility functions for
 * handling REST requests to Ennovatis via utils::threadworker.
 */
struct rest_request_handler {
  /**
   * @brief Fetches the channels from a given channel group and building.
   *
   * This function fetches the channels from the specified channel group and
   * building and populates the REST request object with last used channelid.
   * Results will be available in worker by accessing futures over threads.
   *
   * @param group The channel group to fetch channels from.
   * @param b The building to fetch channels from.
   * @param req The REST request object to populate with fetched channels. (will
   * be copied to worker threads)
   */
  void fetchChannels(const ChannelGroup &group, const ennovatis::Building &b,
                     ennovatis::rest_request req);

  /**
   * @brief Retrieves the result.
   *
   * This function returns a unique pointer to a vector of strings that
   * represents the result if all threads added to the worker are finished.
   *
...
/*
 * @brief The `rest` struct provides utility functions for performing REST
 * requests to Ennovatis.
 */
struct rest {
  /**
   * @brief Fetches Ennovatis data using a REST request.
   *
   * This function sends a REST request to the specified URL using the provided
   * RESTRequest object and returns the response data as a string.
   *
   * @param req The RESTRequest object containing the necessary information for
   * the request.
