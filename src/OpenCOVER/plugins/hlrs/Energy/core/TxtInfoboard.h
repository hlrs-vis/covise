...
struct TxtBoxAttributes {
    TxtBoxAttributes(const osg::Vec3 &pos, const std::string &title, const std::string &font, const float &width,
                     const float &height, const float &margin, const float &titleHeightPercentage, int charSize = 2)
    : position(pos)
    , title(title)
    , fontFile(font)
    , maxWidth(width)
...
public:
    ...
    TxtInfoboard(const osg::Vec3 &position, const std::string &title, const std::string &font, const float &maxWidth,
                 const float &height, const float &margin, const float &titleHeightPercentage, int charSize = 2)
    : m_attributes(
          TxtBoxAttributes(position, title, font, maxWidth, height, margin, titleHeightPercentage, charSize)){};
    ...
    [[nodiscard]] const auto &getMaxWidth(float width) { return m_attributes.maxWidth; }
    [[nodiscard]] const auto &getHeight(float height) { return m_attributes.height; }
    [[nodiscard]] const auto &getFont(const std::string &font) { return m_attributes.fontFile; }
    [[nodiscard]] const auto &getTitle(const std::string &title) { return m_attributes.title; }
    [[nodiscard]] const auto &getCharSize(int charSize) { return m_attributes.charSize; }

private:
    osg::ref_ptr<osgText::Text> createTextBox(const std::string &text, const osg::Vec3 &position, int charSize,
