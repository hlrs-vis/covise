...
struct CylinderAttributes {
    ...
    CylinderAttributes(const float &rad, const float &height, const osg::Vec3 &pos, const ColorMap &colorMap)
    ...
    : CylinderAttributes(rad, height, osg::Vec3(0, 0, 0), ColorMap(maxCol, minCol, defaultCol))
    {}
    float radius;
    float height;
    osg::Vec3 position;
