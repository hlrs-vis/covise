...
void add_attr_to_channel(ennovatis::Channel &channel, const std::string &key,
...
bool sax_channelid_parser::parse_error(std::size_t position,
                                       const std::string &last_token,
                                       const json::exception &ex) {
  if constexpr (debug)
    m_debugLogs.push_back("parse_error(position=" + std::to_string(position) +
