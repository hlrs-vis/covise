...
bool AVRInit(const char *device, int baud_rate)
{
    ...
    if (AVRhCom == INVALID_HANDLE_VALUE)
    {
        // Handle the error.
    ...
    fSuccess = GetCommState(AVRhCom, &dcb);
    ...
    fSuccess = SetCommState(AVRhCom, &dcb);
...
bool AVRClose()
{
    return (CloseHandle(AVRhCom) != 0);
}

/** Wartet die Antwort ab ( max max_time s) und wertet sie aus **/

bool AVRReadBytes(unsigned n, unsigned char *out)
{
    // Attempt a synchronous read operation.
    DWORD nBytesRead = 0;
    BOOL bResult;
    bResult = ReadFile(AVRhCom, out, n, &nBytesRead, NULL);
...
bool AVRSendBytes(const char *command, int c_length)
{
    ...
    DWORD nBytesRead = 0;
    BOOL bResult;
    bResult = WriteFile(AVRhCom, command, c_length, &nBytesRead, NULL);
...
int AVRfd = -1; /** Filedeskriptor ( id fuer Device ) **/
struct termios AVRoldconfig; /** Zwischenspeicher fuer die alte Konfiguration **/
/** der Schnittstelle.				 **/
...
bool AVRReadBytes(unsigned n, unsigned char *out)
{
    ...
    while (Bytes_read < n)
    {
        ...
        ret = read(AVRfd, &out[Bytes_read], n - Bytes_read);
        Bytes_read += ret;
