...
constexpr int SHADER_SCALAR_TIMESTEP_MAPPING_INDEX = 0; // index of the texture that maps from energy grid node index to timestep
                                                        // value
} // namespace
  //
...
Line::Line(std::string name, const Connections &connections)
    : m_name(name)
{
    init(connections);
}
...
{
    return m_name == other.m_name || std::any_of(m_connections.begin(), m_connections.end(), [&other](const auto &pair)
               { return other.m_connections.find(pair.first) != other.m_connections.end(); });
}
    ...
    for (const auto &[name, connection] : m_connections)
    {
        ...
        if (std::find_if(otherConnections.begin(), otherConnections.end(),
                [&name, &start, &end](const auto &otherPair)
                {
                    ...
                    std::string otherReverseName = otherEnd->getName() + " > " + otherStart->getName();

                    if (name == otherReverseName)
                        return true;
                    ...
                    return (start == otherStartPos && end == otherEndPos) || (end == otherStartPos || start == otherEndPos);
                })
