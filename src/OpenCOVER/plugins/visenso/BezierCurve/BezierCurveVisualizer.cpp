...
void BezierCurveVisualizer::drawLine(ref_ptr<MatrixTransform> master, Vec3 point1, Vec3 point2, Vec4 color, float linewidth)
{
    ...
    lineGeoset_state->setMode(GL_LIGHTING, osg::StateAttribute::PROTECTED);
    ref_ptr<PolygonMode> m_Line = new PolygonMode(PolygonMode::FRONT_AND_BACK, PolygonMode::LINE);
    lineGeoset_state->setAttributeAndModes(m_Line, StateAttribute::PROTECTED);
...
void BezierCurveVisualizer::updateGeometry()
{
    ...
    if (showControlPolygonGeom)
    {
        ...
        {
            drawLine(lineDCS, controlPoints[0], controlPoints[1], colorYellow, linewidth);
        }
        else
        {
        ...
        {
            drawLine(lineDCS, controlPoints[controlPoints.size() - 2], controlPoints[controlPoints.size() - 1], colorYellow, linewidth);
        }
        else
        {
...
void BezierCurveVisualizer::drawCurve(ref_ptr<MatrixTransform> master)
{
    ...
    {
        Vec3 p1 = computePointOnCurve(0.0);
        Vec3 p2 = computePointOnCurve(0.001);
        drawLine(master, p1, p2, colorRed);
        for (t = 0.002; t <= 1.0; t += 0.001)
        {
            p1 = p2;
            p2 = computePointOnCurve(t);
            drawLine(master.get(), p1, p2, colorRed);
        }
        break;
    }
...
void BezierCurveVisualizer::visualizeCasteljau(ref_ptr<MatrixTransform> master, double t)
{
    ...
    osg::Sphere *unitSphere = new osg::Sphere(points[0], 10);
...
Vec3 BezierCurveVisualizer::computePointOnCurve(double t)
{
