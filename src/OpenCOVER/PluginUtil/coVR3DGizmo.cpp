...
void coVR3DGizmo::changeGizmoType()
{
    ...
    try
    {
        if (dynamic_cast<coVR3DRotGizmo *>(_gizmo.get()) != nullptr)
        {
            if (_translate)
            {
                _gizmo.reset(new coVR3DTransGizmo(matrix, _size, vrui::coInteraction::ButtonA, "hand", "coVR3DTransGizmo", vrui::coInteraction::Medium, this));
                _type = GIZMO_TYPE::TRANSLATE;
            }
            else if (_scale)
            {
                _gizmo.reset(new coVR3DScaleGizmo(matrix, _size, vrui::coInteraction::ButtonA, "hand", "coVR3DScaleGizmo", vrui::coInteraction::Medium, this));
        ...
        else if (dynamic_cast<coVR3DTransGizmo *>(_gizmo.get()) != nullptr)
        {
            ...
            {
                _gizmo.reset(new coVR3DRotGizmo(matrix, _size, vrui::coInteraction::ButtonA, "hand", "coVR3DRotateGizmo", vrui::coInteraction::Medium, this));
        ...
        else if (dynamic_cast<coVR3DScaleGizmo *>(_gizmo.get()) != nullptr)
        {
            if (_rotate)
            {
                _gizmo.reset(new coVR3DRotGizmo(matrix, _size, vrui::coInteraction::ButtonA, "hand", "coVR3DRotGizmo", vrui::coInteraction::Medium, this));
                _type = GIZMO_TYPE::ROTATE;
            }
            else if (_translate)
            {
