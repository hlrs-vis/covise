...
QDomDocument coConfigImportReader::parse()
{
    ...
    while (!sourceStream->atEnd())
    {
        ...
        if (line.contains(':'))
        {
            ...
            QStringList hostList = line.section(':', 1).split(QRegExp("[,:\\s]"), SplitBehaviorFlags::SkipEmptyParts);
            hosts = hostList.join(",");
...
void coConfigImportReader::updateApplyInstruction(QDomElement &node, QDomElement &instruction)
{
    ...
    if (instruction.tagName() == "Move")
    {
        ...
        QStringList sections = target.split('.', SplitBehaviorFlags::KeepEmptyParts);
    ...
    else if (instruction.tagName() == "Split")
    {
        ...
        QStringList splitKey = instruction.attribute("to").split(' ', SplitBehaviorFlags::SkipEmptyParts);
        QStringList splitValue = splitFrom.split(' ', SplitBehaviorFlags::SkipEmptyParts);
