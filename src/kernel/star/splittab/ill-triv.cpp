...
int main(int argc, char *argv[])
{
    ...
    splitFile << "// Trivial Split.h auto-generated\n" << endl;
    ...
    for (mask = 0; mask < 4096; mask++)
    {
        ...
        if (mask != 0 && mask != 68 && mask != 240 && mask != 244)
        {
            splitFile << "   {   0, { } },  // " << mask << " => " << realMask << endl;
            continue;
        }
        ...
        if (pyr)
        {
            ...
            {
                int numParts;
                Shape **split = pyr->split(numParts);
        ...
        if (pris)
        {
            ...
            {
                int numParts;
                Shape **split = pris->split(numParts);
                if (split)
                {
                    if (numParts > 1)
                        print(mask, split, numParts, splitFile);
        ...
        Hexa hexa(vert);
        int numParts;
        Shape **split = hexa.split(numParts);
