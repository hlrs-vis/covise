...
namespace covise
{
coCallFeedback::coCallFeedback()
{
}

coCallFeedback::coCallFeedback(const coCallFeedback &orig)
{
}
...
void coCallFeedback::init(const char *n, const char *attrib)
{
    ...
    *endOfInfo = '\0';
    ...
    {
        cerr << "coCallFeedback::coCallFeedback: sscanf failed" << endl;
    }
...
void coCallFeedback::unMask(char *string) const
{
    char *oPtr = string;
    while (*oPtr)
    {
...
void coCallFeedback::executeModule(void)
{
...
void coCallFeedback::copyModule()
{
    //   // only the master should fire events for MultiPC versions
...
void coCallFeedback::copyModuleExec()
{
    //   // only the master should fire events for MultiPC versions
    //   if (coVRMSController::instance()->isMaster())
    //   {
...
void coCallFeedback::deleteModule()
{
    //   // only the master should fire events for MultiPC versions
    //   if (coVRMSController::instance()->isMaster())
    //   {
...
const char *coCallFeedback::getParaName(unsigned int i) const
{
    if (i < d_numPara)
...
void coCallFeedback::setVectorParam(const char *name, int numElem, float *field)
{
    char *buf;
...
void coCallFeedback::setVectorParam(const char *name, int numElem, int *field)
{
    char *buf;
...
int coCallFeedback::getBooleanParam(const std::string &paraName, int &value) const
{
...
int coCallFeedback::getIntScalarParam(const std::string &paraName, int &value) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getFloatScalarParam(const std::string &paraName, float &value) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getIntSliderParam(const std::string &paraName, int &min, int &max, int &val) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getFloatSliderParam(const std::string &paraName, float &min, float &max, float &val) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getIntVectorParam(const std::string &paraName, int &numElem, int *&val) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getFloatVectorParam(const std::string &paraName, int &numElem, float *&val) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getStringParam(const std::string &paraName, const char *&val) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getChoiceParam(const std::string &paraName, int &num, char **&labels, int &active) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getFileBrowserParam(const std::string &paraName, char *&val) const
{
    for (unsigned int ctr = 0; ctr < d_numPara; ++ctr)
    {
        if (paraName == getParaName(ctr))
...
int coCallFeedback::getBooleanParam(unsigned int paraNo, int &value) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getIntScalarParam(unsigned int paraNo, int &value) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getFloatScalarParam(unsigned int paraNo, float &value) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getIntSliderParam(unsigned int paraNo, int &min, int &max, int &val) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getFloatSliderParam(unsigned int paraNo, float &min, float &max, float &val) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getIntVectorParam(unsigned int paraNo, int &numElem, int *&val) const
{
    if (paraNo >= d_numPara
    ...
    for (i = 0; i < numElem; i++)
    {
        ...
        {
            cerr << "coCallFeedback::getIntVectorParam: sscanf failed" << endl;
        }
        if (!skipNonSpace(str))
            return -1; // skip to next blank, return if end
    }
...
int coCallFeedback::getFloatVectorParam(unsigned int paraNo, int &numElem, float *&val) const
{
    if (paraNo >= d_numPara
    ...
    for (i = 0; i < numElem; i++)
    {
        ...
        {
            cerr << "coCallFeedback::getFloatVectorParam: sscanf failed" << endl;
        }
        if (!skipNonSpace(str))
            return -1; // skip to next blank, return if end
    }
...
int coCallFeedback::getStringParam(unsigned int paraNo, const char *&val) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getFileBrowserParam(unsigned int paraNo, char *&val) const
{
    if (paraNo >= d_numPara
...
int coCallFeedback::getChoiceParam(unsigned int paraNo, int &num, char **&labels, int &active) const
{
    if (paraNo >= d_numPara
...
void coCallFeedback::setChoiceParam(const char *name, int const pos)
{
    int num;
    int active;
    char **labels;

    getChoiceParam(name, num, labels, active);
    active = pos;
    setChoiceParam("constValueChoiceParam", num, labels, active);
}

void coCallFeedback::setChoiceParam(const char *name, std::string const label)
{
    int num;
    int active;
    char **labels;
    ...
    {
        if (strcmp(label.c_str(), labels[ii]) == 0)
        {
            active = ii;
            break;
        }
    }
    setChoiceParam(name, num, labels, active);
}

void coCallFeedback::setFloatSliderParam(const char *name, float const value)
{
    float min;
    float max;
    float val;
    getFloatSliderParam(name, min, max, val);

    val = value;
    setSliderParam(name, min, max, val);
}

void coCallFeedback::sendFeedback(const char *info, const char *key, const char *data)
{
    CoviseBase::set_feedback_info(info);
    if (data == NULL)
    {
        CoviseBase::send_feedback_message(key, "");
    }
    else
    {
        CoviseBase::send_feedback_message(key, data);
    }
}

void coCallFeedback::print(FILE *outfile)
{
    unsigned int i;
