 ...
 */
static void
    p_ere(p, stop) register struct parse *p;
 ...
 */
static void
    p_ere_exp(p) register struct parse *p;
{
    register char c;
    register sopno pos;
    register int count;
 ...
 */
static void
    p_str(p) register struct parse *p;
{
...
/*
 ...
 ==	register int end2);
 * Giving end1 as REG_OUT essentially eliminates the end1/end2 check.
 *
 ...
 */
static void
    p_bre(p, end1, end2) register struct parse *p;
 ...
 */
static void
    p_bracket(p) register struct parse *p;
{
 ...
 */
static void
    p_b_term(p, cs) register struct parse *p;
register cset *cs;
{
    register char c;
    ...
    default: /* symbol, ordinary character, or range */
        ...
        if ((isupper(start) && isupper(finish)) || (islower(start) && islower(finish)))
        {
            for (i = start; i <= finish; i++)
                if (isalpha(i))
                    CHadd(cs, i);
 ...
 */
static void
    p_b_cclass(p, cs) register struct parse *p;
register cset *cs;
{
 ...
 */
static void
    p_b_eclass(p, cs) register struct parse *p;
register cset *cs;
{
    register char c;
 ...
 */
static void
    bothcases(p, ch) register struct parse *p;
int ch;
{
 ...
 */
static void
    ordinary(p, ch) register struct parse *p;
register int ch;
{
 ...
 */
static void
    nonnewline(p) register struct parse *p;
{
    register char *oldnext = p->next;
    register char *oldend = p->end;
    char bracket[4];
 ...
 */
static void
    repeat(p, start, from, to) register struct parse *p;
 ...
 */
static cset *
    allocset(p) register struct parse *p;
{
 ...
 */
static void
    freeset(p, cs) register struct parse *p;
register cset *cs;
{
    register size_t i;
 ...
 */
static int
    nch(p, cs) register struct parse *p;
register cset *cs;
{
    register size_t i;
    register size_t css = (size_t)p->g->csetsize;
 ...
 */
static void
    mcadd(p, cs, cp) register struct parse *p;
register cset *cs;
register char *cp;
{
 ...
 */
static void
    mcinvert(p, cs) register struct parse *p;
register cset *cs;
{
    (void)p;
#ifdef NDEBUG
 ...
 */
static void
    mccase(p, cs) register struct parse *p;
register cset *cs;
{
    (void)p;
#ifdef NDEBUG
 ...
 */
static void
    doemit(p, op, opnd) register struct parse *p;
sop op;
size_t opnd;
{
    /* avoid making error situations worse */
 ...
 */
static void
    doinsert(p, op, opnd, pos) register struct parse *p;
sop op;
size_t opnd;
sopno pos;
{
 ...
 */
static void
    dofwd(p, pos, value) register struct parse *p;
register sopno pos;
sop value;
{
    /* avoid making error situations worse */
 ...
 */
static void
    enlarge(p, size) register struct parse *p;
register sopno size;
{
    register sop *sp;
 ...
 */
static void
    stripsnug(p, g) register struct parse *p;
register struct re_guts *g;
{
    g->nstates = p->slen;
