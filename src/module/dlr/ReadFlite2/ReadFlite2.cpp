...
void Application::compute(void *)
{
    ...
    {
        strcpy(buf, "ERROR: Can't open grid file >> ");
        strcat(buf, grid_Path);
        Covise::sendError(buf);
        return;
    }
    if ((dataFp = Covise::fopen(data_Path, "r")) == NULL)
    {
        strcpy(buf, "ERROR: Can't open data file >> ");
        strcat(buf, grid_Path);
        Covise::sendError(buf);
        return;
    }
    ...
    if (Mesh != NULL)
    {
        ...
        if (mesh->objectOk())
        {
            ...
            float *tmpd = new float[n_coord];
            fread(&dummy1, sizeof(int), 1, fp);
            fread(tmpd, sizeof(float), (n_coord), fp);
            for (i = 0; i < n_coord; i++)
    ...
    float *tmpd = new float[n_coord];
    fread(&dummy1, sizeof(int), 1, dataFp);
    fread(&n_coord, sizeof(int), 1, dataFp);
    fread(&dummy2, sizeof(int), 1, dataFp);
    if (dummy1 != dummy2)
    {
        Covise::sendError("ERROR wrong FORTRAN block marker");
        return;
    ...
    fread(&dummy1, sizeof(int), 1, dataFp);
    fread(tmpd, sizeof(float), (n_coord), dataFp);
    for (i = 0; i < n_coord; i++)
        r[i] = (float)tmpd[i];
    fread(tmpd, sizeof(float), (n_coord), dataFp);
    for (i = 0; i < n_coord; i++)
