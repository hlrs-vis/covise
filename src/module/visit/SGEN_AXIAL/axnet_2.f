...
c Periodische Raender 2       
     ...
     #  0.,1,lvs6,int((m+1)/2)-1,ase1,f(ixpel2),f(iypel2))
       call MESHSEED(f(ixn),f(iyn),0,0.,0.,0,int((m+1)/2)-1,delta,0.,
     #  0.,1,lvs6,int((m+1)/2)-1,ase1,f(ixper2),f(iyper2))
...
c Vernetzen      
     ...
     #   ase2,1,bvs2,f(ixr+seed),f(iyr+seed))
        seed=seed+ase2
       end do 
       bord=ase1+bord 
c ----------------------------------------------------------------------
...
c Rand 1       
     ...
     #  1,bvs3,f(ixb1+bord),f(iyb1+bord))
c Rand 2      
...
c       
       do za=bord,ase1-1+bord,1
        call GERADE(f(ixb1+za),f(iyb1+za),f(ixb2+za),f(iyb2+za),
     #   ase2,1,bvs3,f(ixr+seed),f(iyr+seed))
        seed=seed+ase2
       end do 
       bord=ase1+bord 
c ----------------------------------------------------------------------
...
c
      integer NL,i,M
c
      DOUBLE PRECISION TBEG,TEND,TV,sw,swm,schr1,schr2,TAB(0:NL-1)
      real L12
c
      swm=(TEND-TBEG)/(NL-1) 
      if (m.eq.1) then
