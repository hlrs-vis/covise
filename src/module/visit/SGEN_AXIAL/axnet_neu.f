...
c
     ...
     #        AUSG,dat1,dat2,dat3,dat4,dat5,dat6,dat8,
...
c ----------------------------------------------------------------------
      parameter (a_gebi=20,a_deck=2)
c ----------------------------------------------------------------------
c DIMENSION DES DATENFELDES, BEI FEHLERMELDUNG VERGROESSERN 
c ----------------------------------------------------------------------
      parameter (DIM_F=1500000,DIM_E=3000000,DIM_BI=100,DIM_PVR=100)
c ----------------------------------------------------------------------
...
c ----------------------------------------------------------------------
     ...
     # dat5,dat6,dat8,lvs1,lvs2,lvs3,lvs4,lvs5,lvs6,lvs7,bvs1,bvs2,bvs3,
     # bvs4,bvs5,rvs,vsfdec,gevs1N,gevs2N,gevs1K,gevs2K,CRE_RB,v_ein,
     # l_mod,wen_kb,weo_kb,weu_kb,wel_kb,wer_kb,wel_km,wer_km,weo_km)
c ----------------------------------------------------------------------
...
c Dreiecksgebiete werden mit NETZX vernetzt       
       if (za.gt.(a_gebi-(3*a_deck))) d_bord2=ase(za,2)+d_bord2 
      end do
c Sonstige Gebiete die mit NETZX vernetzt werden beruecksichtigen
      d_bord2=d_bord2+ase(6,2)+ase(9,2)+ase(11,2)+ase(12,2)+
     # ase(13,2)+ase(14,2) 
c ----------------------------------------------------------------------
c DATENFELD f (real)
c ----------------------------------------------------------------------
...
c Periodische Raender (Hilfspunkte)
      ...
      iypel2=ixpel2+ase(10,1)
      ixper2=iypel2+ase(10,1)
      iyper2=ixper2+ase(10,1)
c Periodische Raender (Hilfspunkte)
      ixpel3=iyper2+ase(10,1)
      iypel3=ixpel3+ase(6,2)
      ixper3=iypel3+ase(6,2)
      iyper3=ixper3+ase(6,2)
...
c ----------------------------------------------------------------------
     ...
     #  alp_hi,schnitt,bi,xu,xo,alpha,val,beta)
c ----------------------------------------------------------------------
...
c ----------------------------------------------------------------------
       ...
       vs1dec=vsdeck*(gevs1N+(gevs1K-gevs1N)/(bi-1)*(schnitt-1))
       vs2dec=vsdeck*(gevs2N+(gevs2K-gevs2N)/(bi-1)*(schnitt-1))
c ----------------------------------------------------------------------
c Berechnung der Hilfspunkte fuer die periodischen Raender
c ----------------------------------------------------------------------
c Periodische Raender 1
       ase1=ase(11,1)
       call GERADE(f(ixn),f(iyn),xu,yu,ase1,1,lvs5,
     #  f(ixpel1),f(iypel1))
       do za=0,ase1-1,1
       ...
       do za=0,ase1-1,1
        f(ixpel1+za)=f(ixpel1+za)-delta 
       end do
c Periodische Raender 2       
       ase1=ase(10,1)
       call MESHSEED(f(ixn),f(iyn),0,0.,0.,0,int((m+1)/2)-1,-delta,0.,
     #  0.,2,lvs6,int((m+1)/2)-1,ase1,f(ixpel2),f(iypel2))
       call MESHSEED(f(ixn),f(iyn),0,0.,0.,0,int((m+1)/2)-1,delta,0.,
     #  0.,2,lvs6,int((m+1)/2)-1,ase1,f(ixper2),f(iyper2))
c Periodische Raender 3
       ase1=ase(6,2)
       call GERADE(f(ixn+int((m+1)/2)-1),f(iyn+int((m+1)/2)-1),xo,yo,
     #  ase1,1,lvs7,f(ixpel3),f(iypel3))
       do za=0,ase1-1,1
        f(ixper3+za)=f(ixpel3+za)+delta
        f(iyper3+za)=f(iypel3+za) 
       end do
       do za=0,ase1-1,1
        f(ixpel3+za)=f(ixpel3+za)-delta 
       end do
c ----------------------------------------------------------------------
...
c ----------------------------------------------------------------------
       ...
       ase2=ase(1,2)
c Rand 1       
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(1),PG(2),0.,0.,0.,
     #  2,lvs1,m-1,ase1,f(ixb1+bord),f(iyb1+bord))
c Rand 2       
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(1),PG(2),0.,0.,aint(bgs),
     #  2,lvs1,m-1,ase1,f(ixb2+bord),f(iyb2+bord))
c Vernetzen      
...
c ----------------------------------------------------------------------
       ...
       ase2=ase(2,2)
c Rand 1       
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(2),PG(3),0.,0.,0.,
     #  1,lvs2,m-1,ase1,f(ixb1+bord),f(iyb1+bord))
c Rand 2       
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(2),PG(3),0.,0.,aint(bgs),
     #  1,lvs2,m-1,ase1,f(ixb2+bord),f(iyb2+bord))
c Vernetzen      
       do za=bord,ase1-1+bord,1
        call GERADE(f(ixb2+za),f(iyb2+za),f(ixb1+za),f(iyb1+za),
     #   ase2,1,bvs1,f(ixr+seed),f(iyr+seed))
...
c Rand 2       
       do za=0,ase1-1,1
        f(ixb2+bord+za)=f(ixpel2+ase2-1+za)      
        f(iyb2+bord+za)=f(iypel2+ase2-1+za)
       end do
c Rand 1       
       pob=0
       do za=0,ase1-1,1
...
c Vernetzen      
     ...
     #   ase2,2,bvs2,f(ixr+seed),f(iyr+seed))
        seed=seed+ase2
       end do 
       bord=ase1+bord 
c ----------------------------------------------------------------------
c Gebiet 6, Achtung: Rapu werden berechnet - G5 muss gleiche Rapu haben
c ----------------------------------------------------------------------
       ase1=ase(6,1)
       ase2=ase(6,2)
c Rand 1       
       pob=ase(1,1)
       call GERADE(f(ixb2+pob),f(iyb2+pob),f(ixpel3),f(iypel3),ase1,
     #  2,bvs2,f(ixb1+bord),f(iyb1+bord))
c Rand 2       
       hilf1=(yo-f(iyb2+pob))/tan(alpha+ael)+f(ixb2+pob)
       hilf2=f(ixpel3+ase2-1)
       call GERADE(hilf1,yo,hilf2,yo,ase1,
     #  2,bvs2,f(ixb2+bord),f(iyb2+bord))
c Rand 3
       pob=bord
       call GERADE(f(ixb1+pob),f(iyb1+pob),f(ixb2+pob),f(iyb2+pob),
     #  ase2,1,lvs7,f(ixb3+bord2),f(iyb3+bord2))
c Rand 4
       do za=0,ase2-1,1
        f(ixb4+bord2+za)=f(ixpel3+za)
        f(iyb4+bord2+za)=f(iypel3+za)
       end do
c Vernetzten      
       call NETZ(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
     #  ase2,f(ixr+seed),f(iyr+seed))   
c
       bord=ase1+bord
       bord2=ase2+bord2
       seed=seed+ase1*ase2
c ----------------------------------------------------------------------
c Gebiet 7, Achtung: Rapu werden berechnet - G6 muss gleiche Rapu haben
c ----------------------------------------------------------------------
       ase1=ase(2,1)
       ase2=ase(7,2)
c Rand 2       
       ...
       call GERADE(hilf1,yo,hilf2,yo,ase1,1,1.2,f(ixb2+bord),
     #  f(iyb2+bord))
c Rand 1      
       pob=ase(1,1)
       do za=0,ase1-1,1
...
c Vernetzen      
     ...
     #   ase2,1,lvs7,f(ixr+seed),f(iyr+seed))
        ...
        bord=ase1+bord
c ----------------------------------------------------------------------
c Gebiet 8, Achtung: Rapu werden berechnet - G7 muss gleiche Rapu haben
c ----------------------------------------------------------------------
       ase1=ase(3,1)
       ase2=ase(8,2)
c Rand 2       
       ...
       call GERADE(hilf1,yo,hilf2,yo,ase1,1,.8,f(ixb2+bord),
     #  f(iyb2+bord))
c Rand 1       
       pob=ase(1,1)+ase(2,1)
       do za=0,ase1-1,1
...
c Rand 1       
       pob=ase(1,1)+ase(2,1)+ase(3,1)-1
       call GERADE(f(ixb2+pob),f(iyb2+pob),f(ixper3),f(iyper3),ase1,
     #  2,bvs3,f(ixb1+bord),f(iyb1+bord))
c Rand 2      
       hilf1=(yo-f(iyb2+pob))/tan(alpha-aer)+f(ixb2+pob)
       hilf2=f(ixper3+ase2-1)
       call GERADE(hilf1,yo,hilf2,yo,ase1,1,bvs3,f(ixb2+bord),
     #  f(iyb2+bord))
c Rand 3
       pob=bord
        call GERADE(f(ixb1+pob),f(iyb1+pob),f(ixb2+pob),f(iyb2+pob),
...
c Rand 4 
       do za=0,ase2-1,1
        f(ixb4+bord2+za)=f(ixper3+za)
        f(iyb4+bord2+za)=f(iyper3+za)
       end do
c Vernetzen      
       call NETZ(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
...
c
       ...
       seed=seed+ase1*ase2
c ----------------------------------------------------------------------
c Gebiet 10, Achtung: Rapu werden berechnet - G9 muss gleiche Rapu haben
c ----------------------------------------------------------------------
       ase1=ase(10,1)
       ase2=ase(10,2)
       do za=0,ase1-1,1
        f(ixb2+bord+za)=f(ixper2+(ase1-1)-za)
        f(iyb2+bord+za)=f(iyper2+(ase1-1)-za)
       end do
...
c       
       do za=bord,ase1-1+bord,1
        call GERADE(f(ixb1+za),f(iyb1+za),f(ixb2+za),f(iyb2+za),
     #   ase2,2,bvs3,f(ixr+seed),f(iyr+seed))
        seed=seed+ase2
       end do 
       bord=ase1+bord 
c ----------------------------------------------------------------------
...
c Rand 1
       por=0
       hilf1=(yu-f(iyr+por))/tan(beta)+f(ixr+por)
       call GERADE(f(ixr+por),f(iyr+por),hilf1,yu,ase1,
     #  1,lvs5,f(ixb1+bord),f(iyb1+bord))
c Rand 2
       do za=0,ase1-1,1
        f(ixb2+bord+za)=f(ixpel1+za)
        f(iyb2+bord+za)=f(iypel1+za)
       end do
c Rand 3
       pob=bord
       call GERADE(f(ixb1+pob),f(iyb1+pob),f(ixb2+pob),f(iyb2+pob),
     #  ase2,1,bvs4,f(ixb3+bord2),f(iyb3+bord2))
c
       pob=ase2-1
       do za=0,(ase2+1)/2-1,1
        hilf2=vsdeck/((ase2+1)/2-1)*za
        f(iyb3+bord2+za)=f(iyb3+bord2+za)-hilf2
        if (za.ne.(ase2+1)/2-1) f(iyb3+bord2+pob-za)=f(iyb3+bord2+pob-
     #   za)-hilf2
       end do
c Rand 4
       pob=ase1-1+bord
       call GERADE(f(ixb1+pob),f(iyb1+pob),f(ixb2+pob),f(iyb2+pob),
     #  ase2,1,bvs4,f(ixb4+bord2),f(iyb4+bord2))
c Vernetzten
       call NETZ2(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
     #  ase2,f(ixr+seed),f(iyr+seed))   
c
       bord=ase1+bord 
...
c
c ----------------------------------------------------------------------
c Gebiet 12, Achtung: Rapu werden berechnet - G1 muss gleiche Rapu haben
c ----------------------------------------------------------------------
       ase1=ase(11,1)
       ase2=ase(12,2)
c Rand 1       
...
c Rand 2       
       ...
       do za=0,ase1-1,1
        f(ixb2+bord+za)=f(ixb1+pob+za)
        f(iyb2+bord+za)=f(iyb1+pob+za)
       end do   
c Rand 3
       pob=bord
        call GERADE(f(ixb2+pob),f(iyb2+pob),f(ixb1+pob),f(iyb1+pob),
     #   ase2,1,bvs1,f(ixb3+bord2),f(iyb3+bord2))
c Rand 4
       pob=ase1-1+bord
        call GERADE(f(ixb2+pob),f(iyb2+pob),f(ixb1+pob),f(iyb1+pob),
     #   ase2,1,bvs1,f(ixb4+bord2),f(iyb4+bord2))
c Vernetzen      
       call NETZ(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
     #  ase2,f(ixr+seed),f(iyr+seed))   
...
c Rand 1      
       ...
       do za=0,ase1-1,1
        f(ixb1+bord+za)=f(ixb1+pob+za)
        f(iyb1+bord+za)=f(iyb1+pob+za)
       end do   
c Rand 3
       pob=bord
        call GERADE(f(ixb2+pob),f(iyb2+pob),f(ixb1+pob),f(iyb1+pob),
...
c Rand 2
       do za=0,ase1-1,1
        f(ixb2+bord+za)=f(ixper1+za)
        f(iyb2+bord+za)=f(iyper1+za)
       end do
c Rand 1, kopiere von Gebiet 13
       pob=0
       do za=1,12,1
        pob=pob+ase(za,1)
       end do
...
c Rand 3        
       pob=bord
       call GERADE(f(ixb1+pob),f(iyb1+pob),f(ixb2+pob),f(iyb2+pob),
     #   ase2,1,bvs5,f(ixb3+bord2),f(iyb3+bord2))
c
       pob=ase2-1
       do za=0,(ase2+1)/2-1,1
        hilf2=vsdeck/((ase2+1)/2-1)*za
...
c Rand 4
       pob=ase1-1+bord
       call GERADE(f(ixb1+pob),f(iyb1+pob),f(ixb2+pob),f(iyb2+pob),
     #   ase2,1,bvs5,f(ixb4+bord2),f(iyb4+bord2))
c Vernetzten
       call NETZ2(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
     #  ase2,f(ixr+seed),f(iyr+seed))   
...
c Mittelpunkt Seite 2
       ...
       xmp2=f(ixr+por)
       ymp2=f(iyr+por)
c Mittelpunkt Seite 3
       xmp3=f(ixpel2+ase1-1)
       ymp3=f(iypel2+ase1-1)
c Mittelpunkt im Dreieck       
       por=0
       call SCHNI_PU(xmp3,ymp3,f(ixr+por),f(iyr+por),xmp1,ymp1,
     #  f(ixpel2+2*(ase1-1)),f(iypel2+2*(ase1-1)),xspu,yspu)
       yspu=yspu-vs1dec 
c
c Gebiet 15
       ase2=ase(15,1)
       ...
       do za=0,ase1-1,1
        f(ixb3+bord2+za)=f(ixr+por+za)
        f(iyb3+bord2+za)=f(iyr+por+za)
       end do   
       call GERADE(xmp2,ymp2,xspu,yspu,ase1,
     #  0,0.,f(ixb4+bord2),f(iyb4+bord2))
c
       ase1=ase(15,2)
       por=0
       do za=1,4,1
       ...
       do za=0,ase2-1,1
        f(ixb1+bord+za)=f(ixr+por+za)
        f(iyb1+bord+za)=f(iyr+por+za)
       end do   
       call GERADE(xmp1,ymp1,xspu,yspu,ase2,
...
c
       ...
       bord2=bord2+ase2
c
c Gebiet 16,Achtung: Rapu werden berechnet -G15 muss gleiche Rapu haben
       ase1=ase(16,1)
       por=0
       do za=1,10,1
        por=por+ase(za,1)*ase(za,2)
       ...
       pob=0
       do za=ase1-1,0,-1
        f(ixb1+bord+pob)=f(ixr+por+za)
        f(iyb1+bord+pob)=f(iyr+por+za)
        pob=pob+1
       end do   
       call GERADE(xmp3,ymp3,xspu,yspu,ase1,
     #  0,0.,f(ixb2+bord),f(iyb2+bord))
c
       ase2=ase(16,2)
       do za=0,ase2-1,1
        f(ixb3+bord2+za)=f(ixpel2+za)
        f(iyb3+bord2+za)=f(iypel2+za)
       end do   
       call GERADE(xmp1,ymp1,xspu,yspu,ase2,
     #  0,0.,f(ixb4+bord2),f(iyb4+bord2))
c      
       call NETZ3(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
     #  ase2,f(ixr+seed),f(iyr+seed))   
c
c Periodische Randknoten durch echte Punkte ersetzten       
       por=seed
       do za=0,ase2-1,1
        f(ixr+por+za)=f(ixpel2+za)
        f(iyr+por+za)=f(iypel2+za)
       end do
c       
       seed=seed+ase1*ase2
       bord=bord+ase1
       bord2=bord2+ase2
c
c Gebiet 17,Achtung: Rapu werden berechnet -G16 muss gleiche Rapu haben
       ase1=ase(17,1)
       por=0
       do za=1,4,1
        por=por+ase(za,1)*ase(za,2)
       ...
       pob=0
       do za=ase1-1,0,-1
        f(ixb2+bord+pob)=f(ixr+por+za)
        f(iyb2+bord+pob)=f(iyr+por+za)
        pob=pob+1
       end do   
       call GERADE(xmp3,ymp3,xspu,yspu,ase1,
...
c
       ...
       pob=ase2-1
       do za=0,ase2-1,1
        f(ixb3+bord2+za)=f(ixpel2+za+pob)
        f(iyb3+bord2+za)=f(iypel2+za+pob)
       end do   
       call GERADE(xspu,yspu,xmp2,ymp2,ase2,
     #  0,0.,f(ixb4+bord2),f(iyb4+bord2))
c      
       call NETZ2(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
...
c Periodische Randknoten durch echte Punkte ersetzten       
       por=seed 
       do za=0,ase2-1,1
        f(ixr+por+za)=f(ixpel2+za+ase2-1)
        f(iyr+por+za)=f(iypel2+za+ase2-1)
       end do
c       
       seed=seed+ase1*ase2
       bord=bord+ase1
...
c Mittelpunkt 2
       ...
       ymp2=f(iyr+por)
c Mittelpunkt 3
       pob=ase(1,1)+ase(2,1)+ase(3,1)+ase(4,1)-ase1
       xmp3=f(ixb2+pob)
       ymp3=f(iyb2+pob)
c Mittelpunkt im Dreieck       
       pob=ase(1,1)+ase(2,1)+ase(3,1)+ase(4,1)-ase1
       call SCHNI_PU(xmp3,ymp3,f(ixper2),f(iyper2),xmp1,ymp1,
     #  f(ixb2+pob-ase1+1),f(iyb2+pob-ase1+1),xspu,yspu)
       yspu=yspu-vs2dec
c
c Gebiet 18
       ase1=ase(18,1)
       ...
       call GERADE(xspu,yspu,xmp2,ymp2,ase1,
     #  0,0.,f(ixb2+bord),f(iyb2+bord))
c
       ase2=ase(18,2)
       por=0
       ...
       por=por-ase2
       do za=0,ase2-1,1
        f(ixb4+bord2+za)=f(ixr+por+ase2-1-za)
        f(iyb4+bord2+za)=f(iyr+por+ase2-1-za)
       end do   
       call GERADE(xmp1,ymp1,xspu,yspu,ase2,
     #  0,0.,f(ixb3+bord2),f(iyb3+bord2))
c      
       call NETZ3(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
     #  ase2,f(ixr+seed),f(iyr+seed))   
       ...
       bord2=bord2+ase2
c
c Gebiet 19,Achtung: Rapu werden berechnet -G18 muss gleiche Rapu haben
       ase1=ase(19,1)
c Rand 1, kopiere von Gebiet 14
       por=0
       do za=1,13,1
        por=por+ase(za,1)*ase(za,2)
       end do
...
c
       ase2=ase(19,2)
c Rand 3, kopiere von Gebiet 4
       pob=ase(1,1)+ase(2,1)+ase(3,1)+ase(4,1)-ase2
       do za=0,ase2-1,1
        f(ixb3+bord2+za)=f(ixb2+pob+ase2-1-za)
        f(iyb3+bord2+za)=f(iyb2+pob+ase2-1-za)
       end do   
c Rand 4
       call GERADE(xmp1,ymp1,xspu,yspu,ase2,
     #  0,0.,f(ixb4+bord2),f(iyb4+bord2))
...
c       
       ...
       bord2=bord2+ase2
c
c Gebiet 20,Achtung: Rapu werden berechnet -G19 muss gleiche Rapu haben
       ase1=ase(20,1)
       por=0
       do za=1,10,1
        por=por+ase(za,1)*ase(za,2)
...
c
       ase2=ase(20,2)
       pob=ase(1,1)+ase(2,1)+ase(3,1)+ase(4,1)-ase(5,2)
       do za=0,ase2-1,1
        f(ixb3+bord2+za)=f(ixb2+pob+za)
        f(iyb3+bord2+za)=f(iyb2+pob+za)
       end do   
       call GERADE(xmp2,ymp2,xspu,yspu,ase2,
     #  0,0.,f(ixb4+bord2),f(iyb4+bord2))
c      
       call NETZ2(f(ixb1+bord),f(iyb1+bord),f(ixb2+bord),f(iyb2+bord),
     #  f(ixb3+bord2),f(iyb3+bord2),f(ixb4+bord2),f(iyb4+bord2),ase1,
...
c ----------------------------------------------------------------------
       if ((bord.ne.d_bord).OR.(seed.ne.d_seed).OR.(bord2.ne.
     #  d_bord2)) then
...
c
      DOUBLE PRECISION LANG,PHI,TAB(DIM)
c
      if (seed.gt.DIM) then
...
c
      do i=1,seed,1 
       x(i)=x1+real(TAB(i))*sign(1.,(x2-x1))*real(DCOS(PHI))
       y(i)=y1+real(TAB(i))*sign(1.,(y2-y1))*real(DSIN(PHI))
      end do
c
      end
c ----------------------------------------------------------------------
...
c ----------------------------------------------------------------------
     ...
     # schnitt,bi,pxu,pxo,wi_ein,val,wi_aus)
c
...
c
      if (valK.le.PI) then
       val=valN+(valK-valN)/(bi-1)*(schnitt-1)
      else
...
c
     ...
     # YTAB(0:NL-1),swm,schr1,schr2,TV,sw,temp(0:500),
     # br(500),brges(500)
      real L12
c
      swm=(TEND-TBEG)/(NL-1) 
...
c
      ...
      print*,L12
      if ((L12.lt.0.6).OR.(L12.gt.1.4)) then
         print*,'L12 auf 1.0 gesetzt'
         L12=1.0
      endif
      if (m.eq.1) then
         do i=2,NL-1,1       
...
c        print*,temp(i-1),(TBEG+(TEND-TBEG)*brges(i)/brges(NL))
        temp(i-1)=TBEG+(TEND-TBEG)*brges(i)/brges(NL)
c          print*,'Differenz:'

c         print*,i,' erfolgreich'      
      end do    
c      endif



c      print*,'von ',TBEG,' bis ',TEND



      do i=0,NL-1,1
c       print*,'     ',temp(i)
       call PSPVAL (temp(i),N,T,AX,BX,CX,DX,AY,BY,CY,DY,XTAB(i),YTAB(i))
      end do   
c
      end
c ----------------------------------------------------------------------
...
c ----------------------------------------------------------------------
     ...
     # dat4,dat5,dat6,dat7,lvs1,lvs2,lvs3,lvs4,lvs5,lvs6,lvs7,bvs1,bvs2,
     # bvs3,bvs4,bvs5,rvs,vsfdec,gevs1N,gevs2N,gevs1K,gevs2K,CRE_RB,
     # v_ein,l_mod,wen_kb,weo_kb,weu_kb,wel_kb,wer_kb,wel_km,wer_km,
     # weo_km)
c
c
c AUSGABE
...
c
      ...
      read(14,'(T31,I3)',ERR=1) dat1
      read(14,'(T33,I3)',ERR=1) dat2
      read(14,'(T31,I3)',ERR=1) dat3
      read(14,'(T31,I3)',ERR=1) dat4
      read(14,'(T32,I3)',ERR=1) dat5
      read(14,'(T30,I3)',ERR=1) dat6
      read(14,'(T31,I3)',ERR=1) dat7
      read(14,'(T41,F5.3)',ERR=1) lvs1 
      read(14,'(T47,F5.3)',ERR=1) lvs2 
      read(14,'(T49,F5.3)',ERR=1) lvs3 
      read(14,'(T49,F5.3)',ERR=1) lvs4 
      read(14,'(T46,F5.3)',ERR=1) lvs5 
...
c Splinepunkte berechnen
       call EINT_L12(ia,XS(1),XS(b),mo,l12,XI)
c
       do j=1,ia,1
...
c AUSGABE
c
c TAB.....Einteilung (TBEG..TEND)  [DOUBLE PRECISION]
c
      implicit none
c
      integer NL,i,M
c
c      DOUBLE PRECISION TBEG,TEND,TV,sw,swm,schr1,schr2,TAB(0:NL-1)
      DOUBLE PRECISION TBEG,TEND,tv(50),sw(50),swm,
     #                 schr1,schr2,TAB(0:NL-1)
      real L12
c
c      swm=(TEND-TBEG)/(NL-1) 
c      if (m.eq.1) then
c        schr1=2*swm/(1+1/L12)
c        schr2=2*swm/(1+L12)
c      elseif ((m.eq.2).and.((NL/2.).ne.int(NL/2.))) then
c        schr1=2*swm/(1+1/L12)
c        schr2=2*swm/(1+L12)
c      elseif ((m.eq.2).and.((NL/2.).eq.int(NL/2.))) then
c        schr1=2*(TEND-TBEG)/((1/L12)*(L12*NL+NL-2))
c        schr2=2*(TEND-TBEG)/(L12*NL+NL-2)
c      endif
c     
c      TV=TBEG 
c
...
c       if (i.eq.0) then
c            sw=0
c       elseif (m.eq.1) then 
c            sw=schr1+(schr2-schr1)*(i-1)/(NL-2)
c       elseif ((m.eq.2).and.((NL/2.).ne.int(NL/2.))) then   
c         if (i.le.((NL-1)/2)) then
c            sw=schr1+(schr2-schr1)*(i-1)/((NL-3)/2.)             
c         else
c            sw=schr2+(schr1-schr2)*(i-1-((NL-1)/2))/((NL-3)/2.)
c         endif   
c       elseif ((m.eq.2).and.((NL/2.).eq.int(NL/2.))) then
c         if (i.le.(NL/2)) then 
c            sw=schr1+(schr2-schr1)*(i-1)/((NL-2)/2.)
c         else
c            sw=schr2+(schr1-schr2)*(i-(NL/2))/((NL-2)/2.) 
c         endif
c       elsec
c         sw=swm 
c       endif
...
c        print*,i,sw(i),tv(i)
      ...
      do i=1,NL,1
        TAB(i-1)=TBEG+(TEND-TBEG)*tv(i)/tv(NL)
      end do   
...
c Variablenuebergabe aus Geometrieberechnung
      ...
      real d_1(npla), d_2(npla+1), beta_1(npla+1), beta_2(npla)
...
c      v_max_leit=Q*(t_grad+1)/(2*PI*r_leit*h_leit)

c      v_betr_leit=v_max_leit*(abs(1-abs(radius/1000-r_nab-r_null)
c     #/(1.01*r_null)))**t_grad
      
c      v_u_leit=v_betr_leit/tan(alpha_0)
c      v_u_lauf=v_u_leit*r_leit/(radius/1000)


c      v_u_lauf_rel=v_u_lauf-u_temp


c
c maximale Geschwindigkeit am Laufrad aus Q und Geometrie
...
c Turbulenzprofil
     ...
     #/(1.02*r_null)))**t_grad
      ...
      beta_1(npla+1)=beta_1(npla)

      k=0
      do i=1,npla,1
        k=k+1
        if (radius.le.d_2(i+1)/2) goto 1

      end do
1     continue


      beta=beta_1(k)+(beta_1(k+1)-beta_1(k))
     #*(radius-d_2(k)/2)/((d_2(npla)-d_2(1))/2/(npla-1))

      
      v_u_lauf_rel=-v_betr/tan(beta)
