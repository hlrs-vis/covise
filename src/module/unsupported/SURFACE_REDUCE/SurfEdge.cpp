...
coDistributedObject **SurfaceEdgeCollapse::createcoDistributedObjects(int red_tri, int red_points, char *Triangle_name, char *Data_name, char *Normals_name)
{
...
{ // if both v1 and v2 lie on the boundary of the surface, we check whether
    ...
    alpha = compute_inner_angle(link[0][0], v1, v2);
    if (fabs(alpha) < 1E-04)
        count++;
    else if (fabs(alpha - M_PI) < 1E-04)
    {
    ...
    alpha = compute_inner_angle(link[num_link - 1][1], v2, v1);
    if (fabs(alpha) < 1E-04)
    {
        count++;
        which_1 = num_link - 1;
...
{ // checks whether the newly computed position of the point the edge collapses
    ...
    for (i = 0; i < num_link; i++)
    {
        ...
        if (!angle[link[i][0]])
            if (n1[0] * n2[0] + n1[1] * n2[1] + n1[2] * n2[2] - 1.0 > 0.1)
                convex = 0;
        n1[0] = n2[0];
        n1[1] = n2[1];
        n1[2] = n2[2];
...
{ // The position of the new point the edge collapses into is computed
    ...
    for (i = 0; i < star.num_tri; i++)
    {
        x_c += coords_x[vertex_list[tri_list[star.tri[i]]]];
        y_c += coords_y[vertex_list[tri_list[star.tri[i]]]];
        z_c += coords_z[vertex_list[tri_list[star.tri[i]]]];
        x_c += coords_x[vertex_list[tri_list[star.tri[i]] + 1]];
        y_c += coords_y[vertex_list[tri_list[star.tri[i]] + 1]];
        z_c += coords_z[vertex_list[tri_list[star.tri[i]] + 1]];
        x_c += coords_x[vertex_list[tri_list[star.tri[i]] + 2]];
        y_c += coords_y[vertex_list[tri_list[star.tri[i]] + 2]];
        z_c += coords_z[vertex_list[tri_list[star.tri[i]] + 2]];
    }

    x_c /= (double)(3 * star.num_tri);
    y_c /= (double)(3 * star.num_tri);
    z_c /= (double)(3 * star.num_tri);
    ...
    {
        k = 3 * i;
        co_x[k] = coords_x[vertex_list[tri_list[star.tri[i]]]] - x_c;
        co_y[k] = coords_y[vertex_list[tri_list[star.tri[i]]]] - y_c;
        co_z[k] = coords_z[vertex_list[tri_list[star.tri[i]]]] - z_c;
        co_x[k + 1] = coords_x[vertex_list[tri_list[star.tri[i]] + 1]] - x_c;
        co_y[k + 1] = coords_y[vertex_list[tri_list[star.tri[i]] + 1]] - y_c;
        co_z[k + 1] = coords_z[vertex_list[tri_list[star.tri[i]] + 1]] - z_c;
        co_x[k + 2] = coords_x[vertex_list[tri_list[star.tri[i]] + 2]] - x_c;
        co_y[k + 2] = coords_y[vertex_list[tri_list[star.tri[i]] + 2]] - y_c;
        co_z[k + 2] = coords_z[vertex_list[tri_list[star.tri[i]] + 2]] - z_c;
    }
            ...
            for (j = 0; j < 3; j++)
            {
                ...
                if ((a == v1) || (b == v1) || (a == v2) || (b == v2))
                {
                    ...
                    { // update edge_array and heap
                        heap->set_item(ept[star.tri[i]][j], e);
                        heap->insert(e);
                    }
                }
            }
...
void SurfaceEdgeCollapse::Reduce(int &red_tri, int &red_pnt)
{
    ...
    while ((heap->getSize() > 20) && (red_tri / (float)num_triangles > cur_percent))
    {
            ...
            if (stars[v1].num_tri + stars[v2].num_tri < MAXTRI)
            {
                ...
                if (check_link(v1, v2, star, link, num_link))
                {
                    ...
                    { // simplify edge
                        ...
                        if (okay && volume_bound >= compute_star_volume(v1, v2, star, link, num_link, x_0, y_0, z_0))
                        {
