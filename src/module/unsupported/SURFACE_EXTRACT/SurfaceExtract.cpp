...
{
    coDistributedObject *tmp_obj_1, *tmp_obj_2, *tmp_obj_3;
    ...
    HandleObjects(tmp_obj_1->createUnknown(), tmp_obj_2->createUnknown(), tmp_obj_3->createUnknown(),
...
{
    coDoSet *grid_set_in, *data_set_in, *idata_set_in;
    coDoSet *T_set, *N_set, *D_set;
    coDistributedObject **grid_objs, **data_objs, **idata_objs;
    ...
    if (grid_object != 0L)
    {
        ...
        else if (strcmp(gtype, "UNIGRD") == 0)
        {
            ...
            ugrid_in->get_min_max(&x_min, &x_max, &y_min, &y_max,
                                  &z_min, &z_max);
    ...
    if (set_num_elem)
    {
            ...
            D_set = new coDoSet(Data_out_name, SET_CREATE);
        {
            int n;
            char **attr, **setting;
            n = grid_set_in->get_all_attributes(&attr, &setting);
            if (n > 0)
                T_set->addAttributes(n, attr, setting);
            if (n > 0)
                N_set->addAttributes(n, attr, setting);
            if (data_object)
            {
                if (n > 0)
                    D_set->addAttributes(n, attr, setting);
            }
        }
        for (i = 0; i < set_num_elem; i++)
        {
            ...
            {
                HandleObjects(grid_objs[i], NULL, idata_objs[i],
                              buf1, buf2, buf3, T_set, N_set, D_set);
            }
        }
    ...
    else
    {
        ...
        if (strcmp(gtype, "UNSGRD") == 0)
        {
            plane = new Plane(numelem, numcoord, DataType);
            plane->createPlane();
        ...
        {
            uplane = new UNI_Plane(numelem, numcoord, DataType);
            uplane->createPlane();
        ...
        {
            splane = new STR_Plane(numelem, numcoord, DataType);
            splane->createPlane();
...
Plane::Plane(int n_elem, int n_nodes, int Type)
{
    NodeInfo *node;
    int i;
    Datatype = Type;
...
UNI_Plane::UNI_Plane(int n_elem, int n_nodes, int Type)
{
    NodeInfo *node;
    int i;
    Datatype = Type;
    ...
    num_elem = n_elem;
    //node_table   = (NodeInfo *)malloc(n_nodes*sizeof(NodeInfo));
    node_table = (NodeInfo *)malloc((n_nodes / 2) * sizeof(NodeInfo));
    node = node_table;

    if (!node)
    {
        cerr << "malloc for " << (n_nodes / 2) * sizeof(NodeInfo) << " bytes failed" << endl;
    }
...
RECT_Plane::RECT_Plane(int n_elem, int n_nodes, int Type)
{
    NodeInfo *node;
    int i;
    Datatype = Type;
...
void Plane::add_vertex(int n1, int n2)
{
    ...
    *vertex++ = *indices;
    ...
    {
        w2 = (double)node_table[n1].dist / (double)(node_table[n1].dist - node_table[n2].dist);
        if (w2 > 1.0)
            w2 = 1.0;
        if (w2 < 0)
            w2 = 0.0;
        ...
        {
            {
                ...
                char **attr, **setting;
                if (grid_in)
                    n = grid_in->get_all_attributes(&attr, &setting);
                else if (sgrid_in)
                    n = sgrid_in->get_all_attributes(&attr, &setting);
                else if (ugrid_in)
                    n = ugrid_in->get_all_attributes(&attr, &setting);
                else if (rgrid_in)
                    n = rgrid_in->get_all_attributes(&attr, &setting);
                if (n > 0)
                    strips_out->addAttributes(n, attr, setting);
            }
        ...
        {
            {
                ...
                if (grid_in)
                    ...
                    n = rgrid_in->get_all_attributes(&attr, &setting);
                if (n > 0)
                    polygons_out->addAttributes(n, attr, setting);
            }
