...
int coNotConnected()
{
   ...
   if (    coSimLibData.soc < 0
        || sendData((void*)&testdata,sizeof(int32),BSWP_MUSTSWAP) != sizeof(int32) )
      return -1;
   else
      return 0;
}
...
static int coSendFTN(int32 type,const char *name,int length)
{
   ...
   /* Send request */
   if (sendData((void*)&type,sizeof(int32),BSWP_MUSTSWAP) != sizeof(int32)) 
      return -1;
   ...
   if (sendData((void*)buffer,64,BSWP_DONTSWAP) != 64) 
      return -1;
   else
      return 0;
}
...
{
   int32 num = numElem;
   if ( sendData((void*)&num,sizeof(int32),BSWP_MUSTSWAP) != sizeof(int32) )
      return -1;
   // rei: error fixed ???? 
   // if ( sendData((void*)data,numElem*sizeof(float),BSWP_MUSTSWAP),numElem*sizeof(float) 
   if ( sendData((void*)data,numElem*sizeof(float),BSWP_MUSTSWAP)
...
int coSend3DataCommon(int numElem, float *data0, float *data1, float *data2)
{
   ...
   if ( sendData((void*)&num,sizeof(int32),BSWP_MUSTSWAP) != sizeof(int32) )
      return -1;
   if ( sendData((void*)data0,numElem*sizeof(float),BSWP_MUSTSWAP) != numElem*sizeof(float) )
      return -1;
   if ( sendData((void*)data1,numElem*sizeof(float),BSWP_MUSTSWAP) != numElem*sizeof(float) )
      return -1;
   if ( sendData((void*)data2,numElem*sizeof(float),BSWP_MUSTSWAP) != numElem*sizeof(float) )
      return -1;
   return 0;
}
#ifdef CO_t3e
...
int coExecModule()
{
   ...
   if (sendData((void*)&execCommand,sizeof(int32),BSWP_MUSTSWAP) != sizeof(int32) )
      return -1;
   else
      return 0;
}                                        
...
{ 
   ...
   if (sendData((void*)&buf,1024,BSWP_DONTSWAP) != 1024 )                          return -1;
...
static int sendMapping(int type, int fortrani, int node, int length, const int *field)
{
   ...
   data.node    = node;
   data.length  = length;
   if (sendData((void*)&data,sizeof(data),BSWP_MUSTSWAP) != sizeof(data) )          return -1;
   
   assert(sizeof(int)==sizeof(int32));
   if (sendData((void*)field,sizeof(int)*length,BSWP_MUSTSWAP) != sizeof(int)*length )
      return -1;
   else
      return 0;
}
 ...
 ***********************************************/
int COSEND(int *data, int *length, int *swaptype)
{
   int size=*length;
   int st  = *swaptype;
   return sendData((const void *)data, size, st);
}
...
{
   int size=*length;
   return recvData((void *)data, size,BSWP_MUSTSWAP);
}
...
static int openClient(unsigned long ip, int port, float timeout)
{
   ...
   do 
   {
      ...
      coSimLibData.soc = socket(AF_INET, SOCK_STREAM, 0);
      if (coSimLibData.verbose>0)
         fprintf(stderr,"openClient(): coSimLibData.soc=%d\n",
                         coSimLibData.soc);
      if (coSimLibData.soc < 0) {
         return -1; 
      } 
      ...
      connectStatus=connect(coSimLibData.soc,(struct sockaddr*)&s_addr_in,
                            sizeof(s_addr_in));
      if (coSimLibData.verbose>0)
         fprintf(stderr,"openClient(): connectStatus=%d\n",
                         connectStatus);
