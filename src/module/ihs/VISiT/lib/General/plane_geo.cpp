...
struct Point *ArcSegmentsXZ(float *m, float *s, float *e, int n)
{
   ...
   mag_a     = float(sqrt(pow(a[0], 2) + pow(a[2], 2)));
   mag_b     = float(sqrt(pow(b[0], 2) + pow(b[2], 2)));
   angle     = float(acos((a[0] * b[0] + a[2] * b[2]) / (mag_a * mag_b)));
   angle_seg = angle / n;
   ...
   {
      angle = (i + 1) * angle_seg;
      p[0]  = float((s[0] - m[0]) * cos(angle) + (s[2] - m[2]) * sin(angle) + m[0]);
      p[1]  = 0.0f;
      p[2]  = float((s[0] - m[0]) * -sin(angle) + (s[2] - m[2]) * cos(angle) + m[2]);
      AddVPoint(p_arc, p);
   }
   AddVPoint(p_arc, e);
