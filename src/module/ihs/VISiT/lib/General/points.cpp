...
struct Point *nCopynPointStruct(struct Point *src, int istart, int srcnum)
{
   ...
   if ((p->z = (float *)calloc(p->maxp,sizeof(float))) == NULL)
      fatal("Space in CopyPointStruct: p->z");
   ssrc = src->x+istart;
   memcpy(p->x,ssrc,srcnum*sizeof(float));
   ssrc = src->y+istart;
   memcpy(p->y,ssrc,srcnum*sizeof(float));
   ssrc = src->z+istart;
   memcpy(p->z,ssrc,srcnum*sizeof(float));
...
void FreePointStruct(struct Point *p)
{
   if (p)
   {
      if (p->nump && p->x) free(p->x);
      if (p->nump && p->y) free(p->y);
      if (p->nump && p->z) free(p->z);
      free(p);
   }
}
