...
int CreateRR_GridRegions(int nob, struct rr_grid *grid)
{
	...
	* (cge->cl->x[0]-cge->cl->x[1]) * grid->phi_scale[0];
    ...
    spara = AllocFlistStruct(cdis2+1);
	...
	reg->para[0] = CalcBladeElementBias(cdis2, 0.0, 1.0, grid->cbias_type, grid->cbias);
	// stretch parameters
	for(j = 0; j < reg->para[0]->num; j++) {
		reg->para[0]->list[j] = pow(reg->para[0]->list[j],0.75);
	}
	if(grid->mesh_ext) {
...
#
	...
	reg = cge->reg[0];
	for(j = 0; j < ii1; j++) {
		u2[0] = cge->reg[1]->arc[2]->list[j];
		u2[1] = cge->reg[1]->line[2]->y[j];
		AddVPoint(reg->line[3],u2);
	}
	u1[0] = cge->reg[3]->arc[1]->list[ii1]-cge->reg[1]->arc[2]->list[ii1];
	u1[1] = cge->reg[3]->line[1]->y[ii1]-cge->reg[1]->line[2]->y[ii1];
	u2[0] = cge->reg[1]->arc[2]->list[ii1];
	u2[1] = cge->reg[1]->line[2]->y[ii1];

	reg->para[3] = CalcBladeElementBias(grid->cledis, 0.0,1.0, grid->clebias_type, grid->clebias);
	CalcLinearCurve(reg->line[3], reg->para[3], u1, u2);

	for(j = ii1-1; j >= 0; j--) {
		u2[0] = cge->reg[3]->arc[1]->list[j];
		u2[1] = cge->reg[3]->line[1]->y[j];
		AddVPoint(reg->line[3],u2);
	}
	...
	reg->line[0] = nCopynPointStruct(cge->reg[0]->line[3],ii1,
					 cge->reg[0]->line[3]->nump-ii1*2);
	reg->arc[0]  = nCopynFlistStruct(cge->reg[0]->arc[3],ii1,
					 cge->reg[0]->arc[3]->num-ii1*2);
	reg->para[0] = CopyFlistStruct(cge->reg[0]->para[3]);
	// ps-envelope (3.3 = 4.2)
	FreePointStruct(reg->line[2]);
	reg->line[2] = nCopynPointStruct(cge->reg[3]->line[1],ii1,
					 cge->reg[3]->line[1]->nump-ii1);
	reg->arc[2]  = nCopynFlistStruct(cge->reg[3]->arc[1],ii1,
					 cge->reg[3]->arc[1]->num-ii1);
	reg->para[2] = nCopynFlistStruct(cge->reg[3]->para[1],ii1,
					 cge->reg[3]->para[1]->num-ii1);
	// ss-upper part, 0..t (3.2 = 2.3)
	FreePointStruct(reg->line[1]);
	reg->para[1] = CopyFlistStruct(reg->para[2]);
	reg->line[1] = nCopynPointStruct(cge->reg[1]->line[2],ii1, reg->para[1]->num);
	reg->arc[1]  = nCopynFlistStruct(cge->reg[1]->arc[2],ii1,reg->para[1]->num);
#ifdef DEBUG_REGIONS
	...
	// outlet line, 6.4
	reg->para[3] = CalcBladeElementBias(grid->cledis, 0.0, 1.0, grid->cxbias_type, grid->cxbias);
	u1[0] = reg->arc[2]->list[reg->arc[2]->num-1] - reg->arc[1]->list[reg->arc[1]->num-1];
	u1[1] = reg->line[2]->y[reg->line[2]->nump-1] - reg->line[1]->y[reg->line[1]->nump-1];
	u1[2] = 0.0;
	u2[0] = reg->arc[1]->list[reg->arc[1]->num-1];
...
#endif
	    // inlet line, 8.1
	    reg->para[0] = CalcBladeElementBias(cdis2, 0.0, 1.0, 0, 1.0);
