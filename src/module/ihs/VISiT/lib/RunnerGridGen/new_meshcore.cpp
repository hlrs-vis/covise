...
int MeshNew_CoreRegion(struct Nodelist *n, struct curve *ml, struct region *reg,
...
float angle14)
{
   ...
   float t[] =                                    // must be identical to spline paras of line 1.4 = 3.1
   {
      0.5f, 0.4f
   };
   ...
   fprintf(fp," MeshNew_CoreRegion %d\n",fcount);
   ...
   reg->nodes[reg->numl] = AllocIlistStruct(reg->line[0]->nump+1);
   for(j = jadd-1; j < reg0->nodes[3]->num-jadd+1; j++)
   {
      Add2Ilist(reg->nodes[0], reg0->nodes[3]->list[j]);
      Add2Ilist(reg->nodes[reg->numl], reg0->nodes[3]->list[j]);
   }
   reg->nodes[1] = nCopyIlistStruct(reg1->nodes[2], reg3->nodes[1]->num);
   reg->nodes[2] = CopyIlistStruct(reg3->nodes[1]);
