...
int MeshNew_GridRegions(struct rr_grid *grid)
{
   ...
   for(i = 0; i < grid->ge_num; i++)
   {
      ...
      fprintf(stderr," MeshNew_XXXRegion %2d",i);fflush(stderr);
#endif
      if(grid->mesh_ext)
         MeshNew_InletRegion(grid->n, grid->ge[i]->ml,
            dummy_inlet,
            grid->psle, grid->ssle,
            grid->cge[i]->reg[0]);
      else
      ...
      MeshNew_SSRegion(grid->n, grid->ge[i]->ml, grid->ssnod,
         grid->ssle, grid->cge[i]->reg[1],
         grid->cge[i]->reg[0], grid->le_dis);
      MeshNew_PSRegion(grid->n, grid->ge[i]->ml, grid->psnod,
         grid->psle, grid->cge[i]->reg[3],
         grid->cge[i]->reg[0], grid->le_dis);
      MeshNew_CoreRegion(grid->n, grid->ge[i]->ml,
         grid->cge[i]->reg[2], grid->cge[i]->reg[0],
         grid->cge[i]->reg[1],
         grid->cge[i]->reg[3], grid->v14_angle[0]);
      MeshNew_SSTERegion(grid->n, grid->ge[i]->ml,
         grid->cge[i]->reg[4], grid->cge[i]->reg[1],
         grid->ssnod, grid->sste,
         grid->outlet);
      MeshNew_OutletRegion(grid->n, grid->ge[i]->ml,
         grid->cge[i]->reg[5],
         grid->cge[i]->reg[1],
         grid->cge[i]->reg[4],
         grid->cge[i]->reg[2], grid->outlet);
      MeshNew_PSTERegion(grid->n, grid->ge[i]->ml,
      ...
      {
         if(grid->mesh_ext)
            MeshNew_ExtRegion(grid->n, grid->ge[i]->ml,
               grid->inlet,
               grid->psle, grid->ssle,
               grid->cge[i]->reg[7],
               grid->cge[i]->reg[0]);
      ...
      fprintf(fp,"i = %3d, equiv = %3d\n",i,
         EquivCheck(&grid->n->n[i*grid->n->offset],
         grid->n->offset));
      fclose(fp);
   ...
   fprintf(stdout,"Creating nodes in %d tip clearance regions ... ",
      grid->gp_num);fflush(stdout);
   for(i = itip; i <= ishroud; i++)
   {
      nnum_prev = grid->n->num;
      MeshNew_SSGapRegion(grid->n, grid->ge[i]->ml,
         grid->sste, grid->ssnod,
         grid->cge[i]->reg[grid->reg_num],
         grid->cge[i]->reg[1], grid->cge[i]->reg[4],
         i-itip);
      MeshNew_PSGapRegion(grid->n, grid->ge[i]->ml,
         grid->pste, grid->psnod,
         grid->cge[i]->reg[grid->reg_num+1],
         grid->cge[i]->reg[3], grid->cge[i]->reg[6],
         i-itip);
   ...
   ReadjustPeriodic(grid->n, grid->psle, grid->ssle, grid->pste, grid->sste, grid->ge_num, grid->rot_clock);
   fprintf(stdout," done!\n");
#endif                                         // GAP
   ...
   // boundary node flags
#ifdef DEBUG_NODES
   fprintf(stderr,"\n ... MeshNew_GridRegions(): creating bc flags ...");fflush(stderr);
#endif
   ...
   for(i = 1; i < grid->ge_num; i++)
   {
      CreateRR_Elements(grid->cge[(i-1)]->reg, grid->e, psblade,
         ssblade, psleperiodic,
         ssleperiodic, psteperiodic, ssteperiodic,
         inlet, outlet,
   ...
   WritePATRAN_SESfile(grid->n->num, grid->e->nume, grid->ge_num, 0, 0,
      "MERIDIANelems", "MERIDIANnodes", "meridian", "meridian_node");
#endif
   ...
   for(i = itip; i < ishroud; i++)
   {
      CreateRR_GapElements(grid->cge[i]->reg, grid->cge[i+1]->reg, grid->e,
         psteperiodic, ssteperiodic, grid->psblade,
         grid->ssblade, grid->psteperiodic, grid->ssteperiodic,
         grid->wall, grid->shroud, grid->reg_num,
         grid->gpreg_num, gpoffset, i-itip, i-ishroud+1
   #ifdef DEBUG_GAPELEM
         , grid->n->n
   #endif
         );
...
#ifdef PATRAN_SES_OUT
   WritePATRAN_SESfile(grid->n->num-nnum_regular, grid->e->nume-elnum_regular, grid->gp_num,
      nnum_regular, elnum_regular, "MERIDIANTIPelems",
      "MERIDIANTIPnodes", "meridiantip", "meridiantip_node");
#endif
#endif                                         // GAP
...
#ifdef DEBUG_NODES
   fprintf(stderr,"MeshNew_GridRegions: DumpNodes ... ");fflush(stderr);
   ...
   fprintf(stderr,"MeshNew_GridRegions: DumpElements ... ");fflush(stderr);
