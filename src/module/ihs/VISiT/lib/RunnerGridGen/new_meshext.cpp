...
int MeshNew_ExtRegion(struct Nodelist *n, struct curve *ml, struct Ilist *inlet,
struct Ilist *psle, struct Ilist *ssle, struct region *reg, struct region *reg0)
{
   ...
   snprintf(fn, sizeof(fn),"rr_debugext_%02d.txt", count++);
   if( (fp = fopen(fn,"w+")) == NULL)
   {
      fprintf(stderr,"Shit happened opening file '%s'!\n",fn);
      exit(-1);
   }
   fprintf(fp," MeshNew_ExtRegion %d\n",count);
#endif
