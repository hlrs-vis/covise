...
int CreateRR_GridRegions(int nob, struct rr_grid *grid)
{
	...
	spara = AllocFlistStruct(grid->cledis+1);
	sline = AllocPointStruct();

	grid->cledis = MIN(grid->cledis, grid->cdis);
	jadd = MIN((grid->cdis-grid->cledis)/2, (grid->psedis));
	if( (grid->cledis = grid->cdis-2*jadd) < 2) {
		grid->cledis = 2;
		jadd = (grid->cdis-grid->cledis) / 2;
	}
	grid->jadd   = jadd;
	...
	for(i = 0; i < grid->ge_num; i++) {
		...
		if(grid->skew_runin) {
			cge = grid->cge[ref];
			phi[0] = cge->cl->x[0] + ( 
				(l0 - cge->cl->y[0])/(cge->cl->y[0]-cge->cl->y[1]) )
...
#
...
#endif
		reg->para[2] = CalcBladeElementBias(grid->le_dis,0.0f, 0.06f, 1, -2.0f);
		reg->para[2] = Add2Bias(reg->para[2], grid->psdis-grid->le_dis+1, 0.06f,t,
								grid->psbias_type, grid->psbias,1);
		reg->para[2] = Add2Bias(reg->para[2], (grid->ssdis - grid->psdis + 1), t, 1.0f,
								grid->ssbias_type, grid->ssbias, 1);
#ifdef DEBUG_REGIONS
		...
		reg->para[3] = CalcBladeElementBias(grid->cledis, 0.0,1.0, grid->clebias_type, grid->clebias);
		t = 0.5f; t2 = 0.4f;							// partition for spline polygon
		// left side, suction side, point, p1 and vector v1
		p1[0] = cge->reg[1]->arc[2]->list[jadd];
		p1[1] = cge->reg[1]->line[2]->y[jadd];
		p1[2] = 0.0f;
		u1[0] = reg->arc[1]->list[0] - p1[0];
		u1[1] = reg->line[1]->y[0] - p1[1];
		u1[2] = 0.0f;
		V_Norm(u1);
		u2[0] = cge->reg[1]->arc[2]->list[jadd+1] -
			cge->reg[1]->arc[2]->list[jadd];
		u2[1] = cge->reg[1]->line[2]->y[jadd+1] - 
			cge->reg[1]->line[2]->y[jadd];
		u2[2] = 0.0;
		V_Norm(u2);
		V_Add(u1, u2, v1);
		// right side (ps), point p3 and vector v3
		p3[0] = cge->reg[3]->arc[1]->list[jadd];
		p3[1] = cge->reg[3]->line[1]->y[jadd];
		p3[2] = 0.0;
		u1[0] = reg->arc[2]->list[0] - p3[0];
		u1[1] = reg->line[2]->y[0] - p3[1];
		u1[2] = 0.0;
		V_Norm(u1);
...
#endif
		node = BSplineKnot(poly, BSPLN_DEGREE);
		// create spline, add points on envelope curve (2.3 & 4.2)
		slen = 0.0;
		// spline
		...
		// add points to line[3] and correct spline points
		// points on 2.3
		for(j = 0; j < jadd; j++) {
			p[0] = cge->reg[1]->arc[2]->list[j];
			p[1] = cge->reg[1]->line[2]->y[j];
			AddVPoint(reg->line[3],p);
		}
		// correction of points due to ill parameterisation
		ix = 0;
		for(j = 0; j < grid->cledis; j++) {
			ix = GetPointIndex(grid->cledis, spara->list, reg->para[3]->list[j], ix);
		...
		// points on 4.2
		for(j = jadd-1; j >= 0; j--) {
			p[0] = cge->reg[3]->arc[1]->list[j];
			p[1] = cge->reg[3]->line[1]->y[j];
			AddVPoint(reg->line[3],p);
		}
		reg->para[3]->num = 0; // reset parametres
		slen = 0.0;
		...
		reg->line[0] = nCopynPointStruct(cge->reg[0]->line[3],jadd,grid->cledis);
		reg->arc[0]	 = nCopynFlistStruct(cge->reg[0]->arc[3],jadd,grid->cledis);
		reg->para[0] = CalcBladeElementBias(grid->cledis, 0.0,1.0, 
											grid->clebias_type, grid->clebias);
		// ps-envelope (3.3 = 4.2)
		FreePointStruct(reg->line[2]);
		reg->line[2] = nCopynPointStruct(cge->reg[3]->line[1],jadd,
										 cge->reg[3]->line[1]->nump-jadd);
		reg->arc[2]	 = nCopynFlistStruct(cge->reg[3]->arc[1],jadd,
										 cge->reg[3]->line[1]->nump-jadd);
		reg->para[2] = nCopynFlistStruct(cge->reg[3]->para[1],jadd,
										 cge->reg[3]->line[1]->nump-jadd);
		// ss-upper part, 0..t (3.2 = 2.3)
		FreePointStruct(reg->line[1]);
		reg->para[1] = CopyFlistStruct(reg->para[2]);
		reg->line[1] = nCopynPointStruct(cge->reg[1]->line[2],jadd,
										 reg->para[1]->num);
		reg->arc[1]	 = nCopynFlistStruct(cge->reg[1]->arc[2],jadd,
										 reg->para[1]->num);
#ifdef DEBUG_REGIONS
		...
		reg->para[3] = CalcBladeElementBias(grid->cledis, 0.0, 1.0, grid->midbias_type, grid->midbias);
		...
		if( (i != 0) && (grid->ssxdis != reg->line[0]->nump) )
			return TEDISCRETE_ERROR;
		grid->ssxdis = reg->line[0]->nump;
		// **************************************************
#ifdef DEBUG_REGIONS
		...
		// outlet line, 7.4
		reg->para[3] = CalcBladeElementBias(reg->para[0]->num, 0.0f, 1.0f,
		...
		if( (i != 0) && (grid->psxdis != reg->line[0]->nump) )
			return TEDISCRETE_ERROR;
		grid->psxdis = reg->line[0]->nump;
		...
		// outlet line, 6.4
		reg->para[3] = CalcBladeElementBias(grid->cledis, 0.0f, 1.0f, grid->cxbias_type, grid->cxbias);
		u1[0] = reg->arc[2]->list[reg->arc[2]->num-1] - reg->arc[1]->list[reg->arc[1]->num-1];
		u1[1] = reg->line[2]->y[reg->line[2]->nump-1] - reg->line[1]->y[reg->line[1]->nump-1];
		u1[2] = 0.0;
		u2[0] = reg->arc[1]->list[reg->arc[1]->num-1];
		...
		if(grid->iinlet != 0 && grid->mesh_ext) {
			...
			fprintf(stderr," 8.2\n");
#endif
			reg->para[1] = CalcBladeElementBias(grid->extdis, 0.0f, 1.0f,
...
#endif
			// inlet line, 8.1
			reg->para[0] = CalcBladeElementBias(cge->reg[0]->para[0]->num, 
												0.0, 1.0, 0, 1.0);
			...
			// readjust the inlet line 1.1=8.4, might be skewed
			// readjust_rrin set above
			//readjust_rrin=0;
			if(readjust_rrin) {
				spara->num  = 0;
				sline->nump = 0;
				...
				slen = 1.0f/slen;
				for(j = 0; j < spara->num; j++) {
					spara->list[j] *= slen;
				}
				ix = reg->line[1]->nump-1;
				for(j = 0; j < reg->line[0]->nump; j++) {
					u1[0] = cge->reg[0]->line[3]->x[j]-reg->line[0]->x[j];
					u1[1] = cge->reg[0]->line[3]->y[j]-reg->line[0]->y[j];
					reg->line[3]->x[j] = cge->reg[0]->line[0]->x[j] =
						u1[0] * spara->list[ix] + reg->line[0]->x[j];
					reg->line[3]->y[j] = cge->reg[0]->line[0]->y[j] =
						u1[1] * spara->list[ix] + reg->line[0]->y[j];
				}
				GetPointRadius(reg->line[3], grid->ge[i]->ml);
				GetPointRadius(cge->reg[0]->line[0], grid->ge[i]->ml);
				FreeFlistStruct(reg->arc[3]);
				FreeFlistStruct(cge->reg[0]->arc[0]);
				reg->arc[3] = GetCircleArclen(reg->line[3]);
				cge->reg[0]->arc[0] = GetCircleArclen(cge->reg[0]->line[0]);
			}                                       // readjust_rrin
		}											// iinlet != 0
