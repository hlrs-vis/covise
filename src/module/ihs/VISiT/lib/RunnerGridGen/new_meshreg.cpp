...
int CreateNew_GridRegions(int nob, struct rr_grid *grid)
{
	...
	v1[0] = grid->phi_skewout[0];
	v1[1] = grid->phi_scale[1];
	v1[2] = grid->phi_skewout[1];
	...
	if((jadd  = grid->psedis/2) < 2) return PSEDIS_ERROR;
	...
	for(i = 0; i < grid->ge_num; i++) {
		...
		if(grid->skew_runin) {
			...
			cge = grid->cge[i];
			phi[1] = cge->cl->x[cge->cl->nump-1] - 
				dphi* EvaluateParameter(grid->ge[i]->para,b);
		}
		arc[0] = phi[0]*r0;
		...
		reg->para[0] = CalcBladeElementBias(grid->cdis+2*jadd-2, 0.0, 1.0, grid->cbias_type, grid->cbias);
		if(grid->mesh_ext) {
			...
			for(j = 0; j < grid->cdis+2*jadd-2; j++) {
...
#
		...
		u1[0] = reg->arc[2]->list[grid->psdis-1] - reg->line[1]->x[grid->psdis-1];
		u1[1] = reg->line[2]->y[grid->psdis-1] - reg->line[1]->y[grid->psdis-1];
		u1[2] = 0.0;
		u2[0] = reg->line[1]->x[grid->psdis-1];
		...
		t  = (1.0f - grid->ge[i]->para) * grid->ss_part[0] +
			grid->ge[i]->para * grid->ss_part[1];
		t2 = grid->ssle_part[0] * (1.0f - grid->ge[i]->para) +
			grid->ssle_part[1] * grid->ge[i]->para;
		reg->para[1] = CalcBladeElementBias(grid->le_dis,0.0,
											t2, 1, -2.0);
		reg->para[1] = Add2Bias(reg->para[1],
								grid->psdis-grid->le_dis+1, t2,t,
								grid->psbias_type, grid->psbias,1);
		reg->para[1] = Add2Bias(reg->para[1],
								(grid->ssdis - grid->psdis + 1),
								t, 1.0, grid->ssbias_type,
								grid->ssbias, 1);
...
#endif
		reg->para[2] = CalcBladeElementBias(grid->le_dis,
											0.0f, 0.06f, 1, -2.0f);
		reg->para[2] = Add2Bias(reg->para[2],
								grid->psdis-grid->le_dis+1, 0.06f,t,
								grid->psbias_type, grid->psbias,1);
		reg->para[2] = Add2Bias(reg->para[2],
								(grid->ssdis - grid->psdis + 1),
								t, 1.0f, grid->ssbias_type,
								grid->ssbias, 1);
#ifdef DEBUG_REGIONS
		...
		// trailing edge (2.4)
		reg->para[3] = CopyFlistStruct(reg->para[0]);
		u1[0] = reg->arc[1]->list[grid->ssdis-1] - reg->line[2]->x[grid->ssdis-1];
		u1[1] = reg->line[1]->y[grid->ssdis-1] - reg->line[2]->y[grid->ssdis-1];
		u1[2] = 0.0;
		u2[0] = reg->line[2]->x[grid->ssdis-1];
		...
		reg->para[3] = CalcBladeElementBias(jadd, 0.0,
											grid->bl_v14_part[0],
											0,1.0);
		reg->para[3] = Add2Bias(reg->para[3], grid->cdis,
								grid->bl_v14_part[0],
								grid->bl_v14_part[1],
								grid->clebias_type, grid->clebias,1);
		reg->para[3] = Add2Bias(reg->para[3],jadd,grid->bl_v14_part[1],
								1.0, 0,1.0,1);
		t = 0.5f; t2 = 0.4f;							// partition for spline polygon
		...
		u1[2] = 0.0f;
		V_Norm(u1);
		u2[0] = cge->reg[1]->line[2]->x[1] - cge->reg[1]->line[2]->x[0];
		...
		u1[2] = 0.0;
		V_Norm(u1);
		u2[0] = cge->reg[3]->line[1]->x[1] - cge->reg[3]->line[1]->x[0];
	...
	ix = 0;
	for(j = 0; j < reg->para[3]->num; j++) {
		ix = GetPointIndex(reg->para[3]->num, spara->list, reg->para[3]->list[j], ix);
	...
	// **************************************************
#ifdef DEBUG_REGIONS
	fprintf(stderr,"modify boundary layer envelope curves\n");
	fprintf(fpdebug,"\nmodify boundary layer envelope curves\n");
	fprintf(fpdebug,"jadd = %d, cge->reg[0]->line[3]->nump = %d\n",jadd, cge->reg[0]->line[3]->nump);
	fprintf(fpdebug,"2.3\n");
#endif
	reg = cge->reg[1];
	// modify 2.3
	// straight line
	p1[0] = cge->reg[0]->arc[3]->list[jadd-1];
	p1[1] = cge->reg[0]->line[3]->y[jadd-1];
	u1[0] = reg->line[2]->x[grid->le_dis-1]-p1[0];
	u1[1] = reg->line[2]->y[grid->le_dis-1]-p1[1];
	for(j = 0; j < grid->le_dis-1; j++) {
		p[0] = p1[0] + u1[0] * reg->para[2]->list[j]/reg->para[2]->list[grid->le_dis-1];
		p[1] = p1[1] + u1[1] * reg->para[2]->list[j]/reg->para[2]->list[grid->le_dis-1];
		reg->line[2]->x[j] = p[0];
		reg->line[2]->y[j] = p[1];
	}
	CalcPointCoords(reg->line[2], &reg->arc[2], grid->ge[i]->ml);
...
#endif
	reg->line[0]->nump = 0;
	FreeFlistStruct(reg->arc[0]);
	for(j = jadd-1; j > 0; j--) {
		AddPoint(reg->line[0], cge->reg[0]->line[3]->x[j],
				 cge->reg[0]->line[3]->y[j],0.0);
	}

	bias = CalcBladeElementBias(grid->psedis-jadd+1, 0.0, 1.0,
								grid->psebias_type, grid->psebias);
	u1[0] = cge->cl->x[0] - cge->reg[0]->line[1]->x[grid->ssmdis-1];
	u1[1] = cge->cl->y[0] - cge->reg[0]->line[1]->y[grid->ssmdis-1];
	u1[2] = 0.0;
	u2[0] = cge->reg[0]->line[1]->x[grid->ssmdis-1];
	u2[1] = cge->reg[0]->line[1]->y[grid->ssmdis-1];
	u2[2] = 0.0;
	CalcLinearCurve(reg->line[0], bias, u1, u2);
	GetPointRadius(reg->line[0], grid->ge[i]->ml);
	reg->arc[0] = GetCircleArclen(reg->line[0]);
	bias->num = 0;
	slen = 0.0;
	// update parameters
	Add2Flist(bias,slen);
	for(j = 1; j < reg->para[0]->num; j++) {
		slen += float(sqrt( pow(reg->line[0]->y[j] - reg->line[0]->y[j-1],2) +
					  pow(reg->arc[0]->list[j] - reg->arc[0]->list[j-1],2)));
		Add2Flist(bias,slen);
	}
	for(j = 0; j < reg->para[0]->num; j++)
		reg->para[0]->list[j] = bias->list[j] / slen;
	FreeFlistStruct(bias);
#ifdef DEBUG_REGIONS
	fprintf(fpdebug,"2.1, para[0]\n");
...
#endif
	// straight line
	p1[0] = cge->reg[0]->arc[3]->list[cge->reg[0]->line[3]->nump-jadd];
	p1[1] = cge->reg[0]->line[3]->y[cge->reg[0]->line[3]->nump-jadd];
	u1[0] = reg->line[1]->x[grid->le_dis-1]-p1[0];
	u1[1] = reg->line[1]->y[grid->le_dis-1]-p1[1];
	for(j = 0; j < grid->le_dis-1; j++) {
		p[0] = p1[0] + u1[0] * reg->para[1]->list[j]/reg->para[1]->list[grid->le_dis-1];
		p[1] = p1[1] + u1[1] * reg->para[1]->list[j]/reg->para[1]->list[grid->le_dis-1];
		reg->line[1]->x[j] = p[0];
		reg->line[1]->y[j] = p[1];
	}
	CalcPointCoords(reg->line[1], &reg->arc[1], grid->ge[i]->ml);
	...
	reg->line[0]->nump = 0;
	FreeFlistStruct(reg->arc[0]);
	ix = cge->reg[0]->line[3]->nump-1;
	for(j = ix-jadd+1; j < ix; j++) {
		AddPoint(reg->line[0], cge->reg[0]->line[3]->x[j],
				 cge->reg[0]->line[3]->y[j],0.0);
	}
	...
	reg = cge->reg[2];
	reg->arc[0]	 = AllocFlistStruct(grid->cdis+1);
	reg->para[0] = AllocFlistStruct(grid->cdis+1);
	// leading edge line (3.1 = 1.4)
	slen = cge->reg[0]->para[3]->list[cge->reg[0]->line[3]->nump-jadd]
		- cge->reg[0]->para[3]->list[jadd-1];
	for(j = jadd-1; j < cge->reg[0]->line[3]->nump-jadd+1; j++) {
		AddPoint(reg->line[0], cge->reg[0]->line[3]->x[j],
				 cge->reg[0]->line[3]->y[j],
				 cge->reg[0]->line[3]->z[j]);
		Add2Flist(reg->arc[0], cge->reg[0]->arc[3]->list[j]);
		Add2Flist(reg->para[0], (cge->reg[0]->para[3]->list[j]-
								 cge->reg[0]->para[3]->list[jadd-1])/slen);
	}
#ifdef DEBUG_REGIONS
	fprintf(fpdebug,"3.1, para[0]\n");
	DumpFlist2File(reg->para[0], fpdebug);
#endif
	// ps-envelope (3.3 = 4.2)
...
#ifdef DEBUG_REGIONS
		fprintf(fpdebug, "te_thickness: t = %f\n",t);
		fprintf(stderr, "0...t\n");
		fprintf(fpdebug, "0...t\n");
		DumpFlist2File(reg->para[0], fpdebug);
#endif
	...
	if(p2[1] > grid->ge[i]->ml->len[grid->ge[i]->ml->p->nump-1]) {
		fprintf(stderr," 5.2: CreateNew_GridRegions: %d: ill intersection point, point shifted!\n", i+1);
	...
	LineIntersect(p3,v3, p1,v1, p2);
	if(p2[1] > grid->ge[i]->ml->len[grid->ge[i]->ml->p->nump-1]) {
		fprintf(stderr," 5.3: CreateNew_GridRegions: %d: ill intersection point, point shifted!\n", i+1);
		p2[1] = grid->ge[i]->ml->len[grid->ge[i]->ml->p->nump-1] * 0.95f;
	}
	t = 0.6f; t2 = 0.6f;
	poly = CurvePolygon(p1,p2,p3, t, t2);
	...
	if(grid->iinlet != 0 && grid->mesh_ext) {
		reg = cge->reg[7];
...
#endif
		// inlet line, 8.1
		reg->para[0] = CalcBladeElementBias(grid->cdis+2*jadd-2, 0.0, 1.0, 0, 1.0);
