...
int MeshRR_PSRegion(struct Nodelist *n, struct curve *ml, struct Ilist *psnod,
					struct Ilist *psle, struct region *reg,struct region *reg0,
					int jadd)
{
	int i, j;
	...
	for(i = 0; i <= jadd; i++) {
		Add2Ilist(reg->nodes[reg->numl],
				  reg0->nodes[3]->list[reg0->nodes[3]->num-1-i]);	 
	...
	for(i = 0; i <= jadd; i++)
		Add2Ilist(reg->nodes[1],
				  reg0->nodes[3]->list[reg0->nodes[1]->num-1-i]);
