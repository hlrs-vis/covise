...
int MeshRR_GridRegions(struct rr_grid *grid)
{
	...
	grid->frictless	   = AllocElementStruct();
        grid->hubAll	   = AllocElementStruct();
        grid->shroudAll	   = AllocElementStruct();
#ifdef RR_IONODES
	...
	for(i = 0; i < grid->ge_num; i++) {
		...
		MeshRR_CoreRegion(grid->n, grid->ge[i]->ml,
						  ...
						  grid->cge[i]->reg[1],
						  grid->cge[i]->reg[3], grid->v14_angle[0], grid->jadd);
	...
	GetAllHubElements(grid->e, grid->hubAll, grid->n, grid->e->nume/(grid->ge_num-1) );        
	GetAllShroudElements(grid->e, grid->shroudAll, grid->n, grid->e->nume/(grid->ge_num-1), grid->ge_num);
#ifdef RR_IONODES
