...
C **************************************************************
C **************************************************************
      SUBROUTINE GEO_LES_COV(coord,coord_num,coord_mod,
...
C           COVISE ...
      integer   ellist, colnode, nodeinfo, colelem, eleminfo
      real      px, py, pz
      dimension px(npoin), py(npoin), pz(npoin)
      dimension ellist(nkd,nelem), nodeinfo(npoin*colnode)
      dimension eleminfo(nelem*colelem)
...
C          REI: coord(i,1)=px(i-1) ?????? --> TESTEN !!!!
        vek(1)=px(i)
        vek(2)=py(i)
        vek(3)=pz(i)
        cind    = colnode*i
        kn_num  = nodeinfo(cind-1)
        mod_num = nodeinfo(cind)
...
c     Maximale Knotennummer:
     ...
     *                     MPI_MAX,MPI_COMM_WORLD,ierr)  
      ...
      if (num_max.ne.knmax_num) then
        ...
        write(luerr,*)'Fehler in Routine GEO_LES_COV'
...
c     EINLESEN DER ELEMENTE:
 ...
 28       continue
          cind    = colelem*i
          el_num  = eleminfo(cind-1)
          mod_num = eleminfo(cind)
