...
C **************************************************************
C **************************************************************
      SUBROUTINE PRINT_GEO_COV(lnods,lnods_num,lnods_mod,
     ...
     *                     cov_coord, cov_lnods,
     *                     cov_lnods_num,
     *                     cov_lnods_proz, cov_coord_num,
     *                     cov_coord_joi, cov_lnods_joi,
     *                     cov_coord_mod, cov_lnods_mod,
     *                     cov_coord_proz)
...
C ---- Parameter fuer COVISE
      integer   covise_run,write_files
      real      cov_coord
      integer   cov_lnods, cov_lnods_num, cov_lnods_proz,
     *          cov_coord_num, cov_coord_joi, cov_lnods_joi,
     *          cov_coord_mod, cov_lnods_mod,cov_coord_proz
      dimension cov_coord(npoin_max*ncd),
     *          cov_lnods(nelem_max*nkd),
     *          cov_lnods_num(nelem_max),
     *          cov_lnods_proz(nelem_max),
     *          cov_coord_num(npoin_max),
     *          cov_coord_joi(npoin_max),
     *          cov_lnods_joi(nelem_max),
     *          cov_coord_mod(npoin_max),
     *          cov_lnods_mod(nelem_max),
     *          cov_coord_proz(npoin_max)
      integer   nkern,nlapp
...
C ---- COVISE
               if (covise_run.eq.1) then
                   cov_coord_num(nnn)=kn_num
                   cov_coord_joi(nnn)=kn_joi
                   cov_coord_mod(nnn)=mod_num
                   cov_coord_proz(nnn)=proz_num
                   do 158 k=1,ncd
                       cov_coord(((k-1)*nkn)+nnn)=co(k)
 158               continue 
               endif
            else
...
c        AUSDRUCK DER KERN-ELEMENTE:
 ...
 131        continue
     ...
     *                                                  proz_num,el_joi
                  ...
                  if (covise_run.eq.1) then
C                   ARGH!!! There runs something totally wrong :-(
                      call erro_init(myid,parallel,luerr)
                      write(luerr,*)'Fehler in print_geo.f:'
                      write(luerr,*)' Als COVISE-Modul muss ncd'
                      write(luerr,*)' immer den Wert 3 haben !!'
                      call erro_ende(myid,parallel,luerr)
                  endif
               else if (ncd.eq.3) then
			      if (write_files.eq.1) then
     ...
     *                                                  proz_num,el_joi
                  ...
                  if (covise_run.eq.1) then
                      cov_lnods_num(nnn)=el_num
                      cov_lnods_joi(nnn)=el_joi
                      cov_lnods_proz(nnn)=proz_num
                      cov_lnods_mod(nnn)=mod_num
                      do 135 k=1,nkd
                         cov_lnods(((k-1)*nel)+nnn)=help(k)
 135                  continue 
                  endif
               endif 
            else
...
c        Initialisierung:
 ...
 161     continue
         ...
         if (covise_run.eq.1) then
             ...
             call sendgeodata(igeb, npoin_ges, nelem_ges, knmax_num,
     *           elmax_num, nkn, nel, ncd, nkd,
     *           cov_coord, cov_lnods, cov_lnods_num,
     *           cov_lnods_proz, cov_coord_num,
     *           cov_coord_joi, cov_lnods_joi,
     *           cov_coord_mod, cov_lnods_mod,
     *           cov_coord_proz)
