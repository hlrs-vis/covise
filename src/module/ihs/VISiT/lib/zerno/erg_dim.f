...
C **************************************************************
C **************************************************************
      SUBROUTINE ERG_DIM(erg_name,nsp_kn_erg,nsp_el_erg,nsp_za_erg)
     ...
     *           warn_print,erro_print,format_read
      ...
      character*80 file_name,erg_name,comment,reihe
...
c     FEHLERMELDUNGEN:
      ...
      if (erro_print) then
         ...
         write(luerr,*)'Fehler in Routine ERG_DIM'
...
c     FEHLERMELDUNG FUER FALSCH GEOEFFNETE FILES:
      ...
      goto 888
 777  continue      
      comment='Fehler beim Oeffnen von File (erg_dim):'
      call erro_init(myid,parallel,luerr)
