...
static float getDL(float dx, float beta);
...
// here we use the REAL blade length given by a solidity parametre
int LSurfRR_BladeElement(struct be *be, float lepar, float rle, float hle,
						 struct edge *te, int nob, float rref, int clock)
{
	int i, j, jnext, err;
	int ile;

	float dbeta, beta[2], l[2], dl, ll, ll2;
	float bb, s, ds, rr, r[2], x, a, xi;
	float p[3], p2[3], z, dphi, par, scale;
	float bllen, deltax, dx;
	...
	for(i = 1; i < NUMP; i++) {
		...
		for(j = jnext; j < be->ml->p->nump-1; j++) {
			if( (be->ml->len[j] <= ll) &&
			    (be->ml->len[j+1]) >= ll) {
				jnext = MAX(0,j-1);
				par = (ll - be->ml->len[j]) / 
