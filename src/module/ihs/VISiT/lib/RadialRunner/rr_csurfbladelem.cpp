...
int CSurfacesRR_BladeElement(struct be *be, float lepar, float tepar,
			     ...
			     int clock, float camb_flag)
{
	...
	// position of te on circumference
	s = be->te_wrap * rte;
	...
	AddPoint(cl, rte, be->te_wrap, hte);
	AddPoint(clmerid, s, l[1], 0.0);
	ll   = ll2 = l[1];
	r[0] = rte;
	x    = 0.0;
