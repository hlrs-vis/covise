...
void AxialRunner::CreateGrid(void)
{
   const char *err_msg;
   char buf[500];
   ...
   if (p_makeGrid->getValue()) {
      ...
      rrg = CreateAR_Mesh(geo->ar);
      if(!gridInitialized) {
         gridInitialized = 1;
         AxialRunner::Grid2CtrlPanel();
      }
	  // see RadialRunner
...
// **************************************************
		...
		// **************************************************
         if(p_ShowComplete->getValue()) {
            ...
            dprintf(4,"nume = %d, numn = %d, noffset = %d, eoffset = %d\n",
...
// **************************************************
		...
		SetElement(&data, rrg->shroud, FIXLABEL);
      ...
      //  7. pressure bc: outlet elements
		...
		coDoSet *bcset = new coDoSet((char*)basename,
...
//  psteperiodicElementFaces->setCurrentObject(psteperiodic_element_faces);
  ...
  *(data+counter) = nNodesFace;
  ...
  //destroy pointers
//  for (ii = 0; ii < 12; i++) {
//    delete boundaryElementFacesParts[ii];
//  }
//  delete [] boundaryElementFacesParts;
  
  //cout << "counter=" << counter << endl;
  //boundaryElementFaces->setCurrentObject(boundary_element_faces);
