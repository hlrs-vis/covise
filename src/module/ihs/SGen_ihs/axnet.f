...
c INTEGER VARIABLEN DEKLARIEREN
c ----------------------------------------------------------------------
      integer ase(a_gebi,2),e
c ----------------------------------------------------------------------
c REAL VARIABLEN DEKLARIEREN
c ----------------------------------------------------------------------
      real f,PI,radius,xu,yu,xo,yo,delta,bgs,alpha,beta,val,valN,
     ...
     #     xl26(50),yl26(50),xl63(50),yl63(50),xperhilf1,yperhilf1,
     #     xperhilf2,yperhilf2
...
c       SPL12=SPL12N+(SPL12K-SPL12N)/(bi-1)*(schnitt-1)
c Generierung der Stuetzpunkte des NACA-Profils 
       call CRE_NACA(k,bi,xi,yi,zi,schnitt,m,SPL12N,SPL12K,b_end,
     # radius,f(ixp),f(iyp),norm_fr,norm_fp,norm_fz)
...
c Gebiet 1
     ...
     #  0.,1,1./lvs1,m-1,ase(1,1),f(ixb1+bord),f(iyb1+bord))

c Gebiet 2
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(2),PG(3),delta,0.,
     #  0.,1,1./lvs2,m-1,ase(2,1),
     #  f(ixb1+bord+ase(1,1)),f(iyb1+bord+ase(1,1)))

c Gebiet 3
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(3),PG(4),0.,0.,0.,
     #  1,lvs3,m-1,ase(3,1),
     #  f(ixb1+bord+ase(1,1)+ase(2,1)),f(iyb1+bord+ase(1,1)+ase(2,1)))

c Gebiet 4
       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(4),PG(5),0.,0.,0.,
     #  1,lvs4,m-1,ase(4,1),
     #  f(ixb1+bord+ase(1,1)+ase(2,1)+ase(3,1)),
     #  f(iyb1+bord+ase(1,1)+ase(2,1)+ase(3,1)))
       
       asespl=ase(1,1)+ase(2,1)+ase(3,1)+ase(4,1)-1

      hilf1=bord+ase(1,1)+ase(2,1)-2
      hilf2=bord+ase(1,1)+ase(2,1)+1
      alpha=-atan2(f(iyb1+hilf1)-f(iyb1+hilf2),
     #           (f(ixb1+hilf2)+delta)-f(ixb1+hilf1))+PI/2.
      print*,alpha

c ----------------------------------------------------------------------
c Berechne den Mittelpunkt und Winkel der oberen und unteren Grenze
c ----------------------------------------------------------------------
       call RAND_SPOTS(f(ixn),f(iyn),int((m+1)/2),yu,yo,alpha,valN,valK,
     #  vbe,alp_hi,schnitt,bi,xu,xo,alpha,val,beta,radius,r_nab,r_kra)
...
c       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(1),PG(2),delta,0.,
c     #  aint(bgs),1,1./lvs1,m-1,ase1,f(ixb2+bord),f(iyb2+bord))

        call SPLINEVERSCHIEBUNG(f(ixb1+bord),f(iyb1+bord),
     #   0,ase1-1,aint(bgs),asespl,ase1,1,delta,
     #   f(ixb2+bord),f(iyb2+bord))
...
c ----------------------------------------------------------------------
       ...
       ase2=ase(2,2)
c Rand 1       
c       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(2),PG(3),delta,0.,
c     #  0.,1,1./lvs2,m-1,ase1,f(ixb1+bord),f(iyb1+bord))
 
        call SPLINEVERSCHIEBUNG(f(ixb1),f(iyb1),
     #   ase(1,1),ase(1,1)+ase1-1,aint(bgs),asespl,ase1,2,delta,
     #   f(ixb2+bord),f(iyb2+bord))

c Rand 2       
c       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(2),PG(3),delta,0.,
c     #  aint(bgs),1,1./lvs2,m-1,ase1,f(ixb2+bord),f(iyb2+bord))

       xperhilf2=f(ixb1+bord+ase1-1)-delta
       yperhilf2=f(iyb1+bord+ase1-1)
       xperhilf1=f(ixb2+bord+ase1-1)-delta
       yperhilf1=f(iyb2+bord+ase1-1)
...
c ----------------------------------------------------------------------
       ...
       ase2=ase(3,2)
c Rand 1       
c       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(3),PG(4),0.,0.,0.,
c     #  1,lvs3,m-1,ase1,f(ixb1+bord),f(iyb1+bord))

        call SPLINEVERSCHIEBUNG(f(ixb1),f(iyb1),
     #   ase(1,1)+ase(2,1),ase(1,1)+ase(2,1)+ase1-1,aint(bgs),
     #   asespl,ase1,3,delta,
     #   f(ixb2+bord),f(iyb2+bord))

c Rand 2       
c       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(3),PG(4),0,0.,aint(bgs),
c     #  1,lvs3,m-1,ase1,f(ixb2+bord),f(iyb2+bord))
c Vernetzen      
       do za=bord,ase1-1+bord,1
        call GERADE(f(ixb2+za),f(iyb2+za),f(ixb1+za),f(iyb1+za),
     #   ase2,1,bvs1,f(ixr+seed),f(iyr+seed))
...
c Rand 1       
     ...
     #  1,lvs4,m-1,ase1,f(ixb1+bord),f(iyb1+bord))
        ...
        call SPLINEVERSCHIEBUNG(f(ixb1),f(iyb1),
     #   ase(1,1)+ase(2,1)+ase(3,1),ase(1,1)+ase(2,1)+ase(3,1)+ase1-1,
     #   aint(bgs),asespl,ase1,4,delta,
     #   f(ixb2+bord),f(iyb2+bord))

c Rand 2       
c       call MESHSEED(f(ixp),f(iyp),0,0.,0.,PG(4),PG(5),0.,0.,aint(bgs),
c     #  1,lvs4,m-1,ase1,f(ixb2+bord),f(iyb2+bord))
c Vernetzen      
       do za=bord,ase1-1+bord,1
        call GERADE(f(ixb2+za),f(iyb2+za),f(ixb1+za),f(iyb1+za),
     #   ase2,1,bvs1,f(ixr+seed),f(iyr+seed))
...
c Periodische Raender 2       
       ase1=ase(3,2)
       call GERADE(xperhilf1,yperhilf1,
     #  xperhilf2,yperhilf2,ase1,1,bvs1,f(ixpel2),f(iypel2))
       do za=0,ase1-1,1
        f(ixper2+za)=f(ixpel2+za)+delta
        f(iyper2+za)=f(iypel2+za) 
       end do
...
c Periodische Raender 3
       ase1=ase(5,2)
       call GERADE(xperhilf1,yperhilf1,xo,yo,
...
c ----------------------------------------------------------------------
     ...
     #    an_grk,e(igrkli),anz_re,e(ireli),anz_rk,e(irkli))
         print*, anz_rk,' Randknoten'
         print*, anz_re,' Randelemente'
c
         call ZUORD_RAND(anz_rk,e(irkli),yu,yo,ase(11,1),f(ixpel1),
     #    f(iypel1),f(ixper1),f(iyper1),ase(3,2),f(ixpel2),f(iypel2),
     #    f(ixper2),f(iyper2),ase(5,2),f(ixpel3),f(iypel3),f(ixper3),
...
c ----------------------------------------------------------------------
c SPLINEVERSCHIEBUNG
c ----------------------------------------------------------------------

      subroutine SPLINEVERSCHIEBUNG(XK,YK,PA,PE,VT,N,NL,NR,delta,XS,YS)

c
c Verschiebt Punkte entlang ihrer Normalen
c
c EINGABE
c
...
c PA...Nummer des Stuetzpunktes ab dem verschoben werden soll
c PE...Nummer des Stuetzpnuktes bis zu dem verschoben werden soll
c N....Nummer des letzten Stuetzpunktes
c NL...Anzahl der Kurvenpunkte
c
c AUSGABE
...
c
      integer N,NL,PA,PE,i,NR
c     parameter (MT=2,IB=3,DIM1=500,DIM2=1000)
      real XK(0:N),YK(0:N),XS(0:NL-1),YS(0:NL-1)
      REAL VT,phi,X1,X2,Y1,Y2,delta
      ...
      if (i.eq.NL-1) then
       ...
       phi=(atan2((Y2-Y1),(X2-X1)))+PI/2.
       
       XS(i)=XK(PA+i)+VT*cos(phi)
       YS(i)=YK(PA+i)+VT*sin(phi)
      end do
...
c RAND_SPOTS
c ----------------------------------------------------------------------
      subroutine RAND_SPOTS(x,y,s,ymin,ymax,alpha,valN,valK,vbe,weinhi,
...
c      alpha=atan2((y(s)-y(s-2)),(x(s)-x(s-2)))
...
c CRE_NACA
c ----------------------------------------------------------------------
      subroutine CRE_NACA(k,b,x,y,z,schnitt,m,L12N,L12K,Nprofile,
     # radius,ip,iz,fr,fp,fz)
c
...
c        
      ...
      call CRE_SP(k,b,r,p,z,schnitt,m,L12N,L12K,Nprofile,radius,ip,iz) 
c
      end
c ----------------------------------------------------------------------
c INP_DAT
...
c CRE_SP
c ----------------------------------------------------------------------
      subroutine CRE_SP(k,b,r,p,z,sc,m,L12N,L12K,Nprofile,ir,ip,iz) 
c
...
c      L12=L12_il*pow(basis_l,(ir-r(1,1))/(r(1,Nprofile)-r(1,1)))
      L12=L12N+(L12K-L12N)*(ir-r(1,1))/(r(1,Nprofile)-r(1,1))
...
c Ausgabe der Knoten      
      do i=1,anz_kno*t,1
       write(16,'(I7,3(F12.8))') i,x(i)/1000.,y(i)/1000.,z(i)/1000.
      end do
c
c Ausgabe der Elemente
      do i=1,anz_elm
...
c
     ...
     # an_rrk,rrkli(ap),DIM,i,j,por,mark,pu
      integer mark1,mark2,mark3,mark4
c
      parameter (DIM=65535)
c
...
c NACA
      ...
      mark1 = ase(1,1)*ase(1,2) + (ase(2,1)-2)*ase(2,2) 
      mark2 = mark1 + ase(2,2)
      mark3 = mark2 + ase(2,2)
      mark4 = mark3 + ase(3,1)*ase(3,2) + (ase(4,1)-2)*ase(4,2)

c
      an_nak=0
      do i=1,anz_rk
       if ((rkli(i).le.mark1).or.(rkli(i).eq.mark2).or.
     #    ((rkli(i).ge.mark3).and.(rkli(i).le.mark4))) then
        an_nak=an_nak+1
        nakli(an_nak)=rkli(i)
       endif
      end do
