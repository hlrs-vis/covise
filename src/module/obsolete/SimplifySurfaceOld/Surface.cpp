...
Surface::Surface(int n_points, int n_vert, int n_poly, char *Type, int *pl_in, int *vl_in, float *x_in, float *y_in, float *z_in, float *nu_in, float *nv_in, float *nw_in)
{
    int i;
    float norm;
...
void
Surface::TriStripsToTriangulation(int *poly_list, int *vertice_list)
...
coDistributedObject **Surface::createcoDistributedObjects(int,
                                                          int,
...
int Surface::check_boundary(int, pair *link, int num_link)
...
int Surface::check_manifold(int, pair *link, int num_link)
...
void Surface::make_link(int v1, int v2, Star star, pair *link, int &num_link)
{
    ...
    // fprintf(stderr, " %d: V1=%d v2=%d",numtest,v1,v2);
    // numtest++;
    count = 0;
...
{ // closes the link if there is a break in the link(v is boundary vertex)
    ...
    if (num_link == 0) // added by Uwe Woessner
        return 0; // otherwise, the next satement is wrong (num_link-1 < 0)
...
{ // check whether the angle enclosed by neighbour triangles of
    ...
    if (value)
    {
        ...
        cosi = a0[0] * a1[0] + a0[1] * a1[1] + a0[2] * a1[2];
...
{ // detect feature edge
    ...
    cosi_max = cos(M_PI * max_angle / 180.0);
    ...
    for (j = 0; j < stars[v].num_tri; j++)
    {
        ...
        { // compute angle between these neighbour triangles
            ...
            cosi = tri1[0] * tri2[0] + tri1[1] * tri2[1] + tri1[2] * tri2[2];
            if (!orientation)
                cosi = -cosi;
...
float Surface::compute_compactness(int v)
{
    float compact;
    float min;
    float area, l1, l2, l3;
    ...
    min = 1.0;
    for (int i = 0; i < stars[v].num_tri; i++)
    {
        ...
        l1 = (coords_x[v1] - coords_x[v]) * (coords_x[v1] - coords_x[v]);
        l1 += (coords_y[v1] - coords_y[v]) * (coords_y[v1] - coords_y[v]);
        l1 += (coords_z[v1] - coords_z[v]) * (coords_z[v1] - coords_z[v]);
        l2 = (coords_x[v2] - coords_x[v1]) * (coords_x[v2] - coords_x[v1]);
        l2 += (coords_y[v2] - coords_y[v1]) * (coords_y[v2] - coords_y[v1]);
        l2 += (coords_z[v2] - coords_z[v1]) * (coords_z[v2] - coords_z[v1]);
        l3 = (coords_x[v2] - coords_x[v]) * (coords_x[v2] - coords_x[v]);
        l3 += (coords_y[v2] - coords_y[v]) * (coords_y[v2] - coords_y[v]);
        l3 += (coords_z[v2] - coords_z[v]) * (coords_z[v2] - coords_z[v]);
        area = 0.5 * sqrt(l1) * sqrt(l3) * sin(compute_inner_angle(v, v1, v2));
        compact = 4 * SQRT_3 * area / (l1 + l2 + l3);
        if (compact < min)
            min = compact;
    }
    return (min);
...
float Surface::compute_compactness(int v, pair *link, int num_link)
{
    float compact;
    float min;
    float area, l1, l2, l3;
    ...
    for (int i = 0; i < num_link; i++)
    {
        v1 = link[i][0];
        v2 = link[i][1];
        l1 = (coords_x[v1] - x) * (coords_x[v1] - x);
        l1 += (coords_y[v1] - y) * (coords_y[v1] - y);
        l1 += (coords_z[v1] - z) * (coords_z[v1] - z);
        l2 = (coords_x[v2] - coords_x[v1]) * (coords_x[v2] - coords_x[v1]);
        l2 += (coords_y[v2] - coords_y[v1]) * (coords_y[v2] - coords_y[v1]);
        l2 += (coords_z[v2] - coords_z[v1]) * (coords_z[v2] - coords_z[v1]);
        l3 = (coords_x[v2] - x) * (coords_x[v2] - x);
        l3 += (coords_y[v2] - y) * (coords_y[v2] - y);
        l3 += (coords_z[v2] - z) * (coords_z[v2] - z);
        area = 0.5 * sqrt(l1) * sqrt(l3) * sin(compute_inner_angle(v, v1, v2));
        compact = 4 * SQRT_3 * area / (l1 + l2 + l3);
        if (compact < min)
            min = compact;
...
{ //Directional curvature in direction (v_i, v) can be approximated by
    ...
    int i;
    double norm;
    double max = 0.0, curv; // Setting max prevents warning: always set correctly in first loop AW
    double a, b, c;
    for (i = 0; i < num; i++)
    {
...
float Surface::compute_tetra_volume(int v1, int v2, int v3, float x0, float y0, float z0)
{ // computes the volume of the tetrahedron build by the triangle (v1, v2, v3)
    // and the vertex with coordinates (x0, y0, z0).
    float vol, det;
...
float Surface::compute_tetra_volume(int t, float x0, float y0, float z0)
{ // computes the volume of the tetrahedron build by the triangle t
    // and the vertex with coordinates (x0, y0, z0).
    return (compute_tetra_volume(vertex_list[tri_list[t]], vertex_list[tri_list[t] + 1], vertex_list[tri_list[t] + 2], x0, y0, z0));
}
...
{ // computes the volume of the polyhedron build by the vertex v to be
    ...
    for (i = 0; i < num_link - 2; i++)
        volume += compute_tetra_volume(retri[i][0], retri[i][1], retri[i][2], x0, y0, z0);

    volume /= 6.0;
    return (volume);
}

float Surface::compute_star_volume(int, int, Star star, pair *, int, float x0, float y0, float z0)
{ // Computes the volume difference of the star build by the edge (v1,v2) to be
    // removed together with her adjacent triangles, and the star build by the
    // new vertex (x0, y0, z0) and the edges stored in the link, thus giving
    // a measure on the volume deficit achieved by collapsing that edge.
    float volume = 0.0;

    for (int i = 0; i < star.num_tri; i++)
        volume += compute_tetra_volume(star.tri[i], x0, y0, z0);
