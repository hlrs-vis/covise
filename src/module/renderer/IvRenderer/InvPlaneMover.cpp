...
// Constructor
//
InvPlaneMover::InvPlaneMover(InvCoviseViewer *v)
    : show_(0)
    , distOffset_(0, 0, 0)
    , feedbackInfo_(NULL)
    , myViewer_(v)
...

{
    ...
    //here we customize the SoJackDragger
    SoSeparator *arrow = new SoSeparator;
    arrow->addChild(handleDrawStyle_);
    arrow->addChild(makeArrow());
    jDrag_->setPart("rotator.rotator", arrow);
    arrow = new SoSeparator;
    arrow->addChild(handleDrawStyle_);
    arrow->addChild(makeArrow());
    jDrag_->setPart("rotator.rotatorActive", arrow);

    SoSeparator *plane = new SoSeparator;
    plane->addChild(handleDrawStyle_);
    plane->addChild(makePlane());
    jDrag_->setPart("translator.yTranslator.translator", plane);
    plane = new SoSeparator;
    plane->addChild(handleDrawStyle_);
    plane->addChild(makePlane());
    jDrag_->setPart("translator.yTranslator.translatorActive", plane);
    jDrag_->setPart("translator.xTranslator.translator", new SoSeparator);
    jDrag_->setPart("translator.xTranslator.translatorActive", new SoSeparator);
    jDrag_->setPart("translator.zTranslator.translator", new SoSeparator);
    jDrag_->setPart("translator.zTranslator.translatorActive", new SoSeparator);

    jDrag_->setPart("scaler.scaler", new SoSeparator);
    jDrag_->setPart("scaler.scalerActive", new SoSeparator);

    jDrag_->setPart("translator.yzTranslator.translatorActive", new SoSeparator);
    jDrag_->setPart("translator.xzTranslator.translatorActive", new SoSeparator);
    jDrag_->setPart("translator.xyTranslator.translatorActive", new SoSeparator);

    jDrag_->setPart("translator.yzTranslator.translator", new SoSeparator);
    jDrag_->setPart("translator.xzTranslator.translator", new SoSeparator);
    jDrag_->setPart("translator.xyTranslator.translator", new SoSeparator);
}
                    ...
                    if (l > 1)
                    {
                        ...
                        if (tok)
                        {
                            ...
                            if (tok)
                            {
                                float dum;
                                int idum;
                                int ret = sscanf(tok, "%f%f%f%f%d", &(mee->planeNormal_[0]), &(mee->planeNormal_[1]), &(mee->planeNormal_[2]), &dum, &idum);
                                if (ret != 5)
                                {
                                    fprintf(stderr, "InvPlanMover::selectionCB: sscanf failed\n");
                                }
...
void
InvPlaneMover::sendFeedback(float *data)
{
    if (!feedbackInfo_)
    {
    ...
    else
    {
        ...
        sprintf(buf, "vertex\nFloatVector\n%f %f %f", data[0], data[1], data[2]);

        strcat(msg, buf);

        if (myViewer_)
        {
            myViewer_->sendCSFeedback((char *)"PARAM", msg);
        }
        ...
        if (myViewer_)
        {
            myViewer_->sendCSFeedback((char *)"EXEC", msg);
        }
    }
}
...
void
InvPlaneMover::deSelectionCB(void *me, SoPath *)
{
    InvPlaneMover *mee = static_cast<InvPlaneMover *>(me);
...
SoPath *
InvPlaneMover::pickFilterCB(void *me, const SoPickedPoint *pick)
{
...
void
InvPlaneMover::setPosition(SbVec3f &point)
{

    // make sure that the normal points in direction to the camera
    SbVec3f camPos = myViewer_->getCamera()->position.getValue();
    SbVec3f no;
    ...
    //   }
    SbVec3f t(0, 0, 0);
    jDrag_->translation = t;
    jDrag_->rotation = SbRotation::identity();
}

// internal callback
void
InvPlaneMover::dragFinishCB(void *me, SoDragger *drag)
{
    ...
    if (mee->show_)
    {
        ...
        // 	cerr << "InvPlaneMover::dragFinishCB(..)       TRANSLATION: " << tt[0] << " : " << tt[1] << " : " << tt[2] << endl;

        // 	cerr << "InvPlaneMover::dragFinishCB(..)       ROT axis: " << axN[0] << " : " << axN[1] << " : " << axN[2] << " angle: " << angle << endl;

        // 	cerr << "InvPlaneMover::dragFinishCB(..)       nnn_: " << mee->nnn_[0] << " : " << mee->nnn_[1] << " : " << mee->nnn_[2] << endl;

        // 	cerr << "InvPlaneMover::dragFinishCB(..)       NORMAL: " << n[0] << " : " << n[1] << " : " << n[2] << endl;
        // 	cerr << "InvPlaneMover::dragFinishCB(..)       DISTANCE: " << d << endl;
        ...
        // send feedback messgae to contoller
...
// internal callback
void
InvPlaneMover::dragStartCB(void *me, SoDragger *drag)
{
    (void)me;
    (void)drag;
...
InvPlaneMover::makeArrow()
{
    ...
    transl->translation.setValue(0, -0.85, 0);
...
InvPlaneMover::makePlane()
{
    ...
    mat->ambientColor.setValue(0.3, 0.1, 0.1);
    mat->diffuseColor.setValue(0.8, 0.7, 0.2);
    mat->specularColor.setValue(0.4, 0.3, 0.1);
