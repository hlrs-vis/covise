    ...
    : coModule(argc, argv, "Read/write COVISE ASCII files")
{
    ...
    p_dataOut = addOutputPort("DataOut",
                              "IntArr|Polygons|Lines|Float|Vec3|UniformGrid|RectilinearGrid|TriangleStrips|StructuredGrid|UnstructuredGrid|Points|Vec2|RGBA|USR_DistFenflossBoco",
                              "OutputObjects");

    // input port
    p_dataIn = addInputPort("DataIn",
                            "Geometry|IntArr|Polygons|Lines|Float|Vec3|UniformGrid|RectilinearGrid|TriangleStrips|StructuredGrid|UnstructuredGrid|Points|Vec2|RGBA|USR_DistFenflossBoco",
                            "InputObjects");
    ...
    p_dirsAsTimesteps = addBooleanParam("timesteps", "Read directories as timesteps");
    p_dirsAsTimesteps->setValue(1);
}
...
void
RW_ASCII::writeObj(const char *offset, const coDistributedObject *new_data, FILE *file)
{
    // get object type
