...
struct Cleaner {
    ...
    void reset(std::function<void()> ntask = std::function<void()>()) {
        task = std::move(ntask);
    }
...
Directory::Directory(const Directory *dir, const std::string &name)
: Entry(dir, name)
{}

Directory::Directory(const Model *m)
: Entry(m)
{}
...
File::File(const Directory *dir, const std::string &name)
: Entry(dir, name)
{}

Entry::Entry(const Directory *parent, std::string name)
: model(parent->model)
, parent(parent)
, name(std::move(name))
{}

Entry::Entry(const Model *model)
: model(model)
{}
...
Path::Path(const Model *model, const std::string &path)
: model(model) {
    if (path.empty()) {
        absolute = false;
        return;
