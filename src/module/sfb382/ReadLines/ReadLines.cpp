...
///////////////////////////////////////////////////////////////
// Module:        ReadLines.cpp
// Author:        Juergen Schulze-Doebold
// Function:      Reads a file containing line coordinates as ASCII code.
// Usage:         Provide filename of lines file.
// History:       2001-04-03  creation date
///////////////////////////////////////////////////////////////
...
coReadLines::coReadLines()
    : coModule("Lines Reader")
{
    int i;

    // Create browser parameter:
    p_filename = addFileBrowserParam("filename", "ASCII data file (.dat)");
    p_filename->setValue("/mnt/pro/cod/sfb382/", "*.dat");

    // Create output ports:
    p_lines = addOutputPort("lines", "coDoLines", "Set of lines");
    p_lines->setInfo("Set of lines for all timesteps.");
    p_colors = addOutputPort("colors", "coDoFloat", "Set of colors");
...
bool coReadLines::parseLine(FILE *fp, coParsedLine *parsed)
{
    ...
    if (strncasecmp(buf, "[NEWLINE]", 9) == 0)
    {
        parsed->type = 2;
...
int coReadLines::compute()
{
    ...
    {
        Covise::sendError("Line data file expected (.dat)");
        return 0;
    }
