...
void Covise_Set_Handler::Compute(int num_in_dataports, char **inportnames, int num_out_dataports, char **outportnames)
{
    //
    // parameters:
    ...
    INDATAPORTS = num_in_dataports;
    OUTDATAPORTS = num_out_dataports;

    // allocate required memory
    data_in = new coDistributedObject *[INDATAPORTS];
    if (OUTDATAPORTS)
        dataoutname = new char *[OUTDATAPORTS];
                                        ...
                                        coDistributedObject ***data_set_out)
{
    ...
    if (OUTDATAPORTS)
    {
        bfr = new char *[OUTDATAPORTS];
        data_new_set = new coDistributedObject **[OUTDATAPORTS];
        D_set = new coDoSet *[OUTDATAPORTS];
    }
    ...
    { // no set - so handle this and all corresponding data-objects
        if (OUTDATAPORTS)
            data = ComputeObject(data_in, data_out_name, INDATAPORTS, OUTDATAPORTS);
        else
            Compute_No_Output(data_in, INDATAPORTS);

        // add objects to set
        for (j = 0; j < OUTDATAPORTS; j++)
        {
            if (data_set_out)
            {
                if (data_set_out[j])
    ...
    { // SET !!!
        ...
        /* 
          ...
          {  data_new_set[i] = new coDistributedObject*[data_set_num_elem];
             data_new_set[i][0] = NULL;
          }
            }
      */
...
coDistributedObject **Covise_Set_Handler::ComputeObject(coDistributedObject **, char **, int, int)
{ // dummy
...
void Covise_Set_Handler::Compute_No_Output(coDistributedObject **, int)
{ // dummy
    Covise::sendError("ERROR: if OUTDATAPORTS is 0, then you should provide a Compute_No_Output-function");
