...
{
    ReadIWS *application = new ReadIWS();
...
void ReadIWS::closeFile(const char *filename)
{
    fclose(iwsFp);
    fprintf(stderr, "File %s closed\n", filename);
}

void ReadIWS::readFile(float scale_factor)
{
...
void ReadIWS::scanHeader(char *line, int *nVert, int *nEdge, int *nCorner, int *nFace, int *nElem)
{
...
char *ReadIWS::testInput(char *line)
{
    char *qualifier = NULL;
...
void ReadIWS::allocCoord(char *line, char *key, int nVert, float **cx, float **cy, float **cz)
{
    if ((strstr(line, "Vertices") != NULL) && (strstr(key, "Vertices") != NULL))
    {
        (*cx) = new float[nVert];
...
void ReadIWS::allocLines(char *line, char *key, int nCorner, int nEdge, int **cl, int **ll)
{
...
void ReadIWS::allocPoly(char *line, char *key, int nFace, int **pl, int **id)
{
...
int ReadIWS::setPolyId(char *line, char *key)
{
    if ((key != NULL) && (strcmp(key, "Faces") == 0) && (strstr(line, "% Faces") == NULL) && (strstr(line, "$") == NULL))
    {
        while (isspace(*line))
...
int ReadIWS::setPolygons(char *line, char *key, int *old_size, int *new_size, int polygon_index, int **polygon_list, int edge_index, int **edge_list)
{
    int num_edges = 0;
    int *tmp_edges;
    int *pl = (*polygon_list);
...
void ReadIWS::line2coord(int *corner_list, int edge_index, int *edge_list, int polygon_index, int *polygon_list, int **polygon_corners)
{
...
bool ReadIWS::isNumber(char *line)
{
    while (isspace(*line))
    {
        ++line;
