...
inline const FuncDefinition *FindFunction(const char *F)
{
    ...
    if (func.nameLength)
    {
        const FuncDefinition *found = lower_bound(Functions, Functions + FUNC_AMOUNT, func);
...
FunctionParser::Data::Data(const Data &cpy)
    ...
    , FuncPtrNames(cpy.FuncPtrNames)
    , FuncPtrs(cpy.FuncPtrs)
    , FuncParserNames(cpy.FuncParserNames)
    , FuncParsers(cpy.FuncParsers)
    , ByteCode(0)
    , ByteCodeSize(cpy.ByteCodeSize)
    , Immed(0)
    , ImmedSize(cpy.ImmedSize)
    , Stack(0)
    , StackSize(cpy.StackSize)
{
    if (ByteCodeSize)
        ByteCode = new unsigned[ByteCodeSize];
    if (ImmedSize)
        Immed = new float[ImmedSize];
    if (StackSize)
...
bool FunctionParser::AddConstant(const string &name, float value)
{
    if (isValidName(name))
    {
        ...
        if (FindVariable(n, data->FuncParserNames) != data->FuncParserNames.end() || FindVariable(n, data->FuncPtrNames) != data->FuncPtrNames.end())
            return false;
                                 ...
                                 FunctionPtr func, unsigned paramsAmount)
{
    ...
    {
        const char *n = name.c_str();
        if (FindVariable(n, data->FuncParserNames) != data->FuncParserNames.end() || FindConstant(n) != data->Constants.end())
            return false;
...
int FunctionParser::CheckSyntax(const char *Function)
{
    ...
    while (true)
    {
        ...
        {
            double val = strtod(&Function[Ind], &Ptr);
            (void)val;
        ...
        // Check for operator
        if (!IsOperator(c) && (c != ',' || functionParenthDepth.empty() || functionParenthDepth.back() != ParenthCnt))
        {
...
float FunctionParser::Eval(const float *Vars)
{
    ...
    for (IP = 0; IP < ByteCodeSize; ++IP)
    {
        ...
        case cGreater:
            ...
            break;
        case cAnd:
            Stack[SP - 1] = (floatToInt(Stack[SP - 1]) && floatToInt(Stack[SP]));
            --SP;
            break;
        case cOr:
            Stack[SP - 1] = (floatToInt(Stack[SP - 1]) || floatToInt(Stack[SP]));
        ...
        case cFCall:
        {
            ...
            float retVal = data->FuncPtrs[index].ptr(&Stack[SP - params + 1]);
            SP -= params - 1;
            Stack[SP] = retVal;
            break;
        }
        ...
        case cPCall:
        {
            ...
            float retVal = data->FuncParsers[index]->Eval(&Stack[SP - params + 1]);
            SP -= params - 1;
            Stack[SP] = retVal;
            break;
        }
...
void FunctionParser::PrintByteCode(std::ostream &dest) const
{
    ...
    for (unsigned IP = 0, DP = 0; IP < data->ByteCodeSize; ++IP)
    {
        ...
        {
            unsigned index = ByteCode[++IP];
            Data::VarMap_t::const_iterator iter = data->FuncPtrNames.begin();
        ...
        {
            unsigned index = ByteCode[++IP];
            Data::VarMap_t::const_iterator iter = data->FuncParserNames.begin();
            while (iter->second != index)
                ++iter;
            dest << "call\t" << iter->first << endl;
            break;
...
private:
    ...
    {
        if (cl.value != cl.voidvalue && cl.size() > 1)
            AddParam(cl.value);
        if (cl.value == cl.voidvalue || cl.size() > 1)
            KillConst(cl);
    }
...
#define ConstantUnaryFun(token, fun)               \
    ...
    {                                              \
        ...
        if (p0->IsImmed())                         \
            ReplaceWithConst(fun(p0->GetImmed())); \
        break;                                     \
    }
#define ConstantBinaryFun(token, fun)                              \
    ...
    {                                                              \
        ...
        if (p0->IsImmed() && p1->IsImmed())                        \
            ReplaceWithConst(fun(p0->GetImmed(), p1->GetImmed())); \
        break;                                                     \
    }
...
public:
    ...
    void FinalOptimize()
    {
        ...
        if (GetOp() == cMul)
        {
            ...
            {
                /***/ if (getp0()->GetOp() == cSin)
                    ReplaceWith(cCsc, getp0()->getp0());
                else if (getp0()->GetOp() == cCos)
                    ReplaceWith(cSec, getp0()->getp0());
                else if (getp0()->GetOp() == cTan)
                    ReplaceWith(cCot, getp0()->getp0());
            }
        }
...
{
    ConstList result;
    result.value = result.voidvalue = GetOp() == cMul ? 1.0f : 0.0f;
...
void CodeTree::Assemble(vector<unsigned> &byteCode,
                        vector<float> &immed) const
{
...
bool CodeTree::operator<(const CodeTree &b) const
{
    ...
    {
        // sort immeds last
        if (IsImmed() != b.IsImmed())
            return IsImmed() < b.IsImmed();
...
#define GROW(n)                         \
    ...
    {                                   \
        stacktop += n;                  \
        if (stack.size() <= stacktop)   \
            stack.resize(stacktop + 1); \
    } while (0)
...
#define EAT(n, opcode)                                                      \
    ...
    {                                                                       \
        ...
        stack.erase(stack.begin() + newstacktop, stack.begin() + stacktop); \
