...
{
    struct sc2004grid *grid = NULL;
    if ((grid = (struct sc2004grid *)calloc(1, sizeof(struct sc2004grid))) == NULL)
    {
        fprintf(stderr, "not enough memory for struct sc2004grid!");
        return NULL;
    }
    ...
    float spacing;
    float x, y, z;
    float xmin = -0.5 * booth->size[0];
    float ymin = -0.5 * booth->size[1];
    float zmin = 0.0;

    int i, j, k;

    // Create Coordinates
    spacing = booth->spacing;
    grid->nelem_x = (int)(booth->size[0] / spacing);
    grid->nelem_y = (int)(booth->size[1] / spacing);
    grid->nelem_z = (int)(booth->size[2] / spacing);
    grid->spacing_x = booth->size[0] / grid->nelem_x;
    grid->spacing_y = booth->size[1] / grid->nelem_y;
    grid->spacing_z = booth->size[2] / grid->nelem_z;
    ...
    DefineBCs(grid, booth, -1);
    GenerateBCs(grid, booth, -1);
    //fprintf(stderr,"grid->bcwall->num=%d\n",grid->bcwall->num);
    //fprintf(stderr,"booth->nobjects=%d\n",booth->nobjects);
    for (i = 0; i < booth->nobjects; i++)
    {
        subtractCube(grid, booth, i);
        DefineBCs(grid, booth, i);
        GenerateBCs(grid, booth, i);
        //fprintf(stderr,"grid->bcwall->num=%d\n",grid->bcwall->num);
    }
    ...
    for (i = 0; i < grid->npoi; i++)
    {
        ...
        {
            fprintf(stderr, "error 1 in CreateSC2004Grid!\n");
        }
    }
    grid->p->nump -= offset;
    ...
    for (i = 0; i < grid->npoi; i++)
    {
        ...
        {
            fprintf(stderr, "error 2 in CreateSC2004Grid!\n");
        }
    }
    ...
    for (i = 0; i < grid->e->nume; i++)
    {
        for (j = 0; j < 8; j++)
        {
            ...
            if ((grid->new_node[vertex] == -1) && (grid->e_to_remove[i] != -1))
            {
                fprintf(stderr, "error 3 in CreateSC2004Grid. Element list uses node that was removed.\n");
            }
            else
            {
    ...
    for (i = 0; i < grid->nelem; i++)
    {
        ...
        {
            fprintf(stderr, "error 4 in CreateSC2004Grid!\n");
        }
    }

    /*
       // 7. remove inlet nodes if there is a wall
      // we can only take inlet node bcs if the nodes appear in the grid->bcin list!
       int *wall_nodes = (int*)calloc(grid->npoi, sizeof(int));
       int removed_inbcs=0;
      for (i=0; i<grid->bcwall->num; i++)
       {
         wall_nodes[grid->bcwall->list[i]]++;
       }
      for (i=0;i<grid->bcin_nodes->num;i++)
       {
   if ( wall_nodes[grid->bcin_nodes->list[i]]>0 )
   {
   removed_inbcs++;
   }
   else
   {
   grid->bcin_nodes->list[i-removed_inbcs]=grid->bcin_nodes->list[i];

   grid->bcin_velos->list[5*(i-removed_inbcs)+0]=grid->bcin_velos->list[5*i+0];
   grid->bcin_velos->list[5*(i-removed_inbcs)+1]=grid->bcin_velos->list[5*i+1];
   grid->bcin_velos->list[5*(i-removed_inbcs)+2]=grid->bcin_velos->list[5*i+2];
   grid->bcin_velos->list[5*(i-removed_inbcs)+3]=grid->bcin_velos->list[5*i+3];
   grid->bcin_velos->list[5*(i-removed_inbcs)+4]=grid->bcin_velos->list[5*i+4];
   }
   }
   grid->bcin_nodes->num-=removed_inbcs;
   grid->bcin_velos->num-=5*removed_inbcs;
   */
    ...
    for (i = 0; i < grid->bcven_nodes->num; i++)
    {
        //isbcnode[grid->new_node[grid->bcven_nodes->list[i]]]=1;
        isbcnode[grid->bcven_nodes->list[i]] = 1;
    }
    for (i = 0; i < grid->bcair_nodes->num; i++)
    {
        //isbcnode[grid->new_node[grid->bcair_nodes->list[i]]]=1;
        isbcnode[grid->bcair_nodes->list[i]] = 1;
    }
   ...
   */
    ...
    float _k = 3.75 * 0.001 * pow(booth->bcin_velo, 2);
    float _eps = 9.40 * 0.0001 * pow(fabs(booth->bcin_velo / l), 3);
    ...
    for (i = 0; i < grid->p->nump; i++)
    {
        if (inlet_nodes[i] == 1)
        {
            ...
            Add2Flist(grid->bcin_velos, booth->bcin_velo);
    ...
    /*
      for (i=0;i<grid->bcin_nodes->num;i++)
      {
         if (grid->p_to_remove[grid->bcin_nodes->list[i]]==-1)
         {
            fprintf(stderr,"error! in nodes list uses removed node!\n");
         }
         grid->bcin_nodes->list[i]=grid->new_node[grid->bcin_nodes->list[i]];
      }
   */
    for (i = 0; i < grid->bcair_nodes->num; i++)
    {
        if (grid->p_to_remove[grid->bcair_nodes->list[i]] == -1)
        {
            fprintf(stderr, "error! aircond. nodes list uses removed node!\n");
        }
        grid->bcair_nodes->list[i] = grid->new_node[grid->bcair_nodes->list[i]];
    }
    for (i = 0; i < grid->bcven_nodes->num; i++)
    {
        if (grid->p_to_remove[grid->bcven_nodes->list[i]] == -1)
        {
            fprintf(stderr, "error! vencond. nodes list uses removed node!\n");
        }
        grid->bcven_nodes->list[i] = grid->new_node[grid->bcven_nodes->list[i]];
    }
   ...
   */
    ...
    for (i = 0; i < grid->bcair_nodes->num; i++)
    {
        node = grid->bcair_nodes->list[i];
        grid->bcair_nodes->list[i] = rep_nodes[node];
    }
    for (i = 0; i < grid->bcin_nodes->num; i++)
    {
    ...
    for (i = 0; i < grid->bcven_nodes->num; i++)
    {
        node = grid->bcven_nodes->list[i];
        grid->bcven_nodes->list[i] = rep_nodes[node];
    }
    for (i = 0; i < grid->bcwallvol->num; i++)
    {
        for (j = 0; j < 4; j++)
...
struct covise_info *CreateGeometry4Covise(struct sc_booth *booth)
{
    ...
    if ((ci = AllocCoviseInfo()) != NULL)
    {
        ...
        for (i = 0; i < booth->nobjects; i++)
        {
            CreateCubePolygons4Covise(ci, booth, i, ipoi, ipol);
        }
    }
    ...
    ipoi = ci->p->nump;
    ipol = ci->pol->num;
    AddPoint(ci->p, -0.5 * booth->size[0],
             -0.5 * booth->size[1],
             0.0);
    AddPoint(ci->p, +0.5 * booth->size[0],
             -0.5 * booth->size[1],
...
int CreateCubePolygons4Covise(struct covise_info *ci, struct sc_booth *booth, int number, int ipoi, int ipol)
{
    ...
    float pos[3];
    float size[3];
    pos[0] = booth->cubes[number]->pos[0];
    pos[1] = booth->cubes[number]->pos[1];
    pos[2] = booth->cubes[number]->pos[2];
    size[0] = booth->cubes[number]->size[0];
    size[1] = booth->cubes[number]->size[1];
    size[2] = booth->cubes[number]->size[2];
...
int subtractCube(struct sc2004grid *grid, struct sc_booth *booth, int number)
{
    ...
    float bxmin = -0.5 * booth->size[0];
    float bymin = -0.5 * booth->size[1];
    float bzmin = 0.;

    /*
       float bxmax = 0.5*booth->size[0];
       float bymax = 0.5*booth->size[1];
       float bzmax = booth->size[2];
   */
    ...
    jlo = (int)ceil((ymin + 0.5 * booth->size[1]) / grid->spacing_y);
    jhi = (int)floor((ymax + 0.5 * booth->size[1]) / grid->spacing_y);
    klo = (int)ceil((xmin + 0.5 * booth->size[0]) / grid->spacing_x);
    khi = (int)floor((xmax + 0.5 * booth->size[0]) / grid->spacing_x);

    fprintf(stderr, "number=%2d", number);
    /*    fprintf(stderr,"imin=%2d imax=%2d jmin=%2d jmax=%2d kmin=%2d kmax=%2d\n",booth->ilo,booth->ihi,booth->jlo,booth->jhi,booth->klo,booth->khi);
   ...
   */
    ...
    /*
      float khiexakt=(xmax+0.5*booth->size[0])/grid->spacing_x;
   ...
   */
    ...
    {
        jlo = booth->jlo; //=0
        ymin = jlo * grid->spacing_y - 0.5 * booth->size[1];
        dont_jlo = 1;
    ...
    {
        klo = booth->klo; // =0
        xmin = klo * grid->spacing_x - 0.5 * booth->size[0];
        dont_jlo = 1;
        //fprintf(stderr,"dont_klo==1!!!\n");
    }
    if (ihi >= booth->ihi)
    {
        ihi = booth->ihi; // =npoi_z
    ...
    if (jhi >= booth->jhi)
    {
        jhi = booth->jhi; // =npoi_y
        ymax = jhi * grid->spacing_y - 0.5 * booth->size[1];
        dont_jhi = 1;
        //fprintf(stderr,"dont_jhi==1!!!\n");
    }
    if (khi >= booth->khi)
    {
        khi = booth->khi; //=npoi_x
        xmax = khi * grid->spacing_x - 0.5 * booth->size[0];
        dont_khi = 1;
   ...
   */
    ...
    float dymin = jlo * grid->spacing_y - (ymin + 0.5 * booth->size[1]);
    float dymax = ymax + 0.5 * booth->size[1] - (jhi * grid->spacing_y);

    float dxmin = klo * grid->spacing_x - (xmin + 0.5 * booth->size[0]);
    float dxmax = xmax + 0.5 * booth->size[0] - (khi * grid->spacing_x);
   ...
   */
    ...
    booth->cubes[number]->ilo = ilo;
    booth->cubes[number]->ihi = ihi;
    booth->cubes[number]->jlo = jlo;
    booth->cubes[number]->jhi = jhi;
    booth->cubes[number]->klo = klo;
    booth->cubes[number]->khi = khi;
...
int DefineBCs(struct sc2004grid *grid, struct sc_booth *booth, int number)
{
   ...
   */
    ...
    if (number == -1)
    {
        ...
        booth->bc_type_minusy = WALL;
        booth->bc_type_plusy = WALL;
        booth->bc_type_minusz = WALL;
        booth->bc_type_plusz = WALL; // hier muss aber noch eine Knoten-RB dazu (Klimaanlage)!

        booth->bc_type_minusx = INLET;
        booth->bc_type_plusx = OUTLET;
    ...
    else
    {
        ...
        // ilo,ihi,...:
        ilo = booth->cubes[number]->ilo;
        ihi = booth->cubes[number]->ihi;
        jlo = booth->cubes[number]->jlo;
        jhi = booth->cubes[number]->jhi;
        klo = booth->cubes[number]->klo;
        khi = booth->cubes[number]->khi;

        /*
      // test
            if (ihi>=grid->npoi_z)
               booth->cubes[number]->ihi=grid->npoi_z-1;
            if (jhi>=grid->npoi_y)
               booth->cubes[number]->jhi=grid->npoi_y-1;
            if (khi>=grid->npoi_x)
               booth->cubes[number]->khi=grid->npoi_x-1;
      */

        // z-direction
        // if ( (inside) or (outside and not normal outside wall) )

        if ((ilo != 0) || ((ilo == 0) && (booth->bc_type_minusz != WALL)))
            booth->cubes[number]->bc_type_minusz = WALL;
        else
            booth->cubes[number]->bc_type_minusz = 0;
        if ((ihi != grid->npoi_z) || ((ihi == grid->npoi_z) && (booth->bc_type_plusz != WALL)))
            booth->cubes[number]->bc_type_plusz = WALL;
        else
            booth->cubes[number]->bc_type_plusz = 0;

        // y-direction
        if ((jlo != 0) || ((jlo == 0) && (booth->bc_type_minusy != WALL)))
            booth->cubes[number]->bc_type_minusy = WALL;
        else
            booth->cubes[number]->bc_type_minusy = 0;
        if ((jhi != grid->npoi_y) || ((jhi == grid->npoi_y) && (booth->bc_type_plusy != WALL)))
            booth->cubes[number]->bc_type_plusy = WALL;
        else
            booth->cubes[number]->bc_type_plusy = 0;

        // x-direction
        if ((klo != 0) || ((klo == 0) && (booth->bc_type_minusx != WALL)))
            booth->cubes[number]->bc_type_minusx = WALL;
        else
            booth->cubes[number]->bc_type_minusx = 0;
        if ((khi != grid->npoi_x) || ((khi == grid->npoi_x) && (booth->bc_type_plusx != WALL)))
            booth->cubes[number]->bc_type_plusx = WALL;
        else
            booth->cubes[number]->bc_type_plusx = 0;

        if (number == 0)
        {
            booth->cubes[number]->bc_special = 77;
        }
        else
        {
            booth->cubes[number]->bc_special = 0;
        }
    }
...
int GenerateBCs(struct sc2004grid *grid, struct sc_booth *booth, int number)
{
    /*
      float xmin = -0.5*booth->size[0];
       float xmax =  0.5*booth->size[0];
      float ymin = -0.5*booth->size[1];
       float ymax =  0.5*booth->size[1];
      float zmin = -0.5*booth->size[2];
       float zmax =  0.5*booth->size[2];
   */
    int npoi_x = grid->npoi_x;
    int npoi_y = grid->npoi_y;
    int npoi_z = grid->npoi_z;
    ...
    if (number != -1)
    {
        ...
        klo = booth->cubes[number]->klo;
        khi = booth->cubes[number]->khi;
        bcminusx = booth->cubes[number]->bc_type_minusx;
        bcplusx = booth->cubes[number]->bc_type_plusx;
        bcminusy = booth->cubes[number]->bc_type_minusy;
        bcplusy = booth->cubes[number]->bc_type_plusy;
        bcminusz = booth->cubes[number]->bc_type_minusz;
        bcplusz = booth->cubes[number]->bc_type_plusz;
    }
    else
    {
        //ilo,ihi,...: elements!!!
        ...
        klo = booth->klo;
        khi = booth->khi;
        bcminusx = booth->bc_type_minusx;
        bcplusx = booth->bc_type_plusx;
        bcminusy = booth->bc_type_minusy;
        bcplusy = booth->bc_type_plusy;
        bcminusz = booth->bc_type_minusz;
        bcplusz = booth->bc_type_plusz;
    }
   ...
   */
    ...
    if (number == -1)
    {
        grid->bcair_nodes = AllocIlistStruct(25);
        grid->bcair_velos = AllocFlistStruct(125);

        int bcnode;

        // Node-bc airconditioning outer geometry
        i = ihi;
        int y1 = (int)floor(0.175 * nelem_y);
        int y2 = (int)ceil(0.275 * nelem_y);
        int y3 = (int)floor(0.450 * nelem_y);
        int y4 = (int)ceil(0.550 * nelem_y);
        int y5 = (int)floor(0.725 * nelem_y);
        int y6 = (int)ceil(0.825 * nelem_y);

        int quit = 0;

        if ((y2 - y1 < 2) || (y3 - y2 < 2) || (y4 - y3 < 2) || (y5 - y4 < 2) || (y6 - y5 < 2))
            quit = 1;

        if ((nelem_y >= MIN_ELEMY_FOR_AIRCONDITIONING) && (quit == 0))
        {

            float l = 4.; // characteristic length
            float kfront = 3.75 * 0.001 * pow(booth->bcair_velo_front, 2);
            float epsfront = 9.40 * 0.0001 * pow(fabs(booth->bcair_velo_front / l), 3);
            float kmiddle = 3.75 * 0.001 * pow((booth->bcair_velo_middle), 2);
            float epsmiddle = 9.40 * 0.0001 * pow(fabs(booth->bcair_velo_middle / l), 3);
            float kback = 3.75 * 0.001 * pow((booth->bcair_velo_back), 2);
            float epsback = 9.40 * 0.0001 * pow(fabs(booth->bcair_velo_back / l), 3);
            ...
            for (j = y1; j < y2 + 1; j++)
            {
                ...
                {
                    bcnode = i * (npoi_x * npoi_y) + j * (npoi_x) + k;
                    Add2Ilist(grid->bcair_nodes, bcnode);
                    Add2Flist(grid->bcair_velos, 0.0);
                    Add2Flist(grid->bcair_velos, 0.0);
                    Add2Flist(grid->bcair_velos, booth->bcair_velo_front);
                    Add2Flist(grid->bcair_velos, kfront);
                    Add2Flist(grid->bcair_velos, epsfront);
                }
            }
            for (j = y3; j < y4 + 1; j++)
            {
                for (k = klo + 1; k < khi; k++)
                {
                    ...
                    Add2Flist(grid->bcair_velos, 0.0);
                    Add2Flist(grid->bcair_velos, 0.0);
                    Add2Flist(grid->bcair_velos, booth->bcair_velo_middle);
                    Add2Flist(grid->bcair_velos, kmiddle);
                    Add2Flist(grid->bcair_velos, epsmiddle);
                }
            }
            for (j = y5; j < y6 + 1; j++)
            {
                for (k = klo + 1; k < khi; k++)
                {
                    ...
                    Add2Flist(grid->bcair_velos, 0.0);
                    Add2Flist(grid->bcair_velos, 0.0);
                    Add2Flist(grid->bcair_velos, booth->bcair_velo_back);
                    Add2Flist(grid->bcair_velos, kback);
                    Add2Flist(grid->bcair_velos, epsback);
                }
            }
        }
        else
        {
            fprintf(stderr, "No airconditioning node-bc generated.\n\
            				Grid is too coarse, we need at least %d elements in y-direction ...\n",
                    MIN_ELEMY_FOR_AIRCONDITIONING);
        }
    }

    // ven
    if ((number != -1) && (booth->cubes[number]->bc_special == 77))
    {
        grid->bcven_nodes = AllocIlistStruct(25);
        grid->bcven_velos = AllocFlistStruct(125);

        int bcnode;

        // Node-bc venconditioning outer geometry
        i = ihi;
        int x1 = (int)floor(klo + 0.2 * (khi - klo));
        int x2 = (int)ceil(klo + 0.4 * (khi - klo));
        int x3 = (int)floor(klo + 0.6 * (khi - klo));
        int x4 = (int)ceil(klo + 0.8 * (khi - klo));

        int y1 = (int)floor(jlo + 0.2 * (jhi - jlo));
        int y2 = (int)ceil(jlo + 0.4 * (jhi - jlo));
        int y3 = (int)floor(jlo + 0.6 * (jhi - jlo));
        int y4 = (int)ceil(jlo + 0.8 * (jhi - jlo));

        int quit = 0;

        if ((y2 - y1 < 2) || (y3 - y2 < 2) || (y4 - y3 < 2) || (x2 - x1 < 2) || (x3 - x2 < 2) || (x4 - x3 < 2))
            quit = 1;

        if ((nelem_y >= MIN_ELEM_FOR_VEN) && (nelem_x >= MIN_ELEM_FOR_VEN) && (quit == 0))
        {

            float l = 4.; // characteristic length
            float _k = 3.75 * 0.001 * pow(booth->bcven_velo, 2);
            float _eps = 9.40 * 0.0001 * pow(fabs(booth->bcven_velo / l), 3);
            ...
            for (j = y1; j < y2 + 1; j++)
            {
                ...
                {
                    bcnode = i * (npoi_x * npoi_y) + j * (npoi_x) + k;
                    Add2Ilist(grid->bcven_nodes, bcnode);
                    Add2Flist(grid->bcven_velos, 0.0);
                    Add2Flist(grid->bcven_velos, 0.0);
                    Add2Flist(grid->bcven_velos, booth->bcven_velo);
                    Add2Flist(grid->bcven_velos, _k);
                    Add2Flist(grid->bcven_velos, _eps);
                }
            }
            for (j = y1; j < y2 + 1; j++)
            {
        ...
        {
            fprintf(stderr, "No ven node-bc generated.\n\
            				Grid is too coarse, we need at least %d elements in y-direction ...\n",
                    MIN_ELEM_FOR_VEN);
        }
    }
...
int CreateGeoRbFile(struct sc2004grid *grid, const char *geofile, const char *rbfile)
{
    ...
    else
    {
        ...
        int n_nodal = grid->bcin_nodes->num + grid->bcair_nodes->num + grid->bcven_nodes->num;
        ...
        for (i = 0; i < grid->bcair_nodes->num; i++)
        {
            fprintf(stream, "%6d 1 %8.5f\n", grid->bcair_nodes->list[i] + 1, grid->bcair_velos->list[5 * i + 0]);
            fprintf(stream, "%6d 2 %8.5f\n", grid->bcair_nodes->list[i] + 1, grid->bcair_velos->list[5 * i + 1]);
            fprintf(stream, "%6d 3 %8.5f\n", grid->bcair_nodes->list[i] + 1, grid->bcair_velos->list[5 * i + 2]);
            fprintf(stream, "%6d 4 %8.5f\n", grid->bcair_nodes->list[i] + 1, grid->bcair_velos->list[5 * i + 3]);
            fprintf(stream, "%6d 5 %8.5f\n", grid->bcair_nodes->list[i] + 1, grid->bcair_velos->list[5 * i + 4]);
        }
        for (i = 0; i < grid->bcven_nodes->num; i++)
        {
            fprintf(stream, "%6d 1 %8.5f\n", grid->bcven_nodes->list[i] + 1, grid->bcven_velos->list[5 * i + 0]);
            fprintf(stream, "%6d 2 %8.5f\n", grid->bcven_nodes->list[i] + 1, grid->bcven_velos->list[5 * i + 1]);
            fprintf(stream, "%6d 3 %8.5f\n", grid->bcven_nodes->list[i] + 1, grid->bcven_velos->list[5 * i + 2]);
            fprintf(stream, "%6d 4 %8.5f\n", grid->bcven_nodes->list[i] + 1, grid->bcven_velos->list[5 * i + 3]);
            fprintf(stream, "%6d 5 %8.5f\n", grid->bcven_nodes->list[i] + 1, grid->bcven_velos->list[5 * i + 4]);
        }
