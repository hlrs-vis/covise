...
void christmas06::CreateUserMenu(void)
{
    ...
    snprintf(path, sizeof(path), "%s/geofile.geo", coCoviseConfig::getEntry("value", "Module.ChristMas06.GeorbPath", "c:/temp").c_str());
    p_geofile = addStringParam("geofile path", "geofile path");
    p_geofile->setValue(path);

    snprintf(path, sizeof(path), "%s/rbfile.geo", coCoviseConfig::getEntry("value", "Module.ChristMas06.GeorbPath", "c:/temp").c_str());
