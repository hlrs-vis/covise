...
{
    struct archgrid *grid = NULL;
    if ((grid = (struct archgrid *)calloc(1, sizeof(struct archgrid))) == NULL)
    {
        fprintf(stderr, "not enough memory for struct archgrid!");
        return NULL;
    }
    ...
    for (i = 0; i < grid->npoi; i++)
    {
        ...
        {
            fprintf(stderr, "error 1 in CreatearchGrid!\n");
        }
    }
    grid->p->nump -= offset;
    ...
    for (i = 0; i < grid->npoi; i++)
    {
        ...
        {
            fprintf(stderr, "error 2 in CreatearchGrid!\n");
        }
    }
    ...
    for (i = 0; i < grid->e->nume; i++)
    {
        for (j = 0; j < 8; j++)
        {
            ...
            if ((grid->new_node[vertex] == -1) && (grid->e_to_remove[i] != -1))
            {
                fprintf(stderr, "error 3 in CreatearchGrid. Element list uses node that was removed.\n");
            }
            else
            {
    ...
    for (i = 0; i < grid->nelem; i++)
    {
        ...
        {
            fprintf(stderr, "error 4 in CreatearchGrid!\n");
        }
    }
