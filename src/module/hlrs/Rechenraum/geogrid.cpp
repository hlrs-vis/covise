...
{
    struct rechgrid *grid = NULL;
    if ((grid = (struct rechgrid *)calloc(1, sizeof(struct rechgrid))) == NULL)
    {
        fprintf(stderr, "not enough memory for struct rechgrid!");
        return NULL;
    }
...
int GenerateFloorBCs(struct rechgrid *grid, struct rech_model *model)
{
    ...
    fgets(buf, 2000, stream);
    if (strncmp(buf, "IN_FLOOR_OPEN_SX9", 13))
    {
        fprintf(stderr, "wrong file format (%s), expected 'IN_FLOOR_OPEN_SX9'\n", model->BCFile);
        return -1;
    }
    ...
    while ((token) && (token[0] != '\n'))
    {
        ...
        {
            xpos_open_sx9.push_back(x);
            ypos_open_sx9.push_back(y);
            for (j = 0; j < spacing; j++) // x-direction
            {
                for (k = 0; k < spacing; k++) // y-direction
                {
                    grid->floorbcs[(y + k) * grid->nelem_x + (x + j)] = 3;
                }
            }
        }
        token = strtok(NULL, " ");
    ...
    fgets(buf, 2000, stream);
    if (strncmp(buf, "IN_FLOOR_OPEN_NEC_CLUSTER", 13))
    {
        fprintf(stderr, "wrong file format (%s), expected 'IN_FLOOR_OPEN_NEC_CLUSTER'\n", model->BCFile);
        return -1;
    }
    ...
    while ((token) && (token[0] != '\n'))
    {
        ...
        {
            xpos_open_nec_cluster.push_back(x);
            ypos_open_nec_cluster.push_back(y);
            for (j = 0; j < spacing; j++) // x-direction
            {
                for (k = 0; k < spacing; k++) // y-direction
                {
                    grid->floorbcs[(y + k) * grid->nelem_x + (x + j)] = 6;
                }
            }
        }
        token = strtok(NULL, " ");
    ...
    fprintf(stderr, "A_open_sx9=%8.5f\n", model->Ain_floor_open_sx9);
    fprintf(stderr, "A_open_nec_cluster=%8.5f\n", model->Ain_floor_open_NEC_cluster);
    fprintf(stderr, "A_lochblech=%8.5f\n", model->Ain_floor_lochblech);

    model->Ain_floor_total = model->Ain_floor_4holes * model->n_floor_4holes + model->Ain_floor_4holesopen * model->n_floor_4holesopen + model->Ain_floor_1hole * model->n_floor_1hole +
                             // model->Ain_floor_open_sx9*model->n_floor_open_sx9 +	// treat SX9 separate here!
                             model->Ain_floor_lochblech * model->n_floor_lochblech;

    model->Ain_floor_open_sx9 = model->Ain_floor_open_sx9 * model->n_floor_open_sx9;
    model->Ain_floor_open_NEC_cluster = model->Ain_floor_open_NEC_cluster * model->n_floor_open_nec_cluster;

    fprintf(stderr, "Ain_floor_total=%8.5f\n", model->Ain_floor_total);
    fprintf(stderr, "Ain_floor_open_sx9=%8.5f\n", model->Ain_floor_open_sx9);
    fprintf(stderr, "Ain_floor_open_NEC_cluster=%8.5f\n", model->Ain_floor_open_NEC_cluster);
    ...
    fprintf(stderr, "vin_open_sx9=%8.5f\n", model->vin_open_sx9);
    fprintf(stderr, "vin_lochblech=%8.5f\n", model->vin_lochblech);
    fprintf(stderr, "vin_open_nec_cluster=%8.5f\n", model->vin_open_nec_cluster);

    fprintf(stderr, "Q_SX9=%8.5f\n", model->vin_open_sx9 * model->Ain_floor_open_sx9);
    fprintf(stderr, "Q_NEC_cluster=%8.5f\n", model->vin_open_nec_cluster * model->Ain_floor_open_NEC_cluster);
    ...
    for (i = 0; i < xpos_open_sx9.size(); i++) // the floor squares
    {
        for (j = 0; j < spacing; j++) // x-direction
        {
            for (k = 0; k < spacing; k++) // y-direction
            {
                ...
                grid->bcinlet_open_sx9.push_back((ypos_open_sx9[i] + k + 1) * grid->npoi_x + (xpos_open_sx9[i] + j) + 1);
                grid->bcinlet_open_sx9.push_back((ypos_open_sx9[i] + k + 1) * grid->npoi_x + (xpos_open_sx9[i] + j));

                grid->bcinlet_open_sx9_vol.push_back((ypos_open_sx9[i] + k) * grid->nelem_x + (xpos_open_sx9[i] + j));
                grid->bcinlet_open_sx9_outer.push_back(-1);

                grid->bcin_type.push_back(152);
                grid->bcin_type2.push_back(5.);
            }
        }
    }
    ...
    for (i = 0; i < xpos_open_nec_cluster.size(); i++) // the floor squares
    {
        for (j = 0; j < spacing; j++) // x-direction
        {
            for (k = 0; k < spacing; k++) // y-direction
            {
                ...
                grid->bcinlet_open_nec_cluster.push_back((ypos_open_nec_cluster[i] + k + 1) * grid->npoi_x + (xpos_open_nec_cluster[i] + j) + 1);
                grid->bcinlet_open_nec_cluster.push_back((ypos_open_nec_cluster[i] + k + 1) * grid->npoi_x + (xpos_open_nec_cluster[i] + j));

                grid->bcinlet_open_nec_cluster_vol.push_back((ypos_open_nec_cluster[i] + k) * grid->nelem_x + (xpos_open_nec_cluster[i] + j));
                grid->bcinlet_open_nec_cluster_outer.push_back(-1);

                grid->bcin_type.push_back(155);
                grid->bcin_type2.push_back(6.);
            }
        }
    }
