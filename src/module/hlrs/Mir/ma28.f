      ...
      subroutine ma28c(dim,matrix1,t,pivot,info)

      use arithmetic
      use declaration

      integer :: dim,pivot(dim)
      type(sparse_matrix) :: matrix1
      ...
      if (intflag .ne. 12345) then
        ...
        licn=size(matrix1%container%element)*15
        lirn=size(matrix1%container%element)*15
        allocate(a(licn))
        allocate(irn(lirn))
        allocate(irn2(lirn))
        allocate(icn(licn))
        ...
        allocate(perminv(matrix1%maxrow))

        n=dim
        nz=size(matrix1%container%element)
        mtype=1
        iflag=1
        u=1.0
        w=0.0
        ...
        do i=1,size(matrix1%container%element)
          a(i)=matrix1%container%element(i)
          icn(i)=matrix1%container%column_number(i)
        enddo

        k=0
        do i=1,matrix1%maxng
          do j=1,matrix1%container%ng(i)
            k=k+1
            irn2(k)=j
          enddo
        enddo
        ...
        do i=1,dim
          perminv(matrix1%container%permutation(i))=i
        enddo
      ...
      subroutine ma28b(dim,matrix1,T,pivot,info)
      ...
      integer, dimension(matrix1%maxrow**2*15) :: icn,irn,irn3
      real(double), dimension(matrix1%maxrow**2*15):: a,a2
      real(double) :: u,w(matrix1%maxrow),rhs(matrix1%maxrow)
      integer :: n,nz,licn,lirn,iflag,mtype
      integer :: ikeep(matrix1%maxrow,5),iw(matrix1%maxrow,8)
      ...
      n=matrix1%maxrow
      nz=size(matrix1%container%element)
      licn=matrix1%maxrow**2
      lirn=matrix1%maxrow**2
      iflag=1
      u=1.0
      w=0.0
      ikeep=0
      ...
      do i=1,size(matrix1%container%element)
       a(i)=dble(matrix1%container%element(i))
       icn(i)=matrix1%container%column_number(i)
      enddo
...
! see code for comments on private variables.
      ...
      common /lpivot/ lpiv(15),lnpiv(15),mapiv,manpiv,iavpiv,  &
                     ianpiv,kountl
