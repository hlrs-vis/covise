...
#include <ext/gmm/gmm_superlu_interface.h>
#include <ext/gmm/gmm_solver_cg.h>
#include <ext/gmm/gmm_solver_gmres.h>
#include <ext/gmm/gmm_solver_bicgstab.h>
#include <ext/gmm/gmm_solver_qmr.h>
...
{
    typedef P APrecond;
    static const APrecond &transform(const P &PP)
    {
        return PP;
    }
};
...
struct actual_precond<P, using_superlu, Matrix>
{
    ...
    static APrecond transform(const PR &)
    {
        return APrecond();
    }
    static const APrecond &transform(const APrecond &PP)
    {
        return PP;
    }
};
                             ...
                             local_solver, global_solver)
{
    ...
    iternc.reduce_noisy();
    iternc.set_maxiter(size_type(-1));
    iteration iter2(iternc);
    iteration iter3(iter2);
    iter3.reduce_noisy();
    ...
    while (!iter.finished(std::min(act_res, precond_res)))
    {
                ...
                if (r > value_type(0))
                {
                    ...
                    while (!iternc.finished(r))
                    {
                        ...
                        for (alpha = mtype(1); alpha >= alpha_min; alpha *= alpha_mult)
                        {
                            ...
                            NS.compute_sub_F(fi, x, isd);
                            gmm::scale(fi, value_type(-1));
                            if ((r_t = gmm::vect_norm2(fi)) <= r * alpha_max_ratio)
                                break;
                        }
                        if (iternc.get_noisy())
                            cout << "(step=" << alpha << ")\t";
                    ...
                    if (SOR_step)
                        gmm::mult(Bi, gmm::scaled(xii, 1.7), u, u);
                    gmm::mult(Bi, xii, rhs, rhs);
                }
            }
            precond_res = gmm::vect_norm2(rhs);
            if (SOR_step)
                cout << "SOR step residu = " << precond_res << endl;
            if (precond_res < residu)
                break;
        }
        ...
        act_res = act_res_new;
        if (iter.get_noisy())
            cout << "(step=" << alpha << ")\t unprecond res = " << act_res << " ";
                    ...
                    const Vector2 &f, iteration &iter)
{
    ...
    SAM.ml2 = &ml2;
    SAM.cor = &cor;
    SAM.precond1 = &precond1;
    SAM.precond2 = &precond2;
    iter2.init();
    SAM.iter = iter2;
