    ...
    while (!iter.finished_vect(r))
    {
        ...
        for (;;)
        {
            ...
            if (unbounded)
                ...
                lambda = (lambda_max + lambda_min) / R(2);
            if (valy <= R(2) * valx && (lambda < R(lambda_init * 1E-8) || (!unbounded && lambda_max - lambda_min < R(lambda_init * 1E-8))))
            {
                blocked = true;
                lambda = lambda_init;
                break;
