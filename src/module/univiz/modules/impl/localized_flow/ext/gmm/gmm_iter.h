...
public:
    ...
    iteration(bool &abortFlag, double r = 1.0E-8, int noi = 0, size_type mit = size_type(-1))
        : rhsn(1.0)
        , maxiter(mit)
        , noise(noi)
        , resmax(r)
    ...
    int get_noisy(void) const
    {
        return noise;
    }
    void set_noisy(int n)
    {
        noise = n;
    }
    void reduce_noisy(void)
    {
        if (noise > 0)
            noise--;
    }

    double get_resmax(void) const
    {
        return resmax;
    }
    void set_resmax(double r)
    {
        resmax = r;
    }

    size_type get_iteration(void) const
    {
        return nit;
    }
    void set_iteration(size_type i)
    {
        nit = i;
    }

    size_type get_maxiter(void) const
    {
        return maxiter;
    }
    void set_maxiter(size_type i)
    {
        maxiter = i;
    }

    double get_rhsnorm(void) const
    {
        return rhsn;
    }
    void set_rhsnorm(double r)
    {
        rhsn = r;
    }
    ...
    bool finished(double nr)
    {
        ...
        return (abortFlag || nit >= maxiter || converged(nr));
    }
    template <typename VECT>
    bool finished_vect(const VECT &v)
    {
