...
struct basic_index : public std::vector<size_t>
{
    ...
    basic_index(size_type j)
        : std::vector<size_t>(j)
        , nb_ref(1)
    {
    }
    template <typename IT>
    basic_index(IT b, IT e)
        : std::vector<size_t>(e - b)
        , nb_ref(1)
    {
        std::copy(b, e, begin());
    }
        ...
        : nb_ref(1)
    {
        ...
        for (; it != ite; ++it)
            i = std::max(i, *it);
        resize(i + 1);
        std::fill(begin(), end(), size_type(-1));
...
struct index_generator
{
    ...
    static void attach(pbasic_index pbi)
    {
        if (pbi)
            pbi->nb_ref++;
    }
...
{
    template <typename IT>
    unsorted_sub_index(IT it, IT ite)
        : sub_index(it, ite)
    {
    }
    template <typename CONT>
    unsorted_sub_index(const CONT &c)
        : sub_index(c)
    {
    }
    unsorted_sub_index()
    ...
    unsorted_sub_index(const unsorted_sub_index &si)
        : sub_index(si)
    {
    }
    unsorted_sub_index &operator=(const unsorted_sub_index &si)
    {
        sub_index::operator=(si);
        return *this;
    }
    void swap(size_type i, size_type j)
    {
...
struct sub_interval
{
    ...
    sub_interval(size_type mi, size_type l)
        : min(mi)
        , max(mi + l)
    {
    }
