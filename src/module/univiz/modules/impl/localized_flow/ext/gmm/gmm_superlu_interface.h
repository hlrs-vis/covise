...
#define DECL_GSSV(NAMESPACE, FNAME, FLOATTYPE, KEYTYPE)                                                                                                                            \
    inline void SuperLU_gssv(superlu_options_t *options, SuperMatrix *A, int *p, int *q, SuperMatrix *L, SuperMatrix *U, SuperMatrix *B, SuperLUStat_t *stats, int *info, KEYTYPE) \
    {                                                                                                                                                                              \
...
#define DECL_GSSVX(NAMESPACE, FNAME, FLOATTYPE, KEYTYPE)                                                                                                                                                                                                                                                                                                                      \
    ...
    {                                                                                                                                                                                                                                                                                                                                                                         \
        ...
        NAMESPACE::FNAME(options, A, perm_c, perm_r, etree, equed, R, C, L, U, work, lwork, B, X, recip_pivot_growth, rcond, ferr, berr, &mem_usage, stats, info);                                                                                                                                                                                                            \
...
template <class T>
void SuperLU_factor<T>::free_supermatrix(void)
{
    if (is_init)
    {
        Destroy_SuperMatrix_Store(&SB);
...
void SuperLU_factor<T>::solve(const VECTX &X_, const VECTB &B, int transp) const
{
    ...
    case LU_CONJUGATED:
        ...
        break;
    default:
        DAL_THROW(dal::failure_error, "invalid value for transposition option");
    }
    StatInit(&stat);
    int info = 0;
    R recip_pivot_gross, rcond;
...
inline void mult(const SuperLU_factor<T> &P, const V1 &v1, const V2 &v2)
{
    P.solve(v2, v1);
}

template <typename T, typename V1, typename V2>
inline void transposed_mult(const SuperLU_factor<T> &P, const V1 &v1, const V2 &v2)
{
