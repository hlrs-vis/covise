...
{
    MAT2 &Q = const_cast<MAT2 &>(QQ);
    MAT3 &R = const_cast<MAT3 &>(RR);
    typedef typename linalg_traits<MAT1>::value_type value_type;
...
void extract_eig(const MAT &A, VECT &V, Ttol tol, TA, std::complex<TV>)
{
    ...
    tol *= Ttol(2);
    for (size_type i = 0; i < n; ++i)
        if ((i == n - 1) || gmm::abs(A(i + 1, i)) < (gmm::abs(A(i, i)) + gmm::abs(A(i + 1, i + 1))) * tol)
    ...
    for (size_type i = 0; i < n; ++i)
    {
        ...
        if ((i == n - 1) || gmm::abs(A(i + 1, i)) < tol_i)
        {
            if (gmm::abs(std::imag(A(i, i))) > tol_cplx)
                DAL_WARNING(1, "A complex eigenvalue has been detected : "
                               << T(A(i, i)) << " : " << gmm::abs(std::imag(A(i, i))) / gmm::abs(std::real(A(i, i))) << " : " << tol_cplx);
            V[i] = std::real(A(i, i));
        }
        else
        {
...
inline void extract_eig(const MAT &A, const VECT &V, Ttol tol)
{
...
void qr_stop_criterion(MAT &A, size_type &p, size_type &q, Ttol tol)
{
    ...
    else
    {
        ...
        while ((q < n - 1 && A(n - 1 - q, n - 2 - q) == T(0)) || (q < n - 2 && A(n - 2 - q, n - 3 - q) == T(0)))
            ++q;
        if (q >= n - 2)
            q = n;
        p = n - q;
        ...
        if (p)
            --p;
        while (p > 0 && A(p, p - 1) != T(0))
            --p;
    }
}

template <typename MAT, typename Ttol>
inline void symmetric_qr_stop_criterion(const MAT &AA, size_type &p, size_type &q,
                                        Ttol tol)
{
    typedef typename linalg_traits<MAT>::value_type T;
    typedef typename number_traits<T>::magnitude_type R;
...
template <typename VECT1, typename VECT2, typename Ttol>
inline void symmetric_qr_stop_criterion(const VECT1 &diag, const VECT2 &sdiag_,
...
void block2x2_reduction(MATH &H, MATQ &Q, Ttol tol)
{
    ...
    std::vector<T> v(2), w(std::max(n, nq));
    v[0] = T(1);
    if (n < 2)
        return;
    tol *= Ttol(2);
...
#define tol_type_for_qr typename number_traits<typename linalg_traits<MAT1>::value_type>::magnitude_type
#define default_tol_for_qr gmm::default_tol(typename linalg_traits<MAT1>::value_type())
                                    ...
                                    Ttol tol, bool exc, bool compute_Q)
{
    ...
    for (m = n - 2; m != 0; --m)
    {
        ...
        if (gmm::abs(h10) * (gmm::abs(v2) + gmm::abs(v3)) <= tol * tst1)
            break;
    }
...
void symmetric_Wilkinson_qr_step(const MAT1 &MM, const MAT2 &ZZ,
                                 bool compute_z)
{
    MAT1 &M = const_cast<MAT1 &>(MM);
    MAT2 &Z = const_cast<MAT2 &>(ZZ);
    typedef typename linalg_traits<MAT1>::value_type T;
    typedef typename number_traits<T>::magnitude_type R;
    ...
    for (size_type k = 1; k < n; ++k)
    {
        ...
        if (k > 1)
            Apply_Givens_rotation_left(M(k - 1, k - 2), M(k, k - 2), c, s);
        Apply_Givens_rotation_left(M(k - 1, k - 1), M(k, k - 1), c, s);
        Apply_Givens_rotation_left(M(k - 1, k), M(k, k), c, s);
        if (k < n - 1)
            Apply_Givens_rotation_left(M(k - 1, k + 1), M(k, k + 1), c, s);
        if (k > 1)
            Apply_Givens_rotation_right(M(k - 2, k - 1), M(k - 2, k), c, s);
                                 ...
                                 const MAT &ZZ, bool compute_z)
{
    ...
    for (size_type k = 1; k < n; ++k)
    {
        ...
        if (k > 1)
            Apply_Givens_rotation_left(a10, a20, c, s);
        Apply_Givens_rotation_left(a11, a21, c, s);
        Apply_Givens_rotation_left(a12, a22, c, s);
        if (k < n - 1)
            Apply_Givens_rotation_left(a13, a23, c, s);

        if (k > 1)
            Apply_Givens_rotation_right(a01, a02, c, s);
        Apply_Givens_rotation_right(a11, a12, c, s);
        Apply_Givens_rotation_right(a21, a22, c, s);
        if (k < n - 1)
            Apply_Givens_rotation_right(a31, a32, c, s);
        ...
        diag[k] = gmm::real(a22);
        if (k > 1)
            sdiag[k - 2] = (gmm::conj(a01) + a10) / R(2);
        sdiag[k - 1] = (gmm::conj(a12) + a21) / R(2);

        x = sdiag[k - 1];
        z = (gmm::conj(a13) + a31) / R(2);
                                ...
                                bool compvect = true)
{
    ...
    while (q < n)
    {
        ...
        if (++ite > n * 100)
            DAL_THROW(failure_error, "QR algorithm failed. Probably, your matrix"
