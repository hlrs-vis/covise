...
#include <ext/gmm/gmm_blas_interface.h>
#include <ext/gmm/gmm_dense_lu.h>
...
#define getrf_interface(lapack_name, base_type)                                    \
    inline size_type lu_factor(dense_matrix<base_type> &A, std::vector<int> &ipvt) \
    {                                                                              \
...
#define getrs_interface(f_name, trans1, lapack_name, base_type)                                                                                    \
    inline void f_name(const dense_matrix<base_type> &A, const std::vector<int> &ipvt, std::vector<base_type> &x, const std::vector<base_type> &b) \
    {                                                                                                                                              \
...
#define getri_interface(lapack_name, base_type)                                                                          \
    inline void lu_inverse(const dense_matrix<base_type> &LU, std::vector<int> &ipvt, const dense_matrix<base_type> &A_) \
    {                                                                                                                    \
        GMMLAPACK_TRACE("getri_interface");                                                                              \
        dense_matrix<base_type> &A = const_cast<dense_matrix<base_type> &>(A_);                                          \
        int n(mat_nrows(A)), info, lwork(-1);                                                                            \
        base_type work1;                                                                                                 \
        if (n)                                                                                                           \
        {                                                                                                                \
            gmm::copy(LU, A);                                                                                            \
...
#define geqrf_interface(lapack_name1, base_type)                                  \
    inline void qr_factor(dense_matrix<base_type> &A)                             \
    {                                                                             \
        GMMLAPACK_TRACE("geqrf_interface");                                       \
        int m(mat_nrows(A)), n(mat_ncols(A)), info, lwork(-1);                    \
        base_type work1;                                                          \
        if (m && n)                                                               \
        {                                                                         \
            ...
            if (info)                                                             \
                DAL_THROW(failure_error, "QR factorization failed");              \
        }                                                                         \
    }
...
#define geqrf_interface2(lapack_name1, lapack_name2, base_type)                                                     \
    inline void qr_factor(const dense_matrix<base_type> &A, dense_matrix<base_type> &Q, dense_matrix<base_type> &R) \
    {                                                                                                               \
...
#define gees_interface(lapack_name, base_type)                                                                                                                                                                      \
    template <typename VECT> inline void implicit_qr_algorithm(const dense_matrix<base_type> &A, const VECT &eigval_, dense_matrix<base_type> &Q, double tol = gmm::default_tol(base_type()), bool compvect = true) \
    {                                                                                                                                                                                                               \
        GMMLAPACK_TRACE("gees_interface");                                                                                                                                                                          \
        typedef bool (*L_fp)(...);                                                                                                                                                                                  \
        L_fp p = 0;                                                                                                                                                                                                 \
        int n(mat_nrows(A)), info, lwork(-1), sdim;                                                                                                                                                                 \
        base_type work1;                                                                                                                                                                                            \
        if (!n)                                                                                                                                                                                                     \
            return;                                                                                                                                                                                                 \
        dense_matrix<base_type> H(n, n);                                                                                                                                                                            \
        gmm::copy(A, H);                                                                                                                                                                                            \
        char jobvs = (compvect ? 'V' : 'N'), sort = 'N';                                                                                                                                                            \
        std::vector<double> rwork(n), eigv1(n), eigv2(n);                                                                                                                                                           \
        lapack_name(&jobvs, &sort, p, &n, &H(0, 0), &n, &sdim, &eigv1[0], &eigv2[0], &Q(0, 0), &n, &work1, &lwork, &rwork[0], &info);                                                                               \
        lwork = int(gmm::real(work1));                                                                                                                                                                              \
        std::vector<base_type> work(lwork);                                                                                                                                                                         \
        lapack_name(&jobvs, &sort, p, &n, &H(0, 0), &n, &sdim, &eigv1[0], &eigv2[0], &Q(0, 0), &n, &work[0], &lwork, &rwork[0], &info);                                                                             \
        if (info)                                                                                                                                                                                                   \
...
#define gees_interface2(lapack_name, base_type)                                                                                                                                                                     \
    ...
    {                                                                                                                                                                                                               \
        ...
        lapack_name(&jobvs, &sort, p, &n, &H(0, 0), &n, &sdim, &eigvv[0], &Q(0, 0), &n, &work1, &lwork, &rwork[0], &rwork[0], &info);                                                                               \
        lwork = int(gmm::real(work1));                                                                                                                                                                              \
        std::vector<base_type> work(lwork);                                                                                                                                                                         \
        lapack_name(&jobvs, &sort, p, &n, &H(0, 0), &n, &sdim, &eigvv[0], &Q(0, 0), &n, &work[0], &lwork, &rwork[0], &rwork[0], &info);                                                                             \
        if (info)                                                                                                                                                                                                   \
            DAL_THROW(failure_error, "QR algorithm failed");                                                                                                                                                        \
        extract_eig(H, const_cast<VECT &>(eigval_), tol);                                                                                                                                                           \
    }
