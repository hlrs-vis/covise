                            ...
                            const VecY &y)
{
    ...
    rank_one_update(A, x, y, typename principal_orientation_type<typename linalg_traits<Matrix>::sub_orientation>::potype());
}
...
{
    Matrix &A = const_cast<Matrix &>(AA);
    rank_two_update(A, x, y, typename principal_orientation_type<typename linalg_traits<Matrix>::sub_orientation>::potype());
}
...
void house_vector(const VECT &VV)
{
...
template <typename VECT>
void house_vector_last(const VECT &VV)
{
    VECT &V = const_cast<VECT &>(VV);
    typedef typename linalg_traits<VECT>::value_type T;
    typedef typename number_traits<T>::magnitude_type R;
...
inline void row_house_update(const MAT &AA, const VECT1 &V, const VECT2 &WW)
{
...
template <typename MAT, typename VECT1, typename VECT2>
inline void col_house_update(const MAT &AA, const VECT1 &V, const VECT2 &WW)
{
    VECT2 &W = const_cast<VECT2 &>(WW);
    MAT &A = const_cast<MAT &>(AA);
    typedef typename linalg_traits<MAT>::value_type value_type;
...
void Hessenberg_reduction(const MAT1 &AA, const MAT2 &QQ, bool compute_Q)
{
    ...
    for (size_type k = 1; k + 1 < n; ++k)
    {
        ...
        if (compute_Q)
            col_house_update(sub_matrix(Q, SUBK, SUBI), v, w);
    }
}
...
void Givens_rotation(T a, T b, T &c, T &s)
{
    ...
    {
        T t = -a / b;
        s = T(R(1) / (sqrt(R(1) + gmm::abs_sqr(t))));
        c = s * t;
    }
    else
    {
        T t = -b / a;
        c = T(R(1) / (sqrt(R(1) + gmm::abs_sqr(t))));
        s = c * t;
    }
}
...
// Apply Q* v
template <typename T>
inline void Apply_Givens_rotation_left(T &x, T &y, T c, T s)
{
    T t1 = x, t2 = y;
    x = gmm::conj(c) * t1 - gmm::conj(s) * t2;
    y = c * t2 + s * t1;
}

// Apply v^T Q
template <typename T>
inline void Apply_Givens_rotation_right(T &x, T &y, T c, T s)
{
    T t1 = x, t2 = y;
    x = c * t1 - s * t2;
    y = gmm::conj(c) * t2 + gmm::conj(s) * t1;
}
