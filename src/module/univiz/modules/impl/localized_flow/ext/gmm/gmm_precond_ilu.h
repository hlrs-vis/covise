...
protected:
    ...
    template <typename M>
    void do_ilu(const M &A, row_major);
...
public:
    ...
    void build_with(const Matrix &A)
    {
        ...
        do_ilu(A, typename principal_orientation_type<typename linalg_traits<Matrix>::sub_orientation>::potype());
    }
    ...
    {
        return sizeof(*this) + (L_val.size() + U_val.size()) * sizeof(value_type) + (L_ind.size() + L_ptr.size()) * sizeof(size_type) + (U_ind.size() + U_ptr.size()) * sizeof(size_type);
    }
};
...
inline void mult(const ilu_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    gmm::copy(v1, v2);
    if (P.invert)
    {
...
template <typename Matrix, typename V1, typename V2>
inline void transposed_mult(const ilu_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    gmm::copy(v1, v2);
    if (P.invert)
    {
...
template <typename Matrix, typename V1, typename V2>
inline void left_mult(const ilu_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    copy(v1, v2);
    if (P.invert)
        gmm::lower_tri_solve(gmm::transposed(P.U), v2, false);
...
template <typename Matrix, typename V1, typename V2>
inline void right_mult(const ilu_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    copy(v1, v2);
    if (P.invert)
        gmm::upper_tri_solve(gmm::transposed(P.L), v2, true);
...
template <typename Matrix, typename V1, typename V2>
inline void transposed_left_mult(const ilu_precond<Matrix> &P, const V1 &v1,
                                 V2 &v2)
{
    copy(v1, v2);
    if (P.invert)
...
template <typename Matrix, typename V1, typename V2>
inline void transposed_right_mult(const ilu_precond<Matrix> &P, const V1 &v1,
                                  V2 &v2)
{
    copy(v1, v2);
    if (P.invert)
