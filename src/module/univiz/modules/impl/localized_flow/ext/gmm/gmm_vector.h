...
public:
    ...
    ref_elt_vector(V *p, size_type ll)
        : pm(p)
        , l(ll)
    {
    }
...
template <typename T, typename V>
inline bool operator==(T v, const ref_elt_vector<T, V> &re)
{
    return (v == T(re));
}
template <typename T, typename V>
...
template <typename T, typename V>
inline T &operator+=(T &v, const ref_elt_vector<T, V> &re)
{
    v += T(re);
    return v;
}
...
template <typename T, typename V>
inline T operator+(const ref_elt_vector<T, V> &re)
{
    return T(re);
}
template <typename T, typename V>
...
template <typename T, typename V>
inline T operator+(const ref_elt_vector<T, V> &re, T v)
{
    return T(re) + v;
}
template <typename T, typename V>
...
template <typename T, typename V>
inline typename number_traits<T>::magnitude_type
abs(const ref_elt_vector<T, V> &re)
{
    return gmm::abs(T(re));
}
template <typename T, typename V>
inline T sqr(const ref_elt_vector<T, V> &re)
{
    return gmm::sqr(T(re));
}
template <typename T, typename V>
inline typename number_traits<T>::magnitude_type
abs_sqr(const ref_elt_vector<T, V> &re)
{
    return gmm::abs_sqr(T(re));
}
template <typename T, typename V>
inline T conj(const ref_elt_vector<T, V> &re)
{
    return gmm::conj(T(re));
}
template <typename T, typename V>
std::ostream &operator<<(std::ostream &o, const ref_elt_vector<T, V> &re)
{
    o << T(re);
    return o;
}
template <typename T, typename V>
inline typename number_traits<T>::magnitude_type
real(const ref_elt_vector<T, V> &re)
{
    return gmm::real(T(re));
}
template <typename T, typename V>
inline typename number_traits<T>::magnitude_type
imag(const ref_elt_vector<T, V> &re)
{
    ...
    : public std::map<size_type, T>::iterator
{
    ...
    wsvector_iterator(const base_it_type &it)
        : base_it_type(it)
    {
    }
};
    ...
    : public std::map<size_type, T>::const_iterator
{
    ...
    wsvector_const_iterator(const base_it_type &it)
        : base_it_type(it)
    {
    }
};
...
public:
    ...
    inline T r(size_type c) const
    {
        ...
        if (it != this->end() && c == it->first)
            return it->second;
        else
            return T(0);
    }

    inline T operator[](size_type c) const
    {
        return r(c);
    }
...
template <typename T>
void wsvector<T>::out_of_range_error(void) const
{
...
template <typename T>
inline void copy(const wsvector<T> &v1,
                 wsvector<T> &v2)
{
...
template <typename T>
inline void copy(const wsvector<T> &v1, const simple_vector_ref<wsvector<T> *> &v2)
{
    simple_vector_ref<wsvector<T> *> *svr = const_cast<simple_vector_ref<wsvector<T> *> *>(&v2);
    wsvector<T> *pv = const_cast<wsvector<T> *>(v2.origin);
    if (vect_size(v1) != vect_size(v2))
        DAL_THROW(dimension_error, "dimensions mismatch");
    *pv = v1;
    svr->begin_ = vect_begin(*pv);
    svr->end_ = vect_end(*pv);
}
template <typename T>
inline void copy(const simple_vector_ref<const wsvector<T> *> &v1,
                 wsvector<T> &v2)
{
    copy(*(v1.origin), v2);
}
template <typename T>
inline void copy(const simple_vector_ref<wsvector<T> *> &v1, wsvector<T> &v2)
{
    copy(*(v1.origin), v2);
}

template <typename T>
inline void clean(wsvector<T> &v, double eps)
{
...
{
    simple_vector_ref<wsvector<T> *> *svr = const_cast<simple_vector_ref<wsvector<T> *> *>(&l);
    wsvector<T> *pv = const_cast<wsvector<T> *>((l.origin));
    clean(*pv, eps);
    svr->begin_ = vect_begin(*pv);
    svr->end_ = vect_end(*pv);
}
...
template <typename T>
inline size_type nnz(const wsvector<T> &l)
{
    return l.nb_stored();
}
...
struct elt_rsvector_
{
    ...
    elt_rsvector_(size_type cc)
    {
        c = cc;
    }
    elt_rsvector_(size_type cc, const T &ee)
    {
        c = cc;
        e = ee;
    }
    bool operator<(const elt_rsvector_ &a) const
    {
        return c < a.c;
    }
    bool operator==(const elt_rsvector_ &a) const
...
struct rsvector_const_iterator
{
    ...
    rsvector_const_iterator(const rsvector_iterator<T> &i)
        : it(i.it)
    {
    }
    rsvector_const_iterator(const IT &i)
        : it(i)
    {
    }
};
...
template <typename T>
class rsvector : public std::vector<elt_rsvector_<T> >
{
public:
...
public:
    ...
    {
        std::swap(nbl, v.nbl);
        std::vector<elt_rsvector_<T> >::swap(v);
    }

    /* Constructeurs */
    explicit rsvector(size_type l)
        : nbl(l)
    {
    }
    rsvector(void)
...
void rsvector<T>::swap_indices(size_type i, size_type j)
{
    ...
    if (i != j)
    {
        ...
        if (iti != this->end() && iti->c == i)
            situation += 1;
        itj = std::lower_bound(this->begin(), this->end(), ej);
        if (itj != this->end() && itj->c == j)
            situation += 2;
        ...
        case 2:
            ...
            --it;
            ite = this->begin();
            for (; it >= ite && it->c >= i; --it, --itj)
                *itj = *it;
            *itj = a;
            break;
        case 3:
            std::swap(iti->e, itj->e);
            break;
        }
    }
}
...
void rsvector<T>::sup(size_type j)
{
    if (nb_stored() != 0)
    {
        ...
        {
            for (iterator ite = this->end() - 1; it != ite; ++it)
                *it = *(it + 1);
...
template <typename T>
void rsvector<T>::w(size_type c, const T &e)
{
#ifdef GMM_VERIFY
    if (c >= nbl)
        out_of_range_error();
...
template <typename T>
T rsvector<T>::r(size_type c) const
{
#ifdef GMM_VERIFY
    if (c >= nbl)
        out_of_range_error();
...
template <typename T>
void rsvector<T>::out_of_range_error(void) const
{
    DAL_THROW(std::out_of_range, "out of range");
}
...
template <typename T>
std::ostream &operator<<(std::ostream &o, const rsvector<T> &v)
{
    gmm::write(o, v);
    return o;
}
...
template <typename T>
inline void copy(const rsvector<T> &v1,
                 rsvector<T> &v2)
{
    if (vect_size(v1) != vect_size(v2))
        DAL_THROW(dimension_error, "dimensions mismatch");
...
template <typename T>
inline void copy(const rsvector<T> &v1, const simple_vector_ref<rsvector<T> *> &v2)
{
    simple_vector_ref<rsvector<T> *> *svr = const_cast<simple_vector_ref<rsvector<T> *> *>(&v2);
    rsvector<T> *pv = const_cast<rsvector<T> *>((v2.origin));
    if (vect_size(v1) != vect_size(v2))
        DAL_THROW(dimension_error, "dimensions mismatch");
    *pv = v1;
    svr->begin_ = vect_begin(*pv);
...
template <typename T>
inline void copy(const simple_vector_ref<const rsvector<T> *> &v1,
                 rsvector<T> &v2)
{
    copy(*(v1.origin), v2);
}
template <typename T>
inline void copy(const simple_vector_ref<rsvector<T> *> &v1, rsvector<T> &v2)
{
    copy(*(v1.origin), v2);
}

template <typename V, typename T>
inline void add(const V &v1,
                rsvector<T> &v2)
{
...
template <typename V, typename T>
void copy(const V &v1, rsvector<T> &v2)
{
    if ((const void *)(&v1) != (const void *)(&v2))
    {
        if (vect_size(v1) != vect_size(v2))
...
template <typename T>
inline void clean(rsvector<T> &v, double eps)
{
    typedef typename number_traits<T>::magnitude_type R;
...
{
    simple_vector_ref<rsvector<T> *> *svr = const_cast<simple_vector_ref<rsvector<T> *> *>(&l);
    rsvector<T> *pv = const_cast<rsvector<T> *>((l.origin));
    clean(*pv, eps);
    svr->begin_ = vect_begin(*pv);
    svr->end_ = vect_end(*pv);
}
...
template <typename T>
inline size_type nnz(const rsvector<T> &l)
{
    return l.nb_stored();
}
...
public:
    ...
    typename std::vector<T>::iterator data_begin(void)
    {
        return data.begin();
    }
    typename std::vector<T>::iterator data_end(void)
    {
        return data.end();
    }
...
template <typename T>
void slvector<T>::w(size_type c, const T &e)
{
#ifdef GMM_VERIFY
    if (c >= size_)
        out_of_range_error();
...
template <typename T>
void slvector<T>::out_of_range_error(void) const
{
    DAL_THROW(std::out_of_range, "out of range");
}
...
template <typename T>
std::ostream &operator<<(std::ostream &o, const slvector<T> &v)
{
    gmm::write(o, v);
    return o;
}
...
template <typename T>
inline size_type nnz(const slvector<T> &l)
{
...
template <typename T, typename V>
inline T sqr(const gmm::ref_elt_vector<T, V> &a)
{
    return T(a) * T(a);
}

template <typename T, typename V>
inline typename gmm::number_traits<T>::magnitude_type
...
template <typename T, typename V>
inline T conj(const gmm::ref_elt_vector<T, V> &a)
{
    return dal::conj(T(a));
}
}
...
{
template <typename T>
void swap(gmm::wsvector<T> &v, gmm::wsvector<T> &w)
{
    v.swap(w);
}
template <typename T>
void swap(gmm::rsvector<T> &v, gmm::rsvector<T> &w)
{
    v.swap(w);
}
template <typename T>
void swap(gmm::slvector<T> &v, gmm::slvector<T> &w)
{
    v.swap(w);
}
}
