...
#include <ext/gmm/gmm_dense_sylvester.h>
             ...
             iteration &outer, Basis &KS)
{
    ...
    while (!outer.finished(beta))
    {
        ...
        if (st.tb_def < p)
        {
            ...
            gmm::copy(KS.mat(), W);
            gmm::copy(scaled(r, nss / beta), mat_col(W, m));
    ...
    if (restore)
    {
        ...
        for (size_type j = end - 1; j >= st.tb_deftot + 2; --j)
        {
            ...
            col_house_update(sub_matrix(tab_p, sub_interval(0, end - st.tb_deftot),
                                        sub_interval(0, jm - st.tb_deftot)),
                             v, w);
            w.resize(n);
        ...
        std::vector<T> d(fin - st.tb_deftot);
        d[0] = T(1);
        ...
        for (size_type j = 0; j + 1 < end - st.tb_deftot; ++j)
        {
            ...
            scale(sub_vector(mat_row(H, st.tb_deftot + j + 1),
                             sub_interval(st.tb_deftot, m - st.tb_deftot)),
                  d[j + 1]);
                 ...
                 idgmres_state &st)
{
    ...
    for (j = 0, ind = 0; j < m - p; ++j)
    {
        if (ritznew[j] == R(-1))
        {
            ...
            {
                st.nb_nolong += 2;
                ++j; //  à adapter dans le cas complexe ...
            }
            else
                st.nb_nolong++;
        }
    ...
    { // An invariant subspace has been found.
        ...
        {
            ++st.nb_want;
            st.tb_defwant = ++p; // il faudrait que ce soit un p local
        }
