...
protected:
    ...
    template <typename M>
    void do_ilutp(const M &, row_major);
...
public:
    void build_with(const Matrix &A)
    {
        ...
        do_ilutp(A, typename principal_orientation_type<typename linalg_traits<Matrix>::sub_orientation>::potype());
    }
    ilutp_precond(const Matrix &A, int k_, double eps_)
        : L(mat_nrows(A), mat_ncols(A))
    ...
    ilutp_precond(int k_, double eps_)
        : K(k_)
        , eps(eps_)
    {
    }
...
void ilutp_precond<Matrix>::do_ilutp(const M &A, row_major)
{
    ...
    for (size_type i = 0; i < n; ++i)
    {
        ...
        {
            DAL_WARNING(2, "pivot " << i << " too small");
            ip = i;
            w[i] = tmp = T(1);
        }
...
inline void mult(const ilutp_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    if (P.invert)
    {
...
template <typename Matrix, typename V1, typename V2>
inline void transposed_mult(const ilutp_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    if (P.invert)
    {
        gmm::copy(v1, P.temporary);
...
template <typename Matrix, typename V1, typename V2>
inline void left_mult(const ilutp_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    if (P.invert)
    {
        gmm::copy(gmm::sub_vector(v1, P.indperm), v2);
...
template <typename Matrix, typename V1, typename V2>
inline void right_mult(const ilutp_precond<Matrix> &P, const V1 &v1, V2 &v2)
{
    if (P.invert)
    {
        copy(v1, v2);
...
template <typename Matrix, typename V1, typename V2>
inline void transposed_left_mult(const ilutp_precond<Matrix> &P, const V1 &v1,
                                 V2 &v2)
{
    if (P.invert)
    {
...
template <typename Matrix, typename V1, typename V2>
inline void transposed_right_mult(const ilutp_precond<Matrix> &P, const V1 &v1,
                                  V2 &v2)
{
    if (P.invert)
    {
