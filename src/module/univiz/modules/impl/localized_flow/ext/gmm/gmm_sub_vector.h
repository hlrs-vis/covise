...
#include <ext/gmm/gmm_sub_index.h>
...
{
    while (itb != itbe && index() == size_type(-1))
    {
        ++itb;
    }
}
...
template <typename IT, typename MIT, typename SUBI, typename ORG,
          typename PT>
inline void set_to_begin(sparse_sub_vector_iterator<IT, MIT, SUBI> &it,
...
template <typename IT, typename MIT, typename SUBI, typename ORG,
          typename PT>
inline void set_to_end(sparse_sub_vector_iterator<IT, MIT, SUBI> &it,
...
struct linalg_traits<sparse_sub_vector<PT, SUBI> >
{
    ...
    typedef typename select_ref<value_type, typename linalg_traits<V>::reference, PT>::ref_type reference;
    typedef typename select_ref<typename linalg_traits<V>::const_iterator,
    ...
    typedef sparse_sub_vector_iterator<typename linalg_traits<V>::const_iterator, pre_iterator, SUBI> const_iterator;
...
template <typename PT, typename SUBI>
std::ostream &operator<<(std::ostream &o, const sparse_sub_vector<PT, SUBI> &m)
{
    gmm::write(o, m);
    return o;
}
...
{
    if (it.index() >= si.max || ite.index() <= si.min)
    {
        it = ite;
        return;
    }
...
struct skyline_sub_vector
{
    ...
    skyline_sub_vector(V &v, const SUBI &s)
        : begin_(vect_begin(v))
        , end_(vect_end(v))
        , origin(linalg_origin(v))
        , si(s)
...
template <typename IT, typename MIT, typename SUBI, typename ORG,
          typename PT>
inline void set_to_begin(skyline_sub_vector_iterator<IT, MIT, SUBI> &it,
...
template <typename IT, typename MIT, typename SUBI, typename ORG,
          typename PT>
inline void set_to_end(skyline_sub_vector_iterator<IT, MIT, SUBI> &it,
                       ORG o, skyline_sub_vector<PT, SUBI> *,
                       linalg_modifiable)
{
    typedef skyline_sub_vector<PT, SUBI> VECT;
...
template <typename PT, typename SUBI>
std::ostream &operator<<(std::ostream &o, const skyline_sub_vector<PT, SUBI> &m)
{
    gmm::write(o, m);
    return o;
}
...
template <typename PT, typename SUBI, typename st_type>
struct svrt_ir
{
...
typename select_return<typename sub_vector_type<const V *, SUBI>::vector_type,
                       typename sub_vector_type<V *, SUBI>::vector_type, const V *>::return_type
sub_vector(const V &v, const SUBI &si)
{
#ifdef GMM_VERIFY
    if (si.last() > vect_size(v))
        DAL_THROW(dimension_error, "sub vector too large");
#endif
    return typename select_return<typename sub_vector_type<const V *, SUBI>::vector_type,
                                  typename sub_vector_type<V *, SUBI>::vector_type, const V *>::return_type(linalg_cast(v), si);
}
...
#endif
    return typename select_return<typename sub_vector_type<const V *, SUBI>::vector_type,
                                  typename sub_vector_type<V *, SUBI>::vector_type, V *>::return_type(linalg_cast(v), si);
}
}
