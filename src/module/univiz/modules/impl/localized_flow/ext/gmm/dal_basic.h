...
#include <vector>
#include <ext/gmm/dal_algobase.h>

namespace dal
{
template <class T, unsigned char pks = 5>
class dynamic_array;
...
#define DNAMPKS__ ((size_type(1) << pks) - 1)
    ...
    bool operator==(const dna_iterator &i) const
    {
        return (i.in == in);
    }
    bool operator!=(const dna_iterator &i) const
...
public:
    dynamic_array(const dynamic_array<T, pks> &da)
    {
        init();
        *this = da;
    }
...
void dynamic_array<T, pks>::swap(dynamic_array<T, pks> &da)
{
    ...
    std::swap(ppks, da.ppks);
    std::swap(m_ppks, da.m_ppks);
}
...
template <class T, unsigned char pks>
dynamic_array<T, pks> &dynamic_array<T, pks>::operator=(const dynamic_array<T, pks> &da)
{
    clear(); /* evitable ... ? */
    ...
    if (ii >= last_accessed)
    {
        ...
        if (ii >= last_ind)
        {
            if ((ii >> (pks + ppks)) > 0)
            {
                while ((ii >> (pks + ppks)) > 0)
                    ppks++;
                array.resize(m_ppks = (size_type(1) << ppks));
                m_ppks--;
            }
...
template <class T, unsigned char pks>
inline void swap(const dynamic_array<T, pks> &x, const dynamic_array<T, pks> &y)
{
    x.swap(y);
}
...
/** 
  ...
  */
template <class T>
class ptr_collection : public std::vector<T *>
{
public:
