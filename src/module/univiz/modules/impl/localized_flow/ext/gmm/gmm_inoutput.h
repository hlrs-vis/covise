                     ...
                     int *prec, int *flag)
{
    ...
    *perline = *width = *flag = *prec = 0;
    if (sscanf(fmt, " (%d%c%d.%d)", perline, &p, width, prec) < 3 || !strchr("PEDF", p))
...
struct HarwellBoeing_IO
{
    ...
    void read(csc_matrix<T, shift> &A);
    ...
    template <typename MAT>
    static void write(const char *filename, const MAT &A);
        ...
        { /* Skip if pattern only  */
            ...
            for (i = 0; i < Valcrd; i++)
            {
                ...
                if (Valflag == 'D')
                {
                    ...
                    char *p;
                    while ((p = const_cast<char *>(strchr(line, 'D'))))
                        *p = 'E';
                }
...
inline void HarwellBoeing_IO::open(const char *filename)
{
    ...
    if (!f)
    {
        DAL_THROW(dal::failure_error, "could not open " << filename);
    }
    /* First line: */
...
HarwellBoeing_IO::read(csc_matrix<T, shift> &A)
{
    ...
    if (!f)
        DAL_THROW(dal::failure_error, "no file opened!");
    if (Type[0] == 'P')
...
HarwellBoeing_IO::write(const char *filename, const csc_matrix<T, shift> &A)
{
    ...
    if (is_complex_double__(T()))
        if (mat_nrows(A) == mat_ncols(A))
            ...
            t = "CRA";
    else if (mat_nrows(A) == mat_ncols(A))
        t = "RUA";
    else
        t = "RRA";
...
public:
    ...
    MatrixMarket_IO(const char *filename)
        : f(0)
    {
        open(filename);
    }
...
MatrixMarket_IO::write(const char *filename, const csc_matrix<T, shift> &A)
{
    ...
    if (is_complex_double__(T()))
        std::copy(&(t2[0]), &(t2[0]) + 4, &(t[0]));
    else
        std::copy(&(t1[0]), &(t1[0]) + 4, &(t[0]));
