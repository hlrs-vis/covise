...
struct linalg_const
{
}; // A reference is either linalg_const,
struct linalg_modifiable
{
}; //  linalg_modifiable or linalg_false.
...
struct abstract_sparse
{
}; // sparse matrix or vector
struct abstract_skyline
{
}; // 'sky-line' matrix or vector
struct abstract_dense
{
}; // dense matrix or vector
struct abstract_indirect
{
}; // matrix given by the product with a vector

struct row_major
{
}; // matrix with a row access.
struct col_major
{
}; // matrix with a column access
struct row_and_col
{
}; // both accesses but row preference
struct col_and_row
{
}; // both accesses but column preference
...
template <typename PT, typename V>
...
inline bool is_sparse(abstract_sparse)
{
    return true;
}
inline bool is_sparse(abstract_dense)
{
    return false;
}
inline bool is_sparse(abstract_skyline)
{
    return true;
}
inline bool is_sparse(abstract_indirect)
{
    return false;
}

template <typename L>
inline bool is_sparse(const L &)
{
    return is_sparse(typename linalg_traits<L>::storage_type());
}

inline bool is_row_matrix_(row_major)
{
    return true;
}
inline bool is_row_matrix_(col_major)
{
    return false;
}
inline bool is_row_matrix_(row_and_col)
{
    return true;
}
inline bool is_row_matrix_(col_and_row)
{
    return true;
}

template <typename L>
inline bool is_row_matrix(const L &)
{
    return is_row_matrix_(typename linalg_traits<L>::sub_orientation());
}

inline bool is_col_matrix_(row_major)
{
    return false;
}
inline bool is_col_matrix_(col_major)
{
    return true;
}
inline bool is_col_matrix_(row_and_col)
{
    return true;
}
inline bool is_col_matrix_(col_and_row)
{
    return true;
}

template <typename L>
inline bool is_col_matrix(const L &)
{
    return is_col_matrix_(typename linalg_traits<L>::sub_orientation());
}

inline bool is_col_matrix(row_major)
{
    return false;
}
inline bool is_col_matrix(col_major)
{
    return true;
}
inline bool is_row_matrix(row_major)
{
    return true;
}
inline bool is_row_matrix(col_major)
{
    return false;
}
...
inline bool is_const_reference(linalg_const)
{
    return true;
}
...
struct select_return_<C1, C2, const L &>
{
    typedef C1 return_type;
};
template <typename C1, typename C2, typename L>
struct select_return_<C1, C2, L &>
{
...
template <typename C1, typename C2, typename L>
struct select_ref_<C1, C2, const L &>
{
    typedef C1 ref_type;
};
template <typename C1, typename C2, typename L>
struct select_ref_<C1, C2, L &>
{
    typedef C2 ref_type;
};
template <typename C1, typename C2, typename PT>
...
{
    typedef typename is_a_reference_<typename linalg_traits<L>::is_reference>::reference reference;
};

template <typename L>
inline bool is_original_linalg(const L &)
{
    return is_original_linalg(typename is_a_reference<L>::reference());
}
inline bool is_original_linalg(linalg_false)
{
    return true;
}
inline bool is_original_linalg(linalg_true)
{
    return false;
}
...
struct select_orientation
{
    typedef typename select_orientation_<C1, C2,
                                         typename principal_orientation_type<typename linalg_traits<L>::sub_orientation>::potype>::return_type return_type;
};
...
template <typename T>
inline std::complex<T> conj_product(std::complex<T> a, std::complex<T> b)
{
    return std::conj(a) * b;
} // to be optimized ?
...
#define magnitude_of_linalg(M) typename number_traits<typename linalg_traits<M>::value_type>::magnitude_type
...
template <typename T>
inline std::complex<T> operator*(int b, const std::complex<T> &a)
{
    return a * T(b);
}
...
{
    typedef typename strongest_value_type<V1, typename strongest_value_type<V2, V3>::value_type>::value_type
        value_type;
};
...
{
    typedef typename temporary_col_matrix_<typename linalg_traits<V>::storage_type,
                                           typename linalg_traits<V>::linalg_type,
                                           V>::matrix_type matrix_type;
};

template <typename S, typename L, typename V>
struct temporary_row_matrix_
{
    typedef abstract_null_type matrix_type;
};
template <typename V, typename L>
...
{
    typedef typename temporary_row_matrix_<typename linalg_traits<V>::storage_type,
                                           typename linalg_traits<V>::linalg_type,
                                           V>::matrix_type matrix_type;
};
...
struct temporary_dense_vector_
{
    typedef abstract_null_type vector_type;
};
template <typename S, typename V>
...
{
    typedef typename temporary_dense_vector_<typename is_a_reference<V>::reference,
...
{
    typedef typename temporary_sparse_vector_<typename is_a_reference<V>::reference,
                                              typename linalg_traits<V>::storage_type, V>::vector_type vector_type;
};
...
{
    typedef typename temporary_skyline_vector_<typename is_a_reference<V>::reference,
                                               typename linalg_traits<V>::storage_type, V>::vector_type vector_type;
};
...
bool same_porigin(PT1, PT2)
{
    return false;
}

template <typename PT>
bool same_porigin(PT pt1, PT pt2)
{
    return (pt1 == pt2);
}
...
template <typename V>
inline size_type vect_size(const V &v)
{
    return linalg_traits<V>::size(v);
}

template <typename MAT>
inline size_type mat_nrows(const MAT &m)
{
    return linalg_traits<MAT>::nrows(m);
}

template <typename MAT>
inline size_type mat_ncols(const MAT &m)
{
    return linalg_traits<MAT>::ncols(m);
}

template <typename V>
inline typename select_return<typename linalg_traits<V>::const_iterator,
...
template <typename V>
inline typename linalg_traits<V>::const_iterator
...
template <typename M>
inline typename select_return<typename linalg_traits<M>::const_row_iterator,
...
template <typename M>
inline typename select_return<typename linalg_traits<M>::const_col_iterator,
...
template <typename M>
inline typename linalg_traits<M>::const_col_iterator
...
template <typename MAT>
inline typename select_return<typename linalg_traits<MAT>::const_sub_row_type,
...
template <typename MAT>
inline typename linalg_traits<MAT>::const_sub_row_type
...
template <typename MAT>
inline typename select_return<typename linalg_traits<MAT>::const_sub_col_type,
...
template <typename MAT>
inline typename linalg_traits<MAT>::const_sub_col_type
...
template <typename IT, typename ORG, typename VECT>
inline void set_to_begin(IT &it, ORG o, VECT *, linalg_false)
{
    it = vect_begin(*o);
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_begin(IT &it, ORG o, const VECT *, linalg_false)
{
    it = vect_const_begin(*o);
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_end(IT &it, ORG o, VECT *, linalg_false)
{
    it = vect_end(*o);
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_end(IT &it, ORG o, const VECT *, linalg_false)
{
    it = vect_const_end(*o);
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_begin(IT &, ORG, VECT *, linalg_const)
{
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_begin(IT &, ORG, const VECT *, linalg_const)
{
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_end(IT &, ORG, VECT *, linalg_const)
{
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_end(IT &, ORG, const VECT *, linalg_const)
{
}

template <typename IT, typename ORG, typename VECT>
inline void set_to_begin(IT &, ORG, VECT *v, linalg_modifiable)
{
    if (is_sparse(*v))
...
template <typename IT, typename ORG, typename VECT>
inline void set_to_begin(IT &, ORG, const VECT *v, linalg_modifiable)
{
    if (is_sparse(*v))
        DAL_THROW(internal_error, "internal_error");
}
...
template <typename IT, typename ORG, typename VECT>
inline void set_to_end(IT &, ORG, VECT *v, linalg_modifiable)
{
    if (is_sparse(*v))
        DAL_THROW(internal_error, "internal_error");
}
...
template <typename IT, typename ORG, typename VECT>
inline void set_to_end(IT &, ORG, const VECT *v, linalg_modifiable)
{
    if (is_sparse(*v))
        DAL_THROW(internal_error, "internal_error");
}
...
template <typename L>
inline void write(std::ostream &o, const L &l)
{
    write(o, l, typename linalg_traits<L>::linalg_type());
}

template <typename L>
void write(std::ostream &o, const L &l,
           abstract_vector)
{
...
template <typename L>
inline void write(std::ostream &o, const L &l, row_and_col)
{
    write(o, l, row_major());
}

template <typename L>
inline void write(std::ostream &o, const L &l, col_and_row)
{
    write(o, l, row_major());
}

template <typename L>
void write(std::ostream &o, const L &l, col_major)
{
    o << "matrix(" << mat_nrows(l) << ", " << mat_ncols(l) << ")" << endl;
    for (size_type i = 0; i < mat_nrows(l); ++i)
    {
        ...
        else
        {
            if (mat_ncols(l) != 0)
                o << ' ' << l(i, 0);
            for (size_type j = 1; j < mat_ncols(l); ++j)
                o << ", " << l(i, j);
        }
        o << " )\n";
    }
