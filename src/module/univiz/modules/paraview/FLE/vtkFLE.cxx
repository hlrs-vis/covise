    ...
    vtkInformationVector *outputVector)
{
    ...
    int UG_Id = 0;
    {
        ...
        output->SetBlock(UG_Id, ugrid);
        ugrid->Delete();
    }
    vtkUnstructuredGrid *outputUG =
        //vtkUnstructuredGrid::SafeDownCast(output->GetDataSet(UG_Id, 0));
        vtkUnstructuredGrid::SafeDownCast(output->GetBlock(UG_Id));
    vtkDataSet *outputUGDS =
        //vtkDataSet::SafeDownCast(output->GetDataSet(UG_Id, 0));
        vtkDataSet::SafeDownCast(output->GetBlock(UG_Id));

    int SG_Id = 1;
    {
        vtkStructuredGrid *sgrid = vtkStructuredGrid::New();
        //output->SetDataSet(SG_Id, 0, sgrid);
        output->SetBlock(SG_Id, sgrid);
        sgrid->Delete();
    }
    vtkStructuredGrid *outputSG =
        //vtkStructuredGrid::SafeDownCast(output->GetDataSet(SG_Id, 0));
        vtkStructuredGrid::SafeDownCast(output->GetBlock(SG_Id));
    vtkDataSet *outputSGDS =
        //vtkDataSet::SafeDownCast(output->GetDataSet(SG_Id, 0));
        vtkDataSet::SafeDownCast(output->GetBlock(SG_Id));
