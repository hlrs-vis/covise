...
###############################################################
 ...
 COVISE_VERSION="2014.11"
  ...
  echo   "*                                                     *"
  echo   "* (c) 1997-2014 HLRS, University of Stuttgart         *"
...
#   check for existing installation     
    ...
    do
        ...
        echo " [$done5]  5)  Install development option"
...
# ask where to install
    ...
    if [ "$COVISE_BRANCH" = "HLRS" ]; then
        ...
        echo "COVISE_BRANCH=$COVISE_BRANCH" >> $INSTDIR/covise-vars.sh
        echo 'PATH=${COVISEDIR}/bin:${PATH}' >>$INSTDIR/covise-vars.sh
        echo "export COVISEDIR COVISE_BRANCH PATH" >> $INSTDIR/covise-vars.sh
        ...
        echo "setenv COVISE_BRANCH $COVISE_BRANCH" >> $INSTDIR/covise-vars.csh
        echo 'set path=(${COVISEDIR}/bin $path)' >>$INSTDIR/covise-vars.csh
    else
        ### create the link-creation scripts @@@@@@@
        echo '#!/bin/sh'                                            > $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise covise"             >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise covise64"           >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise crb"                >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise cover"              >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise clean_covise"       >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise clean_cover"        >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise ARTserver"          >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise MIKEserver"         >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise POLHEMUSserver"     >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise FOBserver"          >> $INSTDIR/coviseLinks
        echo "ln -sf $COVISEDIR/scripts/Covise MOTIONSTARserver"   >> $INSTDIR/coviseLinks
        chmod 755 $INSTDIR/coviseLinks
    fi

    ### shared already existent?
    if [ -d $INSTDIR/covise/icons ] ; then 
      done1="+"
    else
      done1=" "
    fi
    ...
    if [ -d $INSTDIR/covise/data/general/tutorial ] ; then 
      done2="+"
    else
      done2=" "
    fi
    ...
    if [ -d $INSTDIR/covise/data/general/examples ] ; then 
      done3="+"
    else
      done3=" "
    fi
    ...
    ### development option already existent?
    if [ -d $INSTDIR/covise/src/application/examples ] ; then 
      done5="+"
    else
      done5=" "
    fi
...
install_runtime_shared()
{
    ...
    then
        ...
        if [ "$COVISE_BRANCH" = "VISENSO" ]; then
          echo "INFO: modifying the start script"
          mv $COVISEDIR/bin/_exec_for_covise /tmp
          echo "#!/bin/bash"                                           > $COVISEDIR/bin/_exec_for_covise
          echo ""                                                      >>$COVISEDIR/bin/_exec_for_covise
          echo "### included by covinst script"                        >>$COVISEDIR/bin/_exec_for_covise
          echo 'if [ -z "$ARCHSUFFIX" ]; then'                         >>$COVISEDIR/bin/_exec_for_covise
          echo "  export ARCHSUFFIX=\"fawn\""                          >>$COVISEDIR/bin/_exec_for_covise
          echo "fi"                                                    >>$COVISEDIR/bin/_exec_for_covise
          echo 'if [ -z "$COVISEDIR" ]; then'                          >>$COVISEDIR/bin/_exec_for_covise
          echo "  export COVISEDIR=\"$COVISEDIR\""                     >>$COVISEDIR/bin/_exec_for_covise
          echo "fi"                                                    >>$COVISEDIR/bin/_exec_for_covise
          echo 'if [ -z "$COVISE_BRANCH" ]; then'                      >>$COVISEDIR/bin/_exec_for_covise
          echo "  export COVISE_BRANCH=\"$COVISE_BRANCH\""             >>$COVISEDIR/bin/_exec_for_covise
          echo "fi"                                                    >>$COVISEDIR/bin/_exec_for_covise
          cat /tmp/_exec_for_covise | sed -e '1d'                      >>$COVISEDIR/bin/_exec_for_covise
        fi
...
adjust_dot_cshrc_files()
{
	...
	do
	    ...
	    then
            ...
            if [ ! -z "${COVISE_BRANCH}" ]; then
                echo "setenv COVISE_BRANCH ${COVISE_BRANCH}" >> $HOME/.tcshrc
            fi
	...
	do
            ...
            then
                ...
                echo "setenv COVISE_BRANCH HLRS" >> $HOME/.cshrc
...
adjust_dot_bashrc_files()
{
	...
	do
	    ...
	    then
            ...
            if [ ! -z "${COVISE_BRANCH}" ]; then
                echo "export COVISE_BRANCH=${COVISE_BRANCH}" >> $HOME/.bashrc
            fi
	...
	do
            ...
            then
                ...
                echo "export COVISE_BRANCH=HLRS" >> $HOME/.bashrc
                echo "export COVISEDIR=$COVISEDIR" >> $HOME/.bashrc
                echo "export PATH=${COVISEDIR}/bin:\$PATH" >> $HOME/.bashrc
            else
                ans='y'
...
install_runtime_general_modules()
{
	...
	if [ "$COVISE_BRANCH" = "HLRS" ]; then
            ...
            if [ "$ARCHFILE" != "$BASEDIR"/DIST.${ARCHSUFFIX}/co${ARCHSUFFIX}.modules.General.tar.gz ]; then
		continue
	    fi
        fi
...
extract_dev_prog()
{
    ...
    ARCHFILE="$BASEDIR"/SHARED/codev.examples.tar.gz
...
{
    echo " "
    echo "Installing development header files..."
    ...
    ARCHFILE="$BASEDIR"/SHARED/codev.header.tar.gz
